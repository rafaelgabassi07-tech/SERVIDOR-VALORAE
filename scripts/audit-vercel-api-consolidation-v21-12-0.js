...
assert(functions.length === 1 && functions.includes('api/router.js'), `API não está consolidada em 1 function router: ${functions.join(', ')}`);
...
assert(read('vercel.json').includes('/api/router?path=') && read('vercel.json').includes('/api/:path*'), 'vercel.json deve reescrever /api/:path* para /api/router.');
