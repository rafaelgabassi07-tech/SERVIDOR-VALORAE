...
const expectedPublic = '21.12.362';
const expectedPatch = '21.12.362-modal-source-repair-v330';
...
for (const block of [pkg.config, pkg.releaseMetadata]) {
  ...
  assert.equal(block.checkpoint, 'modal-source-repair-v330');
  assert.equal(block.releaseLabel, 'modal-source-repair-v330');
}
...
assert.ok(sw.includes('v21-12-362'));
assert.ok(coreRelease.includes(expectedPatch));
assert.ok(coreRelease.includes('valorae-proxy-server-v21-12-362'));
assert.ok(currentRelease.includes("VALORAE_PUBLIC_VERSION = '21.12.362'"));
