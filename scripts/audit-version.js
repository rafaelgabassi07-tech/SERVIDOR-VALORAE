...
const expectedPublic = '21.12.363';
const expectedPatch = '21.12.363-portfolio-chart-regression-repair-v331';
...
for (const block of [pkg.config, pkg.releaseMetadata]) {
  ...
  assert.equal(block.checkpoint, 'portfolio-chart-regression-repair-v331');
  assert.equal(block.releaseLabel, 'portfolio-chart-regression-repair-v331');
}
...
assert.ok(currentRelease.includes("VALORAE_PUBLIC_VERSION = '21.12.363'"));
