...
const expectedPublic = '21.12.378';
const expectedPatch = '21.12.378-final-decomposition-v346';
...
for (const block of [pkg.config, pkg.releaseMetadata]) {
  ...
  assert.equal(block.checkpoint, 'final-decomposition-v346');
  assert.equal(block.releaseLabel, 'final-decomposition-v346');
}
...
assert.ok(sw.includes('v21-12-378'));
assert.ok(coreRelease.includes(expectedPatch));
assert.ok(coreRelease.includes('valorae-proxy-server-v21-12-378'));
assert.ok(currentRelease.includes("VALORAE_PUBLIC_VERSION = '21.12.378'"));
