...
assert.deepEqual(actual, expected, 'supabase/ deve conter exatamente os três SQLs canônicos');

const texts = Object.fromEntries(expected.map(name => [name, fs.readFileSync(path.join(sqlDir, name), 'utf8')]));
const combined = expected.map(name => texts[name]).join('\n');
...
console.log(`SQL mínimo OK: 3 arquivos, 2 tabelas financeiras, legado bloqueado; sha256=${digest}`);
