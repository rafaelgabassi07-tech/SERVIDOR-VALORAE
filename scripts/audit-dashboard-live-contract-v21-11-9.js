...
check('consolidated api function count', functions.length === 2, `Encontradas: ${functions.join(', ')}`);
check('catch-all api function', functions.includes('api/[...path].js'), 'Todas as rotas /api/* passam pelo router interno.');
check('index api function', functions.includes('api/index.js'), '/api raiz preservado.');
check('router metrics route', has('routes/_router.js', "'/server/metrics'"), '/api/server/metrics via catch-all.');
check('router tests route', has('routes/_router.js', "'/server/tests'"), '/api/server/tests via catch-all.');
check('router cache route', has('routes/_router.js', "'/cache/stats'"), '/api/cache/stats via catch-all.');
check('router source route', has('routes/_router.js', "'/source/status'"), '/api/source/status via catch-all.');
check('router deploy route', has('routes/_router.js', "'/deploy/status'"), '/api/deploy/status via catch-all.');
