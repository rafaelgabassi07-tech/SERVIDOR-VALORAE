...
check(pkg.version === '21.12.0', 'package.json precisa estar em 21.12.0.');
check(Object.keys(pkg.dependencies || {}).length === 0, 'dependencies precisa continuar vazio.');
check(routeManifest().physicalFunctions.length === 1 && routeManifest().physicalFunctions.includes('api/router.js'), 'precisa manter API Routes consolidadas em 1 function física api/router.js para Vercel Free.');
...
check(catalogs.includes('21.12.0'), 'catálogos precisam referenciar 21.12.0.');
