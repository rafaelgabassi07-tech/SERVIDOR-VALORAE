...
check(pkg.version === '21.11.9', 'package.json precisa estar em 21.11.9.');
check(Object.keys(pkg.dependencies || {}).length === 0, 'dependencies precisa continuar vazio.');
check(routeManifest().physicalFunctions.length === 2, 'precisa manter API Routes consolidadas em 2 functions físicas para Vercel Free.');
...
check(catalogs.includes('21.11.9'), 'catálogos precisam referenciar 21.11.9.');
