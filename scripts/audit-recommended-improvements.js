...
assert(pkg.version === '21.11.9', 'package.json precisa estar em 21.11.9.');
assert(Object.keys(pkg.dependencies || {}).length === 0, 'dependencies deve continuar vazio.');
assert(manifest.physicalFunctions.length === 2 && manifest.physicalFunctions.includes('api/[...path].js'), 'Functions físicas devem ficar consolidadas no catch-all para Vercel Free.');
...
console.log('Recommended improvements audit OK: melhorias viáveis v21.11.9 presentes e sem tecnologias não recomendadas.');
