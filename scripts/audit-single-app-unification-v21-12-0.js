...
check('router server tests route', router.includes("'/server/tests'"), '/api/server/tests via router.');
check('router metrics route', router.includes("'/server/metrics'"), '/api/server/metrics via router.');
check('root rewrite to server', vercel.includes('/server.html'), 'Vercel aponta para o app principal.');
check('api consolidated', exists('api/router.js') && !exists('api/index.js') && !exists('api/[...path].js') && !exists('api/server/metrics.js'), 'Function física única api/router.js, sem servidores paralelos.');
