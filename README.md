...
- Adiciona extração canônica de gráficos das páginas de ações e FIIs do Investidor10 para alimentar o APK VALORAE.
- O contrato `/api/v1/asset?view=app&charts=full&chartSource=investidor10&complete=1` agora expõe `assetChartsCanonical`.
- Normaliza `Rentabilidade` e `Rentabilidade Real` exatamente como blocos de períodos do Investidor10: 1 mês, 3 meses, 1 ano, 2 anos, 5 anos e 10 anos quando disponíveis.
- Descobre e consulta URLs internas de gráficos presentes no HTML do Investidor10, em vez de montar gráficos pobres por fallback local.
- Reforça gráficos: `comparacaoIndices`, `receitasLucros`, `lucroCotacao`, `balancoPatrimonial` e `payoutHistorico`.
- Mantém as páginas dedicadas de rankings da v21.12.60 e preserva o núcleo `Valorae-engine.js` sem desmembramento.

### Novidades v21.12.60

- Corrige o ranking da Home do APK para usar como fonte canônica os blocos `Maiores Altas` e `Maiores Baixas` da página principal do Investidor10.
- A rota `/api/v1/market/rankings` agora aceita `source=home` e, para chamadas ao vivo sem tickers, não cai mais para comparação/cesta fixa.
- Mantém `source=dedicated`/`source=pages` como modo opcional, mas esse modo não alimenta a Home do APK por padrão.
- O parser da Home ancora a leitura na região de mercado/Ibovespa e ignora menus/rankings genéricos como `Maiores Altas - 30 dias`.
- Contrato recomendado para o APK: `/api/v1/market/rankings?type=ACAO&source=home&mode=complete&complete=1&strict=1&limit=6&minRows=6`.
- Preserva aliases Android: `rankings.altas`, `rankings.baixas`, `highs/lows`, `topGainers/topLosers`, `maioresAltas/maioresBaixas`.
