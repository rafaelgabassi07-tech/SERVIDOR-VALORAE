...
const STOCK_PROFILE_FIELDS = [
  ...
  { id: 'enterpriseValue', label: 'Valor da firma', group: 'Mercado', aliases: ['valorFirma', 'valor da firma', 'enterpriseValue', 'ev'] },
  { id: 'assets', label: 'Ativos', group: 'Balanço', aliases: ['ativos', 'totalAssets', 'assets', 'ativoTotal'] },
...
function profileDisplay(value, spec = {}) {
  ...
  if (['marketCap', 'equity', 'enterpriseValue', 'assets', 'grossDebt', 'netDebt', 'cash', 'dailyLiquidity'].includes(spec.id)) {
    const n = finiteValue(raw);
...
function addProfileItem(rows, seen, spec, rawValue, source = 'StatusInvest/Investidor10') {
  ...
  const numeric = finiteValue(rawValue?.value ?? rawValue?.valor ?? rawValue?.amount ?? rawValue?.raw ?? rawValue);
...
function buildCompanyProfileItems(payload = {}, assetType = '') {
  ...
  const info = {
    ...
    valorFirma: firstClean(payload.enterpriseValue, payload.valorFirma, merged.valorFirma, merged.enterpriseValue, payload.ev, merged.ev),
    ativos: firstClean(payload.assets, payload.totalAssets, payload.ativos, merged.assets, merged.totalAssets, merged.ativos),
    ...
    disponibilidade: firstClean(payload.cash, payload.caixa, payload.disponibilidades, merged.cash, merged.caixa, merged.disponibilidades),
...
function chartLikeRevenueEntries(node = {}) {
  ...
  if (labels.length && dataSource && dataSource.length === labels.length) {
    labels.forEach((label, index) => out.push({ label, value: dataSource[index] }));
  }
  ...
  for (const dataset of datasets) {
    ...
    if (labels.length && data.length === labels.length && data.every(item => typeof item !== 'object')) {
      labels.forEach((label, index) => out.push({ label, value: data[index] }));
      continue;
    }
    for (const point of data) {
      if (point && typeof point === 'object') {
        out.push({ label: point.name || point.label || point.category || point.x || dataset.name || dataset.label, value: point.y ?? point.value ?? point.percent ?? point.percentage ?? point.valor });
      }
    }
  }
  return out;
...
function collectRevenueRowsFromValue(value, source = 'Investidor10/StatusInvest') {
  ...
  const walk = (node, fallbackLabel = '') => {
    ...
    const latestBucket = latestRevenueBucket(node);
    if (latestBucket !== node) {
      walk(latestBucket, fallbackLabel);
      return;
    }

    const chartEntries = chartLikeRevenueEntries(node);
    if (chartEntries.length) {
      chartEntries.forEach(entry => consume(entry.label, entry.value));
      return;
    }
...
const MARKET_CONTEXT_FIELDS = [
  ...
  { id: 'valuation12m', label: 'Valorização 12M', group: 'Desempenho', unit: '%', aliases: ['valuation12m', 'valorizacao12m', 'valorização 12m', 'variation12m', 'change12m'] },
...
function buildMarketContextItems(payload = {}, assetType = '') {
  const candidates = [payload, payload.results, payload.quote, payload.results?.quote, payload.market, payload.results?.market, payload.companyInfo, payload.empresa, payload.fundo, payload.results?.dadosEmpresa, payload.results?.informacoesEmpresa, payload.results?.informacoesFundo].filter(Boolean);
  const rows = [];
  const seen = new Set();
  for (const spec of MARKET_CONTEXT_FIELDS) {
    ...
    const numeric = finiteValue(raw?.value ?? raw?.valor ?? raw?.amount ?? raw?.raw ?? raw);
    if (!display || display === '-' || display === '--') continue;
    if (numeric === 0 && ['dailyLiquidity', 'ibovParticipation', 'openOptions'].includes(spec.id)) continue;
...
const OWNERSHIP_PATHS = [
  ...
  'results.sections.posicaoAcionaria', 'assetChartBundle.shareholders', 'assetChartsMobile.shareholders'
];
...
function normalizeOwnershipRows(value, source = 'Investidor10 posição acionária') {
  ...
  const consume = (row = {}, fallback = '') => {
    ...
    const label = cleanText(row.shareholder || row.acionista || row.name || row.nome || row.label || row.title || fallback);
    const percent = ownershipPercent(row);
    const value = percent !== null ? revenueDisplay(percent) : cleanText(row.value || row.valor || row.display || row.percentual || row.percent);
    if (!label || !value || value === '-' || value === '--') return;
    rows.push({ label, value, percent, source });
  };
  const walk = (node, fallback = '') => {
    if (!node) return;
    if (Array.isArray(node)) return node.forEach(item => walk(item, fallback));
    if (typeof node !== 'object') return;
    if (node.rows || node.items || node.data || node.values) {
      for (const item of arrayFrom(node.rows || node.items || node.data || node.values)) walk(item, fallback);
    }
    ...
    for (const [key, child] of Object.entries(node)) {
      if (['source', 'metadata', 'diagnostics', 'rows', 'items', 'data', 'values'].includes(key)) continue;
...
function buildOwnershipSection(payload = {}, assetType = '') {
  ...
  for (const path of OWNERSHIP_PATHS) {
    const value = valueAtPath(payload, path);
    if (value !== undefined && value !== null) rows.push(...normalizeOwnershipRows(value));
  }
  const items = rows.map((row, index) => ({ id: `ownership_${index}_${norm(row.label).replace(/\s+/g, '_')}`, label: row.label, value: row.value, group: 'Posição acionária', source: row.source }));
  const chartRows = rows.filter(row => Number.isFinite(row.percent) && row.percent > 0 && row.percent <= 100).slice(0, 8);
...
const FII_CHECKLIST_PATHS = [
  'checklistBuyHold', 'checklist_buy_hold', 'fiiChecklist', 'checklist', 'buyHoldChecklist',
  'results.checklistBuyHold', 'results.checklist_buy_hold', 'results.fiiChecklist', 'results.checklist',
  'results.sections.checklist', 'results.sections.checklistBuyHold'
];
...
function normalizeChecklistRows(value, source = 'Investidor10 checklist') {
  ...
  const walk = (node, fallback = '') => {
    ...
    const label = cleanText(node.label || node.name || node.criterio || node.critério || node.title || node.text || fallback);
    const statusRaw = node.status ?? node.aprovado ?? node.ok ?? node.passed ?? node.value ?? node.valor ?? node.display;
    const valueText = typeof statusRaw === 'boolean' ? (statusRaw ? 'Atende' : 'Não atende') : cleanText(statusRaw || node.result || node.resultado || node.description || node.descricao);
    if (label && valueText) rows.push({ label, value: valueText, source });
    for (const [key, child] of Object.entries(node)) {
      if (['label', 'name', 'criterio', 'critério', 'title', 'text', 'status', 'aprovado', 'ok', 'passed', 'value', 'valor', 'display', 'result', 'resultado', 'description', 'descricao'].includes(key)) continue;
      if (Array.isArray(child)) child.forEach(item => walk(item, key));
    }
  };
  walk(value);
...
function buildFiiChecklistItems(payload = {}, assetType = '') {
  ...
  return rows.map((row, index) => ({ id: `fii_check_${index}_${norm(row.label).replace(/\s+/g, '_')}`, label: row.label, value: row.value, group: 'Checklist do FII', source: row.source }));
}
...
function buildCombinedIndexComparisonChart(ticker = '', charts = []) {
  const indexCharts = arrayFrom(charts)
    .filter(chart => /^asset_vs_(ibov|ifix|cdi|ipca|smll|idiv)$/.test(chart.id || ''))
...
function buildComparisonItemsAndCharts(payload = {}) {
  ...
  return { items: rows.slice(0, 32), charts: finalCharts.slice(0, 7) };
}
...
export function buildAnalysisPageResponse(payload = {}, input = {}) {
  ...
  const coreSections = [
    ...
    ...(isFii ? [section('fii_details', 'FIIs completos', 'fii_details', fiiDetails.items, 'StatusInvest/Investidor10 dados específicos de FIIs', { charts: fiiDetails.charts })] : []),
    ...(isFii ? [section('fii_checklist', 'Checklist do FII', 'metric_grid', fiiChecklistItems, 'Investidor10 checklist de FIIs')] : [])
  ];
