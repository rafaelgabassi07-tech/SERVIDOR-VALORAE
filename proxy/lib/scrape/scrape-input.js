...
export const VALORAE_SCRAPE_INPUT_VERSION = '21.12.52-scrape-fast-response-cache-policy';
...
export function normalizeScrapeInput(input = {}, defaults = {}) {
  ...
  const profile = String(input.profile || input.scrapeProfile || defaults.profile || defaults.scrapeProfile || 'app').trim().toLowerCase() || 'app';
  ...
  const lightweightProfile = ['scrape-fast', 'fast', 'route-fast'].includes(profile);
  const metrics = !['0', 'false', 'no', 'off'].includes(String(input.metrics ?? defaults.metrics ?? '1').toLowerCase());
  const includeDiagnostics = bool(input.includeDiagnostics ?? input.diagnostics, bool(defaults.includeDiagnostics ?? defaults.diagnostics, !lightweightProfile));
  const includeCharts = bool(input.includeCharts ?? input.charts, bool(defaults.includeCharts ?? defaults.charts, !lightweightProfile));
...
export function buildResultKey(normalized = {}) {
  return `result:${stableStringify({
    ...
    url: normalized.url,
    provider: normalized.provider || 'direct',
    profile: normalized.profile || 'app',
    ...
    includeDiagnostics: normalized.includeDiagnostics !== false,
