...
async function attemptFetch(url, { timeoutMs, headers, method, body, signal, validators }) {
  ...
  try {
    ...
    return { text: payload.text, status: res.status, ok: res.ok, validators: responseValidators(res), rawBytes: payload.rawBytes, finalUrl: res.url };
  } finally {
  ...
  return coalesce(key, async () => {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        else if (payload.ok || payload.text) setCache(key, { text: payload.text, status: payload.status, validators: payload.validators, finalUrl: payload.finalUrl }, ttlMs, staleMs);
        if (payload.ok || !isRetryableHttpStatus(payload.status) || attempt >= maxRetries) {
          return payload.revalidated && cached?.value
            ? { text: cached.value.text, status: cached.value.status, cacheStatus: 'REVALIDATED', url, finalUrl: cached.value.finalUrl || url, attempts: attempt + 1 }
            : { text: payload.text, status: payload.status, cacheStatus: payload.ok ? 'LIVE' : 'LIVE_ERROR', url, finalUrl: payload.finalUrl || url, attempts: attempt + 1 };
        }
      } catch (error) {
