...
export const VALORAE_PERFORMANCE_VERSION = '21.12.72-final-ui-charts-news-backup';
...
export const PROFILE_ALIASES = Object.freeze({
  ...
  fast: 'fast',
  chartdeep: 'chartdeep',
  chartsdeep: 'chartdeep',
  'charts-deep': 'chartdeep',
  financialcharts: 'chartdeep',
  'financial-charts': 'chartdeep',
  graficoscompletos: 'chartdeep',
  'graficos-completos': 'chartdeep',
  chartfast: 'chartfast',
  chartsfast: 'chartfast',
  'charts-fast': 'chartfast',
  mobilecharts: 'chartfast',
  'mobile-charts': 'chartfast',
  graficos: 'chartfast',
  gráficos: 'chartfast',
  wallet: 'portfolio',
  carteira: 'portfolio',
...
const PROFILES = {
  ...
  chartfast: {
    ...
    maxHtmlChars: 1_200_000,
    resultCacheTtlMs: 20 * 60 * 1000,
    staleResultCacheMs: 6 * 60 * 60 * 1000,
    includeNewsDefault: false,
    ...
    maxConcurrency: 6,
    description: 'Contrato mobile para gráficos: HTML reduzido + APIs internas essenciais do Investidor10, sem notícias/complementos pesados, com cache stale para resposta rápida.'
  },
  chartdeep: {
    timeoutMs: 18_000,
    valoraeScrapeTimeoutMs: 16_000,
    adaptiveCompletionTimeoutMs: 6500,
    maxHtmlChars: 4_000_000,
    resultCacheTtlMs: 8 * 60 * 1000,
    staleResultCacheMs: 2 * 60 * 60 * 1000,
    includeNewsDefault: false,
    enableInternalApis: true,
    useYahooFallback: true,
    returnHtml: true,
    ...
    hedgedStatusInvest: false,
    maxConcurrency: 4,
    description: 'Modo profundo exclusivo para gráficos financeiros: completa APIs internas do Investidor10 sem notícias/complementos StatusInvest e sem travar a primeira renderização mobile.'
  },
  standard: {
    timeoutMs: 12_000,
...
function chooseProfile(raw = {}, context = {}) {
  ...
  if (['chartdeep','chartsdeep','charts-deep','financialcharts','financial-charts','graficoscompletos','graficos-completos'].includes(view)) return 'chartdeep';
  if (['chartfast','chartsfast','charts-fast','mobilecharts','mobile-charts','graficos','gráficos'].includes(view)) return 'chartfast';
...
export function performanceCapabilities() {
  return {
    ...
    queryParams: ['profile=instant|ultra|tiny|quote|card|chartfast|chartdeep|wallet|analysis|fast|standard|turbo|deep|portfolio', 'view=instant|ultra|tiny|quote|card|wallet|detail|analysis|compact|standard|full', 'complete=1', 'adaptiveCompletion=1|0', 'hedgedStatusInvest=1|0', 'dedupeBatch=1|0', 'staleWhileRevalidate=1|0', 'nocache=1', 'debug=1', 'contracts=full|lite|auto']
  };
}
