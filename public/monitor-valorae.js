(() => {
  ...
  const HASH_ALIASES = { live: 'traffic', feed: 'traffic', command: 'traffic', output: 'traffic', performance: 'health', quality: 'health', comparison: 'benchmark', integration: 'architecture', tests: 'diagnostics', inspector: 'diagnostics' };
  const PATH_ALIASES = { '/tests': 'diagnostics', '/tests.html': 'diagnostics', '/inspector': 'diagnostics', '/inspector.html': 'diagnostics' };
  ...
  function filterEvents() {
    ...
    return state.events.filter(event => {
      ...
      if (!search) return true; return JSON.stringify([event.route,event.ticker,event.requestId,event.appName,event.appChannel,event.sourceStatus,event.cacheStatus,event.method,event.status,event.syncAction,event.errorCode]).toLowerCase().includes(search);
    });
  }
