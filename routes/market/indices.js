...
import { fetchAnalysisTickerSnapshot } from '../../lib/market/indices.js';
...
export default async function handler(req, res) {
  ...
  try {
    const refreshRaw = route.input?.refresh ?? route.input?.nocache ?? route.input?.forceRefresh;
    const bypassCache = ['1', 'true', 'yes', 'sim', 'on'].includes(String(refreshRaw ?? '').trim().toLowerCase());
    const data = await fetchAnalysisTickerSnapshot({ bypassCache, cache: !bypassCache });
    return sendJson(req, res, { version: ValoraeEngine.version, requestId: route.requestId, endpoint: 'market-indices', ...data }, { status: data.ok ? 200 : 502, engineVersion: ValoraeEngine.version, profile: 'market', cacheControl: bypassCache ? 'no-store' : (data.ok ? `private, max-age=${VALORAE_MOBILE_CACHE_POLICY_SECONDS.marketIndices}, stale-while-revalidate=300` : 'no-store') });
  } catch (err) {
