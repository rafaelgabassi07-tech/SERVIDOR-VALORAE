...
const CONTRACT_VERSION = `${VALORAE_RELEASE_PATCH}-integration-manifest`;
...
export default async function handler(req, res) {
  ...
  return sendJson(req, res, {
    ...
    stableRoots: {
      ...
      fieldObservability: 'fieldObservability',
    ...
    endpoints: [
      ...
      endpoint('/api/v1/contract/observability', 'Manifesto e trilha temporária de origem por traceId', 'diagnostic'),
      endpoint('/api/v1/contract/source-adapters', 'Adaptadores isolados, operações, feature flags e métricas por provedor', 'diagnostic'),
      endpoint('/api/v1/contract/html-parser-shadow', 'Parser HTML padrão em modo sombra, paridade e segurança de promoção', 'diagnostic'),
    ],
    appRules: [
      ...
      'fieldObservability é diagnóstico oculto da UI e não deve dirigir valores financeiros.',
      'source adapters são uma fronteira interna; o APK continua consumindo os mesmos contratos financeiros.',
      'htmlParserShadow é diagnóstico oculto; no modo sombra o parser legado continua sendo a única saída financeira.',
