...
import { buildFundamentalsView } from '../../lib/quality/asset-launch-readiness.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (validation) return sendJson(req, res, { version: ValoraeEngine.version, requestId: route.requestId, status: 'ERROR', error: validation }, { status: 400, engineVersion: ValoraeEngine.version, profile: 'asset-fundamentals' });
    ...
    const perfOptions = resolvePerformanceOptions({
      ...
      contracts: 'lite',
    }, { endpoint: 'asset-fundamentals', ticker, type });
    ...
    return sendJson(req, res, {
      ...
      endpoint: 'asset/fundamentals',
      ...fundamentals,
      integrationHint: 'Renderize os grupos com maior completenessPercent primeiro e mostre banner de dados parciais quando status=PARTIAL.',
      preferredAppView: `/api/v1/asset?ticker=${ticker}&view=app`,
    }, { status: 200, engineVersion: ValoraeEngine.version, profile: 'asset-fundamentals', cacheControl: 'private, max-age=30, stale-while-revalidate=120' });
  } catch (err) {
    return sendRouteError(req, res, err, { version: ValoraeEngine.version, requestId: route.requestId, profile: 'asset-fundamentals' });
  }
}
