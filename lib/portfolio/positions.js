...
export function portfolioInceptionDate(positions = [], transactions = [], explicitDate = '') {
  const candidates = [];
  const explicit = normalizeDate(explicitDate);
  if (explicit) candidates.push(explicit);

  for (const transaction of normalizeTransactions(transactions)) {
    if (transaction.quantity > 0 && transaction.date) candidates.push(transaction.date);
  }
  for (const position of normalizePositions(positions)) {
    if (position.firstPurchaseDate) candidates.push(position.firstPurchaseDate);
  }
  return candidates.filter(Boolean).sort()[0] || '';
}

export function tickerInceptionDate(ticker, positions = [], transactions = []) {
  const clean = normalizeTicker(ticker);
  if (!clean) return '';
  const candidates = normalizeTransactions(transactions)
    .filter(transaction => transaction.ticker === clean && transaction.quantity > 0 && transaction.date)
    .map(transaction => transaction.date);
  const position = normalizePositions(positions).find(item => item.ticker === clean);
  if (position?.firstPurchaseDate) candidates.push(position.firstPurchaseDate);
  return candidates.filter(Boolean).sort()[0] || '';
}
