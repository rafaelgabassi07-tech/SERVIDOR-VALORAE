...
export function normalizePositions(input = []) {
  ...
  return list.map((item) => {
    ...
    return { ticker, quantity, avgPrice, currentPrice, invested: round(quantity * avgPrice, 2), marketValue: round(quantity * currentPrice, 2), firstPurchaseDate, firstPurchaseAt, assetClass, sector, segment: sector, name };
...
export function normalizeTransactions(input = []) {
  ...
  return list.map((item) => {
    ...
    const isSell = item?.isSell === true || item?.sell === true || rawQuantity < 0 || /VENDA|SELL|SAIDA|SAÍDA|VENDER/.test(sideRaw);
...
export function quantityAtDate(ticker, targetDate, positions = [], transactions = []) {
  ...
  if (txs.length) return Math.max(0, round(txs.reduce((sum, tx) => sum + tx.quantity, 0), 8));
