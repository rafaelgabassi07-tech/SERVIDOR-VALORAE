...
/**
 * Reconciles the still-open current month with the same live portfolio snapshot shown elsewhere
 * in the app BEFORE Modified Dietz is calculated. Updating only the final chart marketValue would
 * leave monthlyReturnPercent/portfolioReturnPercent tied to an older close.
 */
export function reconcileCurrentMarketSnapshotV5(points = [], { month = '', marketValue = null, complete = false } = {}) {
  const cleanMonth = /^\d{4}-\d{2}$/.test(String(month || '')) ? String(month) : '';
  const currentMarketValue = finite(marketValue);
  if (!complete || !cleanMonth || currentMarketValue === null || currentMarketValue <= 0) {
    return Array.isArray(points) ? points : [];
  }

  return (Array.isArray(points) ? points : []).map(point => {
    if (!point || monthKey(point) !== cleanMonth) return point;
    const dividendsReceived = Math.max(0, finite(point.dividendsReceived) ?? 0);
    const alignedMarketValue = round(currentMarketValue, 2);
    const alignedTotalValue = round(alignedMarketValue + dividendsReceived, 2);
    return {
      ...point,
      marketValue: alignedMarketValue,
      totalValue: alignedTotalValue,
      value: alignedTotalValue,
      patrimonio: alignedTotalValue,
      completeValuation: true,
      partialValuation: false,
      valuationCoveragePercent: 100,
      unavailableValuationTickers: [],
      currentLivePricing: true,
      currentSnapshotAligned: true
    };
  });
}
...
export function selectExposureReturnWindowV5(rows = [], range = 'SINCE_START', months = 12, now = new Date()) {
  ...
  let factor = 1;
  let rebuiltSegmentId = null;
  const rebuilt = visible.map(row => {
    if (rebuiltSegmentId === null || Number(row.chartSegmentId || 0) !== rebuiltSegmentId) {
      rebuiltSegmentId = Number(row.chartSegmentId || 0);
      factor = 1;
    }
