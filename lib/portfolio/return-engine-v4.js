...
export function selectPortfolioReturnWindowV4(rows = [], range = 'SINCE_START', months = 12, now = new Date()) {
  ...
  const calendarVisible = continuousChain.filter(row => row.month >= startMonth);
  if (!calendarVisible.length) return { rows: [], comparisonBaseMonth: '', comparisonStartMonth: startMonth, chainId: latestChainId, leadingInactiveMonths: [] };

  // A janela de calendário pode começar quando a carteira estava zerada. Esses meses são
  // conhecidos e continuam úteis para a trajetória MAX, porém não podem fazer um período
  // YTD/1A/3A parecer investido antes do primeiro capital efetivamente exposto. Remova
  // somente a inatividade que antecede o primeiro mês ativo da janela; lacunas internas
  // entre uma liquidação e uma reentrada permanecem explícitas e não viram performance.
  const firstInvestedIndex = calendarVisible.findIndex(row => row.knownInactive !== true);
  if (firstInvestedIndex < 0) {
    return {
      rows: [],
      comparisonBaseMonth: '',
      comparisonStartMonth: '',
      chainId: latestChainId,
      leadingInactiveMonths: calendarVisible.map(row => row.month)
    };
  }
  const leadingInactiveMonths = calendarVisible.slice(0, firstInvestedIndex).map(row => row.month);
  const visible = calendarVisible.slice(firstInvestedIndex);

  const firstVisibleMonth = visible[0].month;
  const previous = leadingInactiveMonths.length
    ? null
    : (continuousChain
        .filter(row => row.month < firstVisibleMonth && row.knownInactive !== true)
        .at(-1) || null);
  let factor = 1;
...
export function summarizePortfolioReturnV4(series = []) {
  ...
  const active = clean.filter(row => row.knownInactive !== true);
  if (!active.length) {
    return { totalReturnPercent: 0, last12MonthsReturnPercent: 0, lastMonthReturnPercent: 0, averageMonthlyReturnPercent: 0, volatilityMonthlyPercent: 0, bestMonth: null, worstMonth: null };
  }
  const monthly = active.map(row => Number(row.monthlyReturnPercent));
  ...
  const statisticsRows = active.filter(row => row.currentMonthPartial !== true);
  ...
  return {
    totalReturnPercent: round(finite(active.at(-1)?.portfolioReturnPercent) ?? compound(monthly), 2),
