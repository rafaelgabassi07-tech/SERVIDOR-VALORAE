...
export const PORTFOLIO_HISTORY_VERSION = '21.12.382-quote-state-resilience-v350';
...
function mergeHistoryPositions(activePositions = [], transactions = []) {
  ...
  for (const [ticker, txs] of txByTicker.entries()) {
    ...
    const signedTransactionQuantity = txs.reduce((sum, tx) => sum + (tx.side === 'SELL' ? -tx.quantity : tx.quantity), 0);
...
function openingInventoryForPosition(position = {}, transactions = []) {
  ...
  const signedTransactionQuantity = transactions.reduce((sum, tx) => {
    ...
    return sum + (tx.side === 'SELL' ? -quantity : quantity);
  }, 0);
...
function buildMergedPortfolioSeries(histories = [], normalized = [], { range = '1Y', interval, transactions = [] } = {}) {
  ...
  for (const timestamp of timestamps) {
    ...
    perPosition.forEach((entry, idx) => {
      ...
      let price = point && point.timestamp <= timestamp ? toNumber(point.close) : 0;
      const hasRealValuation = price > 0;
      if (!(price > 0)) {
        // No intraday, não retropropaga custo ou uma cotação futura: aguarda o primeiro
        // timestamp em que todas as posições ativas tenham preço real. Em mensal/semanal,
        // mantém somente a componente indisponível pelo custo contábil para não eliminar
        // todos os fechamentos históricos da carteira.
        if (intraday) return;
        price = quantity > 0 && invested > 0 ? invested / quantity : toNumber(entry.position.averagePrice, 0);
    ...
    if (included > 0 && totalValue > 0) {
      ...
      rows.push({
        ...
        source: unavailableValuationTickers.length
          ? (intraday ? 'YahooChartIntraday+CostBasisCarry' : 'YahooChartDaily+CostBasisCarry')
          : (intraday ? 'YahooChartIntraday' : 'YahooChartDaily'),
