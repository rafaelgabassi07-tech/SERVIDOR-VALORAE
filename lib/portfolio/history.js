import { fetchYahooHistory } from '../sources/adapters/index.js';
import { canonicalTicker } from '../market/yahoo.js';
import { applyOperationToPosition, classifyCorporateOperation } from '../sync/financial-integrity.js';
import { getB3RegularSessionBounds, isB3TradingDay } from '../market/b3-calendar.js';

export const PORTFOLIO_HISTORY_VERSION = '21.12.405-regular-session-calendar-v365';
export const PORTFOLIO_HISTORY_ENGINE_HARDENING_V291 = 'VALORAE_REALTIME_PORTFOLIO_HISTORY_ENGINE_V291';

function toNumber(value, fallback = 0) {
  if (typeof value === 'number' && Number.isFinite(value)) return value;
  const n = Number(String(value ?? '').replace(/[^0-9,.-]/g, '').replace(/\.(?=\d{3}(\D|$))/g, '').replace(',', '.'));
  return Number.isFinite(n) ? n : fallback;
}

function parseArrayInput(value) {
  if (Array.isArray(value)) return value;
  if (value === undefined || value === null) return [];
  if (typeof value === 'string') {
    const raw = value.trim();
    if (!raw) return [];
    try {
      const parsed = JSON.parse(raw);
      return Array.isArray(parsed) ? parsed : [];
    } catch {
      return [];
    }
  }
  return [];
}

export function normalizePortfolioPositions(input = {}) {
  const positionRows = parseArrayInput(input.positions);
  if (positionRows.length) {
    return positionRows.map(p => ({
      ticker: canonicalTicker(p.ticker || p.symbol || p.codigo || p.ativo),
      quantity: toNumber(p.quantity ?? p.qtd ?? p.shares ?? p.quantidade),
      averagePrice: toNumber(p.averagePrice ?? p.avgPrice ?? p.precoMedio ?? p.average ?? p.avg),
      firstPurchaseAt: toNumber(p.firstPurchaseAt ?? p.firstPurchaseMillis ?? p.purchaseAt ?? p.dataPrimeiraCompra ?? p.date ?? p.data, 0),
      currentPrice: toNumber(p.currentPrice ?? p.price ?? p.precoAtual ?? p.cotacao ?? p.lastPrice, 0),
      assetClass: p.assetClass || p.assetType || p.type || p.classe || undefined,
      name: p.name || p.nome || p.ticker || p.symbol || undefined,
      account: p.account || p.corretora || undefined,
    })).filter(p => p.ticker && p.quantity > 0);
  }
  const tickers = String(input.tickers || input.ticker || input.symbols || input.symbol || '').split(',').map(s => canonicalTicker(s.trim())).filter(Boolean);
  const quantities = String(input.quantities || input.quantity || input.qtd || '').split(',').map(x => toNumber(x));
  const avgPrices = String(input.avgPrices || input.averagePrices || input.averagePrice || input.precoMedio || '').split(',').map(x => toNumber(x));
  return tickers.map((ticker, i) => ({ ticker, quantity: quantities[i] || 0, averagePrice: avgPrices[i] || 0, currentPrice: 0, firstPurchaseAt: 0 })).filter(p => p.ticker && p.quantity > 0);
}

function parsePortfolioDateSeconds(value) {
  if (value === undefined || value === null) return 0;
  const raw = String(value).trim();
  if (!raw) return 0;
  const numeric = Number(raw);
  if (Number.isFinite(numeric)) {
    if (numeric <= 0) return 0;
    return Math.floor(numeric > 9_999_999_999 ? numeric / 1000 : numeric);
  }
  const iso = Date.parse(raw);
  if (Number.isFinite(iso)) return Math.floor(iso / 1000);
  const br = raw.match(/^(\d{1,2})[\/-](\d{1,2})[\/-](\d{2,4})$/);
  if (br) {
    const day = Number(br[1]);
    const month = Number(br[2]);
    const year = Number(br[3].length === 2 ? `20${br[3]}` : br[3]);
    const ts = Date.UTC(year, month - 1, day) / 1000;
    return Number.isFinite(ts) ? Math.floor(ts) : 0;
  }
  return 0;
}

function normalizeTransactionSide(tx = {}, quantity = 0) {
  const operation = classifyCorporateOperation(tx.operation || tx.side || tx.tipo || tx.type, { isSell: Boolean(tx.isSell || tx.is_sell), quantity });
  return operation.reducesPosition ? 'SELL' : operation.increasesPosition ? 'BUY' : 'NEUTRAL';
}

export function normalizePortfolioTransactions(input = {}) {
  const rows = parseArrayInput(input.transactions);
  return rows.map(tx => {
    const ticker = canonicalTicker(tx.ticker || tx.symbol || tx.codigo || tx.ativo);
    const rawQuantity = toNumber(tx.quantity ?? tx.qtd ?? tx.shares ?? tx.quantidade, 0);
    const classified = classifyCorporateOperation(tx.operation || tx.side || tx.tipo || tx.type, { isSell: Boolean(tx.isSell || tx.is_sell), quantity: rawQuantity });
    const side = normalizeTransactionSide(tx, rawQuantity);
    const quantity = Math.abs(rawQuantity);
    const price = toNumber(tx.price ?? tx.preco ?? tx.unitPrice ?? tx.precoUnitario, 0);
    const grossValue = Math.abs(toNumber(tx.grossValue ?? tx.total ?? tx.valor ?? tx.value, 0)) || Number((quantity * price).toFixed(2));
    const timestamp = parsePortfolioDateSeconds(tx.timestamp ?? tx.time ?? tx.date ?? tx.data ?? tx.firstPurchaseAt);
    return {
      ticker,
      quantity,
      price,
      grossValue,
      timestamp,
      date: timestamp ? new Date(timestamp * 1000).toISOString().slice(0, 10) : String(tx.date || tx.data || ''),
      side,
      operation: tx.operation || tx.side || undefined,
      operationCode: classified.code,
      quantityEffect: classified.quantityEffect,
      costEffect: classified.costEffect,
      assetClass: tx.assetClass || tx.assetType || tx.type || undefined,
      source: tx.source || undefined,
    };
  }).filter(tx => tx.ticker && tx.quantity > 0 && tx.timestamp > 0)
    .sort((a, b) => a.timestamp - b.timestamp || a.ticker.localeCompare(b.ticker));
}

function mergeHistoryPositions(activePositions = [], transactions = []) {
  const byTicker = new Map();
  for (const p of activePositions) byTicker.set(p.ticker, { ...p, quantity: toNumber(p.quantity), averagePrice: toNumber(p.averagePrice), currentPrice: toNumber(p.currentPrice), firstPurchaseAt: toNumber(p.firstPurchaseAt, 0) });
  const txByTicker = new Map();
  for (const tx of transactions) {
    if (!txByTicker.has(tx.ticker)) txByTicker.set(tx.ticker, []);
    txByTicker.get(tx.ticker).push(tx);
  }
  for (const [ticker, txs] of txByTicker.entries()) {
    let quantity = 0;
    let cost = 0;
    let buyGross = 0;
    let buyQty = 0;
    let firstPurchaseAt = 0;
    for (const tx of txs.sort((a, b) => a.timestamp - b.timestamp)) {
      const next = applyOperationToPosition({ quantity, cost }, tx);
      quantity = next.quantity;
      cost = next.cost;
      if (next.operation.increasesPosition) firstPurchaseAt = firstPurchaseAt || tx.timestamp;
      if (next.operation.code === 'BUY' || next.operation.code === 'TRANSFER_IN') {
        buyQty += tx.quantity;
        buyGross += tx.grossValue || tx.quantity * tx.price;
      }
    }
    const existing = byTicker.get(ticker);
    const signedTransactionQuantity = txs.reduce((sum, tx) => sum + Number(tx.quantityEffect ?? classifyCorporateOperation(tx.operation, { quantity: tx.quantity }).quantityEffect) * tx.quantity, 0);
    const inferredOpeningQuantity = Math.max(0, toNumber(existing?.quantity, 0) - signedTransactionQuantity);
    const hasReconciledOpeningInventory = Boolean(existing) && inferredOpeningQuantity > 0.000001;
    const existingFirstSeconds = parsePortfolioDateSeconds(existing?.firstPurchaseAt);
    const knownFirstTransactionSeconds = parsePortfolioDateSeconds(firstPurchaseAt);
    const firstPurchaseBeforeImportedHistory = existingFirstSeconds > 0 && (
      knownFirstTransactionSeconds <= 0 || existingFirstSeconds < knownFirstTransactionSeconds
    );
    const effectiveFirstPurchaseAt = hasReconciledOpeningInventory
      ? (firstPurchaseBeforeImportedHistory ? existingFirstSeconds : 0)
      : (existingFirstSeconds || knownFirstTransactionSeconds || 0);
    const averagePrice = existing?.averagePrice || (quantity > 0 && cost > 0 ? cost / quantity : (buyQty > 0 ? buyGross / buyQty : 0));
    byTicker.set(ticker, {
      ticker,
      quantity: existing?.quantity ?? Math.max(0, quantity),
      averagePrice,
      currentPrice: existing?.currentPrice || 0,
      // Quando o histórico de lançamentos começa no meio da vida da posição, a primeira
      // transação conhecida não é a data de nascimento da carteira. Uma data explícita só
      // limita a série se for realmente anterior ao primeiro lançamento importado.
      firstPurchaseAt: effectiveFirstPurchaseAt,
      assetClass: existing?.assetClass || txs.find(t => t.assetClass)?.assetClass,
      name: existing?.name || ticker,
    });
  }
  return [...byTicker.values()].filter(p => p.ticker && (p.quantity > 0 || txByTicker.has(p.ticker)));
}

function historyCurrentPrice(history = {}, fallback = 0) {
  const lastPoint = Array.isArray(history?.points) ? history.points.filter(p => toNumber(p?.close, 0) > 0).at(-1) : null;
  const price = toNumber(history?.regularMarketPrice, 0) || toNumber(lastPoint?.close, 0) || toNumber(fallback, 0);
  return Number.isFinite(price) && price > 0 ? price : 0;
}

function enrichPositionsWithHistoryPrices(positions = [], histories = []) {
  return positions.map((position, index) => {
    const currentPrice = toNumber(position.currentPrice, 0);
    const remotePrice = historyCurrentPrice(histories[index], currentPrice);
    return {
      ...position,
      currentPrice: currentPrice > 0 ? currentPrice : remotePrice,
      currentPriceSource: currentPrice > 0 ? (position.currentPriceSource || 'payload') : (remotePrice > 0 ? 'YahooChartHistory' : position.currentPriceSource)
    };
  });
}

function currentAnchorPositions(activePositions = [], historyPositions = []) {
  const map = new Map();
  const enrichedByTicker = new Map(historyPositions.map(p => [p.ticker, p]));
  for (const p of activePositions) {
    if (!p?.ticker || toNumber(p.quantity) <= 0) continue;
    const enriched = enrichedByTicker.get(p.ticker) || {};
    map.set(p.ticker, {
      ...enriched,
      ...p,
      currentPrice: toNumber(p.currentPrice, 0) || toNumber(enriched.currentPrice, 0),
      currentPriceSource: toNumber(p.currentPrice, 0) > 0 ? (p.currentPriceSource || 'payload') : enriched.currentPriceSource
    });
  }
  for (const p of historyPositions) {
    if (!p?.ticker || toNumber(p.quantity) <= 0 || map.has(p.ticker)) continue;
    map.set(p.ticker, { ...p, currentAnchorFromHistory: true });
  }
  return [...map.values()].filter(p => p.ticker && toNumber(p.quantity) > 0);
}

function transactionOnlyTickers(activePositions = [], historyPositions = []) {
  const active = new Set(activePositions.map(p => p.ticker).filter(Boolean));
  return historyPositions.map(p => p.ticker).filter(ticker => ticker && !active.has(ticker));
}

function openingInventoryForPosition(position = {}, transactions = []) {
  if (!transactions.length) return { quantity: 0, cost: 0, reconciled: false };
  const currentQuantity = toNumber(position.quantity, 0);
  if (!(currentQuantity > 0)) return { quantity: 0, cost: 0, reconciled: false };
  const signedTransactionQuantity = transactions.reduce((sum, tx) => {
    const quantity = toNumber(tx.quantity, 0);
    return sum + Number(tx.quantityEffect ?? classifyCorporateOperation(tx.operation, { quantity }).quantityEffect) * quantity;
  }, 0);
  const openingQuantity = Number(Math.max(0, currentQuantity - signedTransactionQuantity).toFixed(8));
  if (!(openingQuantity > 0.000001)) return { quantity: 0, cost: 0, reconciled: false };
  const averagePrice = toNumber(position.averagePrice, 0);
  return {
    quantity: openingQuantity,
    cost: Number((openingQuantity * averagePrice).toFixed(8)),
    reconciled: true
  };
}

function stateAtTimestamp(position = {}, transactions = [], timestamp = 0, openingInventory = { quantity: 0, cost: 0 }) {
  if (!transactions.length) {
    const first = Number(position.firstPurchaseAt || 0);
    const firstSeconds = first > 9_999_999_999 ? Math.floor(first / 1000) : Math.floor(first || 0);
    if (firstSeconds > 0 && timestamp < firstSeconds) return { quantity: 0, cost: 0, reconciled: false };
    const quantity = toNumber(position.quantity);
    return { quantity, cost: Number((toNumber(position.averagePrice) * quantity).toFixed(2)), reconciled: false };
  }
  let quantity = toNumber(openingInventory.quantity, 0);
  let cost = toNumber(openingInventory.cost, 0);
  let reconciled = Boolean(openingInventory.reconciled);
  for (const tx of transactions) {
    if (tx.timestamp > timestamp) break;
    const next = applyOperationToPosition({ quantity, cost }, tx);
    quantity = next.quantity;
    cost = next.cost;
  }

  // A posição atual é a fonte de verdade para o estado terminal. Históricos importados podem
  // começar no meio da vida da carteira (por exemplo, apenas uma venda ou um aporte recente).
  // Depois do último lançamento conhecido, reconciliamos pequenas ou grandes lacunas para que
  // o último trecho remoto não termine numa quantidade diferente da carteira atual.
  const lastTransactionTimestamp = toNumber(transactions.at(-1)?.timestamp, 0);
  const currentQuantity = toNumber(position.quantity, 0);
  if (lastTransactionTimestamp > 0 && timestamp >= lastTransactionTimestamp && currentQuantity > 0 && Math.abs(quantity - currentQuantity) > 0.000001) {
    quantity = currentQuantity;
    cost = Number((toNumber(position.averagePrice, 0) * currentQuantity).toFixed(8));
    reconciled = true;
  }
  return {
    quantity: quantity > 0.000001 ? quantity : 0,
    cost: cost > 0.000001 ? cost : 0,
    reconciled
  };
}

function rangeSeconds(range = '1Y') {
  const key = String(range || '1Y').toLowerCase();
  if (key === '1d') return 6 * 60 * 60;
  if (key === '5d') return 5 * 24 * 60 * 60;
  if (key === '1mo' || key === '1m') return 31 * 24 * 60 * 60;
  if (key === '3mo' || key === '3m') return 93 * 24 * 60 * 60;
  if (key === '6mo' || key === '6m') return 186 * 24 * 60 * 60;
  if (key === '5y') return 5 * 366 * 24 * 60 * 60;
  if (key === 'max') return 5 * 366 * 24 * 60 * 60;
  return 366 * 24 * 60 * 60;
}

function positionInvestedValue(p = {}) {
  return Number((toNumber(p.averagePrice) * toNumber(p.quantity)).toFixed(2));
}

function positionCurrentValue(p = {}) {
  const currentPrice = toNumber(p.currentPrice, 0);
  return Number((currentPrice > 0 ? currentPrice * toNumber(p.quantity) : 0).toFixed(2));
}

function positionRealCurrentValue(p = {}) {
  const currentPrice = toNumber(p.currentPrice, 0);
  const quantity = toNumber(p.quantity, 0);
  return Number(((currentPrice > 0 && quantity > 0 ? currentPrice * quantity : 0)).toFixed(2));
}

function isIntradayPortfolioHistory(range = '', interval = '') {
  const r = String(range || '').toLowerCase();
  const i = String(interval || '').toLowerCase();
  return r === '1d' || r === '5d' || /(?:m|h)$/.test(i);
}

const B3_SAO_PAULO_CLOCK = new Intl.DateTimeFormat('en-CA', {
  timeZone: 'America/Sao_Paulo',
  weekday: 'short',
  year: 'numeric',
  month: '2-digit',
  day: '2-digit',
  hour: '2-digit',
  minute: '2-digit',
  hourCycle: 'h23'
});

function b3RegularSessionParts(timestamp = 0) {
  if (!Number.isFinite(Number(timestamp)) || Number(timestamp) <= 0) return null;
  const parts = Object.fromEntries(
    B3_SAO_PAULO_CLOCK.formatToParts(new Date(Number(timestamp) * 1000))
      .filter(part => part.type !== 'literal')
      .map(part => [part.type, part.value])
  );
  const hour = Number(parts.hour);
  const minute = Number(parts.minute);
  if (!Number.isFinite(hour) || !Number.isFinite(minute)) return null;
  return {
    weekday: String(parts.weekday || ''),
    dateKey: `${parts.year}-${parts.month}-${parts.day}`,
    minuteOfDay: hour * 60 + minute
  };
}

export function isB3RegularPortfolioSessionTimestamp(timestamp = 0) {
  const local = b3RegularSessionParts(Number(timestamp));
  if (!local || ['Sat', 'Sun'].includes(local.weekday) || !isB3TradingDay(local.dateKey)) return false;
  const bounds = getB3RegularSessionBounds(local.dateKey);
  return local.minuteOfDay >= bounds.openMinute && local.minuteOfDay <= bounds.closeMinute;
}

function filterB3RegularPortfolioSession(series = [], { range = '1Y', interval } = {}) {
  if (!isIntradayPortfolioHistory(range, interval)) return Array.isArray(series) ? series : [];
  return (Array.isArray(series) ? series : []).filter(row => {
    const timestamp = Number(row?.timestamp || pointTimestampSeconds(row) || 0);
    return isB3RegularPortfolioSessionTimestamp(timestamp);
  });
}

function pointTimestampSeconds(point = {}) {
  const explicit = Number(point.timestamp ?? point.time ?? 0);
  if (Number.isFinite(explicit) && explicit > 0) return Math.floor(explicit > 9_999_999_999 ? explicit / 1000 : explicit);
  const parsed = Date.parse(String(point.date || point.datetime || point.label || ''));
  return Number.isFinite(parsed) ? Math.floor(parsed / 1000) : 0;
}

function pointDateKeyFromTimestamp(timestamp = 0) {
  if (!Number.isFinite(timestamp) || timestamp <= 0) return '';
  return new Date(timestamp * 1000).toISOString().slice(0, 10);
}

function portfolioBucketEndTimestampSeconds(timestamp = 0, interval = '', intraday = false) {
  if (!Number.isFinite(timestamp) || timestamp <= 0 || intraday) return timestamp;
  const key = String(interval || '').toLowerCase();
  const date = new Date(timestamp * 1000);
  if (key === '1mo' || key === '1m' || key === '1month' || key === 'month') {
    return Math.floor(Date.UTC(date.getUTCFullYear(), date.getUTCMonth() + 1, 1) / 1000) - 1;
  }
  if (key === '1wk' || key === '1w' || key === 'week') {
    return timestamp + 7 * 24 * 60 * 60 - 1;
  }
  return timestamp + 24 * 60 * 60 - 1;
}

function isoWeekBucket(date) {
  const current = new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate()));
  const weekday = current.getUTCDay() || 7;
  current.setUTCDate(current.getUTCDate() + 4 - weekday);
  const yearStart = new Date(Date.UTC(current.getUTCFullYear(), 0, 1));
  const week = Math.ceil((((current - yearStart) / 86400000) + 1) / 7);
  return `${current.getUTCFullYear()}-W${String(week).padStart(2, '0')}`;
}

export function portfolioHistoryBucketKey(timestamp = 0, { range = '1Y', interval } = {}) {
  const intraday = isIntradayPortfolioHistory(range, interval);
  if (!Number.isFinite(timestamp) || timestamp <= 0) return 'invalid';
  if (intraday) {
    const seconds = Math.max(60, portfolioIntervalSeconds(interval));
    return `i:${Math.floor(timestamp / seconds)}`;
  }
  const date = new Date(timestamp * 1000);
  const key = String(interval || '').toLowerCase();
  if (key === '1mo' || key === '1m' || key === '1month' || key === 'month') {
    return `${date.getUTCFullYear()}-${String(date.getUTCMonth() + 1).padStart(2, '0')}`;
  }
  if (key === '1wk' || key === '1w' || key === 'week') return isoWeekBucket(date);
  return date.toISOString().slice(0, 10);
}

function portfolioInceptionTimestampSeconds(normalized = [], transactions = []) {
  const transactionsByTicker = new Map();
  for (const tx of transactions) {
    if (!transactionsByTicker.has(tx.ticker)) transactionsByTicker.set(tx.ticker, []);
    transactionsByTicker.get(tx.ticker).push(tx);
  }
  const hasUnknownOpeningInventory = normalized.some(position => {
    const explicitStart = parsePortfolioDateSeconds(position?.firstPurchaseAt);
    const rows = transactionsByTicker.get(position?.ticker) || [];
    const opening = openingInventoryForPosition(position, rows);
    return explicitStart <= 0 && toNumber(opening?.quantity, 0) > 0.000001;
  });
  if (hasUnknownOpeningInventory) return 0;
  const candidates = [
    ...transactions.map(tx => parsePortfolioDateSeconds(tx?.timestamp || tx?.date)),
    ...normalized.map(position => parsePortfolioDateSeconds(position?.firstPurchaseAt))
  ].filter(value => Number.isFinite(value) && value > 0);
  return candidates.length ? Math.min(...candidates) : 0;
}

export function portfolioHistoryPointBelongsToLifetime(timestamp = 0, inceptionTimestamp = 0, { range = '1Y', interval } = {}) {
  if (!(inceptionTimestamp > 0)) return true;
  const intraday = isIntradayPortfolioHistory(range, interval);
  return portfolioBucketEndTimestampSeconds(timestamp, interval, intraday) >= inceptionTimestamp;
}

function normalizePortfolioHistoryPointsForPosition(history = {}, position = {}, { intraday = false, interval = '' } = {}) {
  const firstPurchaseAt = Number(position.firstPurchaseAt || 0);
  const firstPurchaseSeconds = firstPurchaseAt > 0 ? Math.floor(firstPurchaseAt > 9_999_999_999 ? firstPurchaseAt / 1000 : firstPurchaseAt) : 0;
  return (history?.points || [])
    .map(point => {
      const timestamp = pointTimestampSeconds(point);
      const close = toNumber(point.close, NaN);
      if (!timestamp || !Number.isFinite(close) || close <= 0) return null;
      const periodEndTimestamp = portfolioBucketEndTimestampSeconds(timestamp, interval, intraday);
      // Em intervalos mensal/semanal o Yahoo rotula o candle pelo início do período,
      // embora o close represente o fechamento. A compra dentro do mês deve compor esse fechamento.
      if (firstPurchaseSeconds > 0 && periodEndTimestamp < firstPurchaseSeconds) return null;
      const dateKey = pointDateKeyFromTimestamp(timestamp);
      const bucketTimestamp = intraday ? timestamp : Math.floor(Date.parse(`${dateKey}T00:00:00Z`) / 1000);
      return {
        timestamp: bucketTimestamp,
        date: intraday ? new Date(timestamp * 1000).toISOString() : dateKey,
        close
      };
    })
    .filter(Boolean)
    .sort((a, b) => a.timestamp - b.timestamp)
    .filter((point, index, array) => index === 0 || point.timestamp !== array[index - 1].timestamp || point.close !== array[index - 1].close);
}

// seedPortfolioHistorySeriesFromHistories: cria série real baseada nas cotações remotas antes do fallback sintético.
// enrichPositionsWithRemoteCurrentPrices: mantém currentPrice enriquecido por histórico/quote remotos quando disponíveis.
function buildMergedPortfolioSeries(histories = [], normalized = [], { range = '1Y', interval, transactions = [] } = {}) {
  const intraday = isIntradayPortfolioHistory(range, interval);
  const transactionsByTicker = new Map();
  for (const tx of transactions) {
    if (!transactionsByTicker.has(tx.ticker)) transactionsByTicker.set(tx.ticker, []);
    transactionsByTicker.get(tx.ticker).push(tx);
  }
  for (const txs of transactionsByTicker.values()) txs.sort((a, b) => a.timestamp - b.timestamp);
  const perPosition = histories.map((history, index) => {
    const position = normalized[index];
    const positionTransactions = transactionsByTicker.get(position?.ticker) || [];
    const points = normalizePortfolioHistoryPointsForPosition(history, position, { intraday, interval });
    const explicitFirstPurchaseAt = parsePortfolioDateSeconds(position?.firstPurchaseAt);
    return {
      position,
      transactions: positionTransactions,
      openingInventory: openingInventoryForPosition(position, positionTransactions),
      points,
      firstRealQuoteAt: Number(points[0]?.timestamp || 0),
      explicitFirstPurchaseAt
    };
  }).filter(row => row.position);
  if (!perPosition.length) return [];

  // Os timestamps continuam vindo exclusivamente de cotações reais. Porém, uma posição
  // temporariamente sem série não pode apagar todos os outros ativos do fechamento mensal.
  // Nessa componente isolada, preservamos o custo contábil até a fonte voltar, sinalizando
  // partialValuation no ponto em vez de fabricar uma curva para a carteira inteira.
  const timestamps = [...new Set(perPosition.flatMap(row => row.points.map(point => point.timestamp)))]
    .filter(value => Number.isFinite(value) && value > 0)
    .sort((a, b) => a - b);
  if (!timestamps.length) return [];

  const indices = new Array(perPosition.length).fill(0);
  const rows = [];
  for (const timestamp of timestamps) {
    let totalValue = 0;
    let investedValue = 0;
    const positions = {};
    const investedByTicker = {};
    let included = 0;
    let realValuationIncluded = 0;
    let expectedIncluded = 0;
    const unavailableValuationTickers = [];
    const excludedBeforeHistoryTickers = [];
    const stateTimestamp = portfolioBucketEndTimestampSeconds(timestamp, interval, intraday);
    const states = perPosition.map(entry => stateAtTimestamp(entry.position, entry.transactions, stateTimestamp, entry.openingInventory));
    const expectedAtTimestamp = perPosition.map((entry, idx) => {
      const quantity = toNumber(states[idx]?.quantity);
      if (!(quantity > 0)) return false;

      // Sem data de aquisição ou lançamentos que provem a posse antiga, o primeiro candle
      // real é o único marco confiável para incluir o ativo na composição histórica. Isso
      // impede que um ticker recém-listado ou com série curta apague todos os meses anteriores
      // dos demais ativos, sem inventar preço nem custo de mercado para o período ausente.
      const inferredStart = Number(entry.firstRealQuoteAt || 0);
      const explicitStart = Number(entry.explicitFirstPurchaseAt || 0);
      const hasExplicitChronology = explicitStart > 0;
      const beforeKnownOwnership = hasExplicitChronology && stateTimestamp < explicitStart;
      const beforeFirstRealQuote = !intraday && !hasExplicitChronology && inferredStart > 0 && timestamp < inferredStart;
      if (beforeKnownOwnership || beforeFirstRealQuote) {
        if (beforeFirstRealQuote) excludedBeforeHistoryTickers.push(entry.position.ticker);
        return false;
      }
      return true;
    });
    expectedAtTimestamp.forEach(expected => {
      if (expected) expectedIncluded += 1;
    });
    perPosition.forEach((entry, idx) => {
      const points = entry.points;
      while (indices[idx] + 1 < points.length && points[indices[idx] + 1].timestamp <= timestamp) indices[idx] += 1;
      const point = points[indices[idx]];
      const state = states[idx];
      const quantity = toNumber(state.quantity);
      if (!(quantity > 0) || expectedAtTimestamp[idx] !== true) return;
      const invested = Number((state.cost || toNumber(entry.position.averagePrice) * quantity).toFixed(2));
      const price = point && point.timestamp <= timestamp ? toNumber(point.close) : 0;
      const hasRealValuation = price > 0;
      if (!hasRealValuation) {
        unavailableValuationTickers.push(entry.position.ticker);
        return;
      }
      const value = Number((price * quantity).toFixed(2));
      totalValue = Number((totalValue + value).toFixed(2));
      investedValue = Number((investedValue + invested).toFixed(2));
      positions[entry.position.ticker] = value;
      investedByTicker[entry.position.ticker] = invested;
      included += 1;
      if (hasRealValuation) realValuationIncluded += 1;
    });
    if (expectedIncluded > 0 && included < expectedIncluded) continue;
    if (included > 0 && totalValue > 0) {
      const rowInvestedValue = investedValue || Number(normalized.reduce((sum, p) => sum + positionInvestedValue(p), 0).toFixed(2));
      const pnl = totalValue - rowInvestedValue;
      const returnPercent = rowInvestedValue ? Number(((pnl / rowInvestedValue) * 100).toFixed(2)) : undefined;
      rows.push({
        date: intraday ? new Date(timestamp * 1000).toISOString() : pointDateKeyFromTimestamp(timestamp),
        timestamp,
        totalValue,
        investedValue: rowInvestedValue,
        costBasis: rowInvestedValue,
        positions,
        investedByTicker,
        unrealizedPnL: Number(pnl.toFixed(2)),
        unrealizedPnLPct: returnPercent,
        unrealizedPnLPercent: returnPercent,
        returnPercent,
        returnPct: returnPercent,
        source: intraday ? 'YahooChartIntraday' : (String(interval || '').toLowerCase().includes('mo') ? 'YahooChartMonthly' : 'YahooChartDaily'),
        partialValuation: unavailableValuationTickers.length > 0,
        historicalCompositionInferred: excludedBeforeHistoryTickers.length > 0,
        excludedBeforeHistoryTickers: [...new Set(excludedBeforeHistoryTickers)],
        completeValuation: unavailableValuationTickers.length === 0 && realValuationIncluded === expectedIncluded,
        valuationCoveragePercent: expectedIncluded > 0
          ? Number(((realValuationIncluded / expectedIncluded) * 100).toFixed(4))
          : 100,
        expectedValuationPositions: expectedIncluded,
        realValuationPositions: realValuationIncluded,
        unavailableValuationTickers: [...new Set(unavailableValuationTickers)],
        transactionReconciled: states.some(state => state.reconciled === true)
      });
    }
  }
  return rows;
}

function alignIntradaySeriesToCurrentPortfolioValue(series = [], normalized = [], { range = '1Y', interval, enabled = false } = {}) {
  // O histórico bruto permanece fiel à fonte. O alinhamento é opt-in e usado somente
  // pelo gráfico agregado da carteira, reproduzindo o comportamento funcional do baseline.
  if (!enabled || !isIntradayPortfolioHistory(range, interval) || !Array.isArray(series) || series.length < 2) return series;
  const currentValue = Number(normalized.reduce((sum, p) => sum + positionCurrentValue(p), 0).toFixed(2));
  if (!Number.isFinite(currentValue) || currentValue <= 0) return series;
  const last = series[series.length - 1];
  const lastValue = toNumber(last?.totalValue, 0);
  if (!(lastValue > 0)) return series;
  const gap = Math.abs(currentValue - lastValue) / Math.max(currentValue, lastValue);
  const rangeKey = String(range || '').toLowerCase();
  const tolerance = rangeKey === '1d' ? 0.003 : 0.01;
  if (gap <= tolerance) return series;
  const ratio = currentValue / lastValue;
  if (!Number.isFinite(ratio) || ratio <= 0 || ratio < 0.5 || ratio > 2.0) return series;
  return series.map(row => {
    const totalValue = Number((toNumber(row.totalValue, 0) * ratio).toFixed(2));
    const investedValue = Number(toNumber(row.investedValue ?? row.costBasis, 0).toFixed(2));
    const positions = Object.fromEntries(Object.entries(row.positions || {}).map(([ticker, value]) => [ticker, Number((toNumber(value, 0) * ratio).toFixed(2))]));
    const pnl = Number((totalValue - investedValue).toFixed(2));
    const returnPercent = investedValue ? Number(((pnl / investedValue) * 100).toFixed(2)) : undefined;
    return {
      ...row,
      totalValue,
      positions,
      unrealizedPnL: pnl,
      unrealizedPnLPct: returnPercent,
      unrealizedPnLPercent: returnPercent,
      returnPercent,
      returnPct: returnPercent,
      liveAligned: true,
      liveAlignmentRatio: Number(ratio.toFixed(8))
    };
  });
}

function stabilizeIntradayEdgePoints(series = [], { range = '1Y', interval } = {}) {
  if (!isIntradayPortfolioHistory(range, interval) || !Array.isArray(series) || series.length < 4) return series;
  if (String(series.at(-1)?.source || '').toLowerCase() === 'currentprice') return series;
  let clean = series.filter(row => row && toNumber(row.totalValue, 0) > 0).sort((a, b) => Number(a.timestamp || pointTimestampSeconds(a) || 0) - Number(b.timestamp || pointTimestampSeconds(b) || 0));
  if (clean.length < 4) return clean;

  const values = clean.map(row => toNumber(row.totalValue, 0)).filter(value => value > 0).sort((a, b) => a - b);
  const median = values[Math.floor(values.length / 2)] || 0;
  if (!(median > 0)) return clean;

  const adjacentGaps = clean.slice(0, -1).map((row, index) => {
    const leftValue = toNumber(row?.totalValue, 0);
    const rightValue = toNumber(clean[index + 1]?.totalValue, 0);
    return leftValue > 0 && rightValue > 0 ? Math.abs(rightValue - leftValue) / Math.max(leftValue, rightValue) : 0;
  }).filter(Number.isFinite).sort((a, b) => a - b);
  const typicalGap = adjacentGaps[Math.floor(adjacentGaps.length / 2)] || 0;
  const adaptiveEdgeLimit = Math.min(0.18, Math.max(0.035, typicalGap * 7));

  const edgeIsolated = (edge, neighbor, nextNeighbor) => {
    const edgeValue = toNumber(edge?.totalValue, 0);
    const neighborValue = toNumber(neighbor?.totalValue, 0);
    const nextValue = toNumber(nextNeighbor?.totalValue, 0);
    if (!(edgeValue > 0 && neighborValue > 0)) return false;
    const edgeGap = Math.abs(edgeValue - neighborValue) / Math.max(edgeValue, neighborValue);
    const neighborGap = nextValue > 0 ? Math.abs(neighborValue - nextValue) / Math.max(neighborValue, nextValue) : 0;
    const medianGap = Math.abs(edgeValue - median) / Math.max(edgeValue, median);
    return edgeGap > adaptiveEdgeLimit && medianGap > adaptiveEdgeLimit && neighborGap < Math.max(0.12, adaptiveEdgeLimit * 2);
  };

  if (edgeIsolated(clean[0], clean[1], clean[2])) clean = clean.slice(1);
  if (clean.length >= 4 && edgeIsolated(clean[clean.length - 1], clean[clean.length - 2], clean[clean.length - 3])) clean = clean.slice(0, -1);
  return clean;
}

function portfolioIntervalSeconds(interval = '') {
  const raw = String(interval || '').trim().toLowerCase();
  const match = raw.match(/^(\d+)(m|h|d|wk|mo)$/);
  if (!match) return 300;
  const amount = Math.max(1, Number(match[1]) || 1);
  const unit = match[2];
  if (unit === 'm') return amount * 60;
  if (unit === 'h') return amount * 3600;
  if (unit === 'd') return amount * 86400;
  if (unit === 'wk') return amount * 7 * 86400;
  return amount * 30 * 86400;
}

function appendCurrentPortfolioPoint(series = [], normalized = [], { range = '1Y', interval } = {}) {
  // O intraday representa somente o pregão regular da B3. Pré-abertura e after-market não podem
  // alterar o desenho nem o valor de encerramento do gráfico "Preço da carteira".
  const intraday = isIntradayPortfolioHistory(range, interval);
  const regularSeries = filterB3RegularPortfolioSession(series, { range, interval });
  const nowSeconds = Math.floor(Date.now() / 1000);
  if (intraday && !isB3RegularPortfolioSessionTimestamp(nowSeconds)) return regularSeries;

  // O fechamento atual deve refletir somente cotações reais. Custo médio continua sendo
  // custo e nunca substitui silenciosamente uma cotação ausente.
  const currentValue = Number(normalized.reduce((sum, p) => sum + positionRealCurrentValue(p), 0).toFixed(2));
  if (!Number.isFinite(currentValue) || currentValue <= 0) return series;
  const investedValue = Number(normalized.reduce((sum, p) => sum + positionInvestedValue(p), 0).toFixed(2));
  const currentPositions = {};
  const currentInvestedByTicker = {};
  const unavailableValuationTickers = [];
  let realValuationPositions = 0;
  let expectedValuationPositions = 0;
  for (const p of normalized) {
    const quantity = toNumber(p?.quantity, 0);
    if (!(quantity > 0) || !p?.ticker) continue;
    expectedValuationPositions += 1;
    const value = positionRealCurrentValue(p);
    const invested = positionInvestedValue(p);
    if (value > 0) {
      currentPositions[p.ticker] = value;
      realValuationPositions += 1;
    } else {
      unavailableValuationTickers.push(p.ticker);
    }
    if (invested > 0) currentInvestedByTicker[p.ticker] = invested;
  }
  const completeValuation = expectedValuationPositions > 0 && realValuationPositions === expectedValuationPositions;
  if (!completeValuation) return series;
  const now = new Date(nowSeconds * 1000);
  const date = now.toISOString().slice(0, 10);
  const pnl = currentValue - investedValue;
  const returnPercent = investedValue ? Number(((pnl / investedValue) * 100).toFixed(2)) : undefined;
  const currentRow = {
    date: isIntradayPortfolioHistory(range, interval) ? now.toISOString() : date,
    timestamp: nowSeconds,
    totalValue: currentValue,
    investedValue,
    costBasis: investedValue,
    positions: currentPositions,
    investedByTicker: currentInvestedByTicker,
    unrealizedPnL: Number(pnl.toFixed(2)),
    unrealizedPnLPct: returnPercent,
    unrealizedPnLPercent: returnPercent,
    returnPercent,
    returnPct: returnPercent,
    source: completeValuation ? 'currentPrice' : 'currentPricePartial',
    partialValuation: !completeValuation,
    completeValuation,
    valuationCoveragePercent: expectedValuationPositions > 0
      ? Number(((realValuationPositions / expectedValuationPositions) * 100).toFixed(4))
      : 0,
    expectedValuationPositions,
    realValuationPositions,
    unavailableValuationTickers: [...new Set(unavailableValuationTickers)]
  };
  const cleanSeries = regularSeries.filter(row => row && Number(row.totalValue) > 0);
  if (!cleanSeries.length) return [currentRow];
  let clean;
  if (intraday) {
    const last = cleanSeries[cleanSeries.length - 1];
    const lastTimestamp = Number(last.timestamp || pointTimestampSeconds(last));
    clean = cleanSeries.slice();
    // Substitui apenas o bucket ainda aberto. Um ponto remoto de 15 minutos atrás não deve
    // desaparecer em um gráfico de 5 minutos quando o ponto vivo é anexado.
    const replacementWindowSeconds = Math.min(15 * 60, Math.max(60, Math.round(portfolioIntervalSeconds(interval) * 1.5)));
    if (Number.isFinite(lastTimestamp) && Math.abs(nowSeconds - lastTimestamp) <= replacementWindowSeconds) {
      clean[clean.length - 1] = { ...currentRow, timestamp: Math.max(lastTimestamp, nowSeconds) };
      return clean.sort((a, b) => Number(a.timestamp || 0) - Number(b.timestamp || 0));
    }
    return [...clean, currentRow].sort((a, b) => Number(a.timestamp || 0) - Number(b.timestamp || 0));
  }

  const currentBucket = portfolioHistoryBucketKey(nowSeconds, { range, interval });
  clean = cleanSeries.filter(row => {
    const timestamp = Number(row.timestamp || pointTimestampSeconds(row) || 0);
    return portfolioHistoryBucketKey(timestamp, { range, interval }) !== currentBucket;
  });
  return [...clean, currentRow].sort((a, b) => Number(a.timestamp || pointTimestampSeconds(a) || 0) - Number(b.timestamp || pointTimestampSeconds(b) || 0));
}

function summary(series = [], investedValue = 0) {
  if (!series.length) return { points: 0, investedValue };
  const first = series[0];
  const last = series[series.length - 1];
  const values = series.map(p => p.totalValue).filter(Number.isFinite);
  const min = Math.min(...values);
  const max = Math.max(...values);
  const finalInvestedValue = Number((last.investedValue || investedValue || 0).toFixed(2));
  const pnl = last.totalValue - finalInvestedValue;
  return {
    points: series.length,
    investedValue: finalInvestedValue,
    firstValue: first.totalValue,
    lastValue: last.totalValue,
    minValue: min,
    maxValue: max,
    unrealizedPnL: Number(pnl.toFixed(2)),
    unrealizedPnLPct: finalInvestedValue ? Number(((pnl / finalInvestedValue) * 100).toFixed(2)) : undefined,
    periodVariationPct: first.totalValue ? Number((((last.totalValue - first.totalValue) / first.totalValue) * 100).toFixed(2)) : undefined,
  };
}

export async function buildPortfolioHistory(positions, options = {}) {
  const explicitPositions = normalizePortfolioPositions({ positions });
  const optionPositions = explicitPositions.length ? [] : normalizePortfolioPositions(options);
  const activePositions = explicitPositions.length ? explicitPositions : optionPositions;
  const transactions = normalizePortfolioTransactions({ transactions: options.transactions || [] });
  const normalizedBase = mergeHistoryPositions(activePositions, transactions);
  const range = options.range || '1Y';
  const interval = options.interval;
  const timeoutMs = Number(options.timeoutMs || 9000);
  const maxConcurrency = Math.max(1, Math.min(Number(options.maxConcurrency || 4), 8, normalizedBase.length || 1));
  const started = performance.now();
  const histories = new Array(normalizedBase.length);
  const errors = [];
  let cursor = 0;
  async function worker() {
    while (cursor < normalizedBase.length) {
      const i = cursor++;
      const p = normalizedBase[i];
      const h = await fetchYahooHistory(p.ticker, { range, interval, timeoutMs, limit: options.limit });
      if (!h.ok) errors.push({ ticker: p.ticker, error: h.error });
      histories[i] = h;
    }
  }
  await Promise.all(Array.from({ length: maxConcurrency }, () => worker()));

  const normalized = enrichPositionsWithHistoryPrices(normalizedBase, histories);
  const currentPositions = currentAnchorPositions(activePositions, normalized);
  const successfulHistoryTickers = normalized.filter((_, index) => Array.isArray(histories[index]?.points) && histories[index].points.length > 0).map(p => p.ticker);
  const missingHistoryTickers = normalized.map(p => p.ticker).filter(ticker => !successfulHistoryTickers.includes(ticker));
  const reconciledTransactionTickers = normalized
    .filter(position => openingInventoryForPosition(position, transactions.filter(tx => tx.ticker === position.ticker)).reconciled)
    .map(position => position.ticker);
  const investedValue = Number(currentPositions.reduce((sum, p) => sum + p.quantity * p.averagePrice, 0).toFixed(2));
  let series = buildMergedPortfolioSeries(histories, normalized, { range, interval, transactions });
  series = filterB3RegularPortfolioSession(series, { range, interval });
  series = alignIntradaySeriesToCurrentPortfolioValue(series, currentPositions, {
    range,
    interval,
    enabled: ['1', 'true', 'yes', 'sim', 'on'].includes(String(options.liveAlignment ?? options.alignCurrentValue ?? '').trim().toLowerCase())
  });
  // Limpa apenas as bordas remotas. O ponto vivo oficial é anexado depois e nunca pode ser
  // removido como se fosse um outlier, pois ele representa a composição/cotação atual recebida do APK.
  series = stabilizeIntradayEdgePoints(series, { range, interval });
  series = appendCurrentPortfolioPoint(series, currentPositions, { range, interval });
  series = filterB3RegularPortfolioSession(series, { range, interval });
  const portfolioInceptionTimestamp = portfolioInceptionTimestampSeconds(normalized, transactions);
  series = series.filter(row => {
    const timestamp = Number(row?.timestamp || pointTimestampSeconds(row) || 0);
    return portfolioHistoryPointBelongsToLifetime(timestamp, portfolioInceptionTimestamp, { range, interval });
  });
  // “Patrimônio total” aceita somente fechamentos com 100% das posições ativas avaliadas por
  // cotação real. Custo contábil e séries sintéticas permanecem fora do contrato de mercado.
  series = series.filter(row => {
    const coverage = Number(row?.valuationCoveragePercent ?? (row?.completeValuation === true ? 100 : 0));
    return row?.completeValuation === true && row?.partialValuation !== true && coverage >= 99.999 && row?.fallback !== true && Number(row?.totalValue) > 0;
  });
  const remotePointCount = series.filter(row => !/^currentPrice/i.test(String(row?.source || ''))).length;
  return {
    ok: series.length > 0,
    version: PORTFOLIO_HISTORY_VERSION,
    source: remotePointCount > 0 ? 'YahooChart+CurrentPrice' : (series.length ? 'CurrentPrice' : 'Unavailable'),
    routeEngine: 'VALORAE_PORTFOLIO_HISTORY_REBUILD_V292',
    engine: 'portfolio-history-rebuild-v292',
    range,
    interval: interval || undefined,
    positions: normalized,
    count: currentPositions.length,
    activeTickers: currentPositions.map(p => p.ticker),
    historyTickers: normalized.map(p => p.ticker),
    transactionOnlyTickers: transactionOnlyTickers(activePositions, normalized),
    transactionCount: transactions.length,
    portfolioInceptionTimestamp: portfolioInceptionTimestamp || undefined,
    portfolioInceptionDate: portfolioInceptionTimestamp
      ? new Date(portfolioInceptionTimestamp * 1000).toISOString().slice(0, 10)
      : undefined,
    ignoredTransactionCount: 0,
    successfulHistoryTickers,
    missingHistoryTickers,
    historyCoveragePercent: normalized.length ? Number(((successfulHistoryTickers.length / normalized.length) * 100).toFixed(2)) : 0,
    reconciledTransactionTickers,
    remotePointCount,
    fallbackUsed: false,
    partialValuationUsed: series.some(row => row.partialValuation === true),
    partialValuationTickers: [...new Set(series.flatMap(row => row.unavailableValuationTickers || []))],
    historicalCompositionInferred: series.some(row => row.historicalCompositionInferred === true),
    inferredHistoryStartTickers: [...new Set(series.flatMap(row => row.excludedBeforeHistoryTickers || []))],
    summary: summary(series, investedValue),
    series,
    points: series,
    history: series,
    errors,
    durationMs: Math.round(performance.now() - started),
  };
}
