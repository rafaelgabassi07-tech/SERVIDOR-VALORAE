...
export const PORTFOLIO_HISTORY_VERSION = '21.12.353-asset-modal-required-sections-hardening-v321';
...
function mergeHistoryPositions(activePositions = [], transactions = []) {
  ...
  for (const [ticker, txs] of txByTicker.entries()) {
    ...
    byTicker.set(ticker, {
      ...
      // transação conhecida não é a data de nascimento da carteira. Mantemos início aberto
      // para aproveitar as cotações remotas anteriores e reconstruir o estoque inicial.
      firstPurchaseAt: existing?.firstPurchaseAt || (hasReconciledOpeningInventory ? 0 : firstPurchaseAt) || 0,
...
function buildMergedPortfolioSeries(histories = [], normalized = [], { range = '1Y', interval, transactions = [] } = {}) {
  ...
  const perPosition = histories.map((history, index) => {
    ...
    return {
      ...
      points: normalizePortfolioHistoryPointsForPosition(history, position, { intraday })
    };
  ...
  for (const timestamp of timestamps) {
    ...
    const states = perPosition.map(entry => stateAtTimestamp(entry.position, entry.transactions, timestamp, entry.openingInventory));
