...
export const PORTFOLIO_HISTORY_VERSION = '21.12.320-engine-hardening-v291';
...
function normalizeTransactionSide(tx = {}) {
...
export function normalizePortfolioTransactions(input = {}) {
  ...
  return rows.map(tx => {
    ...
    const quantity = toNumber(tx.quantity ?? tx.qtd ?? tx.shares ?? tx.quantidade, 0);
    const price = toNumber(tx.price ?? tx.preco ?? tx.unitPrice ?? tx.precoUnitario, 0);
    const grossValue = toNumber(tx.grossValue ?? tx.total ?? tx.valor ?? tx.value, 0) || Number((quantity * price).toFixed(2));
    ...
    return {
      ...
      side: normalizeTransactionSide(tx),
...
function latestHistoryPrice(history = {}) {
  const lastPoint = [...(history?.points || [])].reverse().find(p => toNumber(p?.close, 0) > 0);
  const lastPointPrice = toNumber(lastPoint?.close, 0);
  const summaryPrice = toNumber(history?.summary?.lastClose, 0);
  const regularPrice = toNumber(history?.regularMarketPrice, 0);
  return regularPrice > 0 ? regularPrice : (lastPointPrice > 0 ? lastPointPrice : summaryPrice);
}

function enrichPositionsWithRemoteCurrentPrices(positions = [], histories = []) {
  return positions.map((position, index) => {
    const remotePrice = latestHistoryPrice(histories[index]);
    if (remotePrice > 0) {
      return { ...position, currentPrice: remotePrice, currentPriceSource: 'YahooChartLatestClose' };
    }
    return { ...position, currentPriceSource: position.currentPrice > 0 ? 'ClientProvided' : 'AveragePriceFallback' };
  });
}

function seedPortfolioHistorySeriesFromHistories(histories = [], positions = [], { range = '1Y', interval, transactions = [] } = {}) {
  ...
  histories.forEach((history, index) => {
    ...
    if (!position) return;
    const cleanPoints = normalizePortfolioHistoryPointsForPosition(history, position, { intraday });
    if (cleanPoints.length) {
      candidateRows.push(cleanPoints[0], cleanPoints[cleanPoints.length - 1]);
      return;
    }
    const lastPrice = latestHistoryPrice(history);
    const prev = toNumber(history?.previousClose, 0);
    const now = Math.floor(Date.now() / 1000);
    if (prev > 0) candidateRows.push({ timestamp: now - (intraday ? 6 * 60 * 60 : 24 * 60 * 60), date: new Date((now - 24 * 60 * 60) * 1000).toISOString(), close: prev });
    if (lastPrice > 0) candidateRows.push({ timestamp: now, date: new Date(now * 1000).toISOString(), close: lastPrice });
  });
  const timestamps = [...new Set(candidateRows.map(p => p.timestamp).filter(v => Number.isFinite(v) && v > 0))].sort((a, b) => a - b);
  ...
  return timestamps.map(timestamp => {
    ...
    histories.forEach((history, index) => {
      const position = positions[index];
      if (!position) return;
      const points = normalizePortfolioHistoryPointsForPosition(history, position, { intraday });
      let point = points.filter(p => p.timestamp <= timestamp).at(-1) || points[0];
      if (!point) {
        const price = timestamp < Math.floor(Date.now() / 1000) ? toNumber(history?.previousClose, 0) : latestHistoryPrice(history);
        if (price > 0) point = { timestamp, close: price };
      }
      if (!point || !(point.close > 0)) return;
      const state = stateAtTimestamp(position, transactionsByTicker.get(position.ticker) || [], timestamp);
      const quantity = toNumber(state.quantity || position.quantity);
      if (!(quantity > 0)) return;
      const value = Number((point.close * quantity).toFixed(2));
      const invested = Number((state.cost || positionInvestedValue({ ...position, quantity })).toFixed(2));
      totalValue = Number((totalValue + value).toFixed(2));
      investedValue = Number((investedValue + invested).toFixed(2));
      valuesByTicker[position.ticker] = value;
      investedByTicker[position.ticker] = invested;
    });
    ...
    return {
      ...
      source: 'YahooChartSeedSeries'
    };
  }).filter(Boolean).filter((row, index, array) => index === 0 || row.timestamp !== array[index - 1].timestamp || Math.abs(row.totalValue - array[index - 1].totalValue) > 0.0001);
}
...
function buildMergedPortfolioSeries(histories = [], normalized = [], { range = '1Y', interval, transactions = [] } = {}) {
  ...
  for (const timestamp of timestamps) {
    ...
    perPosition.forEach((entry, idx) => {
      ...
      const state = stateAtTimestamp(entry.position, entry.transactions, timestamp);
...
export async function buildPortfolioHistory(positions, options = {}) {
  const activePositions = normalizePortfolioPositions({ positions });
  const transactions = normalizePortfolioTransactions({ transactions: options.transactions || [] });
  const normalized = mergeHistoryPositions(activePositions, transactions);
  ...
  const marketPositions = enrichPositionsWithRemoteCurrentPrices(normalized, histories);
  const currentPositions = activePositions.length ? enrichPositionsWithRemoteCurrentPrices(activePositions, histories) : marketPositions.filter(p => p.quantity > 0);
  const investedValue = Number(currentPositions.reduce((sum, p) => sum + p.quantity * p.averagePrice, 0).toFixed(2));
  let series = buildMergedPortfolioSeries(histories, marketPositions, { range, interval, transactions });
  if (series.length < 2) series = seedPortfolioHistorySeriesFromHistories(histories, marketPositions, { range, interval, transactions });
  const remotePointCount = series.length;
  series = appendCurrentPortfolioPoint(series, currentPositions, { range, interval });
  if (series.length < 2) series = fallbackPortfolioHistorySeries(currentPositions.length ? currentPositions : marketPositions, range);
  return {
    ok: series.length > 0,
    version: PORTFOLIO_HISTORY_VERSION,
    source: remotePointCount > 0 ? 'YahooChart+CurrentPrice' : 'CurrentPriceFallback',
    engineHardening: 'PORTFOLIO_HISTORY_ENGINE_HARDENING_V291',
    range,
    ...
    remotePointCount,
    partial: errors.length > 0 && remotePointCount > 0,
    fallbackUsed: remotePointCount === 0 || series.some(row => row.fallback),
    quoteCoverage: { requested: normalized.length, withRemotePrice: marketPositions.filter(p => p.currentPriceSource === 'YahooChartLatestClose').length, errors: errors.length },
