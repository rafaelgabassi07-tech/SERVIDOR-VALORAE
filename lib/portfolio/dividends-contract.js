...
export async function buildDividendsContract(payload = {}) {
  ...
  return coalesce(cacheKey, async () => {
    ...
    const statusTimeout = Math.max(2400, Math.floor(timeoutMs * 0.70));
    const agendaTimeout = Math.max(2400, Math.floor(timeoutMs * 0.75));
    ...
    const [perTicker, agenda] = await Promise.all([
      mapLimit(tickers, statusInvestConcurrency(), t => getConfirmedDividendsByTicker(t, { timeoutMs: statusTimeout })),
      getAgendaDividends(tickers, {
        timeoutMs: agendaTimeout,
        agendaTimeoutMs: Number(payload.agendaTimeoutMs || agendaTimeout),
        futureMonths: payload.agendaMonthsAhead ?? payload.futureMonths ?? payload.monthsForward
      })
    ]);
