...
function dedupeEvents(events = []) {
  ...
  return [...byGeneric.values()].sort((a, b) => (dateMillis(a.paymentDate || a.eligibilityDate) || 0) - (dateMillis(b.paymentDate || b.eligibilityDate) || 0));
}
...
export async function buildDividendsContract(payload = {}) {
  ...
  const cacheKey = `dividends-contract:${stableKey({ tickers, mode, futureMonths: payload.futureMonths, historyMonths: payload.historyMonths, agendaMonthsAhead: payload.agendaMonthsAhead })}`;
