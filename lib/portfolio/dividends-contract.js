...
export async function buildDividendsContract(payload = {}) {
  ...
  return coalesce(cacheKey, async () => {
    ...
    const diagnostics = [];
    const all = [];
    const recoveryMode = String(payload.source || '').trim().toLowerCase() === 'home-agenda-recovery';
    ...
    diagnostics.push({
      ...
      sourcePolicy: recoveryMode
        ? 'Recuperacao mobile: StatusInvest primario por ativo; calendario publico desativado.'
