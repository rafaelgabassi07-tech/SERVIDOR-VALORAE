...
export async function buildDividendsContract(payload = {}) {
  ...
  return coalesce(cacheKey, async () => {
    ...
    const staleOfficialEvents = isPartial
      ? (staleCandidate?.value?.officialEvents || staleCandidate?.value?.events || staleCandidate?.value?.dividends || [])
      : [];
    const responseOfficialEvents = staleOfficialEvents.length
      ? dedupeEvents([...staleOfficialEvents, ...officialEvents])
      : officialEvents;
    if (staleOfficialEvents.length) {
      diagnostics.push({
        provider: 'valorae-cache',
        status: 'MERGED_PARTIAL_COVERAGE',
        liveCount: officialEvents.length,
        staleCount: staleOfficialEvents.length,
        mergedCount: responseOfficialEvents.length,
        reason: 'partial-live-preserve-history'
      });
    }
    ...
    const result = buildDividendResult({
      payload,
      tickers,
      officialEvents: responseOfficialEvents,
      diagnostics,
      cacheStatus: staleOfficialEvents.length ? 'LIVE_PLUS_STALE_COVERAGE' : 'LIVE',
      timedOut: statusBatch.timedOut || isPartial,
      agendaDeferred,
      skippedTickers: statusBatch.skipped,
      staleFallback: staleOfficialEvents.length > 0
    });

    if (responseOfficialEvents.length > 0) {
