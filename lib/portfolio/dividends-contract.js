...
const STOCK_UNITS_FOR_DIVIDEND_DEDUPE = new Set(['TAEE11', 'SANB11', 'KLBN11', 'ALUP11', 'BPAC11', 'IGTI11', 'ENGI11', 'CPLE11', 'AESB11', 'TIET11', 'SULA11']);

function isFiiLikeDividendEvent(event = {}) {
  const ticker = String(event.ticker || event.symbol || '').toUpperCase();
  const assetClass = String(event.assetClass || event.assetType || event.typeAsset || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toUpperCase();
  return assetClass.includes('FII') || assetClass.includes('FUNDO IMOB') || (ticker.endsWith('11') && !STOCK_UNITS_FOR_DIVIDEND_DEDUPE.has(ticker));
}
