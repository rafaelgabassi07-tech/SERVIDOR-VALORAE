...
function dedupeEvents(events = []) {
  ...
  // Rede final: se a data de pagamento vier ausente, usa ticker + elegibilidade + familia.
  const fuzzy = new Map();
...
function enrichPortfolio(event, payload = {}) {
  ...
  return {
    ...
    comDate: event.comDate || event.dateCom || event.dataCom || eligibility,
...
function buildDividendResult({
  ...
  const portfolioUpcoming = split.upcoming.filter(e => e.eligible || !eventEligibilityDate(e));
  const portfolioUpcomingAll = split.upcoming
    .filter(e => e.eligible || !eventEligibilityDate(e))
    .map(e => ({
      ...e,
