...
function eventEligibilityDate(event = {}) {
  return normalizeDate(event.eligibilityDate || event.dateCom || event.comDate || event.dataCom || event.exDate || event.dataEx || event.recordDate || event.referenceDate);
}
...
function identityKey(event = {}, includeType = true) {
  ...
  return [ticker, eventEligibilityDate(event), eventPaymentDate(event), type, eventValueKey(event)].join('|');
}
...
function dedupeEvents(events = []) {
  ...
  for (const event of byPayment.values()) {
    ...
    const value = Number(event.grossValuePerShare || event.declaredValuePerShare || event.netValuePerShare || event.valuePerShare || 0);
...
function paymentMergeKey(event = {}) {
  ...
  const payment = eventPaymentDate(event);
  const type = eventDividendFamily(event);
  const value = eventValueKey(event);
...
function buildDividendResult({
  ...
  return {
    ...
    sourceStatus,
    sourcePolicy: 'STATUSINVEST_PER_TICKER_FIRST_PLUS_INVESTIDOR10_CALENDAR_COMPLEMENT_TAX_AWARE_STALE_IF_ERROR',
    cacheStatus,
    generatedAt: new Date().toISOString(),
    asOf: todayIso(),
    tickers,
...
export async function buildDividendsContract(payload = {}) {
  ...
  return coalesce(cacheKey, async () => {
    ...
    if (duplicateSourceEvents > 0) {
      diagnostics.push({ provider: 'valorae-dedupe', status: 'OK', removedDuplicates: duplicateSourceEvents, message: 'Eventos duplicados normalizados antes de responder ao APK.' });
    }
