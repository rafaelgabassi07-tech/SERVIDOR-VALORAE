...
export function benchmarkAccumulatedMonthMap(
  ...
  // Prefer the exact common portfolio/index denominator. Some real index providers can
  // omit one month even while returning a valid continuous series afterwards. In that
  // case, use the latest real close before the first visible portfolio month; if none
  // exists, anchor the first common visible month at 0 instead of erasing the whole index.
  let base;
  if (comparisonBaseMonth && comparisonBaseMonth < firstVisibleMonth) {
    ...
    const previousRealClose = ordered.slice(0, Math.max(0, startIndex)).at(-1);
    base = exactBase?.value ?? previousRealClose?.value ?? 0;
  } else {
    // At inception the first visible point is a return interval, not a zero baseline.
