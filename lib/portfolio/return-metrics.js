...
export function benchmarkExposureAlignedMonthMap(
  ...
  let segmentBenchmarkBroken = false;
  let firstPortfolioRow = true;

  for (const row of portfolio) {
    const month = String(row.month);
    const segmentId = Number(row.chartSegmentId || 0);
    const segmentChanged = previousSegmentId === null || segmentId !== previousSegmentId;

    if (segmentChanged) {
      previousSegmentId = segmentId;
      previousMonth = '';
      segmentBenchmarkBroken = false;

      // A segment that begins after the first day of the month has no exact monthly benchmark
      // denominator. Do not pretend the benchmark was invested before the portfolio. The line may
      // resume on the next full month, but the summary must treat total benchmark coverage as partial.
      if (row.partialExposureMonth === true && row.segmentStart === true) {
        previousMonth = month;
        segmentBenchmarkBroken = true;
        firstPortfolioRow = false;
        continue;
      }
    }

    let baseMonth = previousMonth;
    if (!baseMonth) {
      baseMonth = firstPortfolioRow && comparisonBaseMonth
        ? comparisonBaseMonth
        : previousSourceMonth(month);
    }

    if (segmentBenchmarkBroken) {
      // Once the first partial month of this exposure segment lacks an exact benchmark base, do not
      // produce a misleading accumulated comparison for later months in the same segment.
      previousMonth = month;
      firstPortfolioRow = false;
      continue;
    }
