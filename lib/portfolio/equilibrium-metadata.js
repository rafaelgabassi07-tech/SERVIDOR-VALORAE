...
function normalizeAssetClassLabel(rawClass = '', ticker = '') {
  ...
  if (clean.includes('ETF') || ETF_TICKERS.has(t) || classified === 'ETF') return 'ETF';
  if (clean.includes('BDR') || /[A-Z]{4}3[0-9]$/.test(t) || classified === 'BDR') return 'BDR';
...
export function exposureLabel(ticker = '', position = {}) {
  ...
  if (FOREIGN_TICKERS.has(t) || /[A-Z]{4}3[0-9]$/.test(t)) return 'Exterior';
  return 'Nacional';
}
...
export function fiiTypeFor(ticker = '', position = {}) {
  ...
  if (/PAPEL|RECEBIVE|CRI|CRA|TITULO|TÍTULO/.test(raw) || FII_PAPER_PREFIXES.some(prefix => t.startsWith(prefix))) return 'Fundo de Papel';
...
export function buildEquilibriumContract(payload = {}) {
  ...
  const requiredCharts = [
    ...consolidated.charts.map(chart => chart.id),
    ...(actions.enabled ? actions.charts.map(chart => chart.id) : []),
    ...(fiisBlock.enabled ? fiisBlock.charts.map(chart => chart.id) : [])
  ];
  const deliveredCharts = allCharts.map(chart => chart.id);
  const missingCharts = requiredCharts.filter(id => !deliveredCharts.includes(id));
  return {
