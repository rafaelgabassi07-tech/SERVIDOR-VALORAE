...
export async function buildPortfolioReturns(payload = {}) {
  ...
  const marketBenchmarks = await Promise.all(requestedBenchmarks.filter(b => marketBenchmarkMap[b]).map(async (benchmark) => {
    ...
    const historyPayload = await getAssetHistory({
      ticker: marketBenchmarkMap[benchmark],
      range: historyRangeForMonths(benchmarkMonths),
      timeoutMs: Number(payload.indexTimeoutMs || payload.timeoutMs || 6500),
      limit: benchmarkLimit
    }).catch(error => ({ status: 'ERROR', points: [], error: error?.message }));
    const integrity = inspectRealHistoryIntegrity(historyPayload);
    const points = integrity.trusted
      ? comparisonPointsFromHistory(historyPayload).map(p => ({ ...p, month: normalizeReturnPointMonth(p), accumulatedPercent: Number(p.returnPercent || 0) })).filter(p => p.month)
      : [];
    const isOfficialB3Index = ['IBOV', 'SMLL', 'IFIX', 'IDIV'].includes(benchmark);
    return {
      ticker: benchmark,
      label: benchmark,
      status: points.length ? 'OK' : (integrity.trusted ? (historyPayload.status || 'EMPTY') : 'REJECTED'),
      points,
      source: historyPayload.source || (isOfficialB3Index ? `B3 Oficial - ${benchmark}` : 'YahooChart'),
