...
import { benchmarkAccumulatedMonthMap, selectPortfolioRowsForRange, summarizeReturnSeries } from './return-metrics.js';
...
function normalizeHistoryDividendEvents(rawEvents = [], positions = [], transactions = []) {
  ...
  const normalized = list.map((event = {}) => {
    ...
    return { ticker, paymentMillis, amount: round(amount, 2), kind };
...
function transactionFlowsBetween(transactions = [], start = 0, end = 0) {
  const flows = { contributions: 0, withdrawals: 0, weightedNetFlow: 0 };
  const duration = Math.max(1, Number(end || 0) - Number(start || 0));
  for (const tx of transactions || []) {
    ...
    const isContribution = Number(tx.quantity || 0) >= 0;
    if (isContribution) flows.contributions += amount;
    else flows.withdrawals += amount;

    // Modified Dietz: cash flows are weighted by the fraction of the month in which
    // the contributed/withdrawn capital was actually exposed to market performance.
    // The old formula removed the whole contribution only from the numerator. A large
    // mid-month purchase could therefore turn a normal 2–5% month into hundreds of
    // percent and visually "multiply" the portfolio in the comparison chart.
    const remainingWeight = Math.max(0, Math.min(1, (Number(end) - Number(tx.millis)) / duration));
    const signedFlow = isContribution ? amount : -amount;
    flows.weightedNetFlow += signedFlow * remainingWeight;
  }
  return {
    contributions: round(flows.contributions, 2),
    withdrawals: round(flows.withdrawals, 2),
    netFlow: round(flows.contributions - flows.withdrawals, 2),
    weightedNetFlow: round(flows.weightedNetFlow, 2)
  };
}
...
export async function buildRealMarketHistory(payload = {}, now = new Date()) {
  ...
  const startDate = portfolioStartDate(payload, positions, transactions) || new Date(firstTxMillis);
  ...
  for (let i = months - 1; i >= 0; i--) {
    ...
    const hasMonthActivity = monthFlows.contributions > 0 || monthFlows.withdrawals > 0 || dividendsInMonth > 0;

    // A total sale is itself part of the return history. The previous implementation skipped the
    // month as soon as no position remained open at month-end, which discarded the sale proceeds
    // from the time-weighted chain and made "Desde o início" forget realized gains/losses. Keep an
    // activity point even with zero closing inventory; monthlyRowsFromCumulative will treat the sale
    // proceeds as a withdrawal/cash-flow rather than as portfolio performance.
    if (!active.length && !hasMonthActivity) continue;
    ...
    for (const [ticker, bucket] of active) {
      ...
      const close = Number.isFinite(livePrice) && livePrice > 0
        ? livePrice
        : monthCloseAtOrBefore(pricesByTicker.get(ticker) || [], monthStart, boundary);
      if (close <= 0) {
        missing = true;
        break;
      }
    ...
    points.push({
      ...
      weightedNetCashFlow: monthFlows.weightedNetFlow,
      investedValue,
...
function returnRangeMonths(range = 'SINCE_START', payload = {}, now = new Date()) {
  ...
  const transactions = normalizeTransactions(payload.transactions || []);
  const firstMillis = transactions
    .map(transaction => Number(transaction.millis || 0))
    .filter(value => Number.isFinite(value) && value > 0)
    .sort((a, b) => a - b)[0];
  if (!firstMillis) return Math.max(12, Math.min(600, Number(payload.historyMonths || 120) || 120));
...
function monthlyRowsFromCumulative(points = []) {
  ...
  for (const rawPoint of (Array.isArray(points) ? points : [])) {
    const month = normalizeReturnPointMonth(rawPoint);
    if (!month) continue;
    const point = { ...rawPoint, month };
    const completeness = [
      point.monthlyReturnPercent, point.returnPercent, point.returnPct, point.value,
      point.totalValue, point.marketValue, point.monthlyContributions,
      point.monthlyWithdrawals, point.weightedNetCashFlow, point.dividendsInMonth
    ].reduce((score, value) => score + (Number.isFinite(Number(value)) ? 1 : 0), 0);
    const previous = rawByMonth.get(month);
    if (!previous || completeness >= previous.completeness) {
      rawByMonth.set(month, { point, completeness });
    }
  }
  const clean = [...rawByMonth.values()]
    .map(({ point }) => ({
      ...point,
      ...
      weightedNetCashFlow: Number(point.weightedNetCashFlow ?? point.weightedCashFlow ?? 0),
      dividendsInMonth: Number(point.dividendsInMonth ?? point.monthDividends ?? 0)
    }))
    .sort((a, b) => a.month.localeCompare(b.month));

  const saneMonthlyReturn = value => {
    const number = Number(value);
    return Number.isFinite(number) && number > -99.9 && number <= 1000 ? number : null;
  };

  let factor = 1;
  return clean.map((point, index) => {
    const previous = clean[index - 1];
    const explicitMonthly = saneMonthlyReturn(point.monthlyReturnPercent);
    const fallbackReturn = saneMonthlyReturn(point.returnPercent ?? point.returnPct ?? point.value ?? NaN);
    let monthlyReturnPercent = explicitMonthly;
    let returnCalculationStatus = explicitMonthly !== null ? 'SOURCE_MONTHLY' : 'CALCULATED';

    if (monthlyReturnPercent === null) {
      const hasCashFlow = point.monthlyWithdrawals > 0 || point.monthlyContributions > 0 || point.dividendsInMonth > 0;
      // O primeiro fechamento é a base da série, não um mês de performance. Usar o
      // retorno sobre custo desse ponto como retorno mensal criava um salto inicial e
      // depois capitalizava esse erro em toda a página Retorno.
      if (!previous) {
        monthlyReturnPercent = 0;
        returnCalculationStatus = 'BASE';
      } else if (
        (previous.marketValue > 0 || point.monthlyContributions > 0) &&
        (point.marketValue >= 0 || hasCashFlow)
      ) {
        // This also covers a full liquidation followed by a later re-entry. Beginning
        // market value can be zero; Modified Dietz then uses the weighted contribution
        // exposure instead of falling back to cumulative return-on-cost.
        monthlyReturnPercent = modifiedDietzMonthlyReturnPercent({
          beginningMarketValue: previous.marketValue,
          endingMarketValue: point.marketValue,
          contributions: point.monthlyContributions,
          withdrawals: point.monthlyWithdrawals,
          weightedNetCashFlow: point.weightedNetCashFlow,
          dividends: point.dividendsInMonth,
          fallbackReturnPercent: fallbackReturn ?? 0
        });
        returnCalculationStatus = previous.marketValue > 0 ? 'MODIFIED_DIETZ' : 'MODIFIED_DIETZ_REENTRY';
      } else {
        monthlyReturnPercent = fallbackReturn ?? 0;
        returnCalculationStatus = fallbackReturn !== null ? 'FALLBACK_SOURCE' : 'NEUTRALIZED_INVALID';
      }
    }

    if (saneMonthlyReturn(monthlyReturnPercent) === null) {
      // Never let a rejected extreme value re-enter the chain through the same unsafe
      // cumulative fallback. With no external contribution/withdrawal, a simple
      // holding-period return (including cash dividend) is a valid secondary check.
      const noExternalFlows = point.monthlyContributions <= 0 && point.monthlyWithdrawals <= 0;
      const holdingPeriodReturn = noExternalFlows && previous?.marketValue > 0 && point.marketValue >= 0
        ? ((point.marketValue + Math.max(0, point.dividendsInMonth) - previous.marketValue) / previous.marketValue) * 100
        : NaN;
      const safeHoldingPeriod = saneMonthlyReturn(holdingPeriodReturn);
      if (safeHoldingPeriod !== null) {
        monthlyReturnPercent = safeHoldingPeriod;
        returnCalculationStatus = 'HOLDING_PERIOD_FALLBACK';
      } else if (fallbackReturn !== null && !hasExternalFlow(point)) {
        monthlyReturnPercent = fallbackReturn;
        returnCalculationStatus = 'FALLBACK_SOURCE';
      } else {
        monthlyReturnPercent = 0;
        returnCalculationStatus = 'NEUTRALIZED_INVALID';
      }
    }

    factor *= (1 + Number(monthlyReturnPercent || 0) / 100);
    if (!Number.isFinite(factor) || factor <= 0) {
      // A corrupt monthly point must not poison every future accumulated value.
      factor = 1;
      monthlyReturnPercent = 0;
      returnCalculationStatus = 'NEUTRALIZED_INVALID';
    }
    return {
      ...
      returnCalculationStatus
    };
  });
}

function hasExternalFlow(point = {}) {
  return Number(point.monthlyContributions || 0) > 0 || Number(point.monthlyWithdrawals || 0) > 0;
}

function rangeSlice(points = [], months = 12) {
  const n = Number(months || 12);
  return n >= points.length ? points : points.slice(-Math.max(1, n));
}
...
function benchmarkMonthMap(points = [], valueField = 'accumulatedPercent', firstVisibleMonth = '', comparisonBaseMonth = '') {
  return benchmarkAccumulatedMonthMap(points, valueField, firstVisibleMonth, comparisonBaseMonth);
}
...
function trailingBenchmarkReturnFromSeries(series = [], months = 12, selector = () => null) {
  ...
  // The whole visible series is already rebased to comparisonBaseMonth. If the
  // requested trailing window spans all visible rows, its final accumulated value
  // is therefore the correct return for that window.
  if (rows.length <= safeMonths) return round(lastValue, 4);
  const baseIndex = rows.length - safeMonths - 1;
  const baseValue = Number(selector(rows[baseIndex]));
...
async function fetchPortfolioReturnBenchmarkHistory(benchmark = '', {
  ...
  // A página Retorno precisa seguir a mesma cadeia que já funciona nos modais de
  // Ação/FII: primeiro a API direta de cotações do índice, depois a camada geral
  // getAssetHistory como contingência. O checkpoint anterior chamava somente a
  // segunda camada, que é Yahoo-only para IFIX/SMLL/IDIV e podia devolver apenas
  // snapshot, deixando o seletor eternamente em "aguardando série".
  if (DIRECT_RETURN_INDEX_CODES.has(code)) {
    const direct = await fetchInvestidor10DirectIndexHistory(code, {
      months,
      timeoutMs: Math.max(7000, Number(timeoutMs) || 9000),
      limit: Math.max(24, Math.min(240, Number(limit) || 120))
    }).catch(error => ({
      ok: false,
      status: 'ERROR',
      ticker: code,
    ...
    const integrity = inspectRealHistoryIntegrity(direct);
    const points = integrity.trusted
      ? comparisonPointsFromHistory(direct)
          .map(point => ({
            ...point,
      ...
      : [];
    attempts.push({
      provider: 'Investidor10DirectIndexHistory',
      status: direct?.status || 'EMPTY',
      count: points.length,
    ...
    if (points.length >= 2) {
      return {
        ...
        attempts,
        provider: 'Investidor10DirectIndexHistory',
        providerParity: 'asset-modal-direct-index-first',
  ...
  const history = await getAssetHistory({
    ...
    timeoutMs: Number(timeoutMs || 9000),
    limit
  ...
  return {
    ...
    attempts,
    provider: 'getAssetHistory',
    providerParity: DIRECT_RETURN_INDEX_CODES.has(code)
      ? 'asset-modal-direct-index-first-with-history-fallback'
      : 'return-history-default',
    directIndexAlternative: false
  };
}
...
export async function buildPortfolioReturns(payload = {}) {
  ...
  const benchmarkFetchMonths = Math.min(600, benchmarkMonths + 1);
  ...
  const currentSnapshotMarketValue = currentSnapshotComplete
    ...
    : 0;
  const history = await buildRealMarketHistory({ ...filteredPayload, limitHistoryToRequestedWindow: true }, now);
  const rawPortfolio = monthlyRowsFromCumulative(history.points || history.history || []);
  const neutralizedReturnMonths = rawPortfolio
    .filter(point => point.returnCalculationStatus === 'NEUTRALIZED_INVALID')
    .map(point => point.month);
  const selectedPortfolio = selectPortfolioRowsForRange(rawPortfolio, range, displayMonths, saoPauloCalendarDate(now));
  const { rows: portfolio, isYearToDate, currentYear } = selectedPortfolio;
  if (!portfolio.length) {
    return {
      status: 'EMPTY',
      contractVersion: 'valorae-portfolio-returns-v2-index-provider-parity',
      range,
      assetFilter,
      summary: {},
      series: [],
  ...
  const [cdi, ipca] = await Promise.all([
    requestedBenchmarks.includes('CDI') ? getCdiAccumulatedSeries(benchmarkFetchMonths, Number(payload.indexTimeoutMs || payload.timeoutMs || 6500)) : Promise.resolve({ status: 'SKIPPED', points: [] }),
    requestedBenchmarks.includes('IPCA') ? getIpcaSeries(benchmarkFetchMonths).catch(error => ({ status: 'ERROR', points: [], error: error?.message })) : Promise.resolve({ status: 'SKIPPED', points: [] })
  ]);
  ...
  const previousPortfolioPoint = rawPortfolio.filter(point => point.month < baseMonth).at(-1) || null;
  // A benchmark must use the same economic base as the portfolio. If there is a
  // portfolio closing before the visible window, that closing is the denominator.
  // At portfolio inception there is no prior performance measurement, so the first
  // visible month itself is the neutral (0%) base for both sides.
  const comparisonBaseMonth = previousPortfolioPoint?.month || baseMonth;
  const cdiMap = benchmarkMonthMap(cdi.points || [], 'accumulatedPercent', baseMonth, comparisonBaseMonth);
  const ipcaAccum = accumulatedFromMonthly((ipca.points || ipca.series || []).map(p => ({ ...p, value: Number(p.monthlyPercent ?? p.value ?? 0) })));
  const ipcaMap = benchmarkMonthMap(ipcaAccum, 'accumulatedPercent', baseMonth, comparisonBaseMonth);
  const marketMaps = Object.fromEntries(marketBenchmarks.map(b => [
    b.ticker,
    benchmarkMonthMap(b.points || [], 'accumulatedPercent', baseMonth, comparisonBaseMonth)
  ]));
  ...
  const returnMetrics = summarizeReturnSeries(series);
  ...
  const cdiTotalPercent = Number.isFinite(lastAlignedCdi) ? lastAlignedCdi : 0;
  ...
  return {
    ...
    source: 'VALORAE Proxy portfolio returns: histórico real + séries de referência validadas',
    summary: {
      totalReturnPercent,
    ...
    diagnostics: {
      ...
      partial: history.partial || neutralizedReturnMonths.length > 0 || (!currentSnapshotComplete && currentPositions.length > 0) || cdi.status !== 'OK' || !cdiUsableInChart || ipca.status !== 'OK' || marketBenchmarks.some(b => b.status !== 'OK'),
      warnings: [
        ...
        ...(neutralizedReturnMonths.length ? [`Retorno mensal descartado por inconsistência numérica em: ${neutralizedReturnMonths.slice(0, 6).join(', ')}. O acumulado foi protegido contra propagação do valor inválido.`] : []),
