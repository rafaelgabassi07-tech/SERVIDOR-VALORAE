  ...
  return {
    ...
    diagnostics: {
      ...
      warnings: [
        ...
        ...(marketBenchmarks.some(b => b.ticker === 'SMLL' && b.status !== 'OK') ? ['SMLL oficial B3 indisponível agora.'] : []),
        ...(marketBenchmarks.some(b => b.ticker === 'IFIX' && b.status !== 'OK') ? ['IFIX oficial B3 indisponível agora; nenhum proxy/ETF foi usado.'] : []),
        ...(marketBenchmarks.some(b => b.ticker === 'IDIV' && b.status !== 'OK') ? ['IDIV oficial B3 indisponível agora.'] : []),
        ...(marketBenchmarks.some(b => ['IBOV', 'SMLL', 'IFIX', 'IDIV'].includes(b.ticker) && (b.simulated || b.proxyTickerUsed)) ? ['Índice oficial B3 rejeitado: fonte simulada/proxy não permitida.'] : [])
      ]
    }
  };
}
