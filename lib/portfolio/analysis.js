...
import { accumulatedReturnForMonths, benchmarkAccumulatedMonthMap, selectPortfolioRowsForRange, summarizeReturnSeries } from './return-metrics.js';
...
export async function buildRealMarketHistory(payload = {}) {
  ...
  const months = Math.max(1, Math.min(600, Number.isFinite(requestedMonths) ? Math.max(requestedMonths, ageMonths || 1) : (ageMonths || 12)));
...
function returnRangeMonths(range = 'SINCE_START', payload = {}) {
  ...
  const now = new Date();
  const ageMonths = ((now.getUTCFullYear() - first.getUTCFullYear()) * 12) + (now.getUTCMonth() - first.getUTCMonth()) + 1;
...
function monthlyRowsFromCumulative(points = []) {
  ...
  return clean.map((point, index) => {
    ...
    if (!Number.isFinite(monthlyReturnPercent)) {
      ...
      if (previous && previous.marketValue > 0 && (point.marketValue > 0 || hasCashFlow)) {
        monthlyReturnPercent = modifiedDietzMonthlyReturnPercent({
          ...
          fallbackReturnPercent: Number(point.returnPercent ?? point.returnPct ?? point.value ?? 0)
        });
      } else {
...
function benchmarkMonthMap(points = [], valueField = 'accumulatedPercent', baseMonth = '') {
  return benchmarkAccumulatedMonthMap(points, valueField, baseMonth);
}

function accumulatedReturnForLastMonths(points = [], months = 12) {
  return accumulatedReturnForMonths(points, months);
}
...
export async function buildPortfolioReturns(payload = {}) {
  ...
  const displayMonths = returnRangeMonths(range, payload);
  ...
  const benchmarkMonths = Math.max(1, Math.min(600, Number(payload.benchmarkMonths || displayMonths) || displayMonths));
  ...
  const selectedPortfolio = selectPortfolioRowsForRange(rawPortfolio, range, displayMonths);
  ...
  // Preserve a legitimate 0% trailing CDI return. Boolean fallback (`||`)
  // incorrectly replaced zero with the all-period CDI value.
  const calculatedCdiLast12MonthsPercent = accumulatedReturnForLastMonths(cdi.points || [], 12);
  const cdiLast12MonthsPercent = calculatedCdiLast12MonthsPercent ?? cdiTotalPercent;
