...
import { buildStrictPortfolioReturnSeries, selectPortfolioReturnWindowV3, summarizePortfolioReturnV3 } from './return-engine-v3.js';
...
export async function buildPortfolioReturns(payload = {}) {
  ...
  const history = await buildRealMarketHistory({
    ...
    // Return v3 is accuracy-first: a missing market close must break the comparable
    // performance chain. Cost basis is accounting data, not a substitute for market value.
    allowHistoricalCostBasisCarry: false
  }, now);
  const strictReturn = buildStrictPortfolioReturnSeries(history.points || history.history || [], {
  ...
  const selectedPortfolio = selectPortfolioReturnWindowV3(rawPortfolio, range, displayMonths, saoPauloCalendarDate(now));
  ...
  if (!portfolio.length) {
    return {
      status: 'EMPTY',
      contractVersion: 'valorae-portfolio-returns-v3-strict-ledger',
      range,
      assetFilter,
      summary: {},
      series: [],
  ...
  // Return v3 owns the portfolio comparison window. Benchmarks must use the exact same
  ...
  const returnMetrics = summarizePortfolioReturnV3(series);
