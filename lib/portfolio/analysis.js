...
import { benchmarkAccumulatedMonthMap, benchmarkExposureAlignedMonthMap } from './return-metrics.js';
import { modifiedDietzMonthlyReturnPercent, weightedPortfolioCashFlows } from './return-calculation.js';
import { buildExactPortfolioReturnSeriesV4, selectPortfolioReturnWindowV4, summarizePortfolioReturnV4 } from './return-engine-v4.js';
...
export async function buildRealMarketHistory(payload = {}, now = new Date()) {
  ...
  for (let i = months - 1; i >= 0; i--) {
    ...
    const beginningActive = [...beginningBuckets.entries()].filter(([, b]) => b.quantity > 0.000001 && b.costBasis > 0);
    ...
    const capitalExposureStartDate = (beginningActive.length > 0 ? monthStart : monthFlows.exposureStart) > 0
      ? new Date(beginningActive.length > 0 ? monthStart : monthFlows.exposureStart).toISOString().slice(0, 10)
      : null;
    const capitalExposureEndDate = (active.length > 0 ? boundary : monthFlows.exposureEnd) > 0
      ? new Date(active.length > 0 ? boundary : monthFlows.exposureEnd).toISOString().slice(0, 10)
      : null;
    const partialExposureMonth = beginningActive.length === 0 || active.length === 0 || isCurrentMonth;
    ...
    if (!active.length && !hasMonthActivity) {
      if (lastTransactionMillis > boundary) {
        ...
        points.push({
          ...
          knownInactive: true,
          source: 'VALORAE Proxy Return v4 • carteira sem capital entre ciclos'
        });
      }
      continue;
    }
...
function monthlyRowsFromCumulative(points = []) {
  ...
  return clean.map((point, index) => {
    ...
    if (monthlyReturnPercent === null) {
      ...
      // No primeiro mês e nas reentradas a carteira pode ter começado depois do dia 1.
      // `weightedNetCashFlow` já usa o intervalo efetivo de exposição, então o primeiro
      // fechamento deve participar da rentabilidade desde a primeira compra em vez de
      // ser zerado artificialmente como uma simples base visual.
      const canApplyModifiedDietz = !previous || previous.marketValue > 0 || point.monthlyContributions > 0;
      if (
        canApplyModifiedDietz &&
        ...
        monthlyReturnPercent = modifiedDietzMonthlyReturnPercent({
          beginningMarketValue: previous?.marketValue ?? 0,
        ...
        returnCalculationStatus = previous?.marketValue > 0 ? 'MODIFIED_DIETZ' : (!previous ? 'MODIFIED_DIETZ_INCEPTION' : 'MODIFIED_DIETZ_REENTRY');
      } else {
...
export function monthlyTableFromSeries(series = []) {
  ...
  for (const point of series) {
    ...
    // Um mês em que a carteira estava comprovadamente sem capital não é retorno zero.
    // É ausência de exposição. Preserve-o como célula vazia (—) para não fazer um ano
    // parecer integralmente investido antes de uma reentrada tardia.
    row[monthKeys[idx]] = point.knownInactive === true
      ? null
      : (Number.isFinite(Number(point.monthlyReturnPercent)) ? round(Number(point.monthlyReturnPercent), 2) : null);
    rows.set(year, row);
  }
...
export async function buildPortfolioReturns(payload = {}) {
  ...
  const history = await buildRealMarketHistory({
    ...
    // Return v4 is accuracy-first: a missing market close must break the comparable
    // performance chain. Cost basis is accounting data, not a substitute for market value.
    allowHistoricalCostBasisCarry: false
  }, now);
  const strictReturn = buildExactPortfolioReturnSeriesV4(history.points || history.history || [], {
    skippedMonths: history.skippedMonths || []
  });
  const rawPortfolio = strictReturn.rows;
  const neutralizedReturnMonths = strictReturn.diagnostics.droppedMonths;
  const selectedPortfolio = selectPortfolioReturnWindowV4(rawPortfolio, range, displayMonths, saoPauloCalendarDate(now));
  ...
  if (!portfolio.length) {
    return {
      status: 'EMPTY',
      contractVersion: 'valorae-portfolio-returns-v4-exact-window',
      range,
      assetFilter,
      summary: {},
      series: [],
  ...
  // Return v4 owns the portfolio comparison window. Benchmarks must use the exact same
  ...
  const series = portfolio.map(point => {
    ...
    const row = {
      ...
      knownInactive: point.knownInactive === true,
  ...
  const returnMetrics = summarizePortfolioReturnV4(series);
  ...
  return {
    ...
    source: 'VALORAE Proxy Return v4: valuation real + fluxos ajustados no tempo + benchmarks na mesma janela',
    summary: {
      totalReturnPercent,
    ...
    diagnostics: {
      ...
      returnComparableMonths: strictReturn.diagnostics.comparableMonths,
      ...
      comparisonBaseMonth,
      comparisonStartMonth: baseMonth,
      leadingInactiveMonthsTrimmed: selectedPortfolio.leadingInactiveMonths || [],
      ...
      partial: history.partial || neutralizedReturnMonths.length > 0 || (!currentSnapshotComplete && currentPositions.length > 0)
      ...
      warnings: [
        ...
        ...(neutralizedReturnMonths.length ? [`Meses fora da cadeia comparável por valuation insuficiente ou retorno não mensurável: ${neutralizedReturnMonths.slice(0, 6).join(', ')}.`] : []),
