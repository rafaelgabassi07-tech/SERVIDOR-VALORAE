...
export async function buildPortfolioReturns(payload = {}) {
  ...
  const displayMonths = returnRangeMonths(range);
  ...
  const benchmarkMonths = Math.max(1, Math.min(120, Number(payload.benchmarkMonths || displayMonths) || displayMonths));
  ...
  const [cdi, ipca] = await Promise.all([
    ...
    requestedBenchmarks.includes('IPCA') ? import('../sources/ipca.js').then(m => m.getIpcaSeries(benchmarkMonths)).catch(error => ({ status: 'ERROR', points: [], error: error?.message })) : Promise.resolve({ status: 'SKIPPED', points: [] })
  ]);
