...
function filterPayloadByAssetClass(payload = {}, assetFilter = 'ALL') {
  ...
  return {
    ...payload,
    positions: (payload.positions || []).filter(p => allowed.has(normalizeTicker(p.ticker || p.symbol || p.codigo || p.ativo))),
    transactions: (payload.transactions || []).filter(t => allowed.has(normalizeTicker(t.ticker || t.symbol || t.codigo || t.ativo))),
    dividendEvents: (payload.dividendEvents || payload.events || payload.dividends || []).filter(e => allowed.has(normalizeTicker(e.ticker || e.symbol || e.codigo || e.ativo)))
  };
}
...
export async function buildPortfolioReturns(payload = {}) {
  ...
  const filteredPayload = filterPayloadByAssetClass({ ...payload, historyMonths: portfolioMonths }, assetFilter);
  ...
  const portfolio = rebasePortfolioRows(rangeSlice(rawPortfolio, displayMonths));
  if (!portfolio.length) {
    return {
      status: 'EMPTY',
  ...
  const [cdi, ipca] = await Promise.all([
    requestedBenchmarks.includes('CDI') ? getCdiAccumulatedSeries(benchmarkMonths, Number(payload.indexTimeoutMs || payload.timeoutMs || 6500)) : Promise.resolve({ status: 'SKIPPED', points: [] }),
    requestedBenchmarks.includes('IPCA') ? getIpcaSeries(benchmarkMonths).catch(error => ({ status: 'ERROR', points: [], error: error?.message })) : Promise.resolve({ status: 'SKIPPED', points: [] })
  ]);
