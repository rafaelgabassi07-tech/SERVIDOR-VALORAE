...
export async function buildPortfolioReturns(payload = {}) {
  ...
  const marketBenchmarks = await Promise.all(requestedBenchmarks.filter(b => marketBenchmarkMap[b]).map(async (benchmark) => {
    ...
    return { ticker: benchmark, label: benchmark, status: points.length ? 'OK' : (historyPayload.status || 'EMPTY'), points, source: historyPayload.source || (benchmark === 'IFIX' ? 'B3 Oficial - IFIX' : 'YahooChart'), official: benchmark === 'IFIX' ? historyPayload.official === true : undefined, simulated: historyPayload.simulated === true, proxyTickerUsed: historyPayload.proxyTickerUsed === true, error: historyPayload.error };
  }));
  ...
  return {
    ...
    diagnostics: {
      ...
      warnings: [
        ...
        ...(marketBenchmarks.some(b => b.ticker === 'IFIX' && (b.simulated || b.proxyTickerUsed)) ? ['IFIX rejeitado: fonte simulada/proxy não permitida.'] : [])
      ]
    }
  };
}
