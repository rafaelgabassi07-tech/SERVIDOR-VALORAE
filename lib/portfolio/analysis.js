...
import { buildExposureOnlyReturnSeriesV5, reconcileCurrentMarketSnapshotV5, selectExposureReturnWindowV5, summarizeExposureReturnV5 } from './return-engine-v5.js';
import { buildReturnAnalytics, buildReturnExposureCycles } from './return-insights.js';
import { normalizeReturnDividendEvents, dividendsEarnedBetween } from './return-dividends.js';
import { resolveHistoricalPortfolioValuation, reconcileReturnOpeningTransactions } from './return-valuation.js';
import { normalizeReturnLedgerTransactions, returnLedgerBucketsAtBoundary, returnLedgerEffectiveTransactions, returnLedgerExposureIntervals, returnLedgerHasUnpricedEconomicTransaction, returnLedgerInceptionDate, returnLedgerWeightedCashFlows } from './return-ledger-engine.js';
...
function portfolioStartDate(payload = {}, positions = [], transactions = []) {
  ...
  const raw = returnLedgerInceptionDate(positions, transactions, explicit);
  if (!raw) return null;
...
export async function buildRealMarketHistory(payload = {}, now = new Date()) {
  ...
  const sourceTransactions = normalizeReturnLedgerTransactions(payload.transactions || []);
  ...
  const inventoryMismatchTickers = openingReconciliation.inventoryMismatchTickers || [];
  const effectiveTransactions = returnLedgerEffectiveTransactions(transactions);
  const exposureIntervals = returnLedgerExposureIntervals(transactions);
  const unmatchedLedgerTransactions = effectiveTransactions.filter(tx => tx.unmatchedQuantity > 0.00000001);
  const realDividendEvents = normalizeReturnDividendEvents(payload.dividendEvents || payload.events || payload.dividends || [], positions, transactions);
  ...
  for (let i = months - 1; i >= 0; i--) {
    ...
    const beginningBuckets = returnLedgerBucketsAtBoundary(transactions, Math.max(0, monthStart - 1));
    const beginningActive = [...beginningBuckets.entries()].filter(([, b]) => b.quantity > 0.000001);
    const buckets = returnLedgerBucketsAtBoundary(transactions, boundary);
    ...
    const monthFlows = returnLedgerWeightedCashFlows(transactions, monthStart, boundary, {
    ...
    const intervalHasExposure = exposureIntervals.some(interval => {
      const intervalEnd = Number(interval.endMillis || Number.POSITIVE_INFINITY);
      return Number(interval.startMillis || 0) <= boundary && intervalEnd >= monthStart;
    });
    const hasCapitalExposure = monthFlows.hadCapitalExposure === true && intervalHasExposure;
    ...
    const hasUnpricedTransaction = returnLedgerHasUnpricedEconomicTransaction(transactions, monthStart, boundary);
  ...
  return {
    status: points.length ? (
      skippedMonths.length || unavailableTickers.length || inventoryMismatchTickers.length || unmatchedLedgerTransactions.length
        ? 'PARTIAL'
        : 'OK'
    ) : 'EMPTY',
    points,
    ...
    realOnly: true,
    partial: Boolean(
      skippedMonths.length || unavailableTickers.length || partialValuationMonths.length ||
      inventoryMismatchTickers.length || unmatchedLedgerTransactions.length
    ),
    skippedMonths,
    ...
    openingInventoryReconciled: false,
    openingInventoryReconciledTickers: [],
    openingInventoryReconciliationCount: 0,
    historicalBackfillDisabled: openingReconciliation.historicalBackfillDisabled === true,
    inventoryMismatchTickers,
    inventoryMismatches: openingReconciliation.inventoryMismatches || [],
    unmatchedLedgerTransactionCount: unmatchedLedgerTransactions.length,
    exposureIntervals,
    unmatchedLedgerTransactions: unmatchedLedgerTransactions.map(tx => ({
      ticker: tx.ticker,
      date: tx.date,
      operationCode: tx.operationCode,
      requestedQuantity: tx.quantity,
      unmatchedQuantity: tx.unmatchedQuantity
    })),
...
function returnRangeMonths(range = 'SINCE_START', payload = {}, now = new Date()) {
  ...
  const positions = normalizePositions(payload.positions || []);
  const transactions = normalizeReturnLedgerTransactions(payload.transactions || []);
...
export async function buildPortfolioReturns(payload = {}) {
  ...
  const currentSnapshotMissingTickers = currentPositions
    ...
    .sort();
  const currentSnapshotPricingComplete = currentPositions.length > 0 && currentSnapshotMissingTickers.length === 0;
  const currentSnapshotMarketValue = currentSnapshotPricingComplete
  ...
  const currentSnapshotComplete = currentSnapshotPricingComplete &&
    !(history.inventoryMismatchTickers || []).length;
  const currentMonth = saoPauloMonthKey(now);
  const returnHistoryPoints = reconcileCurrentMarketSnapshotV5(history.points || history.history || [], {
    month: currentMonth,
    marketValue: currentSnapshotMarketValue,
    complete: currentSnapshotComplete
  });
  const strictReturn = buildExposureOnlyReturnSeriesV5(returnHistoryPoints, {
  ...
  const series = portfolio.map(point => {
    const marketValue = round(Number(point.marketValue ?? point.totalValue ?? point.value ?? 0), 2);
    const dividendsReceived = round(Number(point.dividendsReceived || 0), 2);
    const totalValue = round(Number(point.totalValue ?? point.value ?? 0), 2);
    const row = {
      month: point.month,
      label: point.label,
      date: `${point.month}-01`,
      ...
      monthlyContributions: round(Number(point.monthlyContributions ?? point.contributions ?? 0), 2),
      monthlyWithdrawals: round(Number(point.monthlyWithdrawals ?? point.withdrawals ?? 0), 2),
      weightedNetCashFlow: round(Number(point.weightedNetCashFlow ?? 0), 2),
      dividendsReceived,
  ...
  const cdiTotalComparable = cdiCoverageComplete && Number.isFinite(lastAlignedCdi);
  const cdiTotalPercent = cdiTotalComparable ? lastAlignedCdi : null;
  const cdiLastMonthRaw = trailingBenchmarkReturnFromSeries(series, 1, point => point.cdiReturnPercent);
  const cdiLast12MonthsRaw = trailingBenchmarkReturnFromSeries(series, 12, point => point.cdiReturnPercent);
  const cdiLastMonthPercent = cdiLastMonthRaw == null ? null : cdiLastMonthRaw;
  const cdiLast12MonthsPercent = cdiLast12MonthsRaw == null ? null : cdiLast12MonthsRaw;
  const totalCdiComparison = cdiTotalComparable ? comparisonToCdi(totalReturnPercent, cdiTotalPercent) : { percent: null, label: null };
  const last12CdiComparison = cdiLast12MonthsPercent == null ? { percent: null, label: null } : comparisonToCdi(last12MonthsReturnPercent, cdiLast12MonthsPercent);
  const lastMonthCdiComparison = cdiLastMonthPercent == null ? { percent: null, label: null } : comparisonToCdi(lastMonthReturnPercent, cdiLastMonthPercent);
  ...
  const historicalTransactions = normalizeReturnLedgerTransactions(filteredPayload.transactions || []);
  ...
  return {
    ...
    summary: {
      ...
      cdiTotalPercent: cdiTotalPercent == null ? null : round(cdiTotalPercent, 2),
      cdiLast12MonthsPercent: cdiLast12MonthsPercent == null ? null : round(cdiLast12MonthsPercent, 2),
      cdiLastMonthPercent: cdiLastMonthPercent == null ? null : round(cdiLastMonthPercent, 2),
      totalVsCdiPercent: totalCdiComparison.percent,
      totalVsCdiLabel: totalCdiComparison.label,
      last12MonthsVsCdiPercent: last12CdiComparison.percent,
      last12MonthsVsCdiLabel: last12CdiComparison.label,
      lastMonthVsCdiPercent: lastMonthCdiComparison.percent,
      lastMonthVsCdiLabel: lastMonthCdiComparison.label,
    ...
    diagnostics: {
      ...
      openingInventoryReconciled: false,
      openingInventoryReconciledTickers: [],
      historicalBackfillDisabled: history.historicalBackfillDisabled === true,
      inventoryMismatchTickers: history.inventoryMismatchTickers || [],
      inventoryMismatches: history.inventoryMismatches || [],
      unmatchedLedgerTransactionCount: history.unmatchedLedgerTransactionCount || 0,
      unmatchedLedgerTransactions: history.unmatchedLedgerTransactions || [],
      exposureIntervals: history.exposureIntervals || [],
      ...
      warnings: [
        ...
        ...(history.inventoryMismatchTickers?.length ? [`Posição atual diverge do ledger importado para: ${history.inventoryMismatchTickers.join(', ')}. A posição atual não foi retropropagada para o passado; revise/importa as operações faltantes antes de confiar no mês corrente.`] : []),
        ...(history.unmatchedLedgerTransactions?.length ? [`Operações de saída sem estoque correspondente foram ignoradas como exposição (${history.unmatchedLedgerTransactions.slice(0, 4).map(item => `${item.ticker} ${item.date}`).join(', ')}). Elas permanecem apenas como diagnóstico de histórico incompleto.`] : []),
        ...
        ...(!currentSnapshotComplete && !currentSnapshotMissingTickers.length && history.inventoryMismatchTickers?.length ? ['O snapshot atual não foi usado para sobrescrever o ledger porque a quantidade atual diverge das operações conhecidas.'] : []),
