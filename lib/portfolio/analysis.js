...
import { accumulatedReturnForMonths, benchmarkAccumulatedMonthMap, buildDisplayPortfolioRows, summarizeReturnSeries } from './return-metrics.js';
...
export async function buildPortfolioReturns(payload = {}) {
  ...
  const portfolio = buildDisplayPortfolioRows(rawPortfolio, displayMonths);
  if (!portfolio.length) {
    return {
      status: 'EMPTY',
