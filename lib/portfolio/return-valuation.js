...
export function reconcileReturnOpeningTransactions(positions = [], transactions = []) {
  ...
  for (const position of (Array.isArray(positions) ? positions : [])) {
    ...
    const knownNetQuantity = rows.reduce((sum, transaction) => {
      const quantityDelta = Number(transaction?.quantityDelta ?? transaction?.quantity ?? 0);
      return Number.isFinite(quantityDelta) ? sum + quantityDelta : sum;
    }, 0);
    ...
    reconciliations.push({
      ...
      price: averagePrice,
      grossValue: openingQuantity * averagePrice,
      externalCashFlow: openingQuantity * averagePrice,
      operationCode: 'BUY',
      requiresEconomicPrice: true,
