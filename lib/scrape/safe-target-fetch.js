...
export const VALORAE_SAFE_TARGET_FETCH_POLICY = 'allowlisted-dns-validated-manual-redirects-v2';
...
export async function fetchAllowedScrapeText(rawUrl, {
  ...
  allowPublicRedirectHosts = false,
  headers = {},
  signal,
  ...
  for (let redirectCount = 0; ; redirectCount += 1) {
    ...
    const redirectAllowedHosts = allowPublicRedirectHosts
      ? [...normalizedAllowedHosts(allowedHosts), next.hostname]
      : allowedHosts;
    current = validatePublicScrapeTarget(next.toString(), { allowedHosts: redirectAllowedHosts, allowSubdomains });
  }
}
