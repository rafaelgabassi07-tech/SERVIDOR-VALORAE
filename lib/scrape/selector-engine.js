...
import { createLazyHtmlDocumentContext } from './document-context.js';
...
export function extractSelectors(html = '', selectors = null, options = {}, result = {}) {
  ...
  const documentContext = options.documentContext || createLazyHtmlDocumentContext(html || '', {
    parserMode: options.parserMode,
    selectors,
  });
  const sharedOptions = { ...options, documentContext };

  if (canUseFastSelectors(selectors, options)) {
    const fast = extractFastSelectors(html || '', selectors, options);
    if (fast.ok && fast.safe) {
      let shadow;
      if (shadowFastPathEnabled()) shadow = runHtmlParserShadow(html || '', selectors, fast.results, sharedOptions);
      const standardCandidate = shadow?.diagnostics?.ran ? shadow.results : null;
      const structured = runStructuredDataShadow(html || '', selectors, shadow?.diagnostics?.promoted && shadow.results ? shadow.results : fast.results, sharedOptions);
  ...
  const shadow = runHtmlParserShadow(html || '', selectors, legacy.results || {}, sharedOptions);
  ...
  const structured = runStructuredDataShadow(html || '', selectors, htmlResults, sharedOptions);
  ...
  const response = {
    ...
    metrics: {
      ...
      structuredDocuments: structured.diagnostics?.summary?.documents || 0,
      structuredEndpoints: structured.diagnostics?.summary?.endpoints || 0,
      sharedDocumentParser: shadow.diagnostics?.metrics?.parserEngine || structured.diagnostics?.parserEngine,
      sharedDocumentReused: Boolean(structured.diagnostics?.documentReused),
    },
