...
import { extractNextFlightDocuments, VALORAE_NEXT_FLIGHT_DISCOVERY_VERSION } from './next-flight-discovery.js';
import { isPrivateOrSpecialHost } from './network-safety.js';
import { sanitizeCapturedJson } from './json-safety.js';
...
export const VALORAE_STRUCTURED_DATA_IMPLEMENTATION = 'cheerio-sanitized-hydration-next-flight-and-network-json-safe-discovery-v4';
...
const metrics = globalThis.__VALORAE_STRUCTURED_DATA_METRICS__ || {
  ...
  hydrationAttributes: 0,
  networkJsonDocuments: 0,
  nextFlightFrames: 0,
  nextFlightJsonDocuments: 0,
  frameworkStates: 0,
...
function safeParseJson(raw = '') {
  ...
  if (!source || Buffer.byteLength(source, 'utf8') > maxDocumentBytes()) return null;
...
function addDocument(documents, candidate) {
  ...
  const sanitized = sanitizeCapturedJson(candidate.data);
  ...
  if (!bytes || bytes > maxDocumentBytes()) return;
  const fingerprint = createHash('sha256').update(serialized).digest('base64url').slice(0, 28);
  ...
  documents.push({
    ...
    data: sanitized.data,
    bytes,
    url: String(candidate.url || '').slice(0, 2000) || undefined,
    contentType: String(candidate.contentType || '').slice(0, 160) || undefined,
    status: Number.isFinite(Number(candidate.status)) ? Number(candidate.status) : undefined,
    framework: String(candidate.framework || '').slice(0, 80) || undefined,
    fingerprint,
  });
}
...
function safeUrl(raw = '', baseUrl = '') {
  ...
  if (parsed.username || parsed.password || isPrivateOrSpecialHost(parsed.hostname)) return '';
...
function frameworkMarkers(source = '') {
  const text = String(source || '');
  const out = new Set();
  if (/__NEXT_DATA__|self\.__next_f|_next\//i.test(text)) out.add('nextjs');
  if (/__NUXT__|__NUXT_DATA__|_nuxt\//i.test(text)) out.add('nuxt');
  if (/__remixContext|__remixManifest/i.test(text)) out.add('remix');
  if (/__APOLLO_STATE__/i.test(text)) out.add('apollo');
  if (/__PRELOADED_STATE__|__INITIAL_STATE__/i.test(text)) out.add('redux-or-initial-state');
  if (/data-sveltekit-fetched|__SVELTEKIT_/i.test(text)) out.add('sveltekit');
  if (/astro-island|data-astro-cid/i.test(text)) out.add('astro');
  if (/Highcharts|ApexCharts|new\s+Chart|echarts\.init/i.test(text)) out.add('chart-runtime');
  return [...out];
}

function extractHydrationAttributeDocuments($, documents) {
  const attributes = ['data-page', 'data-props', 'data-state', 'data-hydration', 'data-json', 'data-react-props', 'data-sveltekit-fetched'];
  ...
  $(selector).each((index, element) => {
    ...
    for (const attr of attributes) {
      ...
      addDocument(documents, {
        id: `${attr}-${index + 1}`,
        kind: 'hydration-attribute',
        source: `[${attr}]`,
        data: parsed,
        bytes: Buffer.byteLength(raw, 'utf8'),
        framework: attr === 'data-sveltekit-fetched' ? 'sveltekit' : 'html-hydration',
      });
      if (documents.length > before) added += 1;
    }
  });
  return added;
}

function addAdditionalDocuments(documents, endpoints, endpointSeen, candidates = [], baseUrl = '') {
  let added = 0;
  for (const candidate of Array.isArray(candidates) ? candidates : []) {
    if (documents.length >= maxDocuments()) break;
    const data = candidate?.data;
    if (!data || typeof data !== 'object') continue;
    const bytes = Number(candidate.bytes || Buffer.byteLength(JSON.stringify(data), 'utf8'));
    if (!Number.isFinite(bytes) || bytes <= 0 || bytes > maxDocumentBytes()) continue;
    const url = safeUrl(candidate.url || '', baseUrl);
    const before = documents.length;
    addDocument(documents, {
      id: candidate.id || `network-json-${documents.length + 1}`,
      kind: candidate.kind || 'network-json',
      source: candidate.source || 'playwright-response',
      data,
      bytes,
      url,
      contentType: candidate.contentType,
      status: candidate.status,
      framework: candidate.framework || 'browser-network',
    });
    if (documents.length > before) added += 1;
    if (url && !endpointSeen.has(url) && endpoints.length < maxEndpoints()) {
      endpointSeen.add(url);
      endpoints.push({ url, kind: endpointKind(url), evidence: 'captured-network-json' });
    }
  }
  return added;
}
...
export function discoverStructuredPageData(html = '', options = {}) {
  ...
  const hasAdditionalDocuments = Array.isArray(options.additionalDocuments) && options.additionalDocuments.length > 0;
  if (!hasAdditionalDocuments && !/(?:application\/ld\+json|application\/json|__NEXT_DATA__|self\.__next_f|__NUXT__|__remixContext|__APOLLO_STATE__|__PRELOADED_STATE__|data-(?:page|props|state|hydration|json|react-props|sveltekit-fetched)|Highcharts|ApexCharts|new\s+Chart|fetch\s*\(|axios\.|\/api\/)/i.test(source)) {
  ...
  const frameworks = new Set(frameworkMarkers(source));
  try {
    ...
    $('script').each((index, element) => {
      ...
      for (const framework of frameworkMarkers(`${id} ${type} ${src} ${text}`)) frameworks.add(framework);
      if (!text) return;
      ...
      for (const candidate of extractNextFlightDocuments(text, {
        maxFrames: Math.min(32, maxDocuments()),
        maxRecords: Math.min(48, maxDocuments()),
        maxDocumentBytes: maxDocumentBytes(),
      })) addDocument(documents, candidate);
    ...
    const hydrationAttributes = extractHydrationAttributeDocuments($, documents);
    const networkJsonDocuments = addAdditionalDocuments(
      documents,
      endpoints,
      endpointSeen,
    ...
    const counts = {
      ...
      hydrationAttributes,
      networkJsonDocuments,
      nextFlightFrames: documents.filter(item => item.kind === 'next-flight-frame').length,
      nextFlightJsonDocuments: documents.filter(item => item.kind === 'next-flight-json').length,
      frameworkStates: frameworks.size,
...
function emptyDiscovery(reason = '', error = false) {
  return {
    ...
    documents: [],
    endpoints: [],
    summary: { documents: 0, jsonLd: 0, nextData: 0, applicationJson: 0, inlineAssignments: 0, chartConfigurations: 0, hydrationAttributes: 0, networkJsonDocuments: 0, nextFlightFrames: 0, nextFlightJsonDocuments: 0, frameworkStates: 0, frameworks: [], endpoints: 0, documentKinds: {}, endpointKinds: {} },
...
function recordMetric(status, elapsedMs = 0, counts = null, reason = '') {
  ...
  if (counts) {
    ...
    metrics.hydrationAttributes += counts.hydrationAttributes || 0;
    metrics.networkJsonDocuments += counts.networkJsonDocuments || 0;
    metrics.nextFlightFrames += counts.nextFlightFrames || 0;
    metrics.nextFlightJsonDocuments += counts.nextFlightJsonDocuments || 0;
    metrics.frameworkStates += counts.frameworkStates || 0;
...
export function runStructuredDataShadow(html = '', selectors = {}, legacyResults = {}, options = {}) {
  ...
  const promotionSafe = lostKeys.length === 0 && gainedKeys.length > 0;
  ...
  if (promoted) {
    for (const key of structuredKeys) {
      if (!valuePresent(merged[key]) && valuePresent(extracted.results[key])) merged[key] = extracted.results[key];
    }
  }
  return {
    results: merged,
    ...
    diagnostics: {
      ...
      promoted,
      outputSource: promoted ? 'structured-gap-fill' : 'legacy-preserved',
      comparison: {
...
export function resetStructuredDataMetricsForTests() {
  ...
  Object.assign(metrics, {
    ...
    applicationJsonDocuments: 0, inlineAssignments: 0, chartConfigurations: 0, hydrationAttributes: 0,
    networkJsonDocuments: 0, nextFlightFrames: 0, nextFlightJsonDocuments: 0, frameworkStates: 0, discoveredEndpoints: 0,
...
export function buildStructuredDataManifest() {
  ...
  return {
    ...
    nextFlightDiscoveryVersion: VALORAE_NEXT_FLIGHT_DISCOVERY_VERSION,
    compatibility: 'additive-hidden-from-ui',
    contractImpact: 'none',
    mode,
    enabled: mode !== 'disabled',
    outputPolicy: mode === 'prefer-structured'
      ? 'fill-only-missing-keys-when-no-legacy-key-is-lost'
      : 'legacy-output-always-preserved',
    discoveryOrder: ['json-ld', 'framework-state-scripts', 'nextjs-app-router-flight-json', 'application-json', 'hydration-attributes', 'inline-assignments', 'chart-configurations', 'captured-network-json', 'internal-endpoints', 'html-parser-fallback'],
    ...
    safety: {
      ...
      acceptsOnlyPrevalidatedAdditionalDocuments: true,
      requestHeadersBodiesAndCookiesAreNeverPersisted: true,
      credentialsEmbeddedInUrlsRejected: true,
      privateAndSpecialDiscoveredHostsRejected: true,
      nextFlightJavaScriptNeverExecuted: true,
...
export function classifyKnownInternalEndpoint(url = '') {
  ...
  if (/\/api\/fii\/comparador\//.test(path)) return 'comparadorFiis';
