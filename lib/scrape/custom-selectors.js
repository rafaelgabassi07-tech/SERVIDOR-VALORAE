...
// v21.11.8 closes gaps found in the complete audit: descendant/direct selectors,
...
export const VALORAE_CUSTOM_SELECTORS_VERSION = '21.11.8-custom-selectors-financial-normalizer';
...
function createTextCompactor() {
  const cache = new Map();
  return value => {
    const raw = String(value || '');
    const key = raw.length <= 500 ? raw : `${raw.slice(0, 500)}:${raw.length}`;
...
function extractTableRow(fragment, textFn = compactText) {
  const cells = String(fragment || '').match(/<t[dh]\b[^>]*>[\s\S]*?<\/t[dh]>/gi) || [];
  const values = cells.map(textFn).filter(Boolean);
  return values.join(' ');
}

function extractTableCells(fragment, textFn = compactText) {
  const cells = String(fragment || '').match(/<t[dh]\b[^>]*>[\s\S]*?<\/t[dh]>/gi) || [];
  return cells.map(textFn).filter(Boolean);
}
...
function extractValue(fragment, extract, textFn = compactText) {
  const mode = String(extract || 'text').trim();
  if (!mode || mode === 'text') return /^<tr\b/i.test(String(fragment).trim()) ? extractTableRow(fragment, textFn) : textFn(fragment);
  if (mode === 'html' || mode === 'outerHtml') return fragment;
  if (mode === 'row') return extractTableRow(fragment, textFn);
  if (mode === 'cells') return extractTableCells(fragment, textFn);
  ...
  return textFn(fragment);
}

export function extractCustomSelectors(html = '', selectors = {}, options = {}) {
  const started = performance.now();
  if (!selectors || typeof selectors !== 'object' || Array.isArray(selectors)) return { results: {}, warnings: ['selectors precisa ser objeto.'], metrics: { selectorTimeMs: 0, nodesFound: 0, parseStrategy: 'css-lite' } };
  const source = String(html || '');
  ...
  const textFn = createTextCompactor();
  const results = {};
  const warnings = [];
  let nodesFound = 0;
  ...
  for (const [key, rawSpec] of entries) {
    ...
    const cacheKey = `${spec.selector}::${limit}`;
    let fragments = fragmentCache.get(cacheKey);
    if (fragments) cacheHits += 1;
    else {
      fragments = resolveFragments(source, spec.selector, limit);
      if (fragmentCache.size < 250) fragmentCache.set(cacheKey, fragments);
    }
    nodesFound += fragments.length;
    if (!fragments.length && /\s/.test(String(spec.selector))) warnings.push(`Selector sem resultado: ${key} (${spec.selector})`);
    const values = fragments.map(f => extractValue(f, spec.extract, textFn)).filter(v => v !== undefined && v !== null && String(v).trim() !== '');
    results[key] = values;
  }
  ...
  return { results, warnings, sourceDrift, version: VALORAE_CUSTOM_SELECTORS_VERSION, metrics: { selectorTimeMs: Math.round(performance.now() - started), nodesFound, parseStrategy: 'css-lite', selectorCacheHits: cacheHits } };
}
