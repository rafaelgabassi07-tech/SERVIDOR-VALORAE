...
import { VALORAE_HTML_PARSER_SHADOW_VERSION } from '../core/feature-versions.js';
export { VALORAE_HTML_PARSER_SHADOW_VERSION } from '../core/feature-versions.js';
export const VALORAE_HTML_PARSER_SHADOW_POLICY = 'standards-html-shadow-v1';
export const VALORAE_HTML_PARSER_IMPLEMENTATION = 'cheerio-1.2.0-adaptive-htmlparser2-parse5';
...
export function extractStandardHtmlSelectors(html = '', selectors = {}, options = {}) {
  ...
  let document;
  try {
    const context = options.documentContext || createLazyHtmlDocumentContext(source, {
      parserMode: options.parserMode,
      selectors,
    });
    document = context.resolve('standard-html-selectors');
  } catch (error) {
    return {
      ok: false,
      results: {},
      warnings: [`Falha no parser HTML padrão: ${String(error?.message || error).slice(0, 180)}`],
      metrics: { parserTimeMs: Math.round((performance.now() - started) * 100) / 100, selectorTimeMs: 0, nodesFound: 0, parseStrategy: 'standards-dom-error', selectorCount: 0 },
    };
  }
  const $ = document.$;
  const parserTimeMs = document.reused ? 0 : document.parseTimeMs;
  const selectorStarted = performance.now();
  const results = {};
  const extractedResults = new Map();
  const warnings = [];
  let nodesFound = 0;
  let logicalNodesFound = 0;
  let selectorCount = 0;
  ...
  for (const [key, rawSpec] of entries) {
    ...
    const signature = JSON.stringify([String(spec.selector), String(spec.extract || 'text'), limit]);
    const group = groups.get(signature) || { spec, limit, keys: [] };
    group.keys.push(key);
    groups.set(signature, group);
  }

  for (const { spec, limit, keys } of groups.values()) {
    try {
      ...
      for (const key of keys) extractedResults.set(key, values.slice());
      nodesFound += selection.length;
      logicalNodesFound += selection.length * keys.length;
      if (!selection.length) {
        for (const key of keys) warnings.push(`Selector sem resultado: ${key} (${spec.selector})`);
      }
    } catch (error) {
      for (const key of keys) {
        extractedResults.set(key, []);
        warnings.push(`Selector inválido em ${key}: ${String(error?.message || error).slice(0, 140)}`);
      }
    }
  }

  // Mantém a ordem original das chaves do contrato mesmo quando aliases compartilham uma consulta.
  for (const [key] of entries) {
    if (extractedResults.has(key)) results[key] = extractedResults.get(key);
  }
  ...
  return {
    ...
    metrics: {
      ...
      logicalNodesFound,
      selectorCount,
      selectorGroups: groups.size,
      queriesSaved: Math.max(0, selectorCount - groups.size),
...
export function runHtmlParserShadow(html = '', selectors = {}, legacyResults = {}, options = {}) {
  ...
  return {
    results: promoted ? standard.results : legacyResults,
    diagnostics: {
      version: VALORAE_HTML_PARSER_SHADOW_VERSION,
      policyVersion: VALORAE_HTML_PARSER_SHADOW_POLICY,
      implementation: VALORAE_HTML_PARSER_IMPLEMENTATION,
...
export function buildHtmlParserShadowManifest() {
  ...
  return {
    ...
    featureFlags: [
      ...
      'VALORAE_HTML_DOCUMENT_PARSER',
    ],
    hybridDocumentVersion: VALORAE_HYBRID_DOCUMENT_VERSION,
    performancePolicy: 'adaptive-parser-shared-dom-and-identical-query-deduplication',
