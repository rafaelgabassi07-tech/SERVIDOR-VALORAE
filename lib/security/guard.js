...
import { exposeHeadersCsv, requestHeadersCsv } from '../core/mobile-protocol.js';

export const SECURITY_GUARD_VERSION = '21.12.0';
...
const MAX_RATE_BUCKETS = Math.max(128, Math.min(100_000, Number(process.env.VALORAE_RATE_LIMIT_MAX_BUCKETS || 5_000)));
let rateBucketOperations = 0;
let rateBucketEvictions = 0;
...
function resolveCorsOrigin(req) {
  ...
  const strict = productionRuntime() || truthy(process.env.VALORAE_CORS_STRICT);
...
function safeCorsRequestHeaders(value = '') {
  ...
  const securityHeaders = [
    'X-Requested-With',
    'X-Valorae-Admin-Token',
  ];
  return requestHeadersCsv([...securityHeaders, ...requested]);
}

function exposedResponseHeaders() {
  return exposeHeadersCsv();
}
...
function cleanRateBuckets(now = Date.now(), force = false) {
  rateBucketOperations += 1;
  if (!force && rateBucketOperations % 128 !== 0 && rateBuckets.size < MAX_RATE_BUCKETS) return;
