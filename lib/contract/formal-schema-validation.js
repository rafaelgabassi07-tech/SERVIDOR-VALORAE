...
export function formalRequestSchemaMode() {
  const productionDefault = process.env.VERCEL === '1' || process.env.NODE_ENV === 'production' ? 'enforce' : 'shadow';
  const mode = String(process.env.VALORAE_FORMAL_REQUEST_SCHEMA_MODE || productionDefault).trim().toLowerCase();
  return ['disabled', 'shadow', 'enforce'].includes(mode) ? mode : productionDefault;
}
