...
const DEFAULT_FAILURE_THRESHOLD = boundedEnvNumber('VALORAE_CIRCUIT_FAILURE_THRESHOLD', 4, 1, 20);
const DEFAULT_COOLDOWN_MS = boundedEnvNumber('VALORAE_CIRCUIT_COOLDOWN_MS', 5 * 60 * 1000, 5_000, 60 * 60 * 1000);
const DEFAULT_SUCCESS_RESET = boundedEnvNumber('VALORAE_CIRCUIT_SUCCESS_RESET', 2, 1, 10);
const WINDOW_LIMIT = boundedEnvNumber('VALORAE_CIRCUIT_WINDOW_LIMIT', 40, 8, 200);
const MAX_PROVIDER_ENTRIES = boundedEnvNumber('VALORAE_CIRCUIT_MAX_PROVIDERS', 64, 8, 256);
...
const sharedRuntime = globalThis.__VALORAE_PROVIDER_HEALTH_SHARED_RUNTIME__ || {
  persisted: 0,
  hydrationHits: 0,
  hydrationMisses: 0,
  errors: 0,
...
if (!(sharedRuntime.timers instanceof Map)) sharedRuntime.timers = new Map();
if (!Number.isFinite(Number(sharedRuntime.hydratedAt))) sharedRuntime.hydratedAt = 0;
...
export async function hydrateProviderHealthFromSharedState() {
  return { hydrated: false, reason: 'memory-local-only', hits: 0 };
}
...
export function providerHealthSharedStateStats() {
  return {
    ...
    providerEntries: state.size,
    maxProviderEntries: MAX_PROVIDER_ENTRIES,
  };
}

function trimProviderState(protectedKey = '') {
  while (state.size >= MAX_PROVIDER_ENTRIES) {
    const removable = [...state.keys()].find(key => key !== protectedKey && !DEFAULT_PROVIDERS.includes(key))
      || [...state.keys()].find(key => key !== protectedKey);
    if (!removable) break;
    state.delete(removable);
    const timer = sharedRuntime.timers.get(sharedKey(removable));
    if (timer) clearTimeout(timer);
    sharedRuntime.timers.delete(sharedKey(removable));
  }
}

function entry(name) {
  const key = String(name || 'unknown').slice(0, 160);
  if (!state.has(key)) {
    trimProviderState(key);
    state.set(key, {
      provider: key,
      status: 'healthy',
      failures: 0,
...
export function getProviderHealthSnapshot() {
  // Materialize the canonical providers first. entry() may evict old unknown hosts to keep
  // the state bounded, therefore the snapshot must be built only after that normalization.
  for (const key of DEFAULT_PROVIDERS) entry(key);
  const out = {};
