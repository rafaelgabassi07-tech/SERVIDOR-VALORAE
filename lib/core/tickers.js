...
export function normalizeTicker(value) {
  let ticker = String(value || '')
    ...
    .replace(/[^A-Z0-9]/g, '')
    .slice(0, 16);
  if (/^(?:(?:[A-Z]{4}[0-9]{1,2}B?|[A-Z0-9]{3,6}[0-9]{1,2})F?)SA$/.test(ticker)) ticker = ticker.slice(0, -2);
  if (/^(?:[A-Z]{4}[0-9]{1,2}B?|[A-Z0-9]{3,6}[0-9]{1,2})F$/.test(ticker)) ticker = ticker.slice(0, -1);
