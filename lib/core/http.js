...
export function sendJson(req, res, payload, options = {}) {
  ...
  const contractVersion = effectivePayload?.contractVersion || effectivePayload?.gatewayVersion || effectivePayload?.version || 'valorae-api-v1';
  const deliverySchema = effectivePayload?.delivery?.schemaVersion;
  res.setHeader('X-Valorae-Contract-Version', String(contractVersion).slice(0, 120));
...
export function sendText(res, statusCode, text, type = 'text/plain; charset=utf-8') {
  ...
  res.setHeader('X-Valorae-Contract-Version', 'valorae-api-v1');
