...
import { transformResponsePayload, VALORAE_CONTRACT_RESPONSE_VERSION } from '../contract/response.js';
import { VALORAE_BASELINE_CONTRACT_VERSION } from '../contract/baseline.js';
import { VALORAE_FORMAL_SCHEMA_VERSION } from './feature-versions.js';
import { applyApkCompatibilityHeaders } from './apk-compatibility.js';
...
export function sendJson(req, res, payload, options = {}) {
  ...
  const cacheControl = options.cacheControl || (finalStatus >= 400 ? 'no-store' : 'private, max-age=30');
  const etagMaxBytes = Math.max(16 * 1024, Math.min(1024 * 1024, Number(process.env.VALORAE_ETAG_MAX_BYTES || 192 * 1024)));
  const etagEligible = finalStatus === 200
    && ['GET', 'HEAD'].includes(method)
    && !String(cacheControl).toLowerCase().includes('no-store')
    && bodyBytes <= etagMaxBytes;
  const etag = etagEligible
    ? `"${createHash('sha1').update(stableBody(stripVolatileForEtag(effectivePayload))).digest('base64url')}"`
    : '';
  const reqEtag = String(req?.headers?.['if-none-match'] || '');
  const notModified = Boolean(etag) && reqEtag.split(',').map(s => s.trim()).includes(etag);

  res.statusCode = notModified ? 304 : finalStatus;
  res.setHeader('Content-Type', 'application/json; charset=utf-8');
  if (etag) res.setHeader('ETag', etag);
  else res.removeHeader?.('ETag');
  res.setHeader('Content-Length', String(bodyBytes));
  res.setHeader('X-Valorae-Response-Bytes', String(bodyBytes));
  const cacheStatus = String(effectivePayload?.cache || effectivePayload?.cacheStatus || '').trim().toUpperCase();
  if (/^(HIT|MISS|STALE|BYPASS|COALESCED)$/.test(cacheStatus)) {
    res.setHeader('X-Valorae-Cache', cacheStatus);
  }
  applyApkCompatibilityHeaders(res, req?.valoraeApkCompatibility);
  res.setHeader('X-Valorae-Mobile-Protocol', VALORAE_MOBILE_PROTOCOL_VERSION);
  res.setHeader('X-Valorae-Ecosystem-Contract', RELEASE.ecosystemContract);
  res.setHeader('X-Valorae-Public-Version', RELEASE.publicVersion);
  res.setHeader('X-Valorae-Core-Version', RELEASE.version);
  ...
  const traceId = effectivePayload?.requestId || responseRequestId;
  if (traceId) res.setHeader('X-Valorae-Trace-Id', String(traceId).slice(0, 96));
  const endpointContractVersion = effectivePayload?.contractVersion || effectivePayload?.gatewayVersion || effectivePayload?.version || '';
  const deliverySchema = effectivePayload?.delivery?.schemaVersion;
  // X-Valorae-Contract-Version is the transport-wide protocol contract consumed by
  // every strict APK endpoint. Endpoint-specific schemas stay in the JSON body and
  // are also exposed separately for observability. Mixing both versions caused valid
  // returns and asset-modal payloads to be rejected by the Android continuity guard.
  res.setHeader('X-Valorae-Contract-Version', VALORAE_CONTRACT_RESPONSE_VERSION);
  if (endpointContractVersion) {
    res.setHeader('X-Valorae-Endpoint-Contract-Version', String(endpointContractVersion).slice(0, 120));
  }
  if (deliverySchema) res.setHeader('X-Valorae-Delivery-Schema', String(deliverySchema).slice(0, 32));
  setSecurityHeaders(res, cacheControl);
  applyCors(req, res);
...
export function sendText(res, statusCode, text, type = 'text/plain; charset=utf-8') {
  ...
  res.setHeader('X-Valorae-Public-Version', RELEASE.publicVersion);
  res.setHeader('X-Valorae-Core-Version', RELEASE.version);
  applyApkCompatibilityHeaders(res);
  res.setHeader('X-Valorae-Delivery-Schema', String(VALORAE_ASSET_MODAL_DELIVERY_SCHEMA_VERSION));
  res.setHeader('X-Valorae-Baseline-Contract', VALORAE_BASELINE_CONTRACT_VERSION);
