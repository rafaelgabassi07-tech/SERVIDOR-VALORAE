...
function normalizeComparableRow(item = {}, fallbackTicker = '') {
  ...
  if (!/^[A-Z]{4}[0-9]{1,2}[A-Z]?$/.test(ticker)) return null;
  return {
    ticker,
...
function normalizePeerComparison(value) {
  ...
  const visit = (node, fallback = '', depth = 0) => {
    ...
    if (node && typeof node === 'object') {
      ...
      for (const [key, val] of Object.entries(node)) {
        if (/^[A-Z]{4}[0-9]{1,2}[A-Z]?$/i.test(key) && val && typeof val === 'object') visit({ ...val, ticker: key }, key, depth + 1);
      }
    }
  };
  visit(value);
...
export function buildMobileScraperAssetContract(payload = {}) {
  ...
  const contract = {
    ...
    sobre: results.sobre || results.descricao || results.about || null,
