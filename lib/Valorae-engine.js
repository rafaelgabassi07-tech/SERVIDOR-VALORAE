...
import { isProviderAvailable, recordProviderResult, getProviderHealthSnapshot, providerNameForHost, getProviderCooldown, hydrateProviderHealthFromSharedState, providerHealthSharedStateStats } from './resilience/circuit-breaker.js';
...
import { getFailureCacheShared, setFailureCache, clearFailureCache, failureCacheStats } from './resilience/failure-cache.js';
...
import { buildNativeRequestHeaders, conditionalValidatorHeaders, readTextLimited, responseRetryAfterMs, responseValidators, retryDelayMs } from './http/native-adaptive-fetch.js';
import { providerFetch } from './http/provider-transport.js';
import { sharedStateStats } from './state/shared-runtime-state.js';
import { discoverStructuredPageData, discoveredKnownInternalEndpoints, findStructuredValues } from './scrape/structured-data-discovery.js';
import { fetchAllowedScrapeText } from './scrape/safe-target-fetch.js';
...
async function fetchStatusInvestComplementHtml(ticker, type, options = {}) {
  ...
  await hydrateProviderHealthFromSharedState(['StatusInvest']);
...
export function getValoraeRuntimeStats() {
  return {
    ...
    providerHealthSharedState: providerHealthSharedStateStats(),
    sharedState: sharedStateStats(),
...
async function fetchViaValoraeScrape(url, options = {}) {
  ...
  const promise = (async () => {
    ...
    for (let attempt = 0; attempt <= retryBudget; attempt++) {
      ...
      try {
        const res = await providerFetch(scrapeUrl, {
          method: 'POST',
          headers: {
...
export async function fetchPublicHtml(url, options = {}) {
  ...
  await hydrateProviderHealthFromSharedState([sourceProviderName]);
  ...
  if (useCache && options.refresh !== true && options.cacheBypassed !== true) {
    const recentFailure = await getFailureCacheShared(cacheKey);
  ...
  const fetchWork = async () => {
    ...
    for (const provider of providers) {
      ...
      for (let directAttempt = 0; directAttempt <= directRetryBudget; directAttempt += 1) {
        ...
        try {
          const initialValidators = useCache ? htmlCacheValidators(cacheKey, maxChars, cacheFamilyKey) : {};
          const initialCanonicalUrl = new URL(url).toString();
          const safeFetched = await fetchAllowedScrapeText(url, {
            allowedHosts: allowedTargetHosts(),
            allowSubdomains: false,
            timeoutMs,
            maxRedirects: intEnv('VALORAE_DIRECT_MAX_REDIRECTS', 3),
            signal: controller.signal,
            fetcher: async (targetUrl, { timeoutMs: hopTimeoutMs }) => {
              const baseHeaders = cleanHeaderMap({ ...browserHeaders(targetUrl), ...cleanHeaderMap(options.headers) });
              const validatorHeaders = targetUrl === initialCanonicalUrl
                ? conditionalValidatorHeaders(initialValidators, baseHeaders)
                : {};
              ...
              const response = await providerFetch(targetUrl, {
                ...
                valoraeTransport: { totalTimeoutMs: hopTimeoutMs },
              });
              return {
          ...
          const res = safeFetched.response;
          const contentType = res.headers.get('content-type') || '';
          const finalUrl = safeFetched.finalUrl || res.url || url;
          const status = res.status;
          const retryAfterMs = responseRetryAfterMs(res, { maxMs: 5_000 });
          ...
          await sleep(Math.min(Math.max(retryDelayMs(directAttempt, { baseMs: 180, maxMs: 2500 }), retryAfterMs), Math.max(0, timeoutMs - 50)));
        } catch (err) {
...
function extractEmbeddedInvestidor10Data(html = '', pageUrl = 'https://investidor10.com.br/') {
  const structured = discoverStructuredPageData(html, { url: pageUrl });
  const firstStructured = (patterns) => findStructuredValues(structured, patterns, { limit: 1 })[0]?.value || null;
  const advancedMetrics = extractJsonAssignment(html, [/_sectorIndicators\s*=\s*(\{[\s\S]*?\})\s*;/i, /sectorIndicators\s*=\s*(\{[\s\S]*?\})\s*;/i])
    || firstStructured([/^_?sectorIndicators$/i, /advancedMetrics/i]);
  ...
  const revenueGeography = extractJsonAssignment(html, [/companyRevenuesChartPie\s*=\s*(\{[\s\S]*?\})\s*;/i])
    || revenueEmbeds.revenueGeography
    || firstStructured([/companyRevenuesChartPie/i, /revenueGeography/i, /geograph/i, /region/i]);
  const revenueSegment = extractJsonAssignment(html, [/companyBussinesRevenuesChartPie\s*=\s*(\{[\s\S]*?\})\s*;/i, /companyBusinessRevenuesChartPie\s*=\s*(\{[\s\S]*?\})\s*;/i])
    || revenueEmbeds.revenueSegment
    || firstStructured([/companyBussinesRevenuesChartPie/i, /companyBusinessRevenuesChartPie/i, /revenueSegment/i, /businessRevenue/i]);
  ...
  const rentabilidadeChart = extractRentabilidadeChart(html)
    || firstStructured([/rentabilidadeChart/i, /profitabilityChart/i, /profitabilities/i]);
  const pickId = (patterns, structuredPatterns = []) => {
    for (const re of patterns) {
      const m = html.match(re);
      if (m?.[1]) return m[1];
    }
    const structuredValue = firstStructured(structuredPatterns);
    const scalar = structuredValue && typeof structuredValue === 'object'
      ? (structuredValue.id || structuredValue.value || '')
      : structuredValue;
    return /^\d+$/.test(String(scalar || '').trim()) ? String(scalar).trim() : '';
  };
  ...
  ], [/^(?:companyId|company_id|companyID|idCompany|empresaId|idEmpresa)$/i]);
  ...
  ], [/^(?:tickerId|ticker_id|idTicker|stockId|stock_id|acaoId|idAcao|assetId|asset_id)$/i]);
  const fiiId = pickId([
  ...
  ], [/^(?:fiiId|fii_id|idFii|fundId|fund_id)$/i]);
  return {
    advancedMetrics,
    revenueGeography,
    revenueSegment,
    ...
    structuredData: {
      ...
      policyVersion: structured.policyVersion,
      summary: structured.summary,
      internalEndpoints: discoveredKnownInternalEndpoints(structured),
      outputPolicy: 'legacy-first-structured-fallback-only',
    },
  };
}
...
async function fetchInvestidor10ApiExtras(ticker, type, html, options = {}) {
  ...
  const pageUrl = `https://investidor10.com.br/${type === 'FII' ? 'fiis' : 'acoes'}/${String(ticker || '').toLowerCase()}/`;
  const ids = extractEmbeddedInvestidor10Data(html, pageUrl);
  ...
  if (ids.structuredData) apiExtras.embedded.structuredData = ids.structuredData;
  ...
  const tasks = [];
  const discoveredTasks = Array.isArray(ids.structuredData?.internalEndpoints) ? ids.structuredData.internalEndpoints : [];
  for (const discovered of discoveredTasks) tasks.push([discovered.key, discovered.url]);
  ...
  for (const [key, url] of tasks) {
    const normalizedUrl = String(url || '');
    if (!normalizedUrl || taskUrls.has(normalizedUrl)) continue;
    taskUrls.add(normalizedUrl);
    uniqueTasks.push([key, normalizedUrl]);
  }
  tasks.length = 0;
...
const VALORAE_ANALYSIS_PAGE_VERSION = '21.12.103-analysis-unique-contract';

function resetAnalysisDisplayValue(value) {
  if (value === undefined || value === null || value === '') return '';
  if (Array.isArray(value)) return value.map(resetAnalysisDisplayValue).filter(Boolean).join(' • ');
  if (typeof value === 'object') {
    const candidate = firstDefined(value.display, value.valueDisplay, value.formatted, value.raw, value.value, value.numericValue, value.amount, value.percent, value.name, value.label);
    return resetAnalysisDisplayValue(candidate);
  }
...
function resetNestedValue(obj, path = '') {
  return String(path).split('.').reduce((acc, key) => acc == null ? undefined : acc[key], obj);
}

function resetFirstDisplayFrom(payload = {}, paths = []) {
  for (const path of paths) {
    const value = resetNestedValue(payload, path);
    const display = resetAnalysisDisplayValue(value);
    if (display) return display;
  }
  return '';
}

function resetMetricDisplay(payload = {}, key = '') {
  return resetFirstDisplayFrom(payload, [
    `appPayload.metrics.canonical.${key}`,
    `appMobileSnapshot.metrics.${key}`,
    `normalized.${key}`,
    `results.${key}`,
    `results.indicadores.${key}`,
    `results.fundamentos.${key}`,
    `results.fundamentals.${key}`,
    `results.indicadoresAvancados.${key}`,
    `results.indicadoresFundamentalistas.semComparativos.${key}`,
    `results.sections.indicadores.${key}`,
  ]);
}

function resetFormatBrl(value) {
  const n = normalizeBRNumber(value);
  if (n === undefined) return resetAnalysisDisplayValue(value);
  return `R$ ${n.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
}

function resetFormatPercentDisplay(value) {
  const raw = resetAnalysisDisplayValue(value);
  if (!raw) return '';
...
function resetFormatGenericNumber(value) {
  const raw = resetAnalysisDisplayValue(value);
  if (!raw) return '';
  if (/R\$|US\$|%|[A-Za-zÀ-ÿ]/.test(raw.replace(/\s/g, ''))) return raw;
  const n = normalizeBRNumber(raw);
  if (n === undefined) return raw;
  return n.toLocaleString('pt-BR', { minimumFractionDigits: Math.abs(n) < 10 ? 2 : 2, maximumFractionDigits: 2 });
}

function resetFormatAnalysisRowValue(label = '', value) {
  const l = text(label).normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
  if (/cotacao|preco|valor de mercado|valor patrimonial|patrimonio do fundo|patrimonio liquido|ultimo rendimento|liquidez diaria|liquidez media|taxa de administracao/.test(l)) return resetFormatBrl(value);
  if (/variacao|dividend yield|\bdy\b|yield|payout|margem|roe|roic|roa|cagr|tag along|free float|vacancia/.test(l)) return resetFormatPercentDisplay(value);
  if (/p\/|ev\/|vpa|lpa|giro|divida|patrimonio \/ ativos|passivos \/ ativos|liquidez corrente/.test(l)) return resetFormatGenericNumber(value);
  return resetAnalysisDisplayValue(value);
}

function resetAddAnalysisRow(rows, seen, label, value, detail = '') {
  const cleanLabel = text(label).replace(/\s+/g, ' ').trim();
  const cleanValue = resetFormatAnalysisRowValue(cleanLabel, value);
...
function resetFindStockFundamentalDisplay(payload = {}, keyOrLabel = '') {
  const normalizedTarget = resetNormalizeIndicatorLabel(keyOrLabel);
  const field = STOCK_FUNDAMENTAL_INDICATOR_FIELDS.find(item => {
    const aliases = [item.key, item.label, ...(item.aliases || [])].map(resetNormalizeIndicatorLabel);
    return aliases.includes(normalizedTarget);
  });
  if (!field) return resetMetricDisplay(payload, keyOrLabel);
  return resetIndicatorValueFromObjects(payload, field)
    || resetMetricDisplay(payload, field.key)
    || resetIndicatorValueFromCards(payload, field);
}

function resetFindVariation12mDisplay(payload = {}) {
  const field = {
    key: 'variacao12m',
    label: 'Variação (12M)',
    aliases: ['Variação (12M)', 'Variação 12M', 'Variação 12 meses', 'Valorização (12m)', 'Valorização 12M', 'Valorização 12 meses', 'Rentabilidade 12M', '12M']
  };
  return resetIndicatorValueFromObjects(payload, field)
    || resetMetricDisplay(payload, 'variacao12m')
    || resetMetricDisplay(payload, 'valorizacao12m')
    || resetMetricDisplay(payload, 'rentabilidade12m')
    || resetIndicatorValueFromCards(payload, field)
    || resetFirstDisplayFrom(payload, [
      'results.variacao12m',
      'results.valorizacao12m',
      'results.variacao12Meses',
      'results.valorizacao12Meses',
      'results.rentabilidade12m',
      'results.quote.variacao12m',
      'appPayload.quote.variacao12m',
      'appMobileSnapshot.quote.variacao12m'
    ]);
}


function resetBuildSummaryRows(payload = {}, isFii = false) {
  const rows = [];
  const seen = new Set();
  const results = payload.results || {};
  const sections = results.sections || {};
  ...
  const quote = payload.appMobileSnapshot?.quote || payload.appPayload?.quote || results.quote || results.cotacao || {};
  const ticker = text(firstDefined(payload.ticker, payload.symbol, quote.ticker, quote.symbol)).toUpperCase();
  const type = text(firstDefined(payload.type, quote.type, results.type, isFii ? 'FII' : 'ACAO')).toUpperCase();
  const name = firstDefined(quote.name, results.nome, results.name, results.razaoSocial, results.companyName, results.nomeEmpresa, ticker);

  // Os cinco primeiros itens são o Resumo visual do topo, seguindo o padrão: Cotação, Variação (12M), P/L, P/VP e DY.
  resetAddAnalysisRow(rows, seen, isFii ? 'Cotação da cota' : 'Cotação', firstDefined(quote.priceDisplay, quote.precoAtual, quote.price, resetMetricDisplay(payload, 'precoAtual'), results.precoAtual), 'Resumo do topo');
  resetAddAnalysisRow(rows, seen, 'Variação (12M)', firstDefined(resetFindVariation12mDisplay(payload), quote.variacao12m, quote.change12m, results.variacao12m, results.change12m), 'Resumo do topo');

  if (isFii) {
    resetAddAnalysisRow(rows, seen, 'DY 12M', firstDefined(resetMetricDisplay(payload, 'yield12m'), resetMetricDisplay(payload, 'dividendYield'), results.yield12m, results.dividendYield, results.dy), 'Dividendos');
    resetAddAnalysisRow(rows, seen, 'P/VP', firstDefined(resetFindStockFundamentalDisplay(payload, 'pvp'), resetMetricDisplay(payload, 'pvp'), results.pvp, results.p_vp), 'Valuation');
    resetAddAnalysisRow(rows, seen, 'Liquidez diária', firstDefined(resetMetricDisplay(payload, 'liquidezMediaDiaria'), results.liquidezDiaria, results.volumeMedio, fiiInfo.liquidezDiaria), 'Liquidez');
    resetAddAnalysisRow(rows, seen, 'Último rendimento', firstDefined(resetMetricDisplay(payload, 'ultimoRendimento'), results.ultimoRendimento, fiiInfo.ultimoRendimento), 'Rendimentos');
    resetAddAnalysisRow(rows, seen, 'Valor patrimonial/cota', firstDefined(resetMetricDisplay(payload, 'valorPatrimonialCota'), results.valorPatrimonialPorCota, results.valorPatrimonial, fiiInfo.valorPatrimonialPorCota, fiiInfo.valorPatrimonial), 'Patrimônio');
    resetAddAnalysisRow(rows, seen, 'Patrimônio do fundo', firstDefined(resetMetricDisplay(payload, 'patrimonioLiquido'), results.valorPatrimonialTotal, results.patrimonioLiquido, fiiInfo.valorPatrimonialTotal), 'Patrimônio');
    resetAddAnalysisRow(rows, seen, 'Vacância', firstDefined(resetMetricDisplay(payload, 'vacanciaFisica'), results.vacanciaFisica, results.vacancia, fiiInfo.vacanciaFisica, fiiInfo.vacancia), 'Portfólio');
    resetAddAnalysisRow(rows, seen, 'Cotistas', firstDefined(results.numeroCotistas, fiiInfo.numeroCotistas), 'Portfólio');
    resetAddAnalysisRow(rows, seen, 'Cotas emitidas', firstDefined(results.cotasEmitidas, fiiInfo.cotasEmitidas), 'Portfólio');
    resetAddAnalysisRow(rows, seen, 'Segmento', firstDefined(results.segmentoFii, results.segmento, fiiInfo.segmentoFii, fiiInfo.segmento), 'Perfil');
    resetAddAnalysisRow(rows, seen, 'Tipo de fundo', firstDefined(results.tipoFundo, fiiInfo.tipoFundo), 'Perfil');
    resetAddAnalysisRow(rows, seen, 'Mandato', firstDefined(results.mandato, fiiInfo.mandato), 'Perfil');
    resetAddAnalysisRow(rows, seen, 'Gestão', firstDefined(results.tipoGestao, fiiInfo.tipoGestao), 'Perfil');
    resetAddAnalysisRow(rows, seen, 'Taxa de administração', firstDefined(results.taxaAdministracao, fiiInfo.taxaAdministracao), 'Custos');
    resetAddAnalysisRow(rows, seen, 'CNPJ', firstDefined(results.cnpj, fiiInfo.cnpj), 'Cadastro');
    resetAddAnalysisRow(rows, seen, 'Ticker', ticker, 'Identidade');
    resetAddAnalysisRow(rows, seen, 'Nome do fundo', name, 'Identidade');
    resetAddAnalysisRow(rows, seen, 'Classe', 'FII', 'Identidade');
  } else {
    resetAddAnalysisRow(rows, seen, 'P/L', firstDefined(resetFindStockFundamentalDisplay(payload, 'pl'), resetMetricDisplay(payload, 'pl'), results.pl, results.p_l), 'Valuation');
    resetAddAnalysisRow(rows, seen, 'P/VP', firstDefined(resetFindStockFundamentalDisplay(payload, 'pvp'), resetMetricDisplay(payload, 'pvp'), results.pvp, results.p_vp), 'Valuation');
    resetAddAnalysisRow(rows, seen, 'DY', firstDefined(resetFindStockFundamentalDisplay(payload, 'dividendYield'), resetMetricDisplay(payload, 'dividendYield'), results.dividendYield, results.dy), 'Dividendos');
    resetAddAnalysisRow(rows, seen, 'Liquidez diária', firstDefined(resetMetricDisplay(payload, 'liquidezMediaDiaria'), results.liquidezDiaria, results.volumeMedio, results.averageVolume), 'Liquidez');
    resetAddAnalysisRow(rows, seen, 'Setor', firstDefined(results.setor, results.sector, empresa.dados?.setor, empresa.informacoes?.setor), 'Classificação');
    resetAddAnalysisRow(rows, seen, 'Subsetor', firstDefined(results.subsetor, results.subsector, empresa.dados?.subsetor, empresa.informacoes?.subsetor), 'Classificação');
    resetAddAnalysisRow(rows, seen, 'Segmento', firstDefined(results.segmento, results.segment, empresa.dados?.segmento, empresa.informacoes?.segmento), 'Classificação');
    resetAddAnalysisRow(rows, seen, 'Valor de mercado', firstDefined(resetMetricDisplay(payload, 'valorDeMercado'), results.valorDeMercado, empresa.informacoes?.valorDeMercado), 'Empresa');
    resetAddAnalysisRow(rows, seen, 'Patrimônio líquido', firstDefined(resetMetricDisplay(payload, 'patrimonioLiquido'), results.patrimonioLiquido, empresa.informacoes?.patrimonioLiquido), 'Empresa');
    resetAddAnalysisRow(rows, seen, 'ROE', firstDefined(resetMetricDisplay(payload, 'roe'), results.roe), 'Rentabilidade');
    resetAddAnalysisRow(rows, seen, 'ROIC', firstDefined(resetMetricDisplay(payload, 'roic'), results.roic), 'Rentabilidade');
    resetAddAnalysisRow(rows, seen, 'Margem líquida', firstDefined(resetMetricDisplay(payload, 'margemLiquida'), results.margemLiquida), 'Rentabilidade');
    resetAddAnalysisRow(rows, seen, 'Payout', firstDefined(resetMetricDisplay(payload, 'payout'), results.payout), 'Dividendos');
    resetAddAnalysisRow(rows, seen, 'Free float', firstDefined(results.freeFloat, empresa.informacoes?.freeFloat), 'Governança');
    resetAddAnalysisRow(rows, seen, 'Tag along', firstDefined(results.tagAlong, empresa.informacoes?.tagAlong), 'Governança');
    resetAddAnalysisRow(rows, seen, 'CNPJ', firstDefined(results.cnpj, empresa.dados?.cnpj, empresa.informacoes?.cnpj), 'Cadastro');
    resetAddAnalysisRow(rows, seen, 'Ticker', ticker, 'Identidade');
    resetAddAnalysisRow(rows, seen, 'Nome da empresa', name, 'Identidade');
    resetAddAnalysisRow(rows, seen, 'Classe', type, 'Identidade');
  }

  return rows.slice(0, 60);
}

const STOCK_FUNDAMENTAL_INDICATOR_FIELDS = [
  { key: 'pl', label: 'P/L', aliases: ['P/L', 'PL', 'Preço/Lucro', 'Preco/Lucro'] },
  { key: 'psr', label: 'P/Receita (PSR)', aliases: ['P/Receita (PSR)', 'P/Receita', 'PSR', 'P/SR', 'Preço/Receita', 'Preco/Receita'] },
  { key: 'pvp', label: 'P/VP', aliases: ['P/VP', 'PVP'] },
  { key: 'dividendYield', label: 'Dividend Yield', aliases: ['Dividend Yield', 'DY', 'D.Y'] },
  { key: 'payout', label: 'Payout', aliases: ['Payout'] },
  { key: 'margemLiquida', label: 'Margem Líquida', aliases: ['Margem Líquida', 'Margem Liquida', 'M. Líquida', 'M. Liquida'] },
  { key: 'margemBruta', label: 'Margem Bruta', aliases: ['Margem Bruta', 'M. Bruta'] },
  { key: 'margemEbit', label: 'Margem Ebit', aliases: ['Margem Ebit', 'Margem EBIT', 'M. EBIT', 'M. Ebit'] },
  { key: 'margemEbitda', label: 'Margem Ebitda', aliases: ['Margem Ebitda', 'Margem EBITDA', 'M. EBITDA', 'M. Ebitda'] },
  { key: 'evEbitda', label: 'EV/Ebitda', aliases: ['EV/Ebitda', 'EV/EBITDA'] },
  { key: 'evEbit', label: 'EV/Ebit', aliases: ['EV/Ebit', 'EV/EBIT'] },
  { key: 'pEbitda', label: 'P/Ebitda', aliases: ['P/Ebitda', 'P/EBITDA'] },
  { key: 'pEbit', label: 'P/Ebit', aliases: ['P/Ebit', 'P/EBIT'] },
  { key: 'pAtivo', label: 'P/Ativo', aliases: ['P/Ativo'] },
  { key: 'pCapGiro', label: 'P/Cap.Giro', aliases: ['P/Cap.Giro', 'P/Cap Giro', 'Preço/Capital de Giro', 'Preco/Capital de Giro'] },
  { key: 'pAtivoCircLiq', label: 'P/Ativo Circ. Liq.', aliases: ['P/Ativo Circ. Liq.', 'P/Ativo Circulante Líquido', 'P/Ativo Circulante Liquido'] },
  { key: 'vpa', label: 'VPA', aliases: ['VPA', 'Valor Patrimonial por Ação', 'Valor Patrimonial por Acao'] },
  { key: 'lpa', label: 'LPA', aliases: ['LPA', 'Lucro por Ação', 'Lucro por Acao'] },
  { key: 'giroAtivos', label: 'Giro Ativos', aliases: ['Giro Ativos', 'Giro de Ativos'] },
  { key: 'roe', label: 'ROE', aliases: ['ROE'] },
  { key: 'roic', label: 'ROIC', aliases: ['ROIC'] },
  { key: 'roa', label: 'ROA', aliases: ['ROA'] },
  { key: 'dividaLiquidaPatrimonio', label: 'Dívida Líquida / Patrimônio', aliases: ['Dívida Líquida / Patrimônio', 'Divida Liquida / Patrimonio', 'Dív. Líquida/Patrimônio', 'Divida Liquida Patrimonio', 'Dív. líquida/PL', 'Dívida Líquida/PL'] },
  { key: 'dividaLiquidaEbitda', label: 'Dívida Líquida / Ebitda', aliases: ['Dívida Líquida / Ebitda', 'Divida Liquida / Ebitda', 'Dív. Líquida/EBITDA', 'Dívida Líquida/EBITDA'] },
  { key: 'dividaLiquidaEbit', label: 'Dívida Líquida / Ebit', aliases: ['Dívida Líquida / Ebit', 'Divida Liquida / Ebit', 'Dív. Líquida/EBIT', 'Dívida Líquida/EBIT'] },
  { key: 'dividaBrutaPatrimonio', label: 'Dívida Bruta / Patrimônio', aliases: ['Dívida Bruta / Patrimônio', 'Divida Bruta / Patrimonio', 'Dív. Bruta/Patrimônio', 'Dív. bruta/PL', 'Dívida Bruta/PL'] },
  { key: 'patrimonioAtivos', label: 'Patrimônio / Ativos', aliases: ['Patrimônio / Ativos', 'Patrimonio / Ativos', 'PL/Ativos'] },
  { key: 'passivosAtivos', label: 'Passivos / Ativos', aliases: ['Passivos / Ativos'] },
  { key: 'liquidezCorrente', label: 'Liquidez Corrente', aliases: ['Liquidez Corrente', 'Liq. corrente'] },
  { key: 'cagrReceitas5Anos', label: 'CAGR Receitas 5 anos', aliases: ['CAGR Receitas 5 anos', 'CAGR Receita 5 anos', 'CAGR Receitas 5 Anos', 'cagrReceitas5a'] },
  { key: 'cagrLucros5Anos', label: 'CAGR Lucros 5 anos', aliases: ['CAGR Lucros 5 anos', 'CAGR Lucro 5 anos', 'CAGR Lucros 5 Anos', 'cagrLucros5a'] },
];

const STOCK_FUNDAMENTAL_ALIAS_INDEX = STOCK_FUNDAMENTAL_INDICATOR_FIELDS.reduce((acc, field) => {
  for (const alias of [field.label, field.key, ...(field.aliases || [])]) {
    const normalized = resetNormalizeIndicatorLabel(alias);
    if (normalized) acc[normalized] = field;
  }
  return acc;
}, {});

function resetNormalizeIndicatorLabel(label = '') {
  return text(label)
...
function resetFormatIndicatorDisplay(value, field = {}) {
  const display = resetAnalysisDisplayValue(value);
  if (!display) return '';
  const lower = display.toLowerCase();
  if (/%|r\$|x/.test(lower)) return display;
  if (/yield|payout|margem|roe|roic|roa|cagr/i.test(field.label || field.key || '')) return `${display}%`;
  return display;
}

function resetIndicatorValueFromCards(payload = {}, field = {}) {
  const pools = [
    payload.assetChartBundle?.indicatorCards,
    payload.assetChartsMobile?.indicatorCards,
    payload.appPayload?.charts?.assetChartBundle?.indicatorCards,
    payload.appPayload?.assetChartBundle?.indicatorCards,
    payload.appMobileSnapshot?.assetChartBundle?.indicatorCards,
    payload.results?.assetChartBundle?.indicatorCards,
    payload.results?.assetChartsMobile?.indicatorCards,
    payload.indicatorCards,
  ].filter(Array.isArray);
  const aliases = [field.label, field.key, ...(field.aliases || [])].map(resetNormalizeIndicatorLabel).filter(Boolean);
  for (const cards of pools) {
    for (const card of cards) {
      const label = resetNormalizeIndicatorLabel(firstDefined(card?.label, card?.name, card?.title, card?.key));
      if (!label) continue;
      if (aliases.includes(label) || aliases.some(alias => label === alias || label.includes(alias) || alias.includes(label))) {
        const value = firstDefined(card.display, card.valueDisplay, card.formatted, card.raw, card.value, card.numericValue, card.amount, card.percent);
        const formatted = resetFormatIndicatorDisplay(value, field);
...
function resetIndicatorValueFromObjects(payload = {}, field = {}) {
  const key = field.key;
  const containers = [
    payload.normalized,
    ...
    payload.results,
    payload.results?.indicadores,
    payload.results?.indicadoresFundamentalistas,
    payload.results?.indicadoresFundamentalistas?.semComparativos,
    payload.results?.indicadoresAvancados,
    payload.results?.valuation,
    payload.results?.profitability,
    payload.results?.debt,
    payload.results?.sections?.indicadores,
    payload.appPayload?.metrics?.canonical,
    payload.appMobileSnapshot?.metrics,
  ].filter(Boolean);
  const candidateKeys = [key, ...(field.aliases || []).map(alias => resetNormalizeIndicatorLabel(alias).replace(/(^|\s)([a-z0-9])/g, (_, a, b) => a ? b.toUpperCase() : b).replace(/\s+/g, ''))];
  for (const container of containers) {
    for (const candidate of candidateKeys) {
      if (Object.prototype.hasOwnProperty.call(container, candidate)) {
        const formatted = resetFormatIndicatorDisplay(container[candidate], field);
        if (formatted) return formatted;
      }
    }
    const directByLabel = Object.entries(container).find(([k]) => {
      const normalized = resetNormalizeIndicatorLabel(k);
      return normalized && [field.label, field.key, ...(field.aliases || [])].map(resetNormalizeIndicatorLabel).includes(normalized);
    });
    if (directByLabel) {
      const formatted = resetFormatIndicatorDisplay(directByLabel[1], field);
      if (formatted) return formatted;
    }
  }
  return '';
...
function resetBuildStockFundamentalRows(payload = {}) {
  const rows = [];
  const seen = new Set();
  for (const field of STOCK_FUNDAMENTAL_INDICATOR_FIELDS) {
    const value = resetIndicatorValueFromObjects(payload, field)
      || resetMetricDisplay(payload, field.key)
      || resetIndicatorValueFromCards(payload, field);
    resetAddAnalysisRow(rows, seen, field.label, value, 'Indicador fundamentalista do Investidor10');
  }
  return rows;
}


function indicatorRowsFromPayload(payload = {}, limit = 80) {
  const rows = [];
  const seen = new Set();
  const cardsPools = [
    payload.assetChartBundle?.indicatorCards,
  ...
  for (const cards of cardsPools) {
    for (const card of cards) {
      const label = firstDefined(card?.label, card?.name, card?.title, card?.key);
      const value = firstDefined(card?.display, card?.valueDisplay, card?.formatted, card?.raw, card?.value, card?.numericValue, card?.amount, card?.percent);
      resetAddAnalysisRow(rows, seen, label, value, card?.source || card?.group || 'Indicador capturado');
  ...
  const containers = [
    ...
    payload.normalized,
    payload.results?.indicadores,
    payload.results?.fundamentals,
    payload.results?.fundamentos,
    payload.results?.indicadoresAvancados,
    payload.results?.indicadoresFundamentalistas?.semComparativos,
    payload.appPayload?.metrics?.canonical,
    payload.appMobileSnapshot?.metrics,
  ...
  for (const container of containers) {
    for (const [key, value] of Object.entries(container)) {
      if (rows.length >= limit) return rows;
      resetAddAnalysisRow(rows, seen, key, value, 'Indicador normalizado');
    }
  }
  return rows;
}

function resetBuildFundamentalRows(payload = {}, isFii = false) {
  if (!isFii) {
    const stockRows = resetBuildStockFundamentalRows(payload);
    if (stockRows.length) return stockRows;
  }
  const rows = [];
  const seen = new Set();
  const keys = isFii
    ? ['yield12m', 'pvp', 'liquidezMediaDiaria', 'ultimoRendimento', 'valorPatrimonialCota', 'vacanciaFisica']
    : ['pl', 'pvp', 'dividendYield', 'roe', 'roic', 'margemLiquida', 'payout', 'liquidezMediaDiaria'];
  for (const key of keys) {
    const value = resetMetricDisplay(payload, key);
    if (value) resetAddAnalysisRow(rows, seen, key, value, 'Indicador normalizado');
  }
  const payloadIndicators = indicatorRowsFromPayload(payload, 80);
  for (const row of payloadIndicators) resetAddAnalysisRow(rows, seen, row.label, row.value, row.detail || 'Indicador capturado');
  return rows.slice(0, 80);
}

function resetBuildAnalysisSection({ key, title, kind, rows, source, required = true, reasonWhenEmpty = 'Dados ainda não entregues pela fonte para esta seção.' }) {
  const safeRows = Array.isArray(rows) ? rows.filter(row => resetAnalysisDisplayValue(row?.label) && resetAnalysisDisplayValue(row?.value)) : [];
  return {
    key,
    title,
    kind,
    required,
    source,
    status: safeRows.length ? 'ready' : 'empty',
    itemCount: safeRows.length,
    rows: safeRows,
    previewRows: safeRows,
    reason: safeRows.length ? 'Dados reais enviados pelo contrato novo da página Análise.' : reasonWhenEmpty,
  };
}

function buildAssetAnalysisPage(payload = {}) {
  const results = payload.results || {};
  const ticker = text(firstDefined(payload.ticker, payload.symbol, payload.appMobileSnapshot?.ticker, results.ticker, results.symbol)).toUpperCase();
  const type = text(firstDefined(payload.type, payload.appMobileSnapshot?.type, results.type, inferAssetType(ticker))).toUpperCase();
  const isFii = /FII|FUNDO|FIAGRO|FI-INFRA/i.test(type);
  const summaryRows = resetBuildSummaryRows(payload, isFii);
  const fundamentalRows = resetBuildFundamentalRows(payload, isFii);
  const sections = [
    resetBuildAnalysisSection({
      key: 'assetSummary',
      title: isFii ? 'Resumo do FII' : 'Resumo do Ativo',
      kind: 'summary',
      rows: summaryRows,
      source: 'StatusInvest / Investidor10 / contrato normalizado VALORAE Proxy',
      required: true,
      reasonWhenEmpty: 'Resumo não foi montado porque a fonte não entregou campos mínimos reais.'
    }),
    resetBuildAnalysisSection({
      key: isFii ? 'fiiFundamentalIndicators' : 'stockFundamentalIndicators',
      title: isFii ? 'Indicadores fundamentalistas do FII' : `Indicadores fundamentalistas ${ticker}`.trim(),
      kind: 'fundamental_indicators',
      rows: fundamentalRows,
      source: 'StatusInvest / Investidor10 / Indicadores Fundamentalistas / contrato VALORAE Proxy',
      required: !isFii,
      reasonWhenEmpty: 'Indicadores fundamentalistas não foram enviados porque a fonte não entregou valores reais capturáveis.'
    })
  ];
  ...
  return {
    ...
    ticker,
    type,
    source: 'VALORAE Proxy / StatusInvest primário + Investidor10 referência',
    policy: 'analysis-v4-summary-uses-fundamentals-and-statusinvest-no-synthetic-values',
    generatedAt: payload.metrics?.generatedAt || nowIso(),
    summary: {
      totalSections: sections.length,
      capturedSections: captured,
      pendingSections: sections.length - captured,
      hasSummary: summaryRows.length > 0,
      hasFundamentals: fundamentalRows.length > 0,
      expectedStockFundamentalIndicators: isFii ? 0 : STOCK_FUNDAMENTAL_INDICATOR_FIELDS.length,
      capturedStockFundamentalIndicators: isFii ? 0 : fundamentalRows.length,
    },
    sections,
    sourcePriority: ['StatusInvest', 'Investidor10', 'YahooChart'],
    analysisPrimarySource: results.analysisPrimarySource || 'StatusInvest/Investidor10',
    appGuidance: 'A página Análise deve renderizar módulos próprios. StatusInvest é fonte primária de números quando disponível; o Resumo também usa os Indicadores Fundamentalistas já capturados para completar P/L, P/VP e DY; Investidor10 mantém a referência visual/ordem das seções. Sem dados sintéticos.'
  };
}
...
async function fetchYahooChart(ticker, options = {}) {
  ...
  for (const host of hosts) {
    ...
    const timer = setTimeout(() => controller.abort(), timeoutMs);
    try {
      const res = await providerFetch(url, { signal: controller.signal, headers: { 'User-Agent': browserHeaders(url)['User-Agent'], 'Accept': 'application/json' } });
      if (!res.ok) continue;
...
async function fetchGoogleNews(ticker, aliases = [], limit = DEFAULT_NEWS_LIMIT, options = {}) {
  ...
  const assetOnly = Boolean(options.assetOnly || options.strictAsset || options.assetNewsOnly);
  ...
  const cacheKey = `news:${asOfDay}:${clean || 'GERAL'}:${baseTerms.join('|')}:${safeLimit}:assetOnly=${assetOnly}`;
  ...
  const promise = (async () => {
    ...
    const timer = setTimeout(() => controller.abort(), timeoutMs);
    try {
      const res = await providerFetch(url, { signal: controller.signal, headers: { 'User-Agent': browserHeaders(url)['User-Agent'], 'Accept': 'application/rss+xml,application/xml,text/xml' } });
      ...
      const items = sortNewsNewestFirst(
        rawItems
          ...
          .filter(item => {
            ...
            const now = Date.now();
            const strictAssetMatch = !assetOnly || matchesStrictAssetNews(item, clean, baseTerms);
            return strictAssetMatch && item.relevanceScore > 0 && ts > 0 && ts <= now + 10 * 60_000 && (now - ts) <= 36 * 60 * 60 * 1000;
          })
      ).slice(0, safeLimit);
      if (!items.length) {
      ...
      const value = {
        ...
        timeoutMs,
        assetOnly,
        appPolicy: { canReplacePreviousNews: true, shouldKeepPreviousNews: false, optionalBlock: true, queryWindow: 'when:1d', maxAgeHours: 36, assetOnly },
...
const NEWS_ASSET_ALIAS_STOPWORDS = new Set([
...
function normalizeNewsAssetText(value = '') {
  return String(value || '')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
...
function newsAssetAliasTokens(aliases = []) {
  const tokens = [];
  for (const alias of aliases) {
    const normalized = normalizeNewsAssetText(alias);
    if (normalized.length >= 5) tokens.push(normalized);
    for (const token of normalized.split(' ')) {
      if (token.length >= 5 && !NEWS_ASSET_ALIAS_STOPWORDS.has(token)) tokens.push(token);
    }
  }
  return uniq(tokens).slice(0, 16);
}

function matchesStrictAssetNews(item, ticker, aliases = []) {
  const hay = normalizeNewsAssetText(`${item.title || ''} ${item.snippet || ''} ${item.summary || ''} ${item.source || ''}`);
  const cleanTicker = normalizeNewsAssetText(ticker).replace(/\s+/g, '');
  if (cleanTicker && hay.replace(/\s+/g, '').includes(cleanTicker)) return true;
  return newsAssetAliasTokens(aliases).some(alias => hay.includes(alias));
}
...
export class ValoraeEngine {
  ...
  static async _fetchAtivoUncached(rawTicker, rawType, options = {}) {
    const started = performance.now();
    await hydrateProviderHealthFromSharedState(['Investidor10', 'StatusInvest', 'YahooChart']);
    const ticker = canonicalizeTicker(rawTicker);
    const validation = validarTicker(ticker);
    if (validation) throw new Error(validation);
    const type = rawType || inferAssetType(ticker);
