...
const VALORAE_ANALYSIS_PAGE_VERSION = '21.12.101-analysis-hybrid-statusinvest-primary';
...
function resetBuildSummaryRows(payload = {}, isFii = false) {
  ...
  resetAddAnalysisRow(rows, seen, 'Variação (12M)', firstDefined(resetMetricDisplay(payload, 'variacao12m'), quote.variacao12m, quote.change12m, results.variacao12m, results.change12m), 'Resumo do topo');
  ...
  if (isFii) {
    ...
    resetAddAnalysisRow(rows, seen, 'P/VP', firstDefined(resetMetricDisplay(payload, 'pvp'), results.pvp, results.p_vp), 'Valuation');
    ...
    resetAddAnalysisRow(rows, seen, 'P/L', firstDefined(resetMetricDisplay(payload, 'pl'), results.pl, results.p_l), 'Valuation');
    resetAddAnalysisRow(rows, seen, 'P/VP', firstDefined(resetMetricDisplay(payload, 'pvp'), results.pvp, results.p_vp), 'Valuation');
    resetAddAnalysisRow(rows, seen, 'DY', firstDefined(resetMetricDisplay(payload, 'dividendYield'), results.dividendYield, results.dy), 'Dividendos');
...
function buildAssetAnalysisPage(payload = {}) {
  ...
  const sections = [
    resetBuildAnalysisSection({
      ...
      kind: 'summary',
      rows: summaryRows,
      source: 'Investidor10 / topo do ativo / contrato normalizado VALORAE Proxy',
      required: true,
    ...
    resetBuildAnalysisSection({
      ...
      source: 'Investidor10 / Indicadores Fundamentalistas / contrato VALORAE Proxy',
      required: !isFii,
  ...
  return {
    ...
    policy: 'analysis-v3-statusinvest-primary-investidor10-layout-no-synthetic-values',
    ...
    appGuidance: 'A página Análise deve renderizar módulos próprios. StatusInvest é fonte primária de números quando disponível; Investidor10 mantém a referência visual/ordem das seções. Sem dados sintéticos.'
  };
}
