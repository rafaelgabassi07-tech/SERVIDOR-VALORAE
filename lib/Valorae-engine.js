...
const VALORAE_ANALYSIS_PAGE_VERSION = '21.12.99-analysis-reset-clean-summary';
...
function resetAddAnalysisRow(rows, seen, label, value, detail = '') {
  ...
  const cleanValue = resetAnalysisDisplayValue(value);
...
function resetBuildSummaryRows(payload = {}, isFii = false) {
  ...
  resetAddAnalysisRow(rows, seen, isFii ? 'Nome do fundo' : 'Nome da empresa', name, 'Identidade');
  resetAddAnalysisRow(rows, seen, 'Classe', isFii ? 'FII' : type, 'Identidade');
  resetAddAnalysisRow(rows, seen, isFii ? 'Cotação da cota' : 'Cotação', firstDefined(quote.priceDisplay, quote.precoAtual, quote.price, resetMetricDisplay(payload, 'precoAtual'), results.precoAtual), 'Topo do ativo');
  resetAddAnalysisRow(rows, seen, 'Variação 12M', firstDefined(resetMetricDisplay(payload, 'variacao12m'), quote.variacao12m, quote.change12m, results.variacao12m, results.change12m), 'Topo do ativo');
  ...
  if (isFii) {
    ...
    resetAddAnalysisRow(rows, seen, 'Dividend Yield', firstDefined(resetMetricDisplay(payload, 'dividendYield'), results.dividendYield, results.dy), 'Dividendos');
...
function resetBuildFundamentalRows(payload = {}, isFii = false) {
  ...
  const payloadIndicators = indicatorRowsFromPayload(payload, 40);
...
function buildAssetAnalysisPage(payload = {}) {
  ...
  const sections = [
    ...
    resetBuildAnalysisSection({
      key: 'fundamentalSnapshot',
      title: 'Indicadores fundamentalistas',
      kind: 'metrics',
      rows: fundamentalRows,
      source: 'Investidor10 / indicadores capturados / contrato normalizado VALORAE Proxy',
      required: false,
      reasonWhenEmpty: 'Indicadores serão adicionados em módulo próprio quando a próxima etapa da Análise for implementada.'
    })
  ];
  ...
  return {
    ...
    policy: 'analysis-reset-v1-real-values-only-no-legacy-analysis-renderer-no-synthetic-values',
    ...
    sections,
    appGuidance: 'A página Análise foi reiniciada. O APK deve renderizar somente este contrato novo; não usar gráficos/sections legados para o resumo.'
  };
}
