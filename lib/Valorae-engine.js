...
export async function fetchPublicHtml(url, options = {}) {
  ...
  const fetchWork = async () => {
    ...
    const attempts = [{ provider: 'EnginePolicy', ok: true, skipped: true, plan: enginePlan.reason, directRetryBudget: enginePlan.directRetryBudget, stalePreferred: enginePlan.stalePreferred, sourceScore: enginePlan.sourceScore }];
    ...
    for (const provider of providers) {
      ...
      for (let directAttempt = 0; directAttempt <= enginePlan.directRetryBudget; directAttempt += 1) {
        const controller = new AbortController();
        const timer = setTimeout(() => controller.abort(), timeoutMs);
        const started = performance.now();
        ...
        try {
          ...
          if (!shouldRetryFetch(classification) || directAttempt >= enginePlan.directRetryBudget) break;
          await sleep(180 * Math.pow(2, directAttempt) + Math.floor(Math.random() * 80));
        } catch (err) {
...
export class ValoraeEngine {
  ...
  static async _fetchAtivoUncached(rawTicker, rawType, options = {}) {
    ...
    payload.chartReadiness = buildChartReadinessReport(chartSource);
    payload.chartSeries = buildNormalizedChartSeries(chartSource, { maxSeries: options.maxChartSeries || options.chartSeriesLimit || 8 });
    payload.panelReadiness = buildPanelReadiness(payload);
    payload.consumerDiagnostics = buildConsumerDiagnostics(payload, getValoraeRuntimeStats());
