...
const VALORAE_NEWS_RELIABILITY_VERSION = '21.12.52-news-reliability-upgrade';
...
const KNOWN_B3_UNITS = new Set([
  ...
  'RNEW11','APER11','MODL11','SULA11','BIDI11','IGTI11','CPLE11','BBRK11','BRKM11',
...
const VALORAE_ANALYSIS_PAGE_VERSION = '21.12.98-analysis-asset-summary-structured';

const ACTION_ANALYSIS_BLUEPRINT = [
  { key: 'quoteOverview', title: 'Resumo do ativo', kind: 'summary', required: true, source: 'Investidor10 topo do ativo' },
  { key: 'priceChart', title: 'Cotação', kind: 'chart', required: true, source: 'Investidor10 gráfico de cotação' },
  { key: 'profitability', title: 'Rentabilidade', kind: 'metrics', required: true, source: 'Investidor10 rentabilidade' },
  { key: 'fundamentalIndicators', title: 'Indicadores fundamentalistas', kind: 'metrics', required: true, source: 'Investidor10 indicadores fundamentalistas' },
  { key: 'fundamentalIndicatorHistory', title: 'Histórico de indicadores fundamentalistas', kind: 'chart', required: true, source: 'Investidor10 histórico de indicadores' },
  { key: 'dividendHistory', title: 'Histórico de dividendos', kind: 'chart_table', required: true, source: 'Investidor10 dividendos' },
  { key: 'smartDividendRadar', title: 'Radar de dividendos inteligente', kind: 'projection', required: false, source: 'Investidor10 radar de dividendos' },
  { key: 'payoutHistory', title: 'Payout', kind: 'chart', required: true, source: 'Investidor10 payout' },
  { key: 'bazinCeilingPrice', title: 'Preço teto segundo método Bazin', kind: 'valuation', required: false, source: 'Investidor10 preço teto' },
  { key: 'indexComparison', title: 'Comparação com índices', kind: 'comparison_chart', required: true, source: 'Investidor10 comparação com índices' },
  { key: 'aboutCompany', title: 'Sobre a empresa', kind: 'text', required: true, source: 'Investidor10 sobre a empresa' },
  { key: 'companyData', title: 'Dados sobre a empresa', kind: 'table', required: true, source: 'Investidor10 dados da empresa' },
  { key: 'companyInfo', title: 'Informações sobre a empresa', kind: 'table', required: true, source: 'Investidor10 informações da empresa' },
  { key: 'revenueRegions', title: 'Regiões onde gera receita', kind: 'breakdown_chart', required: true, source: 'Investidor10 regiões de receita' },
  { key: 'revenueBusinesses', title: 'Negócios que geram receita', kind: 'breakdown_chart', required: true, source: 'Investidor10 negócios de receita' },
  { key: 'shareholderPosition', title: 'Posição acionária', kind: 'table', required: false, source: 'Investidor10 posição acionária' },
  { key: 'revenueProfit', title: 'Receitas e lucros', kind: 'financial_chart', required: true, source: 'Investidor10 receitas e lucros' },
  { key: 'profitVsQuote', title: 'Lucro x cotação', kind: 'financial_chart', required: true, source: 'Investidor10 lucro x cotação' },
  { key: 'equityEvolution', title: 'Evolução de patrimônio', kind: 'financial_chart', required: true, source: 'Investidor10 balanço/evolução patrimonial' },
  { key: 'balanceSheet', title: 'Balanço patrimonial', kind: 'statement', required: true, source: 'Investidor10 balanço patrimonial' },
  { key: 'incomeStatement', title: 'DRE / resultados', kind: 'statement', required: false, source: 'Investidor10 demonstrativos' },
  { key: 'cashFlowStatement', title: 'Fluxo de caixa', kind: 'statement', required: false, source: 'Investidor10 demonstrativos' },
  { key: 'relatedCompanies', title: 'Empresas relacionadas', kind: 'cards', required: false, source: 'Investidor10 empresas relacionadas' },
  { key: 'news', title: 'Notícias sobre a empresa', kind: 'news', required: false, source: 'Investidor10 notícias' },
];

const FII_ANALYSIS_BLUEPRINT = [
  { key: 'quoteOverview', title: 'Resumo do FII', kind: 'summary', required: true, source: 'Investidor10 topo do FII' },
  { key: 'priceChart', title: 'Cotação', kind: 'chart', required: true, source: 'Investidor10 gráfico de cotação' },
  { key: 'profitability', title: 'Rentabilidade', kind: 'metrics', required: true, source: 'Investidor10 rentabilidade' },
  { key: 'fiiProfile', title: 'Informações sobre o FII', kind: 'table', required: true, source: 'Investidor10 informações sobre o FII' },
  { key: 'fundamentalIndicators', title: 'Histórico de indicadores fundamentalistas', kind: 'chart', required: true, source: 'Investidor10 histórico de indicadores' },
  { key: 'indexComparison', title: 'Comparação com índices', kind: 'comparison_chart', required: true, source: 'Investidor10 comparação com índices' },
  { key: 'peerComparison', title: 'Comparando com outros FIIs', kind: 'comparison_table', required: true, source: 'Investidor10 comparador de FIIs' },
  { key: 'buyHoldChecklist', title: 'Checklist Buy and Hold', kind: 'checklist', required: false, source: 'Investidor10 checklist do investidor' },
  { key: 'distribution12m', title: 'Distribuições nos últimos 12 meses', kind: 'metrics', required: true, source: 'Investidor10 distribuições 12M' },
  { key: 'dividendYieldHistory', title: 'Dividend Yield', kind: 'chart', required: true, source: 'Investidor10 dividend yield' },
  { key: 'dividendHistory', title: 'Dividendos', kind: 'chart_table', required: true, source: 'Investidor10 dividendos' },
  { key: 'patrimonialInfo', title: 'Informações sobre valor patrimonial', kind: 'table', required: true, source: 'Investidor10 valor patrimonial' },
  { key: 'typeSegmentAverage', title: 'Média do tipo e segmento', kind: 'comparison_table', required: true, source: 'Investidor10 média do tipo e segmento' },
  { key: 'physicalAssets', title: 'Lista de imóveis', kind: 'table', required: false, source: 'Investidor10 lista de imóveis' },
  { key: 'assetDistribution', title: 'Distribuição de ativos do fundo', kind: 'breakdown_chart', required: false, source: 'Investidor10 distribuição de ativos do fundo' },
  { key: 'news', title: 'Notícias sobre o FII', kind: 'news', required: false, source: 'Investidor10 notícias' },
  { key: 'relatedFiis', title: 'FIIs relacionados', kind: 'cards', required: false, source: 'Investidor10 FIIs relacionados' },
];
...
function firstNRows(rows = [], limit = 8) {
  return Array.isArray(rows) ? rows.slice(0, limit).map((row, index) => {
    if (!row || typeof row !== 'object') return { label: `Item ${index + 1}`, value: text(row) };
    const label = text(firstDefined(row.label, row.name, row.period, row.year, row.ticker, row.symbol, row.key, `Item ${index + 1}`));
    const value = text(firstDefined(row.display, row.valueDisplay, row.value, row.amount, row.percent, row.valuePercent, row.netRevenue, row.netProfit, row.netWorth, row.price, row.quote, row.dy, row.pvp, row.status));
    const detail = text(firstDefined(row.detail, row.description, row.segment, row.kind, row.type, row.source));
    return { label, value, detail };
  }).filter(r => r.label || r.value || r.detail) : [];
}

function rowsFromBreakdownMap(map = {}, limit = 8) {
  if (!map || typeof map !== 'object') return [];
  const rows = [];
  for (const [period, items] of Object.entries(map)) {
    if (!Array.isArray(items)) continue;
    for (const item of items.slice(0, limit)) {
      if (!item || typeof item !== 'object') continue;
      rows.push({
        label: text(firstDefined(item.name, item.label, item.segment, item.region, item.country, item.key, period)),
        value: text(firstDefined(item.display, item.valueDisplay, item.valuePercent, item.percent, item.percentage, item.value, item.amount)),
        detail: text(period),
      });
...
function rowsFromGenericObject(value = {}, limit = 80, depth = 0, prefix = '') {
  if (!value || typeof value !== 'object' || depth > 2) return [];
  const rows = [];
  const push = (label, rawValue, detail = '') => {
    const cleanLabel = text(label).trim();
    const cleanValue = text(rawValue).trim();
    const cleanDetail = text(detail).trim();
    if (!cleanLabel || (!cleanValue && !cleanDetail)) return;
    if (/^(_meta|source|version|generatedAt|length)$/i.test(cleanLabel)) return;
    rows.push({ label: cleanLabel, value: cleanValue, detail: cleanDetail });
  };
  if (Array.isArray(value.keyValues)) {
    for (const item of value.keyValues) {
      if (!item || typeof item !== 'object') continue;
      push(firstDefined(item.label, item.name, item.key), firstDefined(item.value, item.display, item.amount, item.percent), item.detail || prefix);
      if (rows.length >= limit) return rows;
    }
  }
  if (Array.isArray(value.rows)) {
    for (const item of value.rows) {
      if (Array.isArray(item)) push(item[0], item.slice(1).join(' • '), prefix);
      else if (item && typeof item === 'object') push(firstDefined(item.label, item.name, item.key, item.indicador), firstDefined(item.value, item.display, item.amount, item.percent, item.valores), firstDefined(item.detail, item.period, item.source, prefix));
      else push(`Item ${rows.length + 1}`, item, prefix);
      if (rows.length >= limit) return rows;
    }
  }
  if (Array.isArray(value.linhas)) {
    for (const item of value.linhas) {
      if (!item || typeof item !== 'object') continue;
      const vals = item.valores && typeof item.valores === 'object'
        ? Object.entries(item.valores).slice(0, 8).map(([k, v]) => `${k}: ${text(v)}`).join(' • ')
        : firstDefined(item.value, item.display, item.amount);
      push(firstDefined(item.indicador, item.label, item.name), vals, prefix);
      if (rows.length >= limit) return rows;
    }
  }
  for (const [key, raw] of Object.entries(value)) {
    if (rows.length >= limit) return rows;
    if (['keyValues','rows','linhas','text','html','tables','links','charts'].includes(key)) continue;
    if (raw === undefined || raw === null || raw === '') continue;
    if (Array.isArray(raw)) {
      for (const item of raw.slice(0, Math.max(1, limit - rows.length))) {
        if (Array.isArray(item)) push(key, item.join(' • '), prefix);
        else if (item && typeof item === 'object') push(firstDefined(item.label, item.name, item.key, item.ticker, key), firstDefined(item.value, item.display, item.amount, item.percent, item.title, item.text), firstDefined(item.detail, item.date, item.period, prefix));
        else push(key, item, prefix);
        if (rows.length >= limit) return rows;
      }
    } else if (raw && typeof raw === 'object') {
      const childRows = rowsFromGenericObject(raw, Math.max(1, limit - rows.length), depth + 1, prefix ? `${prefix} • ${key}` : key);
      rows.push(...childRows);
    } else {
      push(key, raw, prefix);
    }
  }
  if (!rows.length && typeof value.text === 'string' && value.text.trim()) rows.push({ label: prefix || 'Texto', value: value.text.replace(/\s+/g, ' ').trim().slice(0, 260) });
  return rows.slice(0, limit);
}

function rowsFromAnyAnalysisValue(data, limit = 80) {
  if (Array.isArray(data)) return firstNRows(data, limit);
  if (data && typeof data === 'object') {
    const breakdownRows = rowsFromBreakdownMap(data, limit);
    return breakdownRows.length ? breakdownRows : rowsFromGenericObject(data, limit);
  }
  if (typeof data === 'string' && data.trim()) return [{ label: 'Texto', value: data.replace(/\s+/g, ' ').trim().slice(0, 260) }];
  return [];
}

function indicatorRowsFromPayload(payload = {}, limit = 10) {
  const appMetrics = payload.appPayload?.metrics?.canonical || payload.appMobileSnapshot?.metrics || {};
  if (!appMetrics || typeof appMetrics !== 'object') return [];
  return Object.entries(appMetrics).slice(0, limit).map(([key, value]) => {
    if (value && typeof value === 'object') {
      return { label: text(firstDefined(value.label, key)), value: text(firstDefined(value.display, value.value, value.numericValue)), detail: text(firstDefined(value.group, value.category, value.source)) };
    }
    return { label: key, value: text(value) };
  }).filter(r => r.value && r.value !== '[object Object]');
}
...
function analysisDisplayValue(value) {
  ...
  if (typeof value === 'object' && !Array.isArray(value)) {
    const raw = firstDefined(value.display, value.valueDisplay, value.formatted, value.raw, value.value, value.numericValue, value.amount, value.percent, value.name, value.label);
    return analysisDisplayValue(raw);
  }
  if (Array.isArray(value)) return value.map(analysisDisplayValue).filter(Boolean).join(' • ');
  const clean = text(value).replace(/\s+/g, ' ').trim();
  if (!clean || clean === '[object Object]' || /^undefined|null$/i.test(clean)) return '';
  return clean;
}

function metricDisplay(payload = {}, key) {
  const candidates = [
    payload.appPayload?.metrics?.canonical?.[key],
    payload.appMobileSnapshot?.metrics?.[key],
    payload.normalized?.[key],
  ];
  for (const item of candidates) {
    const display = analysisDisplayValue(item);
    if (display) return display;
  }
  return '';
}

function firstDisplayFrom(payload = {}, paths = []) {
  for (const path of paths) {
    const value = path.includes('.') ? nestedValue(payload, path) : payload[path];
    const display = analysisDisplayValue(value);
    if (display) return display;
  }
  return '';
}

function addAnalysisSummaryRow(rows, seen, label, value, detail = '') {
  const cleanLabel = text(label).replace(/\s+/g, ' ').trim();
  const cleanValue = analysisDisplayValue(value);
  const cleanDetail = text(detail).replace(/\s+/g, ' ').trim();
...
function buildQuoteOverviewRows(payload = {}, isFii = false, limit = 40) {
  const rows = [];
  const seen = new Set();
  const results = payload.results || {};
  const sections = results.sections || {};
  const empresa = sections.empresa || {};
  const fiiInfo = sections.informacoesFundo || results.informacoesFundo || {};
  const appQuote = payload.appMobileSnapshot?.quote || payload.appPayload?.quote || {};
  const quote = results.quote || results.cotacao || {};
  const ticker = text(firstDefined(payload.ticker, payload.symbol, appQuote.ticker, quote.ticker)).toUpperCase();
  const type = text(firstDefined(payload.type, appQuote.type, results.type, isFii ? 'FII' : 'ACAO')).toUpperCase();
  const name = firstDefined(appQuote.name, results.nome, results.name, results.razaoSocial, results.companyName, ticker);
  addAnalysisSummaryRow(rows, seen, 'Ticker', ticker, 'Identidade');
  addAnalysisSummaryRow(rows, seen, isFii ? 'Nome do fundo' : 'Nome da empresa', name, 'Identidade');
  addAnalysisSummaryRow(rows, seen, 'Classe', isFii ? 'FII' : type, 'Identidade');
  addAnalysisSummaryRow(rows, seen, isFii ? 'Cotação da cota' : 'Cotação', firstDefined(appQuote.priceDisplay, metricDisplay(payload, 'precoAtual'), quote.priceDisplay, quote.precoAtual, results.precoAtual), 'Topo do Investidor10');
  addAnalysisSummaryRow(rows, seen, 'Variação (12M)', firstDefined(metricDisplay(payload, 'variacao12m'), results.variacao12m, results.change12m, quote.variacao12m), 'Topo do Investidor10');
  addAnalysisSummaryRow(rows, seen, isFii ? 'DY (12M)' : 'Dividend Yield', firstDefined(metricDisplay(payload, isFii ? 'yield12m' : 'dividendYield'), metricDisplay(payload, 'dividendYield'), results.yield12m, results.dividendYield, results.dy), 'Resumo');
  addAnalysisSummaryRow(rows, seen, 'P/VP', firstDefined(metricDisplay(payload, 'pvp'), results.pvp, results.p_vp), 'Resumo');

  if (isFii) {
    addAnalysisSummaryRow(rows, seen, 'Liquidez diária', firstDefined(metricDisplay(payload, 'liquidezMediaDiaria'), results.liquidezDiaria, fiiInfo.liquidezDiaria), 'Resumo');
    addAnalysisSummaryRow(rows, seen, 'Último rendimento', firstDefined(metricDisplay(payload, 'ultimoRendimento'), results.ultimoRendimento, fiiInfo.ultimoRendimento), 'Resumo');
    addAnalysisSummaryRow(rows, seen, 'Valor patrimonial p/ cota', firstDefined(metricDisplay(payload, 'valorPatrimonialCota'), results.valorPatrimonial, results.valorPatrimonialPorCota, fiiInfo.valorPatrimonial, fiiInfo.valorPatrimonialPorCota), 'Patrimônio');
    addAnalysisSummaryRow(rows, seen, 'Valor patrimonial', firstDefined(metricDisplay(payload, 'patrimonioLiquido'), results.valorPatrimonialTotal, results.patrimonioLiquido, fiiInfo.valorPatrimonialTotal), 'Patrimônio');
    addAnalysisSummaryRow(rows, seen, 'Vacância', firstDefined(metricDisplay(payload, 'vacanciaFisica'), results.vacanciaFisica, results.vacancia, fiiInfo.vacanciaFisica, fiiInfo.vacancia), 'Portfólio');
    addAnalysisSummaryRow(rows, seen, 'Número de cotistas', firstDefined(results.numeroCotistas, fiiInfo.numeroCotistas), 'Portfólio');
    addAnalysisSummaryRow(rows, seen, 'Cotas emitidas', firstDefined(results.cotasEmitidas, fiiInfo.cotasEmitidas), 'Portfólio');
    addAnalysisSummaryRow(rows, seen, 'Segmento', firstDefined(results.segmentoFii, results.segmento, fiiInfo.segmentoFii, fiiInfo.segmento), 'Perfil do FII');
    addAnalysisSummaryRow(rows, seen, 'Tipo de fundo', firstDefined(results.tipoFundo, fiiInfo.tipoFundo), 'Perfil do FII');
    addAnalysisSummaryRow(rows, seen, 'Mandato', firstDefined(results.mandato, fiiInfo.mandato), 'Perfil do FII');
    addAnalysisSummaryRow(rows, seen, 'Tipo de gestão', firstDefined(results.tipoGestao, fiiInfo.tipoGestao), 'Perfil do FII');
    addAnalysisSummaryRow(rows, seen, 'Prazo de duração', firstDefined(results.prazoDuracao, fiiInfo.prazoDuracao), 'Perfil do FII');
    addAnalysisSummaryRow(rows, seen, 'Taxa de administração', firstDefined(results.taxaAdministracao, fiiInfo.taxaAdministracao), 'Perfil do FII');
    addAnalysisSummaryRow(rows, seen, 'Público-alvo', firstDefined(results.publicoAlvo, fiiInfo.publicoAlvo), 'Perfil do FII');
    addAnalysisSummaryRow(rows, seen, 'CNPJ', firstDefined(results.cnpj, fiiInfo.cnpj), 'Cadastro');
  } else {
    addAnalysisSummaryRow(rows, seen, 'P/L', firstDefined(metricDisplay(payload, 'pl'), results.pl, results.p_l), 'Resumo');
    addAnalysisSummaryRow(rows, seen, 'Liquidez diária', firstDefined(metricDisplay(payload, 'liquidezMediaDiaria'), results.liquidezDiaria, results.volumeMedio, results.averageVolume), 'Resumo');
    addAnalysisSummaryRow(rows, seen, 'Setor', firstDefined(results.setor, results.sector, empresa.dados?.setor, empresa.informacoes?.setor), 'Classificação');
    addAnalysisSummaryRow(rows, seen, 'Subsetor', firstDefined(results.subsetor, results.subsector, empresa.dados?.subsetor, empresa.informacoes?.subsetor), 'Classificação');
    addAnalysisSummaryRow(rows, seen, 'Segmento', firstDefined(results.segmento, results.segment, empresa.dados?.segmento, empresa.informacoes?.segmento), 'Classificação');
    addAnalysisSummaryRow(rows, seen, 'Valor de mercado', firstDefined(metricDisplay(payload, 'valorDeMercado'), results.valorDeMercado, empresa.informacoes?.valorDeMercado), 'Empresa');
    addAnalysisSummaryRow(rows, seen, 'Valor de firma', firstDefined(results.valorDeFirma, empresa.informacoes?.valorDeFirma), 'Empresa');
    addAnalysisSummaryRow(rows, seen, 'Patrimônio líquido', firstDefined(metricDisplay(payload, 'patrimonioLiquido'), results.patrimonioLiquido, empresa.informacoes?.patrimonioLiquido), 'Empresa');
    addAnalysisSummaryRow(rows, seen, 'ROE', firstDefined(metricDisplay(payload, 'roe'), results.roe), 'Rentabilidade');
    addAnalysisSummaryRow(rows, seen, 'ROIC', firstDefined(metricDisplay(payload, 'roic'), results.roic), 'Rentabilidade');
    addAnalysisSummaryRow(rows, seen, 'Margem líquida', firstDefined(metricDisplay(payload, 'margemLiquida'), results.margemLiquida), 'Rentabilidade');
    addAnalysisSummaryRow(rows, seen, 'Payout', firstDefined(metricDisplay(payload, 'payout'), results.payout), 'Dividendos');
    addAnalysisSummaryRow(rows, seen, 'Free float', firstDefined(results.freeFloat, empresa.informacoes?.freeFloat), 'Governança');
    addAnalysisSummaryRow(rows, seen, 'Tag along', firstDefined(results.tagAlong, empresa.informacoes?.tagAlong), 'Governança');
  }

  addAnalysisSummaryRow(rows, seen, 'Fonte', 'Investidor10 via VALORAE Proxy', 'Rastreamento');
  return rows.slice(0, limit);
}

function resolveAnalysisSectionData(sectionKey, payload = {}, bundle = {}) {
  const results = payload.results || {};
  const sections = results.sections || {};
  const empresa = sections.empresa || {};
  const demonstrativos = sections.demonstrativos || {};
  const isFii = /FII|FIAGRO|FI-INFRA/i.test(text(firstDefined(payload.type, results.type, payload.appMobileSnapshot?.type)));
  const canonical = results.assetChartsCanonical || sections.assetChartsCanonical || payload.assetChartsCanonical || {};
  const coverageChecks = Array.isArray(canonical.coverage?.checks) ? canonical.coverage.checks : [];
  const coverage = coverageChecks.find(check => check.key === sectionKey || check.alias === sectionKey || text(check.title).toLowerCase() === text(sectionKey).toLowerCase());
  const map = {
    quoteOverview: [payload.appMobileSnapshot?.quote, payload.appPayload?.quote, results.quote, results.cotacao, pick(results, ['precoAtual','variacao12m','pl','pvp','dividendYield','yield12m','liquidezDiaria'])],
    priceChart: [bundle.priceHistory, payload.chartSeries?.series, results.priceHistory],
    profitability: [bundle.profitability, bundle.realProfitability, canonical.profitability, results.rentabilidade, sections.rentabilidade, sections.rentabilidadeReal],
    fundamentalIndicators: [results.indicadoresFundamentalistas, sections.indicadores, indicatorRowsFromPayload(payload, 80), results.indicadores, results.fundamentals, results.historicoIndicadores, sections.historicoIndicadores],
    fundamentalIndicatorHistory: [bundle.indicatorHistory, canonical.fii?.fundamentalIndicatorHistory, results.historicoIndicadores, sections.historicoIndicadores],
    dividendHistory: [bundle.dividendEvents, results.dividendHistory, results.historicoDividendos, results.dividendos, sections.dividendos],
    smartDividendRadar: [results.radarDividendos, sections.radarDividendos, results.dividendos?.radar],
    payoutHistory: [bundle.payoutHistory, canonical.financial?.payoutHistory, results.payoutHistory, results.payout],
    bazinCeilingPrice: [results.precoTetoBazin, sections.precoTetoBazin, results.precoJustoGraham],
    indexComparison: [bundle.indexComparison, results.indexComparison, results.comparacaoIndices, sections.comparacaoIndices],
    aboutCompany: [results.sobre, results.description, payload.description, sections.sobre, empresa.sobre],
    companyData: [results.dadosEmpresa, sections.dadosEmpresa, empresa.dados],
    companyInfo: [results.informacoesEmpresa, sections.informacoesEmpresa, empresa.informacoes, pick(results, ['valorDeMercado','valorDeFirma','patrimonioLiquido','numeroPapeis','ativosTotais','ativoCirculante','disponibilidade','freeFloat','tagAlong','liquidezDiaria'])],
    revenueRegions: [bundle.revenueByRegion, canonical.revenueByRegion, results.revenueByRegion, results.regioesReceita, empresa.regioesReceita],
    revenueBusinesses: [bundle.revenueByBusiness, canonical.revenueByBusiness, results.revenueByBusiness, results.negociosReceita, results.segmentosReceita, empresa.negociosReceita],
    shareholderPosition: [results.posicaoAcionaria, sections.posicaoAcionaria, empresa.posicaoAcionaria],
    revenueProfit: [bundle.revenueProfit, canonical.financial?.revenueProfit, results.revenueProfit, demonstrativos.receitasLucros],
    profitVsQuote: [bundle.profitVsQuote, canonical.financial?.profitVsQuote, results.profitVsQuote, demonstrativos.lucroCotacao],
    equityEvolution: [bundle.equityEvolution, canonical.financial?.equityEvolution, results.equityEvolution, demonstrativos.evolucaoPatrimonio],
    incomeStatement: [canonical.financial?.incomeStatement, results.incomeStatement, demonstrativos.resultados, demonstrativos.receitasLucros],
    balanceSheet: [canonical.financial?.balanceSheet, results.balanceSheet, demonstrativos.balancoPatrimonial, bundle.balanceSheet],
    cashFlowStatement: [canonical.financial?.cashFlowStatement, results.cashFlowStatement, demonstrativos.cashFlowStatement],
    relatedCompanies: [results.empresasRelacionadas, sections.empresasRelacionadas, results.relatedCompanies],
    news: [payload.news, results.news, sections.news, results.noticias, sections.comunicados],
    fiiProfile: [bundle.fiiPatrimonialInfo, results.informacoesFundo, sections.informacoesFundo, pick(results, ['cnpj','numeroCotistas','cotasEmitidas','taxaAdministracao','tipoFundo','segmentoFii','mandato','publicoAlvo','tipoGestao','prazoDuracao','vacanciaFisica','vacanciaFinanceira'])],
    peerComparison: [bundle.fiiPeerAverage, results.fiiPeerComparison, results.comparadorFiis, sections.comparacaoFiis],
    buyHoldChecklist: [results.checklistBuyAndHold, results.checklist, sections.checklistBah, sections.checklist],
    distribution12m: [bundle.fiiDistribution12m, results.distribuicoes12m, sections.distribuicoes12m],
    dividendYieldHistory: [bundle.dividendYieldHistory, results.dividendYieldHistory, sections.dividendYieldHistory, results.dividendos],
    patrimonialInfo: [bundle.fiiPatrimonialInfo, results.informacoesFundo, sections.informacoesFundo, sections.valorPatrimonial, results.valorPatrimonial, pick(results, ['valorPatrimonial','valorPatrimonialTotal','patrimonioLiquido','pvp','valorDeMercado'])],
    typeSegmentAverage: [results.mediaTipoSegmento, sections.mediaTipoSegmento, results.fiiPeerComparison],
    physicalAssets: [results.listaImoveis, sections.listaImoveis, canonical.fii?.physicalAssets, bundle.fiiAssetDistribution],
    assetDistribution: [bundle.fiiAssetDistribution, canonical.fii?.assetDistribution, results.fiiAssetDistribution, results.distribuicaoAtivosFundo, sections.distribuicaoAtivosFundo],
    relatedFiis: [results.fiisRelacionados, sections.fiisRelacionados, results.relatedFiis],
  };
  const candidates = map[sectionKey] || [];
  const data = candidates.find(v => valueCount(v) > 0);
  let count = valueCount(data);
  let rows = sectionKey === 'quoteOverview' ? buildQuoteOverviewRows(payload, isFii, 80) : rowsFromAnyAnalysisValue(data, 80);
  if (!rows.length && sectionKey === 'fundamentalIndicators') rows = indicatorRowsFromPayload(payload, 80);
  if (rows.length && !count) count = rows.length;
  const status = count > 0 ? 'captured' : (coverage?.presentInHtml ? 'visible_without_series_yet' : 'pending_or_not_visible');
  return {
    status,
    ...
    reason: count > 0 || rows.length ? 'Dados reais capturados do contrato do Proxy.' : (coverage?.reason || 'Seção prevista no Investidor10; será preenchida somente quando a fonte entregar dados reais.'),
  };
}

function buildAssetAnalysisPage(payload = {}) {
  const bundle = payload.assetChartBundle || payload.assetChartsMobile || payload.appPayload?.assetChartBundle || payload.appPayload?.charts?.assetChartBundle || {};
  const ticker = text(firstDefined(payload.ticker, payload.symbol, bundle.ticker, payload.appMobileSnapshot?.ticker)).toUpperCase();
  const type = text(firstDefined(payload.type, bundle.type, payload.appMobileSnapshot?.type, 'ACAO')).toUpperCase();
  const isFii = type.includes('FII') || type.includes('FUNDO');
  const blueprint = isFii ? FII_ANALYSIS_BLUEPRINT : ACTION_ANALYSIS_BLUEPRINT;
  const sections = blueprint.map(def => {
    const resolved = resolveAnalysisSectionData(def.key, payload, bundle);
    return {
      ...def,
      ...
      coverageKey: resolved.coverageKey,
    };
  });
  const captured = sections.filter(s => s.itemCount > 0).length;
  const visiblePending = sections.filter(s => s.status === 'visible_without_series_yet').length;
  return {
    version: VALORAE_ANALYSIS_PAGE_VERSION,
    ticker,
    type,
    source: 'Investidor10 via VALORAE Proxy',
    policy: 'faithful-sections-full-fundamentals-no-synthetic-values',
    generatedAt: payload.metrics?.generatedAt || nowIso(),
    summary: {
      totalSections: sections.length,
      capturedSections: captured,
      visiblePendingSections: visiblePending,
      pendingSections: sections.filter(s => s.itemCount <= 0).length,
      hasFundamentals: sections.some(s => s.key === 'fundamentalIndicators' && s.itemCount > 0),
      hasCharts: sections.some(s => String(s.kind).includes('chart') && s.itemCount > 0),
    },
    sections,
    appGuidance: 'Renderize todas as seções. Use rows para dados completos e previewRows para visual limpo. Quando status não for captured, mostre estado pendente/indisponível sem criar valores simulados.',
  };
}

function attachAssetAnalysisPage(payload = {}) {
  const page = buildAssetAnalysisPage(payload);
  payload.assetAnalysisPage = page;
  payload.results = payload.results || {};
  payload.results.assetAnalysisPage = page;
  payload.results.sections = payload.results.sections || {};
  payload.results.sections.assetAnalysisPage = page;
  payload.appPayload = payload.appPayload || {};
  payload.appPayload.assetAnalysisPage = page;
  payload.appMobileSnapshot = payload.appMobileSnapshot || {};
  payload.appMobileSnapshot.assetAnalysisPage = page;
  return payload;
}
...
async function fetchGoogleNews(ticker, aliases = [], limit = DEFAULT_NEWS_LIMIT, options = {}) {
  ...
  const safeLimit = Math.max(1, Math.min(25, Number(limit || DEFAULT_NEWS_LIMIT) || DEFAULT_NEWS_LIMIT));
  const isGeneralNews = !clean;
  const baseTerms = (isGeneralNews
    ? ['B3', 'Ibovespa', 'ações brasileiras', 'FIIs', 'dividendos', 'proventos']
    : [`${clean}`, `${clean}.SA`, ...aliases])
    .map(s => String(s || '').trim())
    .filter(s => s && s.length >= 3)
    .slice(0, 8);
  const cacheKey = `news:${clean || 'GERAL'}:${baseTerms.join('|')}:${safeLimit}`;
  ...
  const promise = (async () => {
    ...
    const query = isGeneralNews
      ? '(B3 OR Ibovespa OR "bolsa brasileira" OR ações OR FIIs OR dividendos OR proventos OR resultados OR balanço)'
      : `(${quotedAliases.join(' OR ')}) (B3 OR ações OR ação OR bolsa OR dividendos OR proventos OR resultados OR balanço OR FII OR "fundo imobiliário")`;
    ...
    try {
      ...
      const items = rawItems.map(item => ({ ...item, query, relevanceScore: scoreNews(item, clean, baseTerms) }))
        .filter(item => item.relevanceScore > 0)
        .sort((a, b) => (b.relevanceScore - a.relevanceScore) || (Date.parse(b.pubDate || 0) - Date.parse(a.pubDate || 0)))
        .slice(0, safeLimit);
      if (!items.length) {
      ...
      const value = {
        ...
        fetchedAt: nowIso(),
        timeoutMs,
        appPolicy: { canReplacePreviousNews: true, shouldKeepPreviousNews: false, optionalBlock: true },
...
function parseRssItems(xml) {
  ...
  while ((m = re.exec(xml)) && out.length < 50) {
    ...
    if (title && link) {
      ...
      out.push({ title, link, pubDate: Number.isFinite(dateMs) ? new Date(dateMs).toISOString() : undefined, source, snippet: desc });
    }
  }
  return out;
}
...
export class ValoraeEngine {
  ...
  static async _fetchAtivoUncached(rawTicker, rawType, options = {}) {
    ...
    payload = attachAssetAnalysisPage(payload);
