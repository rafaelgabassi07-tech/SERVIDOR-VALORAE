...
const VALORAE_ANALYSIS_PAGE_VERSION = '21.12.96-investidor10-analysis-page-contract';
...
function resolveAnalysisSectionData(sectionKey, payload = {}, bundle = {}) {
  ...
  const map = {
    quoteOverview: [payload.appMobileSnapshot?.quote, payload.appPayload?.quote, results.quote, results.cotacao],
    priceChart: [bundle.priceHistory, payload.chartSeries?.series, results.priceHistory],
    profitability: [bundle.profitability, bundle.realProfitability, canonical.profitability],
    fundamentalIndicators: [indicatorRowsFromPayload(payload), results.indicadores, results.fundamentals],
    fundamentalIndicatorHistory: [bundle.indicatorHistory, canonical.fii?.fundamentalIndicatorHistory, results.historicoIndicadores],
    dividendHistory: [bundle.dividendEvents, results.dividendHistory, results.historicoDividendos, results.dividendos],
    smartDividendRadar: [results.radarDividendos, sections.radarDividendos],
    payoutHistory: [bundle.payoutHistory, canonical.financial?.payoutHistory, results.payoutHistory],
    bazinCeilingPrice: [results.precoTetoBazin, sections.precoTetoBazin],
    indexComparison: [bundle.indexComparison, results.indexComparison, results.comparacaoIndices],
    aboutCompany: [results.sobre, results.description, payload.description, sections.sobre],
    companyData: [results.dadosEmpresa, sections.dadosEmpresa, sections.informacoesEmpresa],
    companyInfo: [results.informacoesEmpresa, sections.informacoesEmpresa],
    revenueRegions: [bundle.revenueByRegion, canonical.revenueByRegion, results.revenueByRegion, results.regioesReceita],
    revenueBusinesses: [bundle.revenueByBusiness, canonical.revenueByBusiness, results.revenueByBusiness, results.negociosReceita, results.segmentosReceita],
    shareholderPosition: [results.posicaoAcionaria, sections.posicaoAcionaria],
    revenueProfit: [bundle.revenueProfit, canonical.financial?.revenueProfit, results.revenueProfit],
    profitVsQuote: [bundle.profitVsQuote, canonical.financial?.profitVsQuote, results.profitVsQuote],
    equityEvolution: [bundle.equityEvolution, canonical.financial?.equityEvolution, canonical.financial?.balanceSheet, results.equityEvolution, results.balanceSheet],
    incomeStatement: [canonical.financial?.incomeStatement, results.incomeStatement, sections.demonstrativos?.incomeStatement],
    cashFlowStatement: [canonical.financial?.cashFlowStatement, results.cashFlowStatement, sections.demonstrativos?.cashFlowStatement],
    relatedCompanies: [results.empresasRelacionadas, sections.empresasRelacionadas, results.relatedCompanies],
    news: [payload.news, results.news, sections.news],
    fiiProfile: [bundle.fiiPatrimonialInfo, results.informacoesFundo, sections.informacoesFundo],
    peerComparison: [bundle.fiiPeerAverage, results.fiiPeerComparison, results.comparadorFiis],
    buyHoldChecklist: [results.checklist, sections.checklist],
    distribution12m: [bundle.fiiDistribution12m, results.distribuicoes12m],
    dividendYieldHistory: [bundle.dividendYieldHistory, results.dividendYieldHistory],
    patrimonialInfo: [bundle.fiiPatrimonialInfo, results.informacoesFundo, sections.informacoesFundo],
    typeSegmentAverage: [results.mediaTipoSegmento, sections.mediaTipoSegmento, results.fiiPeerComparison],
    physicalAssets: [results.listaImoveis, canonical.fii?.physicalAssets, bundle.fiiAssetDistribution],
    assetDistribution: [bundle.fiiAssetDistribution, canonical.fii?.assetDistribution, results.fiiAssetDistribution, results.distribuicaoAtivosFundo],
  ...
  let previewRows = [];
  if (Array.isArray(data)) previewRows = firstNRows(data, 8);
  else if (data && typeof data === 'object') previewRows = rowsFromBreakdownMap(data, 8);
  else if (typeof data === 'string') previewRows = [{ label: 'Texto', value: data.slice(0, 220) }];
  if (!previewRows.length && sectionKey === 'fundamentalIndicators') previewRows = indicatorRowsFromPayload(payload, 8);
  if (previewRows.length && !count) count = previewRows.length;
  ...
  return {
    ...
    reason: count > 0 ? 'Dados reais capturados do contrato do Proxy.' : (coverage?.reason || 'Seção prevista no Investidor10; será preenchida somente quando a fonte entregar dados reais.'),
  };
}
...
function buildAssetAnalysisPage(payload = {}) {
  ...
  return {
    ...
    policy: 'faithful-sections-no-synthetic-values',
    ...
    sections,
    appGuidance: 'Renderize todas as seções. Quando status não for captured, mostre estado pendente/indisponível sem criar valores simulados.',
  };
}
