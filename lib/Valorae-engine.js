      ...
      for (let directAttempt = 0; directAttempt <= directRetryBudget; directAttempt += 1) {
        ...
        try {
          const res = await fetch(url, {
            ...
            redirect: 'follow',
            signal: controller.signal,
            headers: cleanHeaderMap({ ...browserHeaders(url), ...cleanHeaderMap(options.headers) }),
          });
          ...
          await sleep(Math.min(180 * Math.pow(2, directAttempt) + Math.floor(Math.random() * 80), Math.max(0, timeoutMs - 50)));
        } catch (err) {
...
function normalizeDividendDateText(d) {
  const m = String(d || '').match(/(\d{1,2})\/(\d{1,2})\/(\d{2}|\d{4})/);
  if (!m) return '';
  const yy = String(m[3]);
...
function toMobileDividendEvent(event = {}, ticker = '') {
  ...
  const out = {
    ...
    dateCom: text(firstDefined(event.dateCom, event.dataCom, event.data_com, event.exDate, event.recordDate)),
    paymentDate: text(firstDefined(event.paymentDate, event.dataPagamento, event.payDate, event.date_payment, event.date, event.data)),
    valuePerShare,
    ...
    status: text(firstDefined(event.status, event.type, event.tipo, event.kind, 'Provento')),
...
function buildMobileAssetChartBundle(payload = {}) {
  ...
  const fiiAssetDistribution = mapBreakdownPayload(canonicalFii.physicalAssets || results.listaImoveis || results.ativosFundo || results.distribuicaoAtivos);
