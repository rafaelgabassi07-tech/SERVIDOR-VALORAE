...
import { formatBrDate } from '../core/dates.js';
...
function dateLabel(value = '') {
  ...
  if (!raw) return '';
  return formatBrDate(raw, raw).slice(0, 10);
}
...
function displayByUnit(raw, unit = '') {
  ...
  if (unit === 'R$') return brl(value) || cleanText(value);
...
function buildValuationModelItems(payload = {}, assetType = '') {
  ...
  for (const spec of VALUATION_MODEL_FIELDS) {
    ...
    if (raw === undefined || raw === null) continue;
    const display = displayByUnit(raw, spec.unit);
    const numeric = finiteValue(raw?.value ?? raw?.valor ?? raw?.amount ?? raw?.raw ?? raw);
    if (!display || display === '-' || display === '--') continue;
...
const CHECKLIST_PATHS = [
  ...
  'sections.checklistBuyHold', 'sections.checklist',
...
function buildChecklistItems(payload = {}, assetType = '') {
  ...
  return uniqueChecklistRows(rows).slice(0, 14).map((row, index) => ({
    id: `checklist_${index}_${norm(row.label).replace(/\s+/g, '_')}`,
    label: row.label,
    value: row.value,
    group: 'Checklist da fonte',
    source: row.source
  }));
}
...
function buildFiiChecklistItems(payload = {}, assetType = '') {
  ...
  return uniqueChecklistRows(rows).slice(0, 14).map((row, index) => ({ id: `fii_check_${index}_${norm(row.label).replace(/\s+/g, '_')}`, label: row.label, value: row.value, group: 'Checklist do FII', source: row.source }));
}
...
function buildSourceCoverageDiagnostics(assetType = '', sections = []) {
  ...
  const expected = [
    ...
    { id: 'checklist', source: 'Investidor10 checklist', expected: true },
...
const FII_ACCOUNTING_FIELDS = [
  ...
  { id: 'rentabilidadePatrimonial', label: 'Rentabilidade patrimonial', group: 'Rentabilidade', unit: '%', aliases: ['rentabilidadePatrimonial', 'Rentabilidade patrimonial - (R$)'] },
...
export function buildAnalysisPageResponse(payload = {}, input = {}) {
  ...
  const coreSections = [
    ...
    section('checklist', 'Checklist da fonte', 'metric_grid', checklistItems, 'Investidor10 checklist buy and hold'),
