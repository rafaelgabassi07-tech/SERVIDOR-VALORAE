...
function displayFromCard(payload, spec) {
  ...
  for (const card of indicatorCards(payload)) {
    ...
    if (display && display !== '0') return { value: display, source: card.source || 'Investidor10', raw: card.value };
  }
  return null;
}
...
function displayForSpec(payload, spec) {
  ...
  return display ? { value: display, source: 'Investidor10', raw } : null;
}
...
function buildSummaryItems(payload = {}, assetType = '') {
  ...
  const specById = (id) => STOCK_INDICATORS.find(spec => spec.id === id) || {};
  const pl = displayForSpec(payload, specById('pl'))?.value || '';
  const pvp = displayForSpec(payload, specById('pvp'))?.value || '';
  const dy = displayForSpec(payload, specById('dividendYield'))?.value || '';
  const rows = [
    ...
    ...(isFii ? [] : [{ id: 'pl', label: 'P/L', value: pl, group: 'Valuation', source: 'Investidor10' }]),
    { id: 'pvp', label: 'P/VP', value: pvp, group: 'Valuation', source: 'Investidor10' },
    { id: 'dy', label: isFii ? 'DY 12M' : 'DY', value: dy, group: 'Dividendos', source: 'Investidor10' }
  ];
...
function extractDateCandidate(value = '') {
  ...
  const namedMonthYear = raw.match(/\b(?:jan|janeiro|fev|fevereiro|mar|março|marco|abr|abril|mai|maio|jun|junho|jul|julho|ago|agosto|set|setembro|out|outubro|nov|novembro|dez|dezembro)[-\/ ](?:19|20)\d{2}\b/i);
  if (namedMonthYear) return namedMonthYear[0];
  const quarterYear = raw.match(/\b(?:[1-4]\s*T|T\s*[1-4]|Q[1-4])[-\/ ]?(?:19|20)\d{2}\b/i);
  if (quarterYear) return quarterYear[0];
  const yearQuarter = raw.match(/\b(?:19|20)\d{2}[-\/ ]?(?:[1-4]\s*T|T\s*[1-4]|Q[1-4])\b/i);
  if (yearQuarter) return yearQuarter[0];
...
function buildDividendSummaryItems(payload = {}, assetType = '') {
  ...
  const dy = displayForSpec(payload, STOCK_INDICATORS.find(spec => spec.id === 'dividendYield') || {})?.value || '';
  const rows = [
    ...
    { id: 'dy_12m', label: assetType === 'FII' ? 'DY 12M' : 'DY', value: dy, group: 'Dividendos', source: 'Investidor10' },
...
function buildDividendRadarItems(payload = {}, assetType = '', ticker = '') {
  ...
  for (const month of DIVIDEND_RADAR_MONTHS) {
    ...
    items.push({
      ...
      label: month.label,
      value: count > 0 ? `Ativo${count > 1 ? ` (${count})` : ''}` : 'Sem padrão',
      group: 'Data Com',
...
function chartPeriodSortKey(label = '') {
  ...
  if (!raw) return null;
  const normalized = norm(raw).replace(/\s+/g, ' ').trim();
  ...
  if (m) {
    const month = monthMap.get(m[1]) || monthMap.get(m[1].slice(0, 3));
...
function sortChartPointsChronologically(points = []) {
  const rows = arrayFrom(points);
  if (rows.length < 2) return rows;
  const keyed = rows.map((point, index) => ({ point, index, key: chartPeriodSortKey(point?.label ?? point?.period ?? point?.date ?? '') }));
  const parsed = keyed.filter(item => item.key !== null).length;
  if (parsed < 2 || keyed.length - parsed > 1) return rows;
...
function sortChartRowsChronologically(rows = []) {
  const source = arrayFrom(rows);
  if (source.length < 2) return source;
  const keyed = source.map((row, index) => ({ row, index, key: chartPeriodSortKey(chartPointLabel(row)) }));
  const parsed = keyed.filter(item => item.key !== null).length;
  if (parsed < 2 || keyed.length - parsed > 1) return source;
  return keyed.sort((a, b) => (a.key ?? Number.MAX_SAFE_INTEGER) - (b.key ?? Number.MAX_SAFE_INTEGER) || a.index - b.index).map(item => item.row);
}

function sortPeriodLabelsChronologically(labels = []) {
  const source = arrayFrom(labels).map(cleanText).filter(Boolean);
  if (source.length < 2) return source;
  const keyed = source.map((label, index) => ({ label, index, key: chartPeriodSortKey(label) }));
  const parsed = keyed.filter(item => item.key !== null).length;
  if (parsed < 2 || keyed.length - parsed > 1) return source.sort((a, b) => String(a).localeCompare(String(b), 'pt-BR', { numeric: true }));
  return keyed.sort((a, b) => (a.key ?? Number.MAX_SAFE_INTEGER) - (b.key ?? Number.MAX_SAFE_INTEGER) || a.index - b.index).map(item => item.label);
}
...
function indexedDualSeriesChart(id, title, rows = [], source = 'VALORAE Proxy', left = {}, right = {}, options = {}) {
  const normalized = sortChartRowsChronologically(arrayFrom(rows)).slice(-14);
...
function normalizePoints(rows = [], valueKeys = ['value', 'close', 'price'], limit = 18) {
  return sortChartPointsChronologically(arrayFrom(rows).map(row => chartPoint(row, valueKeys)).filter(Boolean)).slice(-limit);
}
...
function chartAxisKind(chart = {}) {
  const explicit = cleanText(chart.axisKind || chart.axis || chart.xAxisKind).toLowerCase();
  if (explicit === 'time' || explicit === 'category') return explicit;
  const text = [chart.id, chart.title, chart.chartType, chart.type].map(cleanText).join(' ').toLowerCase();
  if (/donut|composition|composi[cç][aã]o|horizontal|distribution|distribui[cç][aã]o|asset_distribution|revenue_breakdown|regi[aã]o|segmento|neg[oó]cio|state|estado|setor|classe|categoria/.test(norm(text))) return 'category';
  return 'time';
}

function chartSourceVisualModel(source = '') {
  const raw = cleanText(source).toLowerCase();
  if (raw.includes('investidor10')) return 'investidor10';
  if (raw.includes('statusinvest')) return 'statusinvest';
  if (raw.includes('yahoo')) return 'yahoo';
  if (raw.includes('b3')) return 'b3';
  return 'valorae';
}

function isInvestidor10AnalysisChartSource(candidate = {}) {
  const raw = [
    candidate.source,
    candidate.sourceVisualModel,
    candidate.id,
    candidate.title,
    candidate.provider,
  ...
  if (!raw) return false;
  if (/simulad|sint[eé]tic|mock|fake|reconstru|reconstructed|fallback falso|snapshot degradado/.test(raw)) return false;
  if (/(status\s*invest|statusinvest|yahoo|finance chart api|b3\/|bcb|google)/.test(raw) && !raw.includes('investidor10')) return false;
  return raw.includes('investidor10') ||
    raw.includes('cotacao-lucro') ||
    raw.includes('cotação-lucro') ||
    raw.includes('api de balancos') ||
    raw.includes('api de balanços');
}

function chartSourceFidelity(candidate = {}) {
  if (hasFakeComparisonFlags(candidate)) return 'reconstructed';
  const explicit = cleanText(candidate.sourceFidelity || candidate.fidelity).toLowerCase();
  if (['exact', 'derived', 'reconstructed', 'official', 'source'].includes(explicit)) return explicit;
  return 'exact';
}

function withChartContractMetadata(chart = {}, options = {}) {
  if (!chart || typeof chart !== 'object') return chart;
  const axisKind = options.axisKind || chartAxisKind({ ...chart, ...options });
  const preserveSourceOrder = options.preserveSourceOrder ?? chart.preserveSourceOrder ?? axisKind === 'category';
  const sourceFidelity = chartSourceFidelity({ ...chart, ...options });
  return {
    ...chart,
    axisKind,
    preserveSourceOrder: Boolean(preserveSourceOrder),
    sourceVisualModel: options.sourceVisualModel || chart.sourceVisualModel || chartSourceVisualModel(chart.source || options.source || ''),
    sourceFidelity
  };
}

function hasMinimumDisplayableChartPoints(chart = {}) {
  const series = arrayFrom(chart.series);
  const pointCount = series.reduce((sum, serie) => sum + arrayFrom(serie?.points).length, 0);
  if (pointCount <= 0) return false;
  const axisKind = cleanText(chart.axisKind).toLowerCase();
  const visual = [chart.id, chart.title, chart.chartType, chart.type, chart.sourceVisualModel]
    .map(cleanText)
    .join(' ')
    .toLowerCase();
  const categoryLike = axisKind === 'category' || /donut|composition|composi[cç][aã]o|horizontal|distribui[cç][aã]o|asset_distribution|revenue_breakdown|regi[aã]o|segmento|neg[oó]cio|categoria/.test(norm(visual));
  if (categoryLike) return pointCount >= 1;
  return series.some(serie => arrayFrom(serie?.points).length >= 2);
}

function finalizeAnalysisChart(chart = {}) {
  const enriched = withChartContractMetadata(chart);
  if (enriched.sourceFidelity === 'reconstructed' || hasFakeComparisonFlags(enriched)) return null;
  if (!isInvestidor10AnalysisChartSource(enriched)) return null;
  if (!hasMinimumDisplayableChartPoints(enriched)) return null;
  return enriched;
}

function finalizeAnalysisSections(sections = []) {
  return arrayFrom(sections).map(section => {
    const charts = arrayFrom(section.charts).map(finalizeAnalysisChart).filter(Boolean);
    const itemCount = arrayFrom(section.items).length + charts.length;
    return { ...section, charts, itemCount, status: itemCount > 0 ? 'ready' : (section.status || 'empty') };
  });
}
...
function analysisChart(id, title, chartType, points, source = 'VALORAE Proxy', options = {}) {
  ...
  return withChartContractMetadata({
    id,
    title,
    chartType,
    source,
...
function multiSeriesChart(id, title, chartType, rows = [], source = 'VALORAE Proxy', seriesSpecs = [], options = {}) {
  const normalized = sortChartRowsChronologically(arrayFrom(rows)).slice(-(options.limit || 14));
  const minPoints = options.minPoints || 2;
  const maxSeries = options.maxSeries || 6;
  const candidateSeries = seriesSpecs
    .map(spec => {
      const seen = new Map();
      for (const row of normalized) {
        const point = chartPoint(row, spec.valueKeys || [spec.id, 'value']);
        if (!point || !cleanText(point.label) || !Number.isFinite(point.value)) continue;
        if (options.ignoreZero !== false && Math.abs(point.value) === 0) continue;
        seen.set(point.label, point);
      }
      const points = sortChartPointsChronologically([...seen.values()]).slice(-(options.pointLimit || 10));
      return { id: spec.id || spec.label || 'series', label: spec.label || spec.id || 'Série', points };
    })
    .filter(serie => serie.points.length >= minPoints)
    .slice(0, maxSeries);
  if (candidateSeries.length < (options.minSeries || 1)) return null;
  const periodCounts = new Map();
  for (const serie of candidateSeries) {
    for (const label of new Set(serie.points.map(point => cleanText(point.label)).filter(Boolean))) {
      periodCounts.set(label, (periodCounts.get(label) || 0) + 1);
    }
  }
  const requiredAlignedSeries = options.requireFullAlignment ? candidateSeries.length : Math.min(2, candidateSeries.length);
  const commonPeriodCandidates = [...periodCounts.entries()]
    .filter(([, count]) => count >= requiredAlignedSeries)
    .map(([label]) => label);
  const commonPeriods = sortPeriodLabelsChronologically(commonPeriodCandidates)
    .slice(-(options.pointLimit || 10));
  // Para gráficos financeiros com mais de duas séries, não relaxar o alinhamento.
  // O APK renderiza barras agrupadas por período; aceitar períodos diferentes por série
  // gera barras deslocadas ou linhas que parecem quebradas no modal. Se a fonte não
  // entregar interseção suficiente, o chamador cai no gráfico simples/duplo seguro.
  if (options.requireFullAlignment && commonPeriods.length < minPoints) return null;
  const aligned = commonPeriods.length >= minPoints
    ? candidateSeries.map(serie => {
        const byLabel = new Map(serie.points.map(point => [cleanText(point.label), point]));
        return { ...serie, points: commonPeriods.map(label => byLabel.get(label)).filter(Boolean) };
      }).filter(serie => serie.points.length >= minPoints)
    : candidateSeries;
  if (aligned.length < (options.minSeries || 1)) return null;
  return withChartContractMetadata({
    id,
    title,
    chartType,
...
function collectHistoricalIndicatorSources(payload = {}) {
  ...
  return [
    { source: 'StatusInvest histórico', value: payload.historicalIndicators },
...
function addHistoricalIndicatorRecord(records, seen, rawLabel, rawPeriod, rawValue, specs, source = 'Investidor10') {
...
function normalizeHistoricalIndicatorRecords(payload = {}, assetType = '') {
  ...
  const walk = (value, source = 'Investidor10', inheritedLabel = '') => {
    if (!value) return;
    if (Array.isArray(value)) {
  ...
  for (const item of arrayFrom(bundle.dividendYieldHistory || payload.dividendYieldHistory || payload.results?.dividendYieldHistory)) {
    addHistoricalIndicatorRecord(records, seen, 'Dividend Yield', item.year || item.period || item.label || item.date, item, specs, item.source || 'Investidor10 histórico de proventos');
  }
  for (const item of arrayFrom(bundle.payoutHistory || payload.payoutHistory || payload.results?.payoutHistory)) {
    addHistoricalIndicatorRecord(records, seen, 'Payout', item.year || item.period || item.label || item.date, item, specs, item.source || 'Investidor10');
  }
...
function buildHistoricalIndicatorCharts(payload = {}, assetType = '') {
  const records = normalizeHistoricalIndicatorRecords(payload, assetType)
    .filter(record => isInvestidor10AnalysisChartSource({ source: record.source }));
  const specs = historySpecsForAsset(assetType);
  const charts = [];
  for (const spec of specs) {
    ...
    const chart = analysisChart(
      ...
      'line',
      rows.reverse(),
      'Investidor10 histórico de indicadores',
...
function addFinancialStatementRecord(records, seen, metric, period, value, source = 'Investidor10 APIs de balanços') {
  if (!metric) return;
...
function collectFinancialStatementSources(payload = {}) {
  ...
  return [
    { source: 'Investidor10 APIs de balanços', value: payload.financialStatements },
    { source: 'Investidor10 APIs de balanços', value: payload.statements },
    { source: 'Investidor10 APIs de balanços', value: payload.demonstrativos },
    { source: 'Investidor10 APIs de balanços', value: payload.results?.financialStatements },
    { source: 'Investidor10 APIs de balanços', value: payload.results?.statements },
    { source: 'Investidor10 APIs de balanços', value: payload.results?.demonstrativos },
    ...
    { source: 'Investidor10 APIs de balanços', value: sections.demonstrativos },
    ...
    { source: 'Investidor10 APIs de balanços', value: payload.results?.financialChartsCanonical || payload.financialChartsCanonical },
    { source: 'Investidor10 APIs de balanços', value: payload.results?.assetChartsCanonical?.financial || payload.assetChartsCanonical?.financial },
    ...
    { source: 'Investidor10 assetChartBundle', value: { incomeStatement: bundle.incomeStatement || bundle.revenueProfit, balanceSheet: bundle.balanceSheet || bundle.equityEvolution, cashFlowStatement: bundle.cashFlowStatement } }
  ].filter(entry => entry.value !== undefined && entry.value !== null);
}
...
function addFinancialValuesByPeriod(records, seen, metric, values = {}, source = 'Investidor10 APIs de balanços') {
...
function consumeFinancialChartObject(records, seen, value = {}, source = 'Investidor10 APIs de balanços', groupHint = null) {
...
function normalizeFinancialStatementRecords(payload = {}) {
  ...
  const consumeRow = (row = {}, source = 'Investidor10 APIs de balanços', groupHint = null, inheritedPeriod = '') => {
  ...
  const walk = (value, source = 'Investidor10 APIs de balanços', groupHint = null, inheritedPeriod = '') => {
    if (value === undefined || value === null) return;
    if (Array.isArray(value)) {
...
function alignStatementSeriesByPeriod(series = []) {
  const safeSeries = series
    .map(serie => ({
      ...
      points: arrayFrom(serie.points)
        ...
        .sort((a, b) => (chartPeriodSortKey(a.label) ?? Number.MAX_SAFE_INTEGER) - (chartPeriodSortKey(b.label) ?? Number.MAX_SAFE_INTEGER) || String(a.label).localeCompare(String(b.label), 'pt-BR', { numeric: true }))
    }))
  ...
  const preferredPeriodCandidates = [...periodCounts.entries()]
    .filter(([, count]) => count >= Math.min(2, safeSeries.length))
    .map(([label]) => label);
  const preferredPeriods = sortPeriodLabelsChronologically(preferredPeriodCandidates)
    .slice(-10);
...
function buildFinancialStatementCharts(payload = {}) {
  const records = normalizeFinancialStatementRecords(payload)
  ...
  for (const group of FINANCIAL_STATEMENT_GROUPS) {
    ...
    for (const metric of group.metrics) {
      ...
      const points = sortChartPointsChronologically(Array.from(unique.values()))
        .slice(-10);
      if (points.length >= 2) candidateSeries.push({ id: metric.id, label: metric.label, points });
      if (candidateSeries.length >= 5) break;
    }
    const series = alignStatementSeriesByPeriod(candidateSeries).slice(0, 5);
    if (series.length) {
      charts.push({
        id: `${group.id}_statement`,
        title: `${group.title} por período`,
        chartType: 'grouped_bar',
        source: 'Investidor10 APIs de balanços normalizadas pelo Proxy',
        unit: 'R$',
        series
      });
    }
...
function appendInvestidor10CurrentQuotePoint(rows = [], payload = {}) {
  ...
  const livePoint = {
    ...
    source: 'Investidor10 cotação atual normalizada'
  };
...
function buildAssetCharts(payload = {}) {
  ...
  // Cotação/Yahoo/StatusInvest direto não entra mais como gráfico visual da Análise/modal.
  // Só é permitido o histórico de preço quando ele já veio no bundle canônico do Investidor10.
  const priceRows = appendInvestidor10CurrentQuotePoint(bundle.priceHistory, payload);
  const price = analysisChart('price_history', 'Cotação em tempo real', 'line', priceRows, 'Investidor10 gráfico de cotação', { valueKeys: ['close', 'price', 'value'], minPoints: 2, limit: 24, unit: 'R$', seriesLabel: 'Cotação' });
  if (price) charts.push(price);

  const dividends = analysisChart(isFii ? 'fii_monthly_distribution' : 'dividend_history', isFii ? 'Rendimento mensal' : 'Histórico de proventos', 'bar', bundle.dividendMonthly || bundle.fiiDistribution12m || bundle.dividendYearly, 'Investidor10 histórico de proventos', { valueKeys: ['amount', 'value', 'total', 'valuePerShare'], minPoints: 2, limit: 14, unit: 'R$' });
  if (dividends) charts.push(dividends);

  const dyHistory = analysisChart('dividend_yield_history', 'Dividend Yield histórico', 'bar', bundle.dividendYieldHistory || bundle.fiiDividendYieldHistory || payload.dividendYieldHistory || payload.results?.dividendYieldHistory, 'Investidor10 histórico de proventos', { valueKeys: ['yieldPercent', 'dividendYield', 'dy', 'valuePercent', 'percent', 'percentage', 'value'], minPoints: 2, limit: 14, unit: '%' });
  if (dyHistory) charts.push(dyHistory);

  const revenueRows = bundle.revenueProfit || payload.statements?.revenueProfit || payload.results?.statements?.revenueProfit;
  const revenueProfit = multiSeriesChart('revenue_profit', 'Receitas e Lucros', 'grouped_bar', revenueRows, 'Investidor10 APIs de balanços', [
    { id: 'net_revenue', label: 'Receita líquida', valueKeys: ['netRevenue', 'net_revenue', 'revenue', 'receitaLiquida', 'receita'] },
    { id: 'gross_profit', label: 'Lucro bruto', valueKeys: ['grossProfit', 'lucroBruto'] },
    { id: 'ebitda', label: 'EBITDA', valueKeys: ['ebitda'] },
    { id: 'ebit', label: 'EBIT', valueKeys: ['ebit'] },
    { id: 'net_profit', label: 'Lucro líquido', valueKeys: ['netProfit', 'net_profit', 'profit', 'lucroLiquido', 'lucro'] }
  ], { unit: 'R$', maxSeries: 5, minSeries: 2, pointLimit: 10, requireFullAlignment: true }) ||
    dualSeriesChart('revenue_profit', 'Receitas e Lucros', 'grouped_bar', revenueRows, 'Investidor10 APIs de balanços', { id: 'revenue', label: 'Receita', valueKeys: ['netRevenue', 'revenue', 'receita'] }, { id: 'profit', label: 'Lucro', valueKeys: ['netProfit', 'profit', 'lucro'] });
  ...
  const equityRows = bundle.equityEvolution || bundle.balanceSheet || bundle.fiiPatrimonialInfo || payload.statements?.balanceSheet || payload.results?.statements?.balanceSheet;
  const equity = isFii
    ? analysisChart('fii_patrimonial_value', 'Valor patrimonial', 'bar', equityRows, 'Investidor10', { valueKeys: ['netWorth', 'equity', 'patrimonioLiquido', 'valorPatrimonial', 'patrimonialValue', 'value'], minPoints: 2, limit: 12, unit: 'R$' })
    : (multiSeriesChart('equity_evolution', 'Balanço e patrimônio', 'grouped_bar', equityRows, 'Investidor10 APIs de balanços', [
        { id: 'net_worth', label: 'Patrimônio líquido', valueKeys: ['netWorth', 'equity', 'patrimonioLiquido', 'pl'] },
        { id: 'total_assets', label: 'Ativos', valueKeys: ['totalAssets', 'assets', 'ativos', 'ativoTotal'] },
        { id: 'total_liabilities', label: 'Passivos', valueKeys: ['totalLiabilities', 'liabilities', 'passivos', 'passivoTotal'] },
        { id: 'net_debt', label: 'Dívida líquida', valueKeys: ['netDebt', 'net_debt', 'dividaLiquida', 'dívidaLiquida'] },
        { id: 'cash', label: 'Disponibilidade', valueKeys: ['cash', 'disponibilidade', 'caixa'] }
      ], { unit: 'R$', maxSeries: 5, minSeries: 2, pointLimit: 10, requireFullAlignment: true }) ||
      analysisChart('equity_evolution', 'Evolução patrimonial', 'bar', equityRows, 'Investidor10 APIs de balanços', { valueKeys: ['netWorth', 'equity', 'patrimonioLiquido', 'value'], minPoints: 2, limit: 12 }));
  if (equity) charts.push(equity);

  const payout = analysisChart('payout_history', 'Payout histórico', 'bar', bundle.payoutHistory || payload.payoutHistory || payload.results?.payoutHistory, 'Investidor10', { valueKeys: ['payout', 'valuePercent', 'percent', 'percentage', 'value'], minPoints: 2, limit: 14, unit: '%' });
  ...
  const assetDistribution = analysisChart('fii_asset_distribution', 'Distribuição de ativos do fundo', 'donut_composition', assetDistributionPoints, 'Investidor10', { valueKeys: ['value'], minPoints: 1, limit: 12, unit: '%' });
...
const STOCK_PROFILE_FIELDS = [
  ...
  { id: 'segment', label: 'Segmento', group: 'Classificação', aliases: ['segmento', 'segment'] },
  { id: 'listingSegment', label: 'Segmento de listagem', group: 'Governança', aliases: ['segmentoListagem', 'segmento de listagem', 'segmentoB3', 'listingSegment'] },
  { id: 'cnpj', label: 'CNPJ', group: 'Cadastro', aliases: ['cnpj', 'documento'] },
  { id: 'site', label: 'Site', group: 'Cadastro', aliases: ['site', 'website', 'url'] },
  { id: 'foundationYear', label: 'Ano de fundação', group: 'Cadastro', aliases: ['anoFundacao', 'ano de fundação', 'foundationYear'] },
  { id: 'listedSince', label: 'Ano de estreia na bolsa', group: 'Cadastro', aliases: ['anoEstreiaBolsa', 'ano de estreia na bolsa', 'listedSince'] },
  { id: 'employees', label: 'Funcionários', group: 'Cadastro', aliases: ['numeroFuncionarios', 'número de funcionários', 'funcionarios', 'employees'] },
  { id: 'listedTickers', label: 'Papéis da empresa', group: 'Mercado', aliases: ['papeisEmpresa', 'papéis da empresa', 'tickers', 'listedTickers'] },
  { id: 'fractionalTickers', label: 'Papéis fracionados', group: 'Mercado', aliases: ['papeisFracionados', 'papéis fracionados', 'fractionalTickers'] },
...
function addProfileItem(rows, seen, spec, rawValue, source = 'Investidor10') {
...
function collectCompanyProfileSources(payload = {}) {
  ...
  return [
    { source: 'StatusInvest/Investidor10 apresentação', value: payload.profilePresentation },
    { source: 'StatusInvest/Investidor10 apresentação', value: payload.assetPresentation },
    { source: 'StatusInvest/Investidor10 apresentação', value: results.profilePresentation },
    { source: 'StatusInvest/Investidor10 apresentação', value: results.assetPresentation },
    { source: 'Investidor10 apresentação', value: payload.assetChartsCanonical?.presentation || payload.assetChartsCanonical?.profilePresentation },
    { source: 'Investidor10 apresentação', value: results.assetChartsCanonical?.presentation || results.assetChartsCanonical?.profilePresentation },
    { source: 'Investidor10 apresentação', value: results.assetChartsCanonical?.company?.presentation || results.assetChartsCanonical?.fii?.presentation },
    { source: 'Investidor10 cadastro', value: results.assetChartsCanonical?.company?.info || results.assetChartsCanonical?.fii?.info },
...
function collectProfilePresentations(payload = {}, merged = {}) {
  const results = payload.results || {};
  const candidates = [
    payload.profilePresentation, payload.assetPresentation, payload.presentation,
    results.profilePresentation, results.assetPresentation, results.presentation,
    results.sections?.sobre?.presentation, results.sections?.empresa?.profilePresentation, results.sections?.empresa?.sobre?.presentation,
    results.sections?.fundo?.profilePresentation, results.sections?.fundo?.sobre?.presentation,
    merged.presentation, merged.profilePresentation, merged.assetPresentation
  ];
  return candidates.filter(candidate => candidate && typeof candidate === 'object');
}

function profilePresentationSections(presentation = {}) {
  const sections = arrayFrom(presentation.sections || presentation.narrativeBlocks || presentation.blocks)
    .map((block, index) => ({
      id: cleanText(block.id || block.key || `profile_text_${index}`),
      label: cleanText(block.title || block.label || (index === 0 ? 'Sobre' : 'História')),
      value: cleanText(block.text || block.value || block.summary || block.description),
      source: cleanText(block.source || presentation.source || 'Investidor10 perfil textual')
    }))
    .filter(block => block.value.length >= 60);
  if (!sections.length && cleanText(presentation.summary).length >= 60) {
    sections.push({ id: 'profile_text_summary', label: 'Sobre', value: cleanText(presentation.summary), source: cleanText(presentation.source || 'Investidor10 perfil textual') });
  }
...
function buildCompanyProfileItems(payload = {}, assetType = '') {
  ...
  const presentations = collectProfilePresentations(payload, merged);
  const presentationSections = presentations.flatMap(profilePresentationSections);
  const primaryPresentationSummary = firstClean(...presentations.map(p => p.summary), presentationSections[0]?.value);

  const info = {
    ...merged,
    descricao: firstClean(primaryPresentationSummary, payload.assetDescription, payload.description, payload.descricao, payload.profilePresentation?.summary, payload.assetPresentation?.summary, results.profilePresentation?.summary, results.assetPresentation?.summary, merged.summary, merged.descricao, merged.description, merged.sobre, merged.resumo),
    ...
    anoFundacao: firstClean(payload.anoFundacao, payload.foundationYear, merged.anoFundacao, merged.foundationYear),
    anoEstreiaBolsa: firstClean(payload.anoEstreiaBolsa, payload.listedSince, merged.anoEstreiaBolsa, merged.listedSince),
    numeroFuncionarios: firstClean(payload.numeroFuncionarios, payload.employees, merged.numeroFuncionarios, merged.employees),
    papeisEmpresa: firstClean(payload.papeisEmpresa, payload.listedTickers, merged.papeisEmpresa, merged.listedTickers),
    papeisFracionados: firstClean(payload.papeisFracionados, payload.fractionalTickers, merged.papeisFracionados, merged.fractionalTickers),
    segmentoListagem: firstClean(payload.segmentoListagem, payload.listingSegment, merged.segmentoListagem, merged.listingSegment),
  ...
  const sourceLabel = cleanText(presentations[0]?.source) || 'StatusInvest/Investidor10 perfil';
  if (description) rows.push({ id: 'profile_description', label: isFii ? 'Sobre o fundo' : 'Sobre a empresa', value: description, group: 'Descrição', source: sourceLabel });
  for (const block of presentationSections) {
    if (rows.length >= 8) break;
    if (rows.some(row => norm(row.value) === norm(block.value))) continue;
    rows.push({
      id: `profile_narrative_${block.id || rows.length}`,
      label: block.label || (isFii ? 'Sobre o fundo' : 'Sobre a empresa'),
      value: block.value,
      group: 'Descrição',
      source: block.source || sourceLabel
    });
  }
  ...
  const officialLabels = new Set([...specs.flatMap(spec => [spec.label, spec.id, ...(spec.aliases || [])]).map(norm), 'descricao', 'description', 'sobre', 'resumo', 'summary', 'sections', 'narrativeblocks', 'profilepresentation', 'assetpresentation']);
...
function collectRevenueRowsFromValue(value, source = 'Investidor10') {
  const rows = [];
...
function collectRevenueRows(payload = {}, paths = [], source = 'Investidor10') {
  const rows = [];
  for (const path of paths) {
...
function dedupeRevenueRows(rows = []) {
  ...
  for (const row of rows) {
    ...
    if (!current || percent > current.percent) map.set(key, { label, period, percent, source: row.source || 'Investidor10' });
  }
...
function revenueChartFromRows(id, title, rows = [], source = 'Investidor10') {
...
function buildRevenueBreakdown(payload = {}) {
  const businessRows = dedupeRevenueRows([
    ...collectRevenueRows(payload, REVENUE_BUSINESS_PATHS, 'Investidor10 negócios de receita'),
    ...collectRevenueRowsByKey(payload, REVENUE_BUSINESS_KEY_HINTS, 'Investidor10 negócios de receita')
  ]);
  const regionRows = dedupeRevenueRows([
    ...collectRevenueRows(payload, REVENUE_REGION_PATHS, 'Investidor10 regiões de receita'),
    ...collectRevenueRowsByKey(payload, REVENUE_REGION_KEY_HINTS, 'Investidor10 regiões de receita')
  ]);
...
function buildSourceComparativeItemsAndCharts(payload = {}, assetType = '') {
  ...
  // Mantém os comparativos setoriais como dados/tabela. Não expõe o antigo gráfico
  // "Indicadores x setor" para não misturar visual StatusInvest na Análise/modal.
  return { items, charts: [] };
}


const PEER_FUNDAMENTAL_METRICS = [
  { id: 'pl', label: 'P/L', aliases: ['pl', 'pL', 'P/L'], lowerBetter: true },
  { id: 'pvp', label: 'P/VP', aliases: ['pvp', 'pVP', 'P/VP'], lowerBetter: true },
  { id: 'dividendYield', label: 'Dividend Yield', aliases: ['dividendYield', 'dy', 'Dividend Yield', 'DY'], lowerBetter: false, unit: '%' },
  { id: 'roe', label: 'ROE', aliases: ['roe', 'ROE'], lowerBetter: false, unit: '%' }
];

function peerMetricDisplay(raw, unit = '') {
  const display = cleanText(raw?.display || raw?.formatted || raw?.fmt || raw?.text);
  if (display) return display;
  if (unit === '%') return percentDisplay(rawNumericValue(raw));
  return decimalDisplay(rawNumericValue(raw)) || cleanText(rawNumericValue(raw));
}

function metricFromPeer(peer = {}, metric = {}) {
  const metrics = objectFrom(peer.metrics || peer.indicators || peer.fundamentals || peer.valores || peer.values);
  for (const alias of [metric.id, metric.label, ...(metric.aliases || [])]) {
    const value = metrics?.[alias] ?? peer?.[alias];
    if (value !== undefined && value !== null && cleanText(value) !== '') {
      const numeric = finiteValue(rawNumericValue(value));
      return { numeric, display: peerMetricDisplay(value, metric.unit), raw: value };
    }
  }
  return null;
}

function collectPeerFundamentalCompanies(payload = {}) {
  const candidates = [
    payload.peerFundamentalComparator,
    payload.relatedCompanies,
    payload.peers,
    payload.comparadorAcoes,
    payload.results?.peerFundamentalComparator,
    payload.results?.relatedCompanies,
    payload.results?.peers,
    payload.results?.comparadorAcoes,
    payload.results?.sections?.empresa?.peerFundamentalComparator,
    payload.results?.sections?.empresa?.relatedCompanies
  ];
  const rows = [];
  const seen = new Set();
  for (const value of candidates) {
    for (const item of arrayFrom(value)) {
      const ticker = cleanText(item?.ticker || item?.symbol || item?.code || item?.label || item?.name).toUpperCase();
      if (!ticker || seen.has(ticker)) continue;
      seen.add(ticker);
      rows.push({ ...item, ticker, symbol: ticker });
    }
  }
  return rows;
}

function collectPeerFundamentalComparativeCards(payload = {}) {
  return [
    ...indicatorCards(payload),
    ...arrayFrom(payload.sourceComparatives),
    ...arrayFrom(payload.results?.sourceComparatives),
...
function findAssetMetricValue(payload = {}, metric = {}, cards = []) {
  for (const card of cards) {
    const label = norm(card?.label || card?.name || card?.key);
    const wanted = [metric.id, metric.label, ...(metric.aliases || [])].map(norm);
    if (!wanted.some(key => key && (label === key || label.includes(key) || key.includes(label)))) continue;
    const numeric = finiteValue(card?.value ?? card?.raw ?? card?.valor ?? card?.amount);
    const display = comparisonDisplay(card, card?.unit || metric.unit || '');
    if (numeric !== undefined || display) return { numeric, display, raw: card };
  }
  for (const alias of [metric.id, ...(metric.aliases || [])]) {
    const raw = rawField(payload, [alias]);
    if (raw !== undefined && raw !== null && cleanText(raw) !== '') {
      return { numeric: finiteValue(raw), display: metric.unit === '%' ? percentDisplay(raw) : (decimalDisplay(raw) || cleanText(raw)), raw };
    }
  }
  return null;
}

function peerFundamentalWinner(metric, values = []) {
  const numericValues = values.filter(item => item && Number.isFinite(item.numeric));
  if (!numericValues.length) return null;
  return numericValues.reduce((best, item) => {
    if (!best) return item;
    return metric.lowerBetter ? (item.numeric < best.numeric ? item : best) : (item.numeric > best.numeric ? item : best);
  }, null);
}

function comparativeBenchmarkEntry(source = {}, keyAliases = [], fallbackLabel = '', unit = '') {
  if (!source) return null;
  const normalizedAliases = keyAliases.map(norm).filter(Boolean);
  let value;
  if (Array.isArray(source)) {
    value = source.find(item => normalizedAliases.includes(norm(item?.scope || item?.kind || item?.group || item?.label || item?.name || item?.key)));
  } else if (typeof source === 'object') {
    for (const [key, candidate] of Object.entries(source)) {
      if (normalizedAliases.includes(norm(key)) || normalizedAliases.includes(norm(candidate?.scope || candidate?.kind || candidate?.group || candidate?.label || candidate?.name))) {
        value = candidate;
        break;
      }
    }
  ...
  const numeric = finiteValue(value?.value ?? value?.valor ?? value?.numeric ?? value?.raw ?? value);
  const display = comparisonDisplay(value, value?.unit || unit || '');
  if (numeric === undefined && !display) return null;
  return {
    ticker: fallbackLabel,
    label: cleanText(value?.label || value?.name || fallbackLabel) || fallbackLabel,
    numeric,
    display,
    kind: norm(fallbackLabel).replace(/\s+/g, '_') || 'benchmark'
  };
}

function benchmarkComparativesForMetric(cards = [], metric = {}) {
  const wanted = [metric.id, metric.label, ...(metric.aliases || [])].map(norm);
  for (const card of cards) {
    const label = norm(card?.label || card?.name || card?.key);
    if (!wanted.some(key => key && (label === key || label.includes(key) || key.includes(label)))) continue;
    const comparisons = card?.comparisons || card?.comparativos || card?.comparative || card?.benchmarks || card?.averages || {};
    const directComparisons = { ...(comparisons && typeof comparisons === 'object' && !Array.isArray(comparisons) ? comparisons : {}) };
    const directSector = card?.setor ?? card?.sector;
    const directSubsector = card?.subsetor ?? card?.subSector ?? card?.subsector;
    const directSegment = card?.segmento ?? card?.segment;
    if (directSector !== undefined && directSector !== null) directComparisons.setor = directSector;
    if (directSubsector !== undefined && directSubsector !== null) directComparisons.subsetor = directSubsector;
    if (directSegment !== undefined && directSegment !== null) directComparisons.segmento = directSegment;
    return [
      comparativeBenchmarkEntry(directComparisons, ['setor', 'sector'], 'Setor', card.unit || metric.unit || ''),
      comparativeBenchmarkEntry(directComparisons, ['subsetor', 'sub_setor', 'subsector', 'sub sector', 'subSector'], 'Subsetor', card.unit || metric.unit || ''),
      comparativeBenchmarkEntry(directComparisons, ['segmento', 'segment'], 'Segmento', card.unit || metric.unit || '')
    ].filter(Boolean);
  }
  return [];
}

function buildPeerFundamentalComparator(payload = {}, assetType = '') {
  if (!isCompanyLikeAssetType(assetType)) return { items: [], charts: [] };
  const ticker = cleanText(payload.ticker || payload.symbol || payload.results?.ticker || payload.results?.symbol).toUpperCase() || 'Ativo';
  const groups = objectFrom(payload.comparativeGroups || payload.results?.comparativeGroups || payload.results?.sections?.empresa?.comparativeGroups || payload.profile || payload.results?.profile);
  const segmentName = cleanText(groups.segment || groups.segmento || payload.segment || payload.subSector || payload.subsetor || payload.results?.segment || payload.results?.subsetor);
  const cards = collectPeerFundamentalComparativeCards(payload);
  const peers = collectPeerFundamentalCompanies(payload).filter(peer => peer.ticker !== ticker).slice(0, 5);
  const rows = [];
  const seen = new Set();
  if (segmentName || peers.length) {
    rows.push({
      id: 'peer_fundamental_scope',
      label: 'Grupo comparado',
      value: [segmentName ? `Segmento: ${segmentName}` : '', peers.length ? `Pares: ${peers.map(peer => peer.ticker).join(', ')}` : ''].filter(Boolean).join(' • '),
      group: 'Comparador por segmento',
      source: 'Investidor10 Comparador/Empresas Relacionadas'
    });
  }
  for (const metric of PEER_FUNDAMENTAL_METRICS) {
    const assetMetric = findAssetMetricValue(payload, metric, cards);
    const values = [];
    if (assetMetric && (assetMetric.numeric !== undefined || assetMetric.display)) values.push({ ticker, numeric: assetMetric.numeric, display: assetMetric.display || cleanText(assetMetric.raw), kind: 'asset' });
    for (const peer of peers) {
      const peerMetric = metricFromPeer(peer, metric);
      if (!peerMetric || (peerMetric.numeric === undefined && !peerMetric.display)) continue;
      values.push({ ticker: peer.ticker, numeric: peerMetric.numeric, display: peerMetric.display || cleanText(peerMetric.raw), kind: 'peer' });
    }
    const benchmarks = benchmarkComparativesForMetric(cards, metric);
    const benchmarkValues = [...values, ...benchmarks.filter(item => item.numeric !== undefined || item.display)];
    if (benchmarkValues.length < 2) continue;
    const winner = peerFundamentalWinner(metric, benchmarkValues);
    const comparison = benchmarkValues.map(item => `${item.ticker || item.label}: ${item.display || decimalDisplay(item.numeric)}`).join(' • ');
    const key = norm(`${metric.id}|${comparison}`);
    if (seen.has(key)) continue;
    seen.add(key);
    rows.push({
      id: `peer_fundamental_${metric.id}`,
      label: metric.label,
      value: `${comparison}${winner ? ` • Melhor: ${winner.ticker}` : ''}`,
      group: metric.lowerBetter ? 'Menor é melhor' : 'Maior é melhor',
      source: peers.length ? 'Investidor10 Empresas Relacionadas + médias Setor/Subsetor/Segmento' : 'Investidor10 comparativos Setor/Subsetor/Segmento'
    });
  }
  return { items: rows.slice(0, 12), charts: [] };
}
...
const CRITICAL_SOURCE_SECTION_IDS = new Set([
  'summary',
...
const SOURCE_SECTION_REFERENCE_LABELS = {
  summary: 'Cotação, P/L, P/VP, DY e variação 12M',
  market_context: 'Mercado, risco, liquidez e índices',
  fundamental_indicators: 'Indicadores fundamentalistas',
  valuation_models: 'Preço justo Graham/Bazin',
  source_comparatives: 'Comparativos por setor/subsetor/segmento',
  peer_fundamental_comparator: 'Empresas relacionadas e comparador do mesmo segmento',
  indices_events: 'Índices e participação',
  governance_events: 'Dados de mercado, cadastro e eventos',
  checklist: 'Checklist Investidor10 Buy and Hold',
  dividend_radar: 'Radar de dividendos',
  historical_indicators: 'Histórico de indicadores',
  financial_statements: 'DRE, balanço e fluxo de caixa',
  asset_charts: 'Receitas/lucros, lucro x cotação, DY, patrimônio e composições Investidor10',
  revenue_breakdown: 'Regiões e negócios que geram receita',
  ownership: 'Posição acionária',
  company_profile: 'Sobre a empresa/fundo e dados cadastrais',
  comparisons: 'Comparação com índices e ativos',
  fii_accounting: 'Dados patrimoniais e contábeis do FII',
  fii_details: 'Sobre, estratégia, taxas e estrutura do FII',
  fii_portfolio: 'Carteira, imóveis, distribuição e exposição do FII',
  fii_checklist: 'Checklist Investidor10 para FII'
};
...
function buildSourceCoverageDiagnostics(assetType = '', sections = []) {
  ...
  const expected = [
    ...
    { id: 'peer_fundamental_comparator', source: 'Investidor10 comparador de fundamentos por segmento', expected: true },
    ...
    { id: 'historical_indicators', source: 'Investidor10', expected: true },
    { id: 'financial_statements', source: 'Investidor10', expected: companyLike },
    { id: 'asset_charts', source: 'Investidor10 gráficos visuais oficiais', expected: true },
    { id: 'revenue_breakdown', source: 'Investidor10', expected: companyLike },
    { id: 'ownership', source: 'Investidor10', expected: companyLike },
    { id: 'company_profile', source: 'Investidor10', expected: true },
    { id: 'comparisons', source: 'Investidor10 comparadores quando disponíveis', expected: true },
    { id: 'fii_accounting', source: 'Investidor10', expected: isFii },
    { id: 'fii_details', source: 'Investidor10', expected: isFii },
    { id: 'fii_portfolio', source: 'Investidor10', expected: isFii },
  ...
  return expected.map(item => {
    ...
    return {
      ...item,
      referenceLabel: SOURCE_SECTION_REFERENCE_LABELS[item.id] || item.id,
      critical: CRITICAL_SOURCE_SECTION_IDS.has(item.id),
      status: coverageStatus(section, item.expected),
...
function chartFidelityCounters(sections = []) {
  const counters = { total: 0, exact: 0, derived: 0, reconstructed: 0, unknown: 0 };
  for (const section of sections) {
    for (const chart of arrayFrom(section.charts)) {
      counters.total += 1;
      const fidelity = cleanText(chart.sourceFidelity || chart.fidelity).toLowerCase();
      if (fidelity === 'exact' || fidelity === 'source' || fidelity === 'official') counters.exact += 1;
      else if (fidelity === 'derived') counters.derived += 1;
      else if (fidelity === 'reconstructed' || hasFakeComparisonFlags(chart)) counters.reconstructed += 1;
...
function buildDataQualitySummary(sourceCoverage = [], sections = []) {
  const expected = sourceCoverage.filter(item => item.expected);
  const implemented = expected.filter(item => item.status === 'implemented');
  const missing = expected.filter(item => item.status === 'missing');
  const criticalMissing = missing.filter(item => item.critical);
  const charts = chartFidelityCounters(sections);
  const chartIssues = charts.unknown + charts.reconstructed;
  const status = criticalMissing.length || chartIssues ? 'attention' : (missing.length ? 'partial' : 'complete');
  const appActionHints = [
    ...(criticalMissing.length ? [`Priorizar captura/contrato das seções críticas: ${criticalMissing.map(item => item.id).join(', ')}.`] : []),
    ...(charts.unknown ? ['Revisar gráficos sem sourceFidelity explícito antes de exibir no modo profissional.'] : []),
    ...(charts.reconstructed ? ['Bloquear gráficos reconstruídos/sintéticos no Proxy e no APK.'] : []),
  ];
  return {
    status,
    expectedSections: expected.length,
    implementedSections: implemented.length,
    missingSections: missing.length,
    criticalMissingSections: criticalMissing.length,
    coveragePercent: expected.length ? Math.round((implemented.length / expected.length) * 100) : 100,
    totalItems: sections.reduce((sum, section) => sum + Number(section.itemCount || 0), 0),
    totalCharts: charts.total,
    exactCharts: charts.exact,
    derivedCharts: charts.derived,
    unknownFidelityCharts: charts.unknown,
    blockedReconstructedCharts: charts.reconstructed,
    strictChartPolicyOk: charts.unknown === 0 && charts.reconstructed === 0,
    missingExpectedSectionIds: missing.map(item => item.id),
    criticalMissingSectionIds: criticalMissing.map(item => item.id),
    sourceCaptureMap: expected.map(item => ({
      id: item.id,
    ...
    appActionHints,
    policy: 'Proxy entrega somente seções reais ou derivadas diretamente das fontes; gráficos reconstruídos/sintéticos não devem chegar ao APK.'
  };
}
...
function hasFakeComparisonFlags(candidate = {}) {
  ...
  const flags = [candidate.simulated, candidate.synthetic, candidate.proxyTickerUsed, candidate.fake, candidate.mocked, candidate.reconstructedFromYahooSnapshot, candidate.reconstructedFromMonthlyReturns, String(candidate.sourceFidelity || '').toLowerCase() === 'reconstructed'];
  ...
  return /(simulad|sint[eé]tic|proxy ticker|proxyticker|etf\/proxy|ticker substituto|fallback falso|snapshot degradado|último snapshot conhecido|ultimo snapshot conhecido|reconstructed|reconstru[ií]d|rentabilidade mensal)/i.test(text);
}
...
function collectComparisonCandidates(payload = {}) {
  ...
  const bundleComparisons = [
    ...arrayFrom(valueAtPath(payload, 'assetChartBundle.indexComparison')),
    ...arrayFrom(valueAtPath(payload, 'assetChartsMobile.indexComparison')),
    ...arrayFrom(bundle.indexComparison)
  ].map(candidate => ({ ...candidate, source: 'Investidor10 comparador de índices' }));
  candidates.push(...bundleComparisons);
  return candidates;
}
...
function buildCombinedIndexComparisonChart(ticker = '', charts = []) {
  ...
  return {
    ...
    chartType: 'multi_line',
    source: 'Investidor10 comparador de índices consolidado',
    unit: '%',
...
function buildComparisonItemsAndCharts(payload = {}) {
  ...
  for (const item of arrayFrom(payload.peers || payload.results?.peers || payload.comparadorAcoes || payload.results?.comparadorAcoes || payload.comparacaoFiis || payload.results?.comparacaoFiis)) {
    ...
    if (value) rows.push({ id: `peer_${label}`, label, value, group: 'Ações/FIIs semelhantes', source: item.source || 'Investidor10' });
  }
  ...
  for (const candidate of arrayFrom(payload.peerComparisons || payload.results?.peerComparisons || payload.comparisons?.peers)) {
    ...
    if (seenCharts.has(id)) continue;
    seenCharts.add(id);
    rows.push({ id, label: `${ticker || 'Ativo'} x ${label}`, value: comparisonPeriodValue(aligned), group: 'Ativo x par semelhante', source: candidate.source || 'Investidor10' });
    charts.push({ id, title: `${ticker || 'Ativo'} x ${label}`, chartType: 'multi_line', source: candidate.source || 'VALORAE Proxy comparadores estruturados', unit: '%', series: aligned });
  }
...
function addFiiDetailItem(rows, seen, id, label, rawValue, group = 'FII', source = 'Investidor10', spec = {}) {
...
function mergedFiiInfo(payload = {}) {
  ...
  return {
    ...
    yield12m: firstClean(payload.yield12m, payload.dividendYield, payload.dy, merged.yield12m, merged.dividendYield, merged.dy, merged['DY atual'], merged['Yield 12 meses']),
    ultimoRendimento: firstClean(payload.ultimoRendimento, merged.ultimoRendimento, merged.lastIncome, merged.lastDividend, merged['ÚLTIMO RENDIMENTO'], merged['Último rendimento'], merged['Média mensal 12 meses']),
    ...
    valorPatrimonialCota: firstClean(payload.valorPatrimonialCota, payload.valorPatrimonialPorCota, merged.valorPatrimonialCota, merged.valorPatrimonialPorCota, merged.vpCota, merged['VAL. PATRIMONIAL P/ COTA'], merged['Valor patrimonial por cota'], merged['Valor patrimonial por cota']),
    patrimonioLiquido: firstClean(payload.patrimonioLiquido, payload.valorPatrimonialTotal, merged.patrimonioLiquido, merged.valorPatrimonialTotal, merged.netWorth, merged['VALOR PATRIMONIAL'], merged['Valor patrimonial'], merged['Valor patrimonial'], merged['Patrimônio Líquido']),
    ...
    cotasEmitidas: firstClean(payload.cotasEmitidas, merged.cotasEmitidas, merged.issuedShares, merged['COTAS EMITIDAS'], merged['Cotas emitidas'], merged['Número de cotas']),
...
function buildFiiDetailItems(payload = {}, assetType = '') {
  ...
  collectFirstArrayFromPaths(payload, FII_PROPERTIES_PATHS).slice(0, 18).forEach((property, index) => {
    ...
    addFiiDetailItem(rows, seen, `property_${index + 1}_${norm(label).replace(/\s+/g, '_')}`, label, value, 'Lista de imóveis', property.source || 'Investidor10 lista de imóveis');
  });

  const distributionRows = dedupeRevenueRows(normalizeDistributionPoints(collectFirstArrayFromPaths(payload, FII_ASSET_DISTRIBUTION_PATHS), ['percentual', 'valuePercent', 'percent', 'percentage', 'value']).map(point => ({ label: point.label, percent: point.value, source: 'Investidor10 distribuição de ativos' })));
...
function buildFiiDetailCharts(payload = {}, assetType = '') {
  ...
  const income = analysisChart('fii_detail_monthly_income', 'Rendimento mensal do FII', 'bar', bundle.fiiDistribution12m || bundle.dividendMonthly || payload.distribuicoes12m || payload.results?.distribuicoes12m, 'Investidor10 rendimentos de FIIs', { valueKeys: ['amount', 'value', 'valor', 'rendimento', 'income', 'valuePerShare'], minPoints: 2, limit: 14, unit: 'R$' });
  ...
  const distribution = analysisChart('fii_detail_asset_distribution', 'Distribuição de ativos do FII', 'donut_composition', assetDistributionPoints, 'Investidor10 distribuição de ativos', { valueKeys: ['value'], minPoints: 1, limit: 12, unit: '%' });
...
function normalizeFiiPortfolioStateRows(rows = []) {
  ...
  for (const row of arrayFrom(rows)) {
    ...
    byState.set(key, { estado, quantidade: qty, source: row.source || 'Investidor10 lista de imóveis' });
  }
...
function buildFiiPortfolioCharts(payload = {}, assetType = '') {
  ...
  if (points.length) {
    charts.push({ id: 'fii_portfolio_by_state', title: 'Imóveis por estado', chartType: 'donut_composition', source: 'Investidor10 lista de imóveis', unit: '', series: [{ id: 'states', label: 'Estados', points }] });
  }
  return charts;
}
...
export function buildAnalysisPageResponse(payload = {}, input = {}) {
  ...
  const peerFundamentalComparator = buildPeerFundamentalComparator(payload, assetType);
  ...
  const coreSections = [
    ...
    ...(companyLike ? [section('peer_fundamental_comparator', 'Comparador de fundamentos do segmento', 'peer_comparator', peerFundamentalComparator.items, 'Investidor10 Comparador de Ativos e Empresas Relacionadas', { charts: peerFundamentalComparator.charts })] : []),
    ...
    ...(companyLike ? [section('financial_statements', 'DRE, Balanço e Fluxo de Caixa', 'table', statementItems, 'StatusInvest/Investidor10/Yahoo fundamentos normalizados pelo Proxy; gráficos somente Investidor10', { charts: statementCharts })] : []),
    ...
    section('asset_charts', 'Gráficos do Ativo', 'chart', chartPreviewItems(assetCharts), 'Investidor10 séries visuais estruturadas', { charts: assetCharts }),
    ...
    ...(companyLike ? [section('revenue_breakdown', 'Negócios e Regiões de Receita', 'chart', revenueBreakdown.items, 'Investidor10 negócios e regiões de receita normalizados pelo Proxy', { charts: revenueBreakdown.charts })] : []),
    section('comparisons', assetComparisonTitle(assetType), 'comparison', comparisons.items, 'Investidor10 comparadores estruturados quando disponíveis; gráficos externos filtrados', { charts: comparisons.charts }),
    ...
    ...(isFii ? [section('fii_portfolio', 'Portfólio do FII', 'fii_details', fiiPortfolio.items, 'Investidor10 imóveis e distribuição de ativos', { charts: fiiPortfolio.charts })] : []),
  ...
  const sections = finalizeAnalysisSections(rawSections);
  ...
  const dataQuality = buildDataQualitySummary(sourceCoverage, sections);
  ...
  return {
    ...
    sourcePolicy: 'Fundamentos, histórico, cadastro e proventos podem usar StatusInvest e Investidor10; gráficos visuais da Análise e dos modais são expostos somente quando a série vem do Investidor10; Yahoo/B3/BCB ficam restritos a dados numéricos auxiliares e índices quando disponíveis; sem dados sintéticos.',
    sources: [
      { id: 'statusinvest', role: 'structured_numbers_dividends_profile_when_available' },
      { id: 'investidor10', role: 'exclusive_visual_chart_source_indicator_history_revenue_breakdown_profile_when_available' },
      { id: 'yahoo', role: 'numeric_market_and_index_auxiliary_source_no_analysis_modal_charts' },
    ...
    diagnostics: {
      ...
      peerFundamentalComparatorItems: peerFundamentalComparator.items.length,
...
export const _test = { STOCK_INDICATORS, STOCK_HISTORICAL_INDICATORS, FII_HISTORICAL_INDICATORS, buildAnalysisPageResponse, buildSummaryItems, buildFundamentalItems, buildDividendSummaryItems, buildDividendHistoryItems, buildHistoricalIndicatorItems, buildHistoricalIndicatorCharts, buildFinancialStatementItems, buildFinancialStatementCharts, normalizeFinancialStatementRecords, buildMarketContextItems, buildValuationModelItems, buildSourceComparativeItemsAndCharts, buildPeerFundamentalComparator, buildIndicesEventsItems, buildGovernanceEventsItems, buildChecklistItems, buildOwnershipSection, buildFiiChecklistItems, buildDividendRadarItems, dividendEvents, buildSourceCoverageDiagnostics, buildDataQualitySummary, buildRevenueBreakdown, buildComparisonItemsAndCharts, buildFiiDetails, buildFiiAccounting, buildFiiPortfolio, buildAssetCharts, computeTwelveMonthVariation };
