...
function dividendEvents(payload = {}) {
  const candidatePaths = [
    ...
    'assetChartBundle.dividendEvents', 'assetChartsMobile.dividendEvents',
    'appMobileSnapshot.assetChartBundle.dividendEvents', 'appMobileSnapshot.assetChartsMobile.dividendEvents',
    'results.assetChartBundle.dividendEvents', 'results.assetChartsMobile.dividendEvents',
  ...
  for (const path of candidatePaths) {
    ...
    if (Array.isArray(candidate)) events.push(...candidate);
  }
  ...
  for (const event of events) {
    ...
    const paymentDate = cleanText(
      ...
      event.pgto || event.dataPgto || event.datePayment || event.date || event.data || event.referenceDate
    );
    const comDate = cleanText(
      event.dateCom || event.comDate || event.dataCom || event.data_com || event.dataBase || event.baseDate ||
      event.recordDate || event.eligibilityDate || event.exDate || event.dataEx || event.com
    );
...
function buildDividendRadarItems(payload = {}, assetType = '', ticker = '') {
  ...
  for (const event of events) {
    ...
    const paymentMonth = monthNumberFromDate(event.paymentDate || event.payDate || event.dataPagamento || event.pagamento || event.date || event.data || event.referenceDate);
...
function normalizeChecklistRows(value, source = 'Investidor10 checklist') {
  ...
  const walk = (node, fallback = '') => {
    ...
    const valueText = typeof statusRaw === 'boolean' ? (statusRaw ? 'Atende' : 'Não atende') : cleanText(statusRaw || node.result || node.resultado || node.description || node.descricao) || 'Não informado';
...
function hasAtLeastYearsOfEvents(events = [], years = 5) {
  const now = Date.now();
  const minTime = now - years * 366 * 24 * 60 * 60 * 1000;
  return events.some(event => {
    const t = Date.parse(event.paymentDate || event.comDate || event.date || '');
    return Number.isFinite(t) && t <= now && t <= minTime + 366 * 24 * 60 * 60 * 1000;
  });
}
...
function statusForExtractedChecklistRow(row = {}, criteria = [], payload = {}, assetType = '') {
  const raw = norm(row.value);
  if (/nao atende|não atende|reprov|false|pendente|invalido/.test(raw)) return 'Não atende';
  if (/atende|aprov|true|sim|ok|check/.test(raw)) return 'Atende';
  const key = norm(row.label);
...
function buildChecklistItems(payload = {}, assetType = '') {
  ...
  const criteriaRows = STOCK_CHECKLIST_CRITERIA.map(spec => {
    const matched = [...extractedByLabel.entries()].find(([key]) => key.includes(norm(spec.label).slice(0, 24)) || norm(spec.label).includes(key.slice(0, 24)))?.[1];
  ...
  const known = new Set(STOCK_CHECKLIST_CRITERIA.map(spec => norm(spec.label).slice(0, 24)));
  const extraRows = extracted.filter(row => {
    const key = norm(row.label);
    return ![...known].some(knownKey => key.includes(knownKey) || knownKey.includes(key.slice(0, 24)));
  });
...
function buildFiiChecklistItems(payload = {}, assetType = '') {
  ...
  const known = new Set(FII_CHECKLIST_CRITERIA.map(spec => norm(spec.label).slice(0, 24)));
  const extraRows = extracted.filter(row => {
    const key = norm(row.label);
    return ![...known].some(knownKey => key.includes(knownKey) || knownKey.includes(key.slice(0, 24)));
  });
...
export function buildAnalysisPageResponse(payload = {}, input = {}) {
  ...
  const coreSections = [
    ...
    ...(stockLike ? [section('dividend_radar', `Radar de Dividendos Inteligente para ${ticker}`, 'dividend_radar', dividendRadarItems, 'Investidor10/StatusInvest proventos normalizados')] : []),
