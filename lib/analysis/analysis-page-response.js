...
function multiSeriesChart(id, title, chartType, rows = [], source = 'VALORAE Proxy', seriesSpecs = [], options = {}) {
  const normalized = arrayFrom(rows).slice(-(options.limit || 14));
  const minPoints = options.minPoints || 2;
  ...
  const candidateSeries = seriesSpecs
    .map(spec => {
      ...
      const points = [...seen.values()].sort((a, b) => String(a.label).localeCompare(String(b.label), 'pt-BR', { numeric: true })).slice(-(options.pointLimit || 10));
  ...
  const commonPeriods = [...periodCounts.entries()]
...
function buildCompanyProfileItems(payload = {}, assetType = '') {
  ...
  const info = {
    ...merged,
    descricao: firstClean(payload.assetDescription, payload.description, payload.descricao, payload.profilePresentation?.summary, payload.assetPresentation?.summary, results.profilePresentation?.summary, results.assetPresentation?.summary, merged.summary, merged.descricao, merged.description, merged.sobre, merged.resumo),
