...
const STOCK_PROFILE_FIELDS = [
  ...
  { id: 'enterpriseValue', label: 'Valor da firma', group: 'Mercado', aliases: ['valorFirma', 'valor da firma', 'valor de firma', 'enterpriseValue', 'ev'] },
...
function firstMoneyExpression(value) {
  ...
  if (!raw) return '';
  const money = raw.match(/R\$\s*[\d.]+(?:,\d+)?(?:\s*(?:Trilh(?:ão|ões)|Bilh(?:ão|ões)|Milh(?:ão|ões)|Mil))?/i);
...
function profileDisplay(value, spec = {}) {
  ...
  if (['marketCap', 'equity', 'enterpriseValue', 'assets', 'grossDebt', 'netDebt', 'cash', 'dailyLiquidity'].includes(spec.id)) {
    const n = finiteValue(raw);
    if (n !== undefined) return `R$ ${compactDisplay(n)}`;
    const money = firstMoneyExpression(raw);
    return money || cleanText(raw);
  }
  if (spec.id === 'shares') {
    const n = finiteValue(raw);
