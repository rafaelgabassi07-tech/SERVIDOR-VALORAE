...
function buildAssetCharts(payload = {}) {
  const bundle = payload.assetChartBundle || payload.assetChartsMobile || payload.results?.assetChartBundle || payload.results?.assetChartsMobile || {};
  const charts = [];
  const price = analysisChart('price_history', 'Cotação histórica', 'line', bundle.priceHistory || payload.historicoPrecos || payload.results?.historicoPrecos, 'Yahoo Finance / StatusInvest', { valueKeys: ['close', 'price', 'value'], minPoints: 2, limit: 18, unit: 'R$' });
  if (price) charts.push(price);
  const dividends = analysisChart('dividend_history', 'Histórico de proventos', 'bar', bundle.dividendMonthly || bundle.fiiDistribution12m, 'StatusInvest proventos', { valueKeys: ['value', 'amount', 'total'], minPoints: 1, limit: 12, unit: 'R$' });
  ...
  const equity = analysisChart('equity_evolution', 'Evolução de patrimônio', 'bar', bundle.equityEvolution || bundle.balanceSheet || payload.statements?.balanceSheet || payload.results?.statements?.balanceSheet, 'StatusInvest/Yahoo fundamentals', { valueKeys: ['netWorth', 'equity', 'patrimonioLiquido', 'value'], minPoints: 1, limit: 12 });
