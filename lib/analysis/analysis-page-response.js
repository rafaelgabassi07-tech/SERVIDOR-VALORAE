...
function dateLabel(value = '') {
  ...
  if (!raw) return '';
  const yearMonth = raw.match(/^(\d{4})[-/.](\d{1,2})(?![-/.]\d)/);
...
function buildDividendHistoryItems(payload = {}) {
  return dividendEvents(payload).slice(0, 24).map((event, index) => {
    const reference = dateLabel(event.referenceDate || event.period || event.month);
    const payment = dateLabel(event.paymentDate) || reference;
    const com = dateLabel(event.comDate) || reference;
    ...
    return amount ? {
      ...
      label: payment ? `Pagamento ${payment}` : (reference ? `Competência ${reference}` : kind),
      value: amount,
      group: [kind, com ? `Data com ${com}` : '', reference ? `Competência ${reference}` : ''].filter(Boolean).join(' • '),
      source: cleanText(event.source || event.rawProvider || event.provider || 'StatusInvest proventos')
    } : null;
  }).filter(Boolean);
}
