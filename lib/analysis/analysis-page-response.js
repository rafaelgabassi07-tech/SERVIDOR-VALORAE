...
function dividendEvents(payload = {}) {
  ...
  for (const event of events) {
    ...
    const paymentDate = cleanText(
      ...
      event.period || event.periodo || event.competence || event.competencia || event.month || event.mes ||
      event.label || event.name || event.nome
    );
    const comDate = cleanText(
      ...
      event.dataAprovacao || event.dataDeclaracao || event.exDividendDate || event.dataExDividendo
    );
