...
const FII_MODAL_VERSION = '26.asset-modal.fii.v7';
...
function extractInvestidor10FiiPeerComparison(html = '', ticker = '') {
  ...
  return {
    id: 'fii_peer_comparison',
    title: 'Comparando com outros FIIs',
    subtitle: 'Tabela do Investidor10 filtrada por mesmo tipo e segmento do fundo.',
    filterLabel: 'Mesmo tipo e segmento',
    source: 'Investidor10',
...
async function fetchInvestidor10FiiBundle(ticker, timeoutMs = 6500) {
  ...
  const peerComparison = html ? extractInvestidor10FiiPeerComparison(html, ticker) : extractInvestidor10FiiPeerComparison('', ticker);
...
async function buildFiiYahooIndexComparison(ticker, timeoutMs = 8500) {
  ...
  return {
    id: 'fii_asset_vs_indices_full',
    title: `Comparação de ${ticker} com índices`,
    subtitle: 'Ativo x IFIX x CDI x IPCA x IBOV x SMLL x IDIV x IVVB11, no padrão do gráfico de Retorno.',
    ...
    sourcePolicy: 'Ativo, IFIX, SMLL, IDIV, IBOV e IVVB11 usam Yahoo Finance Chart API com símbolos diretos quando aplicável; IFIX/IDIV/SMLL permanecem sem Investidor10/B3/ETF/proxy. CDI e IPCA usam Banco Central SGS, seguindo a mesma família de benchmarks do gráfico de Retorno.'
  };
}
...
export async function buildFiiModalContract(payload = {}) {
  ...
  const [oneYearHistory, investidor10, comparison] = await Promise.all([
    ...
    fetchInvestidor10FiiBundle(ticker, Number(payload.investidor10TimeoutMs || 7500))
      .catch(error => ({ ok: false, items: [], sections: [], quickMetrics: {}, peerComparison: { id: 'fii_peer_comparison', title: 'Comparando com outros FIIs', subtitle: 'Mesmo tipo e segmento', filterLabel: 'Mesmo tipo e segmento', source: 'Investidor10', status: 'EMPTY', columns: [], rows: [] }, returnsRows: [], historicalIndicators: { columns: [], rows: [], status: 'EMPTY' }, error: error?.message || String(error), diagnostics: { found: false } })),
...
export const _test = { extractInvestidor10FiiInformation, extractInvestidor10FiiQuickMetrics, normalizeFiiHistoricalIndicatorsApi, returnsRowsFromInvestidor10, cleanInvestidor10InfoValue, normalizeComparisonPoints, comparisonItemFromSeries, firstHistoricalIndicatorValue, comparisonFetchPlans, extractInvestidor10FiiPeerComparison, parseFiiPeerRowChunk, FII_INDEX_BENCHMARKS, FII_COMPARISON_PERIODS, FII_MODAL_VERSION, normalizeAccumulatedBenchmarkPoints };
