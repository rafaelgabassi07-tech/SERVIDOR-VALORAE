...
const FII_MODAL_VERSION = '26.asset-modal.fii.v3';
...
function extractInvestidor10FiiQuickMetrics(html = '', ticker = '') {
  ...
  const start = plain.search(new RegExp(`${tk}\s+Cotação`, 'i'));
  const section = plain.slice(Math.max(0, start), start >= 0 ? start + 1200 : 2200);
  const pick = (labelPattern, untilPattern = '(?:GGRC11|P\/VP|Liquidez|VARIAÇÃO|COTAÇÃO|Resumo|$)') => {
    const re = new RegExp(`${labelPattern}\\s+([\\s\\S]*?)(?=${untilPattern})`, 'i');
    return cleanInvestidor10InfoValue(section.match(re)?.[1] || '');
  };
  const priceChunk = pick(`${tk}\\s+Cotação`, `${tk}\\s+DY|P\/VP|Liquidez|VARIAÇÃO|COTAÇÃO|$`);
  const priceMatch = priceChunk.match(/R\$\s*[\d.,]+/i)?.[0] || '';
  const changeMatch = priceChunk.replace(priceMatch, '').match(/[+-]?\s*\d{1,3}(?:[,.]\d+)?\s*%/)?.[0] || '';
  const dy = pick(`${tk}\\s+DY\\s*\\(12M\\)`, 'P\/VP|Liquidez|VARIAÇÃO|COTAÇÃO|$');
  const pvp = pick('P\/VP', 'Liquidez|VARIAÇÃO|COTAÇÃO|$');
  const liquidity = pick('Liquidez\\s+Diária', 'VARIAÇÃO|COTAÇÃO|$');
  const variation = pick('VARIA[ÇC][ÃA]O\\s*\\(12M\\)', 'COTAÇÃO|$');
  const out = {
...
export async function buildFiiModalContract(payload = {}) {
  ...
  const [oneYearHistory, investidor10] = await Promise.all([
  ...
  const variation12m = Number.isFinite(Number(quick.variation12mPercent)) ? round(Number(quick.variation12mPercent), 4) : null;
  const price = Number(quick.price);
  const dy12m = Number(quick.dy12m);
  const pvp = Number(quick.pvp);
  const dailyLiquidity = Number(quick.dailyLiquidity);
  ...
  const metrics = [
    { id: 'price', label: `${ticker} cotação`, value: quick.priceDisplay || '—', numericValue: Number.isFinite(price) ? round(price, 4) : null, source: 'Investidor10' },
    { id: 'dy12m', label: `${ticker} DY (12M)`, value: quick.dy12mDisplay || (Number.isFinite(dy12m) ? formatPercent(dy12m) : '—'), numericValue: Number.isFinite(dy12m) ? round(dy12m, 4) : null, source: 'Investidor10' },
    { id: 'pvp', label: 'P/VP', value: quick.pvpDisplay || (Number.isFinite(pvp) ? String(round(pvp, 2)).replace('.', ',') : '—'), numericValue: Number.isFinite(pvp) ? round(pvp, 4) : null, source: 'Investidor10' },
    { id: 'daily_liquidity', label: 'Liquidez diária', value: quick.dailyLiquidityDisplay || (Number.isFinite(dailyLiquidity) ? formatCompactMoney(dailyLiquidity) : '—'), numericValue: Number.isFinite(dailyLiquidity) ? round(dailyLiquidity, 2) : null, source: 'Investidor10' },
    { id: 'variation_12m', label: 'Variação (12M)', value: quick.variation12mDisplay || (variation12m === null ? '—' : formatPercent(variation12m, false)), numericValue: variation12m, source: 'Investidor10' }
  ];

  const now = new Date().toISOString();
  const hasInvestidor10Data = Boolean(investidor10?.items?.length || investidor10?.returnsRows?.length || investidor10?.historicalIndicators?.rows?.length || metrics.some(m => m.source === 'Investidor10' && m.value !== '—'));
  ...
  return {
    ...
    name,
    updatedAt: now,
    sourcePolicy: 'Modal único com StatusInvest, Fundamentus, BCB e fallbacks legados descartados. Para FIIs, cards rápidos, rentabilidade, informações cadastrais e histórico de indicadores fundamentalistas vêm do Investidor10. A única exceção preservada é o motor de cotação em tempo real via Yahoo, usado apenas para a série do gráfico e sem alimentar fundamentos ou informações.',
    sources: [
    ...
    quoteSummary: {
      ...
      priceDisplay: quick.priceDisplay || '—',
      changePercent: Number.isFinite(Number(quick.changePercent)) ? round(Number(quick.changePercent), 4) : null,
      changeDisplay: quick.changeDisplay || '',
...
export const _test = { extractInvestidor10FiiInformation, extractInvestidor10FiiQuickMetrics, normalizeFiiHistoricalIndicatorsApi, returnsRowsFromInvestidor10, cleanInvestidor10InfoValue };
