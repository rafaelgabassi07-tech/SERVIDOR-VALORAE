...
async function fetchInvestidor10FiiBundle(ticker, timeoutMs = 6500, options = {}) {
  ...
  const [historicalFetch, vacancyFetch, announcementsFetch] = fastMode
    ? [
...
async function buildFiiModalContractFresh(payload = {}) {
  ...
  const timeoutMs = fiiStageTimeoutMs(payload, fastMode ? 3800 : 8500);
  const [oneYearHistory, investidor10, comparison, yahooLogo] = await Promise.all([
    fetchYahooHistory(ticker, { range: fastMode ? '1M' : (payload.range || '1Y'), interval: payload.interval || '1d', timeoutMs: Math.min(fastMode ? 2400 : timeoutMs, timeoutMs), limit: Number(payload.limit || (fastMode ? 80 : 260)) })
      .catch(error => ({ ok: false, points: [], error: error?.message || String(error), source: 'YahooChart' })),
    fetchInvestidor10FiiBundle(ticker, Math.min(Number(payload.investidor10TimeoutMs || payload.fundamentalTimeoutMs || timeoutMs || 7200), fastMode ? 2600 : 6500), { mode: stage })
