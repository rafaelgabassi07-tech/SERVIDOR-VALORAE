...
function extractFiiDividendChartsFromHtml(html = '', ticker = '', { referencePrice = null, quickMetrics = {}, distributions12m = null } = {}) {
  ...
  if (!monthlyYield.length && monthlyDividends.length) {
    const refFrom12mDy = finiteNumberOrNull(total12m) !== null && finiteNumberOrNull(currentDy) !== null && Number(currentDy) > 0
...
async function buildFiiModalContractFresh(payload = {}) {
  ...
  const fiiBundlePromise = fetchInvestidor10FiiBundle(
    ticker,
    Math.min(Number(payload.investidor10TimeoutMs || payload.fundamentalTimeoutMs || timeoutMs || 7200), fastMode ? 2600 : 6500),
    { mode: stage }
