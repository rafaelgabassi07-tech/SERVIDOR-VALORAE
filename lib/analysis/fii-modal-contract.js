...
function extractInvestidor10FiiChecklistMarkup(html = '', ticker = '') {
  const source = String(html || '');
  const sectionStart = source.search(/\bid=["']checklist["']/i);
  if (sectionStart < 0) return { items: [], section: '' };
  const section = source.slice(sectionStart, sectionStart + 30000);
  const starts = [...section.matchAll(/<div\b[^>]*class=["'][^"']*\bchecklist-item\b[^"']*["'][^>]*>/gi)];
...
function ensureFiiBuyHoldChecklist({ checklist = null, ticker = '', html = '', quickMetrics = {}, infoItems = [], propertyPortfolio = null, vacancyHistory = null, patrimonialInfo = null, dividendCharts = null } = {}) {
  ...
  const genericVacancy = parseBrNumber(firstInfoValue(infoItems, ['vacancia']));
  ...
  const items = calculatedItems.map(calculated => {
    const direct = directItems.get(calculated.id);
    const preferred = direct && (direct.passed === true || direct.passed === false) ? {
      ...calculated,
      ...direct,
      id: calculated.id,
      label: fiiChecklistCriterion(calculated.id).label,
      source: direct.source || 'Investidor10 checklist buy and hold',
      dataNature: direct.dataNature || 'DIRECT',
      calculated: false
    } : calculated;
    return {
      ...preferred,
  ...
  return {
    ...
    subtitle: 'Critérios públicos e cálculos do VALORAE sobre métricas reais do Investidor10; itens sem evidência permanecem como não informados.',
    ...
    diagnostics: {
      ...
      reason: checklist?.diagnostics?.reason || 'canonical_criteria_merged_with_modal_metrics',
      ...
      hasVacancy: Number.isFinite(physicalVacancy)
    }
  };
}
