...
import { fetchYahooHistory, fetchYahooQuote, getCdiAccumulatedSeries, getIpcaSeries, fetchInvestidor10DirectIndexHistory } from '../sources/adapters/index.js';
...
import { FII_MODAL_CRITICAL_SECTIONS, FII_MODAL_RECOVERABLE_SECTIONS } from './asset-modal-sections.js';

const FII_MODAL_VERSION = '26.asset-modal.fii.v27-contract-batching';


const fiiIdResolutionCache = new Map();
const FII_ID_CACHE_TTL_MS = 6 * 60 * 60 * 1000;
const FII_REQUIRED_DEEP_SECTIONS = FII_MODAL_CRITICAL_SECTIONS;
const FII_RECOVERABLE_SECTIONS = FII_MODAL_RECOVERABLE_SECTIONS;
...
function fiiSectionRecoveryTargets(payload = {}) {
  ...
  const explicitlyMissing = [
    ...parseFiiSectionList(payload.requestedRecoverySections),
...
const FII_INDEX_BENCHMARKS = Object.freeze([
  { key: 'ifix', code: 'IFIX', label: 'IFIX', ticker: 'IFIX', yahooSymbol: 'IFIX.SA', yahooSymbols: ['IFIX.SA', '^IFIX'], kind: 'official_return_index', sourceLabel: 'Investidor10 API direta do índice IFIX + contingência Yahoo/B3' },
  ...
  { key: 'ibov', code: 'IBOV', label: 'IBOV', ticker: 'IBOV', yahooSymbol: '^BVSP', kind: 'official_return_index', sourceLabel: 'Investidor10 API direta do índice IBOV + contingência Yahoo/B3' },
  { key: 'smll', code: 'SMLL', label: 'SMLL', ticker: 'SMLL', yahooSymbol: 'SMLL.SA', yahooSymbols: ['SMLL.SA', '^SMLL'], kind: 'official_return_index', sourceLabel: 'Investidor10 API direta do índice SMLL + contingência Yahoo/B3' },
  { key: 'idiv', code: 'IDIV', label: 'IDIV', ticker: 'IDIV', yahooSymbol: 'IDIV.SA', yahooSymbols: ['IDIV.SA', '^IDIV'], kind: 'official_return_index', sourceLabel: 'Investidor10 API direta do índice IDIV + contingência Yahoo/B3' },
...
const FII_BUY_HOLD_CHECKLIST_CRITERIA = Object.freeze([
  ...
  },
  {
    ...
    help: 'Mede a consistência média de distribuição nos 24 meses mais recentes.'
  },
  {
  ...
  },
  {
    ...
    help: 'Critério de escala patrimonial mínima do fundo.'
  },
  {
  ...
  },
  {
    ...
    help: 'Avalia perda financeira recente por vacância ou inadimplência.'
  }
]);
...
function explicitFiiCheckboxStateNearWindowCenter(rawWindow = '') {
...
function detectChecklistPassed(section = '', labelStart = 0, labelLength = 0, rawHtmlWindow = '') {
  const checkboxState = explicitFiiCheckboxStateNearWindowCenter(rawHtmlWindow);
  ...
  const combinedRawWindow = `${rawHtmlWindow || ''} ${rawWindow}`;
  const window = normalizeLooseText(combinedRawWindow);
  const hasExplicitNegative = /\b(?:nao atende|não atende|reprovado|reprovada|falhou|negativo|cancel|close|unchecked|uncheck|xmark|times|false|disabled)\b/.test(window)
    || /(?:class|data-[a-z-]+|aria-label|title)\s*=\s*["'][^"']*(?:unchecked|uncheck|fail|failed|false|negative|danger|muted|gray|grey|times|xmark|close|cancel)[^"']*["']/i.test(combinedRawWindow);
  ...
  const hasExplicitPositive = /\b(?:check|check circle|check_circle|checked|check box|fa check|icon check|material icons check|is checked|status true|aprovado|aprovada|atende|positivo|success|done|true|sim)\b/.test(window)
    || /(?:class|data-[a-z-]+|aria-label|title)\s*=\s*["'][^"']*(?:checked|check-circle|check_circle|checkmark|check-mark|fa-check|icon-check|success|positive|passed|approved|true)[^"']*["']/i.test(combinedRawWindow)
    || /✓|✔|☑/.test(combinedRawWindow);
...
function extractInvestidor10FiiChecklistMarkup(html = '', ticker = '') {
  const source = String(html || '');
  ...
  const section = source.slice(sectionStart, sectionStart + 30000);
  ...
  for (const itemHtml of blocks) {
    ...
    const blockText = cleanInvestidor10InfoValue(labelHtml || htmlToPlainText(itemHtml));
    const normalizedLabel = normalizeLooseText(blockText);
    const criterion = FII_BUY_HOLD_CHECKLIST_CRITERIA.find(candidate =>
    ...
    else passed = detectChecklistPassed(itemHtml, Math.max(0, normalizeLooseText(itemHtml).indexOf(normalizeLooseText(criterion.label))), criterion.label.length);
...
function extractInvestidor10FiiBuyHoldChecklist(html = '', ticker = '') {
  const directMarkup = extractInvestidor10FiiChecklistMarkup(html, ticker);
  if (directMarkup.items.length) {
    const items = directMarkup.items;
    const passedCount = items.filter(item => item.passed === true).length;
    const failedCount = items.filter(item => item.passed === false).length;
    return {
      id: 'fii_buy_hold_checklist',
      title: ticker ? `Checklist do Investidor Buy and Hold sobre ${ticker}` : 'Checklist do Investidor Buy and Hold',
      subtitle: 'Critérios e marcações públicas lidos diretamente dos checkboxes do Investidor10.',
      status: 'OK',
      source: 'Investidor10',
      sourceUrl: ticker ? `https://investidor10.com.br/fiis/${ticker.toLowerCase()}/` : undefined,
      total: items.length,
      ...
      unknown: 0,
      items,
      disclaimer: extractChecklistDisclaimer(htmlToPlainText(directMarkup.section)) || 'Esta ferramenta de checklist é fornecida apenas para fins informativos e não constitui recomendação de investimento. A pontuação baseia-se em parâmetros de mercado, mas não garante resultados futuros.',
      diagnostics: {
        found: true,
        ...
        calculatedCount: 0,
        unknownCount: 0,
        policy: 'public_checkbox_attribute_checked_is_source_of_truth'
      }
    };
  }
  const plain = htmlToPlainText(html);
  ...
  const rawSectionStart = rawSource.search(/(?:\bid=["']checklist["']|\bclass=["'][^"']*\bchecklist(?:-do-investidor|-investidor|-buy-hold)?\b[^"']*["']|CHECKLIST\s+DO\s+INVESTIDOR)/i);
  const rawHtmlSection = rawSectionStart >= 0 ? rawSource.slice(rawSectionStart, rawSectionStart + 30000) : rawSource.slice(0, 30000);
  const matches = [];
  const seen = new Set();
  for (const criterion of FII_BUY_HOLD_CHECKLIST_CRITERIA) {
    ...
    seen.add(criterion.id);
    const rawNeedle = bestText.split(/\s+/).filter(Boolean).slice(0, 5).join(' ');
    const rawIndex = rawNeedle ? rawHtmlSection.toLowerCase().indexOf(rawNeedle.toLowerCase()) : -1;
    const rawHtmlWindow = rawIndex >= 0
      ? rawHtmlSection.slice(Math.max(0, rawIndex - 900), Math.min(rawHtmlSection.length, rawIndex + bestText.length + 1200))
      : '';
    const passed = detectChecklistPassed(section, bestIndex, bestText.length, rawHtmlWindow);
  ...
  const items = matches.map(({ criterion, passed }) => ({
    ...
    label: criterion.label,
    passed,
    status: passed === true ? 'PASSED' : (passed === false ? 'FAILED' : 'SOURCE_PENDING'),
    statusLabel: passed === true ? 'Atende' : (passed === false ? 'Não atende' : 'Em apuração'),
    help: criterion.help,
    source: 'Investidor10 checklist buy and hold',
    evidence: passed === true || passed === false ? 'marcação explícita no HTML' : 'marcação explícita ausente',
    dataNature: passed === true || passed === false ? 'DIRECT' : 'SOURCE_PENDING',
    calculated: false,
    sourceUrl: ticker ? `https://investidor10.com.br/fiis/${ticker.toLowerCase()}/` : undefined
  }));
  const passedCount = items.filter(item => item.passed === true).length;
...
function checklistItemFromMetric({ criterion, value = null, passed = null, valueDisplay = '', source = 'Investidor10 + métricas do modal' } = {}) {
  ...
  const display = valueDisplay || (finiteNumberOrNull(value) !== null ? String(value) : '—');
  return {
    id: criterion.id,
    label: criterion.label,
    ...
    help: criterion.help,
    source: known ? source : 'Investidor10 — dado aguardando atualização',
    evidence: known
      ? `Critério calculado com a métrica pública ${display}.`
      : 'A linha foi preservada no checklist, mas a métrica necessária ainda não chegou na resposta da fonte.',
    dataNature: known ? 'CALCULATED' : 'SOURCE_PENDING',
    calculated: known,
    sourceUrl: undefined
  };
}
...
function fiiListingEvidenceFromSources(html = '', dividendCharts = null) {
  ...
  for (const match of plain.matchAll(/(?:constitu[ií]do|in[ií]cio|listad[oa]|criad[oa]|fundad[oa])\D{0,60}\b(19\d{2}|20\d{2})\b/gi)) {
    ...
    if (Number.isFinite(year) && year >= 1990 && year <= currentYear) explicitYears.push(year);
  }
  const eventYears = (Array.isArray(dividendCharts?.events) ? dividendCharts.events : [])
    ...
    .filter(year => Number.isFinite(year) && year >= 1990 && year <= currentYear);
  const explicitListedYear = explicitYears.length ? Math.min(...explicitYears) : null;
  const firstDividendYear = eventYears.length ? Math.min(...eventYears) : null;
  if (Number.isFinite(explicitListedYear)) {
    const yearsListed = currentYear - explicitListedYear;
    return {
      explicitListedYear,
      firstDividendYear,
      referenceYear: explicitListedYear,
      yearsListed,
      passed: yearsListed >= 5,
      basis: 'explicit_page_year',
      source: 'Investidor10 dados do fundo',
      evidence: `Ano de constituição/listagem ${explicitListedYear}; ${yearsListed} anos de histórico.`
    };
  }
  if (Number.isFinite(firstDividendYear) && currentYear - firstDividendYear >= 5) {
    const yearsObserved = currentYear - firstDividendYear;
    return {
      explicitListedYear: null,
      firstDividendYear,
      referenceYear: firstDividendYear,
      yearsListed: yearsObserved,
      passed: true,
      basis: 'dividend_history_lower_bound',
      source: 'Investidor10 histórico de rendimentos',
      evidence: `Há eventos públicos desde ${firstDividendYear}, comprovando ao menos ${yearsObserved} anos de histórico.`
    };
  }
  return {
    explicitListedYear: null,
    firstDividendYear,
    referenceYear: firstDividendYear,
    yearsListed: Number.isFinite(firstDividendYear) ? currentYear - firstDividendYear : null,
    passed: null,
    basis: Number.isFinite(firstDividendYear) ? 'short_dividend_history_is_not_failure' : 'source_pending',
    source: 'Investidor10 dados do fundo',
    evidence: Number.isFinite(firstDividendYear)
      ? `O histórico disponível começa em ${firstDividendYear}, mas uma janela curta não comprova que o fundo tenha menos de 5 anos.`
      : 'Ano de constituição/listagem ainda não retornado pela fonte.'
  };
}

function firstListedYearFromHtml(html = '', dividendCharts = null) {
  return fiiListingEvidenceFromSources(html, dividendCharts).referenceYear;
}
...
function namedFiiVacancyPercent({ html = '', infoItems = [], kind = 'physical' } = {}) {
  const wanted = kind === 'financial' ? /financeir/ : /f[ií]sic/;
  const item = (Array.isArray(infoItems) ? infoItems : []).find(row => {
    const identity = normalizeLooseText([row?.id, row?.label, row?.title].filter(Boolean).join(' '));
    return /vacancia/.test(identity) && wanted.test(identity);
  });
  const itemValue = parseBrNumber(item?.numericValue ?? item?.value ?? item?.displayValue ?? '');
  if (Number.isFinite(itemValue) && itemValue >= 0 && itemValue <= 100) return itemValue;

  const raw = String(html || '');
  const jsonNames = kind === 'financial'
    ? ['financialVacancy', 'financial_vacancy', 'vacanciaFinanceira', 'vacancia_financeira']
    : ['physicalVacancy', 'physical_vacancy', 'vacanciaFisica', 'vacancia_fisica'];
  ...
  const plain = htmlToPlainText(raw).replace(/\s+/g, ' ');
  const label = kind === 'financial' ? 'financeir[ao]' : 'f[ií]sic[ao]';
  for (const match of plain.matchAll(new RegExp(`vac[aâ]ncia\\s+${label}([^%]{0,70}?)([0-9]{1,3}(?:[.,][0-9]+)?)\\s*%`, 'gi'))) {
    const context = normalizeLooseText(match[1] || '');
    if (/abaixo|acima|criterio|ultimos 12 meses/.test(context)) continue;
    const value = parseBrNumber(match[2]);
    if (Number.isFinite(value) && value >= 0 && value <= 100) return value;
  }
  return null;
}
...
function monthlyYieldPeriodKey(point = {}) {
  const rawPeriod = String(point?.period || point?.date || '').trim();
  const year = Number(point?.year || rawPeriod.match(/^(19\d{2}|20\d{2})/)?.[1]);
  const month = Number(point?.month || rawPeriod.match(/^(?:19\d{2}|20\d{2})[-/](\d{1,2})/)?.[1]);
  if (!Number.isFinite(year) || !Number.isFinite(month) || month < 1 || month > 12) return '';
  return `${String(year).padStart(4, '0')}-${String(month).padStart(2, '0')}`;
}

function averageRecentFiiYieldMonths(dividendCharts = null, months = 24) {
  const rows = Array.isArray(dividendCharts?.yieldSeriesByFrequency?.monthly)
    ? dividendCharts.yieldSeriesByFrequency.monthly
    : [];
  const byPeriod = new Map();
  rows.forEach(point => {
    const value = finiteNumberOrNull(point?.yieldPercent ?? point?.value);
    const period = monthlyYieldPeriodKey(point);
    if (value !== null && value >= 0 && period) byPeriod.set(period, value);
  });
  const requiredMonths = Math.max(1, Number(months) || 24);
  const normalized = [...byPeriod.entries()]
    .map(([period, value]) => ({ period, value }))
    .sort((a, b) => a.period.localeCompare(b.period));
  // Não decide um critério de 24 meses com uma janela curta ou duplicada.
  if (normalized.length < requiredMonths) return null;
  const recent = normalized.slice(-requiredMonths);
  return recent.reduce((sum, point) => sum + point.value, 0) / recent.length;
}

function canonicalChecklistCriterionId(item = {}) {
  const rawId = String(item?.id || '').trim();
  if (FII_BUY_HOLD_CHECKLIST_CRITERIA.some(criterion => criterion.id === rawId)) return rawId;
  const identity = normalizeLooseText([rawId, item?.label, item?.title, item?.criterion].filter(Boolean).join(' '));
  if (!identity) return '';
  for (const criterion of FII_BUY_HOLD_CHECKLIST_CRITERIA) {
    const aliases = [criterion.label, ...(criterion.variants || [])].map(normalizeLooseText);
    if (aliases.some(alias => identity === alias || identity.includes(alias) || alias.includes(identity))) return criterion.id;
  }
  if (/listad|bolsa/.test(identity) && /5|cinco/.test(identity)) return 'listed_5y';
  if (/dividend yield|\bdy\b/.test(identity) && /24/.test(identity) && /9/.test(identity)) return 'dy_24m_above_9';
  if (/liquidez/.test(identity) && /1 milhao|1000000/.test(identity)) return 'daily_liquidity_1m';
  if (/cotistas/.test(identity) && /20 mil|20000/.test(identity)) return 'shareholders_20k';
  if (/patrimonio liquido/.test(identity) && /500 milhoes|500000000/.test(identity)) return 'equity_500m';
  if (/imoveis|propriedades/.test(identity) && /5|cinco/.test(identity)) return 'properties_5_plus';
  if (/vacancia fisica/.test(identity) && /10/.test(identity)) return 'physical_vacancy_below_10';
  if (/vacancia financeira/.test(identity) && /10/.test(identity)) return 'financial_vacancy_below_10';
  return '';
}

function canonicalChecklistDirectItems(checklist = null) {
  const mapped = new Map();
  (Array.isArray(checklist?.items) ? checklist.items : []).forEach(item => {
    const id = canonicalChecklistCriterionId(item);
    if (id && !mapped.has(id)) mapped.set(id, { ...item, id });
  });
  return mapped;
}
...
function ensureFiiBuyHoldChecklist({ checklist = null, ticker = '', html = '', quickMetrics = {}, infoItems = [], propertyPortfolio = null, vacancyHistory = null, patrimonialInfo = null, dividendCharts = null } = {}) {
  const symbol = String(ticker || '').toUpperCase();
  const directItems = canonicalChecklistDirectItems(checklist);
  const listingEvidence = fiiListingEvidenceFromSources(html, dividendCharts);
  const listedYear = listingEvidence.referenceYear;
  const yearsListed = listingEvidence.yearsListed;
  const avgDy24m = averageRecentFiiYieldMonths(dividendCharts, 24);

  const infoNumber = (ids = [], parser = parseBrNumber) => {
    for (const id of ids) {
      const parsed = parser(firstInfoValue(infoItems, [id]));
      if (Number.isFinite(parsed)) return parsed;
    }
    return null;
  };
  const metricNumber = (aliases = [], parser = parseBrNumber) => {
    const patterns = aliases.map(normalizeLooseText).filter(Boolean);
    for (const metric of Array.isArray(patrimonialInfo?.metrics) ? patrimonialInfo.metrics : []) {
      const identity = normalizeLooseText([metric?.id, metric?.label, metric?.title].filter(Boolean).join(' '));
      if (!patterns.some(pattern => identity.includes(pattern) || pattern.includes(identity))) continue;
      const parsed = parser(metric?.numericValue ?? metric?.value ?? metric?.valueDisplay ?? metric?.displayValue ?? '');
      if (Number.isFinite(parsed)) return parsed;
    }
    return null;
  };
  const quickNumber = (keys = [], parser = Number) => {
    for (const key of keys) {
      const raw = quickMetrics?.[key];
  ...
  const dailyLiquidity = quickNumber(['dailyLiquidity', 'liquidezDiaria', 'averageDailyLiquidity'], Number)
    ?? infoNumber(['liquidez_media_diaria', 'liquidez_diaria', 'liquidez'], parseMoneyScale)
    ?? metricNumber(['liquidez diaria', 'liquidez media diaria'], parseMoneyScale);
  const shareholders = infoNumber(['numero_cotistas', 'cotistas', 'numero_de_cotistas'], parseCountScale)
    ?? quickNumber(['shareholders', 'shareholdersCount', 'cotistas'], parseCountScale)
    ?? metricNumber(['numero de cotistas', 'cotistas'], parseCountScale);
  const equity = infoNumber(['valor_patrimonial', 'patrimonio_liquido', 'patrimonio'], parseMoneyScale)
    ?? quickNumber(['equity', 'netAssetValue', 'patrimonialValue'], parseMoneyScale)
    ?? metricNumber(['patrimonio liquido', 'valor patrimonial', 'patrimonio'], parseMoneyScale);
  const properties = countFiiProperties(propertyPortfolio)
    ?? infoNumber(['numero_imoveis', 'quantidade_imoveis', 'imoveis'], parseCountScale)
    ?? metricNumber(['numero de imoveis', 'quantidade de imoveis', 'imoveis'], parseCountScale);
  const genericVacancy = infoNumber(['vacancia', 'taxa_vacancia'], parseBrNumber)
    ?? quickNumber(['vacancy', 'vacancia'], parseBrNumber);
  const physicalVacancy = namedFiiVacancyPercent({ html, infoItems, kind: 'physical' })
    ?? metricNumber(['vacancia fisica'], parseBrNumber)
    ?? averageVacancyPercent(vacancyHistory)
    ?? (Number.isFinite(genericVacancy) ? genericVacancy : null);
  const financialVacancy = namedFiiVacancyPercent({ html, infoItems, kind: 'financial' })
    ?? metricNumber(['vacancia financeira'], parseBrNumber)
    ?? quickNumber(['financialVacancy', 'vacanciaFinanceira'], parseBrNumber);

  const source = 'VALORAE — cálculo sobre dados públicos do Investidor10';
  const listedCriterionItem = checklistItemFromMetric({
    criterion: fiiChecklistCriterion('listed_5y'),
    value: Number.isFinite(yearsListed) ? yearsListed : null,
    valueDisplay: Number.isFinite(yearsListed) ? `${yearsListed} anos observados` : '—',
    passed: listingEvidence.passed,
    source: listingEvidence.source
  });
  listedCriterionItem.evidence = listingEvidence.evidence;
  listedCriterionItem.source = listingEvidence.source;
  const calculatedById = new Map([
    listedCriterionItem,
    checklistItemFromMetric({ criterion: fiiChecklistCriterion('dy_24m_above_9'), value: Number.isFinite(avgDy24m) ? round(avgDy24m, 4) : null, valueDisplay: Number.isFinite(avgDy24m) ? formatPercent(avgDy24m, false) : '—', passed: Number.isFinite(avgDy24m) ? avgDy24m >= 9 : null, source }),
    checklistItemFromMetric({ criterion: fiiChecklistCriterion('daily_liquidity_1m'), value: Number.isFinite(dailyLiquidity) ? round(dailyLiquidity, 2) : null, valueDisplay: Number.isFinite(dailyLiquidity) ? formatCompactMoney(dailyLiquidity) : '—', passed: Number.isFinite(dailyLiquidity) ? dailyLiquidity >= 1_000_000 : null, source }),
    ...
    checklistItemFromMetric({ criterion: fiiChecklistCriterion('equity_500m'), value: Number.isFinite(equity) ? round(equity, 2) : null, valueDisplay: Number.isFinite(equity) ? formatCompactMoney(equity) : '—', passed: Number.isFinite(equity) ? equity >= 500_000_000 : null, source }),
    checklistItemFromMetric({ criterion: fiiChecklistCriterion('properties_5_plus'), value: Number.isFinite(properties) ? Math.round(properties) : null, valueDisplay: Number.isFinite(properties) ? `${Math.round(properties)} imóveis` : '—', passed: Number.isFinite(properties) ? properties >= 5 : null, source }),
    checklistItemFromMetric({ criterion: fiiChecklistCriterion('physical_vacancy_below_10'), value: Number.isFinite(physicalVacancy) ? round(physicalVacancy, 4) : null, valueDisplay: Number.isFinite(physicalVacancy) ? formatPercent(physicalVacancy, false) : '—', passed: Number.isFinite(physicalVacancy) ? physicalVacancy < 10 : null, source }),
    checklistItemFromMetric({ criterion: fiiChecklistCriterion('financial_vacancy_below_10'), value: Number.isFinite(financialVacancy) ? round(financialVacancy, 4) : null, valueDisplay: Number.isFinite(financialVacancy) ? formatPercent(financialVacancy, false) : '—', passed: Number.isFinite(financialVacancy) ? financialVacancy < 10 : null, source })
  ].map(item => [item.id, item]));

  const items = FII_BUY_HOLD_CHECKLIST_CRITERIA.map(criterion => {
    const calculated = calculatedById.get(criterion.id) || checklistItemFromMetric({ criterion });
    const direct = directItems.get(criterion.id);
    const directResolved = direct && (direct.passed === true || direct.passed === false);
    const merged = directResolved ? {
      ...calculated,
      ...direct,
      id: criterion.id,
      label: criterion.label,
      help: direct.help || criterion.help,
      status: direct.passed ? 'PASSED' : 'FAILED',
      statusLabel: direct.passed ? 'Atende' : 'Não atende',
      source: direct.source || 'Investidor10 checklist buy and hold',
      evidence: direct.evidence || 'Estado lido diretamente da marcação pública da fonte.',
      dataNature: 'DIRECT',
      calculated: false
    } : calculated;
    return {
  ...
  const passedCount = items.filter(item => item.passed === true).length;
  const failedCount = items.filter(item => item.passed === false).length;
  const pendingCount = items.length - passedCount - failedCount;
  const fullyResolved = pendingCount === 0 && items.length === FII_BUY_HOLD_CHECKLIST_CRITERIA.length;
  return {
    id: 'fii_buy_hold_checklist',
    title: symbol ? `Checklist do Investidor Buy and Hold sobre ${symbol}` : 'Checklist do Investidor Buy and Hold',
    subtitle: fullyResolved
      ? 'Todos os critérios foram preenchidos com marcações ou métricas públicas da fonte.'
      : 'Todos os critérios estão presentes; as linhas ainda pendentes serão atualizadas assim que a métrica correspondente chegar da fonte.',
    status: fullyResolved ? 'OK' : (passedCount + failedCount > 0 ? 'PARTIAL' : 'SOURCE_PENDING'),
    source: directItems.size ? 'Investidor10 + VALORAE' : source,
    sourceUrl: symbol ? `https://investidor10.com.br/fiis/${symbol.toLowerCase()}/` : undefined,
    total: items.length,
    passed: passedCount,
    failed: failedCount,
    ...
    items,
    disclaimer: checklist?.disclaimer || 'Esta leitura é informativa, independente da carteira do usuário, e não constitui recomendação de investimento.',
    diagnostics: {
      ...(checklist?.diagnostics || {}),
      reason: checklist?.diagnostics?.reason || 'canonical_criteria_merged_with_all_available_source_metrics',
      portfolioIndependent: true,
      fullyResolved,
      expectedCriteria: FII_BUY_HOLD_CHECKLIST_CRITERIA.length,
      deliveredCriteria: items.length,
      derivation: directItems.size ? 'canonical_direct_items_plus_calculated_gaps' : 'calculated_by_valorae_from_investidor10_metrics',
      dataNature: directItems.size ? 'MIXED' : 'CALCULATED',
      directCount: items.filter(item => item.dataNature === 'DIRECT').length,
      calculatedCount: items.filter(item => item.dataNature === 'CALCULATED').length,
      pendingCount,
      missingCriterionIds: items.filter(item => item.passed == null).map(item => item.id),
      listedYear,
      yearsListed,
      listingEvidenceBasis: listingEvidence.basis,
      explicitListedYear: listingEvidence.explicitListedYear,
      firstDividendYear: listingEvidence.firstDividendYear,
      hasAverageDy24m: Number.isFinite(avgDy24m),
      averageDy24mSampleCount: Math.min(24, Array.isArray(dividendCharts?.yieldSeriesByFrequency?.monthly) ? dividendCharts.yieldSeriesByFrequency.monthly.length : 0),
      ...
      hasPhysicalVacancy: Number.isFinite(physicalVacancy),
      hasFinancialVacancy: Number.isFinite(financialVacancy)
    }
  };
}
...
function normalizeInvestidor10FiiPeerApi(raw = null, ticker = '') {
  const symbol = String(ticker || '').toUpperCase();
  ...
  for (const candidate of candidates) {
    ...
    const peerTicker = String(candidate.title || candidate.ticker || candidate.symbol || candidate.code || '')
      .toUpperCase()
      .match(/\b[A-Z]{4}\d{2}\b/)?.[0] || '';
    if (!peerTicker || seen.has(peerTicker)) continue;
    seen.add(peerTicker);
    const dividendYield = finiteNumberOrNull(candidate.dividend_yield ?? candidate.dividendYield ?? candidate.dy);
    const pvp = finiteNumberOrNull(candidate.p_vp ?? candidate.pvp ?? candidate.priceToBook);
    const patrimonialValue = finiteNumberOrNull(candidate.net_worth ?? candidate.netWorth ?? candidate.patrimonial_value ?? candidate.patrimonialValue);
    const fundType = normalizePeerTableText(candidate.type || candidate.fund_type || candidate.fundType || '');
    const segment = normalizePeerTableText(candidate.segment || candidate.segmento || '');
    rows.push({
      ticker: peerTicker,
      isReference: peerTicker === symbol,
      dividendYield,
      dividendYieldDisplay: dividendYield === null ? '—' : formatPercent(dividendYield, false),
      pvp,
      pvpDisplay: pvp === null ? '—' : Number(pvp).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }),
      patrimonialValue,
      patrimonialValueDisplay: patrimonialValue === null ? '—' : formatCompactMoney(patrimonialValue),
      fundType: fundType || '—',
      segment: segment || '—',
      source: 'Investidor10 API comparador de FIIs',
      sourceUrl: peerTicker ? `https://investidor10.com.br/fiis/${peerTicker.toLowerCase()}/` : undefined
    });
  }
  return rows;
}

function extractInvestidor10FiiPeerApiUrl(html = '', fiiId = '') {
  const source = String(html || '');
  const direct = source.match(/\bdata-url=["']([^"']*\/api\/fii\/comparador\/table\/\d+\/[^"']*)["']/i)?.[1]
    || source.match(/((?:https:\/\/investidor10\.com\.br)?\/api\/fii\/comparador\/table\/\d+\/[^"'`<>\s]*)/i)?.[1]
    || '';
  if (direct) {
    const decoded = decodeHtmlEntities(direct).replace(/\\\//g, '/');
    return decoded.startsWith('http') ? decoded : `https://investidor10.com.br${decoded.startsWith('/') ? '' : '/'}${decoded}`;
  }
  const id = String(fiiId || '').trim();
  return /^\d+$/.test(id) ? `https://investidor10.com.br/api/fii/comparador/table/${id}/segment_type/` : '';
}

async function fetchInvestidor10FiiPeerComparison({ ticker = '', html = '', fiiId = '', timeoutMs = 9000, fallback = null } = {}) {
  const url = extractInvestidor10FiiPeerApiUrl(html, fiiId);
  if (!url) {
    return fallback || emptyFiiPeerComparison(ticker, { mode: 'api_comparator', reason: 'comparator_api_url_not_found' });
  }
  const response = await fetchJson(url, {
    timeoutMs: Math.min(18000, Math.max(4200, Number(timeoutMs) || 9000)),
    ttlMs: 30 * 60 * 1000,
    staleMs: 24 * 60 * 60 * 1000,
    retries: 0,
    headers: {
      ...
      Referer: ticker ? `https://investidor10.com.br/fiis/${String(ticker).toLowerCase()}/` : 'https://investidor10.com.br/fiis/'
    }
  }).catch(error => ({ json: null, status: 0, cacheStatus: 'ERROR', error: error?.message || String(error) }));
  const rows = normalizeInvestidor10FiiPeerApi(response?.json, ticker);
  if (rows.length) {
    return buildFiiPeerComparisonPayload(rows, ticker, {
      mode: 'api_comparator_segment_type',
      endpoint: url,
      httpStatus: response?.status || 0,
      cacheStatus: response?.cacheStatus,
      parsedRows: rows.length,
      typeRows: rows.filter(row => row.fundType && row.fundType !== '—').length,
      segmentRows: rows.filter(row => row.segment && row.segment !== '—').length,
      policy: 'investidor10_comparator_api_no_peer_page_fanout_no_inference'
    }, 'Investidor10 API comparador de FIIs');
  }
  return {
    ...(fallback || emptyFiiPeerComparison(ticker)),
    diagnostics: {
      ...
      comparatorApi: {
        ...
        httpStatus: response?.status || 0,
        cacheStatus: response?.cacheStatus,
        error: response?.error || (response?.parseError ? 'parse-error' : 'empty-comparator-api'),
        parsedRows: 0
      }
    }
  };
...
async function fetchInvestidor10FiiBundle(ticker, timeoutMs = 6500, options = {}) {
  ...
  const sourceTimeoutMs = fastMode ? Math.max(Number(timeoutMs) || 0, 12000) : timeoutMs;
  const response = await fetchText(url, {
  ...
  const wantsHistorical = !targetedRecovery || recoveryTarget.sections.has('historicalIndicators') || wantsChecklist;
  const wantsVacancy = !targetedRecovery || recoveryTarget.sections.has('vacancyHistory') || wantsChecklist;
  const wantsPeerComparison = !targetedRecovery ||
    recoveryTarget.sections.has('peerComparison') ||
    recoveryTarget.sections.has('patrimonialInfo') ||
    recoveryTarget.sections.has('checklist');
  const [historicalFetch, vacancyFetch, announcementsFetch, enrichedPeerComparison] = fastMode
    ...
    : await Promise.all([
      ...
      wantsPeerComparison
        ? fetchInvestidor10FiiPeerComparison({ ticker, html, fiiId: ids.fiiId, timeoutMs, fallback: peerComparison })
          .catch(error => ({ ...peerComparison, diagnostics: { ...(peerComparison?.diagnostics || {}), comparatorApi: { error: error?.message || String(error), policy: 'direct_comparator_api_then_rendered_source_fallback' } } }))
...
async function fetchOfficialReturnIndexComparisonHistory(item, period, timeoutMs) {
  ...
  const directIndex = await fetchInvestidor10DirectIndexHistory(item.code, {
  ...
  const directPoints = directIntegrity.trusted
    ? normalizeReturnPageHistoryPoints(directIndex, period, directIndex?.source || item.sourceLabel)
    : [];
  if (directPoints.length >= 2) {
    return {
      ...
      points: directPoints,
      plan: { range: `${Number(period.months || 120)}M`, interval: '1mo', directInvestidor10IndexApi: true },
      ok: true,
      symbolUsed: item.code,
  ...
  if (points.length >= 2) {
    return {
      history,
      integrity,
      directIndexAttempt: directIndex,
      points,
...
async function buildFiiYahooIndexComparison(ticker, timeoutMs = 8500) {
  ...
  // Inicia as séries de mercado imediatamente; CDI/IPCA podem levar mais tempo no BCB,
  // mas não devem bloquear o início de Yahoo/Investidor10.
  const periodMarketTasks = FII_COMPARISON_PERIODS.map(period => {
    const tickers = [
    ...
    const marketTickers = tickers.filter(item => !String(item.kind || '').startsWith('bcb_'));
    return {
      period,
      tickers,
      marketTickers,
      historiesPromise: Promise.allSettled(marketTickers.map(item => fetchComparisonSeriesHistory(item, period, perCallTimeout, {})))
    };
  });
  const [cdiSeries, ipcaSeries] = await macroPromise;
  ...
  const periodEntries = await Promise.all(periodMarketTasks.map(async task => {
    const { period, tickers, marketTickers } = task;
    const marketResults = await task.historiesPromise;
    const macroTickers = tickers.filter(item => String(item.kind || '').startsWith('bcb_'));
    const macroResults = await Promise.allSettled(macroTickers.map(item => fetchComparisonSeriesHistory(item, period, perCallTimeout, macroSeries)));
    const resultsByKey = new Map();
    marketTickers.forEach((item, index) => resultsByKey.set(item.key || item.code, marketResults[index]));
    macroTickers.forEach((item, index) => resultsByKey.set(item.key || item.code, macroResults[index]));
    const histories = tickers.map(item => resultsByKey.get(item.key || item.code));
    const series = histories.map((result, index) => {
      ...
      return {
        ...
        points: value.points,
        source: value.directIndexAlternative
        ...
        endpoint: item.kind?.startsWith('bcb_')
          ? (value.history?.source || item.sourceLabel)
          : value.directIndexAlternative
            ? value.history?.endpoint || value.history?.sourceUrl || item.sourceLabel
            : value.reusedReturnProxy
  ...
  return {
    ...
    diagnostics,
    source: 'Investidor10 API direta de índices + Yahoo Finance Chart API + Banco Central SGS',
    ...
    sourcePolicy: 'IFIX, SMLL, IDIV e IBOV usam primeiro a API histórica direta do próprio índice no Investidor10; Yahoo/B3 ficam como contingência. Snapshots isolados, ETF substituto, ticker proxy, mock e série simulada são rejeitados. CDI e IPCA usam Banco Central SGS. A projeção de R$ 1.000,00 é calculada somente a partir das séries reais ativas do período selecionado.'
  };
}
...
async function buildFiiModalContractFresh(payload = {}) {
  ...
  const fiiBundlePromise = fetchInvestidor10FiiBundle(
    ticker,
    Math.min(Number(payload.investidor10TimeoutMs || payload.fundamentalTimeoutMs || timeoutMs || 18000), fastMode ? 2600 : 18000),
  ...
  const shouldFetchDividendYieldHistory = !fastMode
    && (!recoveryTarget.targeted || recoveryTarget.sections.has('dividendCharts') || recoveryTarget.sections.has('checklist'));
  ...
  const fiiBundleTask = fastMode
    ...
    : fiiBundlePromise;
  const [oneYearHistory, investidor10, comparison, historicalDividendYield] = await Promise.all([
    historyTask,
    fiiBundleTask,
  ...
  const checklistItems = Array.isArray(investidor10?.checklist?.items) ? investidor10.checklist.items : [];
  const hasResolvedChecklist = checklistItems.length >= FII_BUY_HOLD_CHECKLIST_CRITERIA.length
    && checklistItems.slice(0, FII_BUY_HOLD_CHECKLIST_CRITERIA.length).every(item =>
  ...
  return {
    ...
    dataTruth: {
      ...
      calculatedSections: [
        { id: 'checklist', nature: 'MIXED', rule: 'oito critérios canônicos; marcação direta da fonte prevalece e lacunas são calculadas apenas com métricas públicas reais' },
    ...
    quoteSummary: {
      ...
      logoUrl: '',
      logoSource: '',
      logoPolicy: 'DISABLED_FOR_FII_NO_OFFICIAL_LOGO',
    ...
    diagnostics: {
      ...
      fiiLogoPolicy: 'DISABLED_NO_OFFICIAL_LOGOS',
      yahooLogoStatus: 'NOT_APPLICABLE',
      yahooLogoUrl: '',
...
function normalizeFiiModalPayload(payload = {}, defaultTimeoutMs = 22000) {
  const stage = normalizeFiiModalStage(payload);
...
export async function buildFiiModalContract(payload = {}) {
  const modalPayload = normalizeFiiModalPayload(payload, 22000);
