...
const FII_MODAL_VERSION = '26.asset-modal.fii.v4';
...
async function buildFiiYahooIndexComparison(ticker, timeoutMs = 8500) {
  const perCallTimeout = Math.min(5200, Math.max(2600, Math.floor(timeoutMs * 0.7)));
  const periodEntries = await Promise.all(FII_COMPARISON_PERIODS.map(async period => {
    const tickers = [{ key: 'asset', code: ticker, label: ticker, ticker, sourceLabel: 'Yahoo Finance Chart API ativo' }, ...FII_INDEX_BENCHMARKS.map(item => ({ ...item, sourceLabel: `Yahoo Finance Chart API índice direto ${item.yahooSymbol}` }))];
    const histories = await Promise.allSettled(tickers.map(item => fetchYahooHistory(item.ticker, {
      range: period.range,
      interval: period.interval,
      timeoutMs: perCallTimeout,
      limit: period.key === '10y' ? 180 : 280
    })));
    const series = histories.map((result, index) => {
      const item = tickers[index];
      if (result.status !== 'fulfilled') return null;
      const points = normalizeComparisonPoints(result.value);
      if (points.length < 2) return null;
      return {
        ...
        points,
        source: item.sourceLabel || result.value?.source || 'Yahoo Finance Chart API',
        yahooSymbol: result.value?.symbol || item.yahooSymbol || `${item.ticker}.SA`
      };
    }).filter(Boolean);
  ...
  const quoteSettled = await Promise.allSettled(FII_INDEX_BENCHMARKS.map(item => fetchYahooQuote(item.ticker, { timeoutMs: Math.min(3600, perCallTimeout), interval: '1d' })));
  const indexQuotes = FII_INDEX_BENCHMARKS.map((item, index) => indexQuoteFromYahooQuote(item, quoteSettled[index].status === 'fulfilled' ? quoteSettled[index].value : {}));
  const seriesByPeriod = Object.fromEntries(periodEntries);
  const defaultPeriod = seriesByPeriod['2y']?.series?.length ? '2y' : (Object.keys(seriesByPeriod).find(key => seriesByPeriod[key]?.series?.length) || '2y');
  ...
  return {
    ...
    indexQuotes,
    source: 'Yahoo Finance Chart API',
    sourcePolicy: 'IFIX, IDIV e SMLL consultados exclusivamente pelo Yahoo Finance Chart API com IFIX.SA, IDIV.SA e SMLL.SA.'
  };
}
...
export const _test = { extractInvestidor10FiiInformation, extractInvestidor10FiiQuickMetrics, normalizeFiiHistoricalIndicatorsApi, returnsRowsFromInvestidor10, cleanInvestidor10InfoValue, normalizeComparisonPoints, comparisonItemFromSeries, firstHistoricalIndicatorValue };
