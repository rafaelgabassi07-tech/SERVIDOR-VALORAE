...
const FII_MODAL_VERSION = '26.asset-modal.fii.v20';
...
function announcementFromRow({ rowText = '', href = '', anchorText = '', ticker = '', index = 0, baseUrl = '' } = {}) {
  ...
  const dateMatch = cleanRow.match(/\b\d{2}\/\d{2}\/\d{4}\b/);
  const dateDisplay = dateMatch?.[0] || '';
  let titleCandidate = cleanAnchor && !/^abrir$/i.test(cleanAnchor) ? cleanAnchor : cleanRow;
  if (dateDisplay) titleCandidate = titleCandidate.split(dateDisplay)[0] || titleCandidate;
...
async function enrichInvestidor10FiiAnnouncementPdfLinks(payload, { timeoutMs = 4500 } = {}) {
  ...
  for (let index = 0; index < Math.min(items.length, 8); index += 1) {
    ...
    try {
      ...
      const pdfUrl = /\.pdf(?:$|[?#])/i.test(finalUrl)
...
function extractInvestidor10FiiAnnouncements(html = '', ticker = '') {
  ...
  const sectionHtml = findAnnouncementsHtmlSection(html, symbol);
...
function announcementKindFromUrl(url = '') {
  ...
  return /\.pdf(?:$|[?#])/i.test(clean) ? 'pdf' : 'external';
}
...
function buildTypeSegmentAverageRows({ html = '', ticker = '', quickMetrics = {}, infoItems = [], peerComparison = null, patrimonial = {} } = {}) {
  ...
  return {
    ...
    rows,
    diagnostics: { parsedFrom: section ? 'investidor10_html_section' : 'peer_comparison_fallback', sectionFound: Boolean(section), peerRows: peerRows.length }
  };
}
...
function buildFiiPatrimonialInfoPayload({ html = '', ticker = '', quickMetrics = {}, infoItems = [], canonical = {}, peerComparison = null } = {}) {
  ...
  return {
    ...
    segmentAverage,
    diagnostics: {
      parsedFrom: section ? 'investidor10_html_section' : 'canonical_supplementary_fallback',
...
async function fetchInvestidor10FiiBundle(ticker, timeoutMs = 6500) {
  ...
  const announcements = html ? await enrichInvestidor10FiiAnnouncementPdfLinks(extractInvestidor10FiiAnnouncements(html, ticker), { timeoutMs }) : emptyFiiAnnouncements(ticker, { reason: 'no_html' });
  return {
    ok: Boolean(html),
    url,
...
async function fetchComparisonSeriesHistory(item, period, timeoutMs, macroSeries = {}) {
  ...
  if (item.kind === 'official_return_index') {
    ...
    // Fallback final: mantém a chamada direta Yahoo local como segunda chance,
    ...
    const series = histories.map((result, index) => {
      ...
      return {
        ...
        source: value.reusedReturnProxy
          ...
          : `${item.sourceLabel || value.history?.source || 'Yahoo Finance Chart API'}${value.symbolUsed && value.symbolUsed !== item.yahooSymbol ? ` · símbolo alternativo ${value.symbolUsed}` : ''}${value.plan?.fallback ? ' · fallback de intervalo' : ''}`,
