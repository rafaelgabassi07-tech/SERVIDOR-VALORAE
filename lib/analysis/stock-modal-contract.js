...
export const STOCK_MODAL_VERSION = '26.asset-modal.stock.v25';
...
function stockPayoutNumeric(value, field = '') {
  if (value == null || value === '') return null;
  if (typeof value === 'number') return Number.isFinite(value) ? value : null;
  if (Array.isArray(value)) {
    for (let i = value.length - 1; i >= 0; i -= 1) {
      const n = stockPayoutNumeric(value[i], field);
      if (n !== null) return n;
    }
    return null;
  }
  if (typeof value === 'object') {
    ...
    for (const key of (keysByField[field] || ['value','valor','y','amount','total'])) {
      if (Object.prototype.hasOwnProperty.call(value, key)) {
        const n = stockPayoutNumeric(value[key], field);
        if (n !== null) return n;
      }
    }
    return null;
...
function normalizeStockPayoutDedicatedSource(raw, { ticker = '' } = {}) {
  ...
  const addField = (labelRaw, field, rawValue, order = 0) => {
    if (!field) return;
    const value = stockPayoutNumeric(rawValue, field);
  ...
  const consumeSeries = (series, categories = [], inheritedName = '') => {
    for (const [seriesIndex, item] of (series || []).entries()) {
      ...
      if (field && data.length) {
        data.forEach((point, index) => addField(stockPayoutLabelFromPoint(point, categories[index], index), field, point, index + seriesIndex * 1000));
      }
    }
  };
  ...
  const consumeRows = (rows, categories = []) => {
    for (const [rowIndex, row] of (rows || []).entries()) {
      ...
      const label = stockPayoutLabelFromPoint(row, categories[rowIndex], rowIndex);
      addField(label, 'netIncome', row.netIncome ?? row.netProfit ?? row.lucroLiquido ?? row.lucro_liquido ?? row.lucro ?? row.profit ?? row.resultado, rowIndex);
      addField(label, 'payoutPercent', row.payoutPercent ?? row.payout ?? row.payOut ?? row.pay_out ?? row.payoutRatio, rowIndex);
      addField(label, 'dividendYieldPercent', row.dividendYieldPercent ?? row.dividendYield ?? row.dy ?? row.yield, rowIndex);
    }
  };
  ...
  const visit = (node, depth = 0, inheritedName = '') => {
    ...
    if (label) {
      addField(label, 'netIncome', node.netIncome ?? node.netProfit ?? node.lucroLiquido ?? node.lucro_liquido ?? node.lucro ?? node.profit ?? node.resultado, 0);
      addField(label, 'payoutPercent', node.payoutPercent ?? node.payout ?? node.payOut ?? node.pay_out ?? node.payoutRatio, 0);
      addField(label, 'dividendYieldPercent', node.dividendYieldPercent ?? node.dividendYield ?? node.dy ?? node.yield, 0);
    }
    ...
    for (const [key, value] of Object.entries(node)) {
      ...
      if (field && Array.isArray(value)) {
        value.forEach((point, index) => addField(stockPayoutLabelFromPoint(point, categories[index], index), field, point, index));
        continue;
      }
      if (field && value && typeof value === 'object' && !Array.isArray(value)) {
        for (const [period, periodValue] of Object.entries(value)) addField(period, field, periodValue, 0);
        continue;
      }
...
function finalizeStockPayoutPoint(point, ticker = '') {
  const netIncome = Number(point.netIncome);
  const payout = Number(point.payoutPercent);
  const dy = Number(point.dividendYieldPercent);
  return {
...
function buildStockPayoutChartPayload({ ticker = '', canonical = {}, historicalIndicators = {}, dividendHistory = {}, payoutRaw = null, html = '' } = {}) {
  ...
  if (hasCurrentValues) {
    ...
    const latestProfit = Number.isFinite(Number(currentNetIncome)) ? Number(currentNetIncome) : [...profitByYear.entries()].sort((a, b) => Number(b[0]) - Number(a[0]))[0]?.[1];
    const key = 'ult12m';
    ...
    pointMap.set(key, {
      ...
      label: 'Últ 12M',
      year: null,
      netIncome: Number.isFinite(Number(existing.netIncome)) && Math.abs(Number(existing.netIncome)) >= 1_000_000 ? existing.netIncome : (Number.isFinite(Number(latestProfit)) ? Number(latestProfit) : existing.netIncome),
      payoutPercent: Number.isFinite(Number(existing.payoutPercent)) ? existing.payoutPercent : (Number.isFinite(Number(payout)) ? Number(payout) : undefined),
      dividendYieldPercent: Number.isFinite(Number(existing.dividendYieldPercent)) ? existing.dividendYieldPercent : (Number.isFinite(Number(dy)) ? Number(dy) : undefined)
    });
  }
...
function stockHistoryColumnLabel(raw = '') {
  ...
  if (/^(atual|current|ultimo|ultima|hoje|ttm|ultimos_12_meses|ult_12_meses|last_12_months|lTM)$/i.test(key)) return 'Atual';
...
function normalizeStockHistoricalIndicatorsDataset(data, options = {}) {
  ...
  const processPointArray = (label, values, source = localSource, fallbackColumns = []) => {
    ...
    values.forEach((item, index) => {
      ...
      if (!col) return;
      rowValues[col] = stockHistoryValueFromCell(item, item?.type || item?.unit || '');
    });
    pushRow(label, rowValues, source);
  };
  const processValueObject = (label, rawValues, source = localSource) => {
    if (!label || rawValues == null) return;
    if (Array.isArray(rawValues)) {
      if (rawValues.length && rawValues.every(item => item && typeof item === 'object' && !Array.isArray(item))) {
        processPointArray(label, rawValues, source);
      }
      return;
    }
    ...
    for (const [period, rawValue] of Object.entries(rawValues)) {
      ...
      if (!col) continue;
      values[col] = stockHistoryValueFromCell(rawValue, rawValue?.type || rawValue?.unit || '');
    }
  ...
  const parseArrayRow = (row, fallbackColumns = [], source = localSource) => {
    ...
    valueCells.forEach((cell, index) => {
      ...
      if (!col) return;
      rowValues[col] = stockHistoryValueFromCell(cell);
    });
    pushRow(label, rowValues, source);
  };
  const parseObjectRow = (row, fallbackColumns = [], source = localSource) => {
    ...
    if (!label) return;
    const directValues = row.values ?? row.valores ?? row.periodValues ?? row.period_values ?? row.history ?? row.historico;
    if (Array.isArray(directValues)) return processPointArray(label, directValues, row.source || source, fallbackColumns);
    if (directValues && typeof directValues === 'object') return processValueObject(label, directValues, row.source || source);
    const values = {};
    for (const [key, value] of Object.entries(row)) {
      const col = stockHistoryColumnLabel(key);
      if (!col) continue;
      values[col] = stockHistoryValueFromCell(value, row.type || row.unit || '');
    }
  ...
  const categories = [data?.categories, data?.labels, data?.years, data?.anos, data?.periods, data?.periodos, data?.xAxis?.categories, data?.xaxis?.categories, data?.options?.xaxis?.categories, data?.options?.xAxis?.categories]
    .find(list => Array.isArray(list) && list.length) || [];
  const normalizedCategories = categories.map(stockHistoryColumnLabel).filter(Boolean);
  const seriesCandidate = Array.isArray(data?.series) ? data.series : (Array.isArray(data?.datasets) ? data.datasets : (Array.isArray(data?.data?.series) ? data.data.series : []));
  if (normalizedCategories.length && Array.isArray(seriesCandidate) && seriesCandidate.length) {
    ...
    for (const series of seriesCandidate) {
      ...
      dataValues.forEach((rawValue, index) => {
        ...
        if (col) values[col] = stockHistoryValueFromCell(rawValue, rawValue?.type || rawValue?.unit || series?.unit || '');
      });
    ...
    if (root && typeof root === 'object' && !Array.isArray(root)) {
      for (const [label, rawValues] of Object.entries(root)) {
        ...
        if (isStockHistoricalMetricLabel(label)) {
          processValueObject(label, rawValues, localSource);
          continue;
        }
  ...
  if (!rows.length) {
    const candidateKeys = ['historicoIndicadores','historico_indicadores','historicalIndicators','indicatorHistory','fundamentalIndicatorHistory','indicadores','indicators','table','tabela','chart','payload','result','response','results'];
...
async function fetchInvestidor10StockBundle(ticker, timeoutMs = 6500) {
  ...
  const canonicalHistoricalRaw = canonical?.company?.fundamentalIndicatorHistory || canonical?.fundamentalIndicatorHistory || {};
  const htmlHistoricalRaw = html ? extractInvestidor10StockHistoricalIndicatorsFromHtml(html, symbol) : {};
  let historicalIndicators = buildStockHistoricalIndicators(canonicalHistoricalRaw, symbol, fundamentalIndicators);
  if (!(historicalIndicators.rows || []).length && (htmlHistoricalRaw.rows || []).length) {
    historicalIndicators = buildStockHistoricalIndicators(htmlHistoricalRaw, symbol, fundamentalIndicators);
  }
