...
export const STOCK_MODAL_VERSION = '26.asset-modal.stock.v42';
...
function stockHistoryLabelFromObject(row = {}) {
  ...
  return row.label ?? row.indicador ?? row.indicator ?? row.indicatorName ?? row.indicator_name
  ...
  for (const serie of series) {
    ...
    for (const point of data) {
      if (Array.isArray(point)) {
        ...
        const percent = point.y ?? point.percent ?? point.percentage ?? point.value ?? point.valor;
...
function revenueKeyMatchesKind(key = '', kind = 'region') {
  ...
  if (!k) return false;
  if (kind === 'business') {
    return /(negocio|negocios|business|bussines|segment|segmento|segmentos|product|produto|produtos|activity|atividade|receitapornegocio|receitaspornegocio|revenuebybusiness|businessrevenue|revenuesegment|segmentrevenue|productrevenue|companybusiness|bussinessrevenue)/i.test(k);
  }
  return /(regiao|regioes|region|regions|geograph|geography|geographic|country|countries|pais|paises|receitaporregiao|receitasporregiao|revenuebyregion|regionrevenue|revenuegeography|geographyrevenue|geographicrevenue|companyrevenueschartpie)/i.test(k);
}
...
function revenueTotalFromCandidate(candidate = {}) {
  ...
  const visit = (node, depth = 0) => {
    ...
    const totalDisplay = cleanText(node.totalAmountDisplay || node.totalDisplay || node.totalValueDisplay || node.valorTotalDisplay || node.receitaTotalDisplay || node.revenueTotalDisplay || node.total || node.valorTotal || node.receitaTotal || node.revenueTotal || '');
    if (totalDisplay) {
      return {
