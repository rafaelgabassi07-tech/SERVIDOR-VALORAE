...
export const STOCK_MODAL_VERSION = '26.asset-modal.stock.v47';
...
function stockChecklistFundamentalNumber(fundamentalIndicators = {}, id = '') {
  ...
  if (!item) return null;
  const direct = Number(item.numericValue);
...
function extractStockChecklistCompanyFacts(html = '') {
  ...
  return {
    ...
    dailyLiquidity: Number.isFinite(Number(liquidity)) ? Number(liquidity) : null,
    dailyLiquidityDisplay: liquidityDisplay,
    userRating: Number.isFinite(Number(rating)) ? Number(rating) : null,
...
function stockChecklistProfitEvidence(canonical = {}) {
  ...
  for (const point of sources) {
    ...
    if (!meta.label || !Number.isFinite(Number(income))) continue;
...
function deriveStockChecklistStatusFromInvestidor10({ criterionId = '', html = '', fundamentalIndicators = {}, canonical = {}, buyHoldRanking = null } = {}) {
  ...
  const pick = (...values) => values.find(value => Number.isFinite(Number(value)));
  const valueResult = (value, threshold, comparator, source, display = '') => {
    if (!Number.isFinite(Number(value))) return { passed: null, source, evidence: 'sem_valor_real_suficiente' };
    const n = Number(value);
  ...
  switch (criterionId) {
    case 'listed_5y': {
      ...
      if (!Number.isFinite(Number(year))) return { passed: null, source: 'Investidor10 dados da empresa', evidence: 'ano_de_estreia_indisponivel' };
    ...
    case 'dividends_5y_above_5': {
      ...
      return valueResult(value, 5, '>=', ranking?.dyAverage5y != null ? 'Investidor10 ranking buy and hold' : 'Investidor10 indicadores fundamentalistas', Number.isFinite(Number(value)) ? `DY médio/atual ${formatPercent(value, false)}` : '');
    }
    case 'roe_above_10': {
      const value = pick(ranking?.roe, metric('roe'));
      return valueResult(value, 10, '>=', ranking?.roe != null ? 'Investidor10 ranking buy and hold' : 'Investidor10 indicadores fundamentalistas', Number.isFinite(Number(value)) ? `ROE ${formatPercent(value, false)}` : '');
    }
    case 'debt_below_equity': {
      const value = pick(ranking?.grossDebtEquity, metric('divida_bruta_patrimonio'));
      return valueResult(value, 1, '<', ranking?.grossDebtEquity != null ? 'Investidor10 ranking buy and hold' : 'Investidor10 indicadores fundamentalistas', Number.isFinite(Number(value)) ? `Dívida Bruta/Patrimônio ${formatNumber(value, 2)}` : '');
    }
    case 'revenue_growth_5y': {
      const value = pick(ranking?.cagrRevenue5y, metric('cagr_receitas_5_anos'));
      return valueResult(value, 10, '>=', ranking?.cagrRevenue5y != null ? 'Investidor10 ranking buy and hold' : 'Investidor10 indicadores fundamentalistas', Number.isFinite(Number(value)) ? `CAGR Receita 5 anos ${formatPercent(value, false)}` : '');
    }
    case 'profit_growth_5y': {
      const value = pick(ranking?.cagrProfit5y, metric('cagr_lucros_5_anos'));
      return valueResult(value, 10, '>=', ranking?.cagrProfit5y != null ? 'Investidor10 ranking buy and hold' : 'Investidor10 indicadores fundamentalistas', Number.isFinite(Number(value)) ? `CAGR Lucro 5 anos ${formatPercent(value, false)}` : '');
    }
    ...
    case 'investidor10_user_rating': {
      if (Number.isFinite(Number(facts.userRating))) return valueResult(facts.userRating, 4, '>=', 'Investidor10 avaliação dos usuários', facts.userRatingDisplay || '');
...
function normalizeStockHistoricalIndicatorsDataset(data, options = {}) {
  ...
  if (!rows.length) {
    const candidateKeys = ['historicoIndicadores','historico_indicadores','historico_indicadores_fundamentalistas','indicators_history','indicator_history','historicalIndicators','historical_indicators','indicatorHistory','fundamentalHistory','fundamentalsHistory','fundamentalIndicatorHistory','fundamental_indicator_history','fundamentals','fundamental','ratios','ratioHistory','indicadores','indicators','metrics','metricas','table','tabela','chart','chartData','payload','data','result','response','results'];
...
function collectStockHistoricalIndicatorCandidates(payload, options = {}) {
  ...
  const walk = (value, path = 'root', depth = 0) => {
    ...
    if (/(historico|history|indicador|indicator|fundamental|fundamento|ratio|metric)/i.test(key)) add(value, path, 'semantic_key');
    if (Array.isArray(value)) {
...
function buildStockHistoricalIndicatorSources({ html = '', ticker = '', apiExtras = {} } = {}) {
  ...
  for (const item of directSources) {
    sources.push(item);
    for (const nested of collectStockHistoricalIndicatorCandidates(item)) sources.push(nested);
  }
  const embedded = apiExtras?.embedded || {};
  for (const key of ['historicalIndicators','historicoIndicadores','indicatorHistory','fundamentalIndicatorHistory']) {
...
function parseStockRevenueRowsFromSection(section = '', fallbackPeriod = '', kind = 'region') {
  const rows = [];
  ...
  for (const match of source.matchAll(re)) {
    ...
    if (revenueLabelLooksInvalidForKind(label, kind)) continue;
...
function revenueYearFromCandidate(candidate = {}) {
  ...
  const visit = (node, depth = 0) => {
    ...
    const keyMatch = Object.keys(node || {}).find(key => /^20\d{2}$/.test(String(key)));
    if (keyMatch) return String(keyMatch);
    for (const arr of [node.years, node.anos, node.periods, node.periodos]) {
      if (Array.isArray(arr)) {
        const found = arr.map(value => String(value || '').match(/\b(20\d{2})\b/)?.[1]).find(Boolean);
        if (found) return found;
      }
    }
...
function normalizeRevenueCandidateRow(node = {}, index = 0, kind = 'region', source = 'Investidor10 distribuição de receitas API') {
  ...
  const rawLabel = cleanText(
    node.label || node.name || node.nome || node.title
    || node.region || node.regiao || node.geography || node.local || node.country || node.pais
    || node.segment || node.segmento || node.business || node.negocio || node.product || node.produto || node.category || node.categoria || node.activity || node.atividade || node.x || ''
  );
  ...
  if (revenueLabelLooksInvalidForKind(label, kind)) return null;
  const percentRaw = node.percentDisplay || node.percentualDisplay || node.percentageDisplay || node.shareDisplay || node.participationDisplay || node.participacaoDisplay || embeddedPercentDisplay || node.percentual || node.percent || node.percentage || node.sharePercent || node.share || node.yPercent || node.y_percentage || node.participation || node.participacao || node.y;
  ...
  const amountRaw = node.amountDisplay || node.valorDisplay || node.valueDisplay || node.totalDisplay || node.receitaDisplay || node.revenueDisplay || node.formattedValue || node.formatted_value || node.displayValue || node.display_value || node.dataLabel || node.data_label || embeddedAmountDisplay || nestedRevenueValue(node, ['custom.amountDisplay','custom.valorDisplay','custom.valueDisplay','custom.receitaDisplay','custom.revenueDisplay','custom.value','custom.valor','extra.amountDisplay','extra.valorDisplay','meta.amountDisplay','meta.valorDisplay']) || node.amount || node.valor || node.total || node.receita || node.revenue;
...
function pairedRevenueAmountRowsFromCandidate(candidate, kind = 'region', source = 'Investidor10 distribuição de receitas API') {
  const labels = pickRevenueLabelsFromCandidate(candidate);
  const values = pickRevenueSeriesValuesFromCandidate(candidate);
  if (!labels.length || !values.length) return [];
  const staged = labels.map((label, index) => {
    const safeLabel = stripRevenueDecorationsFromLabel(label);
    if (revenueLabelLooksInvalidForKind(safeLabel, kind)) return null;
    const rawValue = values[index];
    let amount = null;
    let amountDisplay = '';
    if (rawValue && typeof rawValue === 'object') {
      const rawAmount = rawValue.amount ?? rawValue.valor ?? rawValue.receita ?? rawValue.revenue ?? rawValue.y ?? rawValue.value ?? rawValue.total;
      amount = parseRevenueAmountNumber(rawAmount);
      amountDisplay = cleanText(rawValue.amountDisplay || rawValue.valorDisplay || rawValue.receitaDisplay || rawValue.revenueDisplay || rawValue.valueDisplay || '');
    } else {
      amount = parseRevenueAmountNumber(rawValue);
    }
    const displayFromCandidate = revenueAmountDisplayFromCandidate(candidate, index);
    if (displayFromCandidate && /R\$/i.test(displayFromCandidate)) {
      amountDisplay = displayFromCandidate;
      amount = parseRevenueAmountNumber(displayFromCandidate) ?? amount;
    }
    if (!Number.isFinite(Number(amount)) || Number(amount) <= 100) return null;
    return { label: safeLabel, amount: Number(amount), amountDisplay };
  }).filter(Boolean);
  ...
  return staged.map((row, index) => {
    const percent = round((row.amount / Number(total)) * 100, 4);
    if (!Number.isFinite(Number(percent)) || percent <= 0 || percent > 100) return null;
    return {
      id: `${kind}_${index}_${metricId(row.label)}`,
      label: row.label,
      amountDisplay: row.amountDisplay || formatRevenueMoneyAbbrev(row.amount) || '—',
      amount: round(row.amount, 2),
      percent,
      percentDisplay: revenuePercentDisplayFromComputedShare(percent),
      source
    };
  }).filter(Boolean);
}
...
function revenueLabelLooksInvalidForKind(label = '', kind = 'region') {
  const raw = cleanText(label).replace(/\s+/g, ' ').trim();
  const normalized = compactKey(raw);
  if (!raw || raw.length < 2 || raw.length > (kind === 'business' ? 96 : 64)) return true;
  if (/^(?:total|ano|year|data|value|valor|percentual|percent|receita|image|select|button|fonte|status)$/i.test(raw)) return true;
  if (/^(?:P\/?L|P\/?VP|P\/?Receita|PSR|DY|ROE|ROIC|ROA|VPA|LPA|EV\/?Ebitda|EV\/?Ebit|P\/?Ebitda|P\/?Ebit|Dividend Yield|Payout|Margem.*|Liquidez.*|CAGR.*)$/i.test(raw)) return true;
  if (/\b(?:not[íi]cias?|coment[áa]rios?|discuss[aã]o|respostas?|responder|enviar|carregar|press[- ]?release|comunicado|divulga[cç][aã]o|publicado|cotacao|cota[cç][aã]o|ibov|selic|d[úu]vida|iniciante|vou\s+vender|vou\s+comprar|wallet|carteira|ranking|forum|usu[aá]rio|pergunta)\b/i.test(raw)) return true;
  ...
  if (kind === 'region' && /\b(?:diesel|gasolina|petr[oó]leo|querosene|nafta|g[aá]s|[óo]leo|energia|servi[cç]os?|produto|segmento)\b/i.test(raw)) return true;
  if (kind === 'business' && /\b(?:brasil|china|europa|[áa]sia|am[ée]ricas?|estados\s+unidos|argentina|exterior|pa[ií]ses|regi[oõ]es?)\b/i.test(raw) && raw.split(/\s+/).length <= 3) return true;
  const words = raw.split(/\s+/).filter(Boolean);
  const lowerWords = words.filter(word => /^[a-záéíóúâêôãõàç]{3,}$/.test(word));
  if (words.length >= 7 && lowerWords.length / Math.max(1, words.length) > 0.55 && !/\b(?:combust[ií]vel|bunker|querosene|avia[cç][aã]o|renov[aá]veis|energia|servi[cç]os|explora[cç][aã]o|distribui[cç][aã]o)\b/i.test(raw)) return true;
  return false;
}

function revenueContextLooksBlocked(key = '', value = null) {
  const source = `${key || ''} ${typeof value === 'string' ? value : ''}`;
  return /(?:noticias?|news|comentarios?|comments?|discuss(?:ao|ion)|forum|posts?|respostas?|answers?|comunicados?|announcements?|press[-_ ]?release|artigos?|articles?|blog|feed|timeline|opiniao|opini[aã]o|user|usuario|usu[aá]rio|avatar|rating|avaliacao|avalia[cç][aã]o|ranking|relacionadas|related)/i.test(source);
}

function revenuePercentDisplayFromComputedShare(percent) {
...
function revenueLabelFromNode(node = {}, kind = 'region') {
  const aliases = kind === 'business'
    ? ['label','name','nome','title','titulo','segment','segmento','business','negocio','negócio','produto','product','category','categoria','activity','atividade']
    : ['label','name','nome','title','titulo','region','regiao','região','geography','geografia','local','country','pais','país','mercado'];
  const raw = firstRevenueAliasValue(node, aliases);
  const label = stripRevenueDecorationsFromLabel(raw || '');
  if (revenueLabelLooksInvalidForKind(label, kind)) return '';
  return label;
}

function revenuePercentFromNode(node = {}) {
  const value = firstRevenueAliasValue(node, [
    ...
    'representatividade','representatividadePercentual','participacaoReceita','participaçãoReceita','revenueShare','valuePercent','valorPercentual','yPercent','y_percentage'
  ]);
  const n = parseBrNumber(value);
...
function revenueAmountFromNode(node = {}) {
  const value = firstRevenueAliasValue(node, [
    ...
    'amount','valor','value','total','receita','revenue','grossRevenue','netRevenue','net_revenue','receitaLiquida','receita_liquida','vendas','sales','y'
  ]);
  ...
  for (const serie of series) {
    ...
    for (const point of data) {
      if (Array.isArray(point)) {
        const label = point[0];
        const percent = parseBrNumber(point[1]);
        if (cleanText(label) && Number.isFinite(Number(percent))) rows.push(normalizeRevenueCandidateRow({ label, percent }, rows.length, kind, source));
      } else if (point && typeof point === 'object') {
        const label = point.name || point.label || point.category || point.categoria || point.x || point.region || point.regiao || point.country || point.pais || point.segment || point.segmento || point.business || point.negocio || point.product || point.produto || point.activity || point.atividade;
        const percent = point.percent ?? point.percentage ?? point.percentual ?? point.porcentagem ?? point.share ?? point.participation ?? point.participacao;
        if (cleanText(label) && Number.isFinite(Number(parseBrNumber(percent)))) rows.push(normalizeRevenueCandidateRow({ ...point, label, percent }, rows.length, kind, source));
      }
    }
  }
  return rows.filter(Boolean);
...
function chartJsRevenueRowsFromCandidate(candidate, kind = 'region', source = 'Investidor10 distribuição de receitas API') {
  ...
  for (const dataset of datasets) {
    ...
    const rows = labels.map((label, index) => {
      const rawValue = values[index];
      const percent = typeof rawValue === 'object' ? (rawValue?.y ?? rawValue?.value ?? rawValue?.percent ?? rawValue?.percentage ?? rawValue?.valor) : rawValue;
      const numeric = parseBrNumber(percent);
      if (!Number.isFinite(Number(numeric)) || Number(numeric) <= 0 || Number(numeric) > 100) return null;
      const amountDisplay = typeof rawValue === 'object'
        ? cleanText(rawValue.amountDisplay || rawValue.valorDisplay || rawValue.valueDisplay || rawValue.receitaDisplay || rawValue.revenueDisplay || '')
        : revenueAmountDisplayFromCandidate(candidate, index);
      return normalizeRevenueCandidateRow({ label, percent: numeric, amountDisplay }, index, kind, source);
    }).filter(Boolean);
  ...
  if (values.length) {
    return labels.map((label, index) => {
      const rawValue = values[index];
      const numeric = typeof rawValue === 'object' ? parseBrNumber(rawValue?.y ?? rawValue?.value ?? rawValue?.percent ?? rawValue?.percentage ?? rawValue?.valor) : parseBrNumber(rawValue);
      if (!Number.isFinite(Number(numeric)) || Number(numeric) <= 0 || Number(numeric) > 100) return null;
      return normalizeRevenueCandidateRow({ label, percent: numeric, amountDisplay: revenueAmountDisplayFromCandidate(candidate, index) }, index, kind, source);
    }).filter(Boolean);
  }
...
function objectMapRevenueRowsFromCandidate(candidate, kind = 'region', source = 'Investidor10 distribuição de receitas API') {
  ...
  const ignored = new Set(['labels','categories','names','series','datasets','data','values','items','rows','children','options','chart','xAxis','xaxis','total','totalAmountDisplay','totalDisplay','valorTotal','receitaTotal','revenueTotal','year','ano','period','periodo','title','id','source','status']);
  const rows = [];
  ...
  const walk = (node) => {
    ...
    if (Array.isArray(node)) {
      ...
      if (primitive.length >= 2) {
        ...
        const parsed = parseStockRevenueRowsFromSection(joined, '', kind).map(row => ({ ...row, id: `${kind}_${rows.length}_${metricId(row.label)}`, source }));
        if (parsed.length) {
          rows.push(...parsed);
          return;
        }
...
function revenueCandidateValuesFromNode(root, kind = 'region') {
  ...
  const walk = (node, depth = 0, keyHint = '') => {
    if (node == null || depth > 7 || revenueContextLooksBlocked(keyHint, node)) return;
    if (Array.isArray(node)) {
    ...
    for (const [key, value] of Object.entries(node).slice(0, 160)) {
      if (revenueContextLooksBlocked(key, value)) continue;
      if (revenueKeyMatchesKind(key, kind)) add(value);
      const keyAllowsTraversal = /^(?:payload|result|results|response|chart|chartData|data|dados|items|itens|rows|series|datasets|values|valores|years|anos|periods|periodos|receita|receitas|faturamento|distribuicao|distribuição|distribution|breakdown|segmentos|segments|geografica|geográfica|geography|company|asset|ticker|empresa)$/i.test(key);
      if (keyAllowsTraversal || /^20\d{2}$/.test(key) || revenueKeyMatchesKind(key, kind) || depth === 0) {
...
function rowsFromRevenueCandidate(candidate, kind = 'region', source = 'Investidor10 distribuição de receitas API') {
  ...
  for (const sourceCandidate of expandedCandidates) {
    ...
    rows.push(
      ...
      ...pairedRevenueAmountRowsFromCandidate(sourceCandidate, kind, source),
      ...pairedRows
    );
  }
  ...
  const walk = (node, keyHint = '') => {
    ...
    for (const [key, value] of Object.entries(node)) {
      if (['rows', 'items', 'data', 'values', 'series', 'datasets', 'children', 'pontos', 'pontosGrafico', 'chartData', 'payload', 'result', 'results', 'response', 'chart'].includes(key)
        || /^20\d{2}$/.test(key)
...
function sectionBetweenPlain(plain = '', startRe, endReList = []) {
  ...
  const end = endReList.map(re => section.search(re)).filter(i => i > 20).sort((a, b) => a - b)[0];
  if (Number.isFinite(end)) section = section.slice(0, end);
  return section;
}
...
function buildStockRevenueBreakdownPayload({ html = '', canonical = {}, ticker = '', name = '' } = {}, kind = 'region') {
  ...
  let items = parseStockRevenueRowsFromSection(section, year, kind);
  ...
  const canonicalCandidates = isRegion
    ? [canonical?.revenueRegion, canonical?.revenueGeography, canonical?.revenueByRegion, canonical?.regionRevenue, canonical?.geographicRevenue, canonical?.geographyRevenue, canonical?.regioesReceita, canonical?.regioesOndeGeraReceita, canonical?.receitaPorRegiao, canonical?.receitasPorRegiao, canonical?.revenueBreakdowns?.region, canonical?.revenueBreakdowns?.geography, canonical?.revenueBreakdowns?.byRegion, canonical?.embedded?.revenueGeography, canonical?.embedded?.revenueRegion, canonical?.embedded?.revenueByRegion, embedded?.revenueGeography, ...restRevenueCandidates]
    : [canonical?.revenueBusiness, canonical?.revenueSegment, canonical?.revenueByBusiness, canonical?.businessRevenue, canonical?.segmentRevenue, canonical?.productRevenue, canonical?.businessSegments, canonical?.businessBreakdown, canonical?.segmentBreakdown, canonical?.productBreakdown, canonical?.negociosReceita, canonical?.negociosQueGeramReceita, canonical?.receitaPorNegocio, canonical?.receitasPorNegocio, canonical?.produtosReceita, canonical?.receitaPorProduto, canonical?.revenueBreakdowns?.business, canonical?.revenueBreakdowns?.segment, canonical?.revenueBreakdowns?.byBusiness, canonical?.embedded?.revenueSegment, canonical?.embedded?.revenueBusiness, canonical?.embedded?.revenueByBusiness, canonical?.embedded?.businessRevenue, canonical?.embedded?.segmentRevenue, canonical?.embedded?.productRevenue, embedded?.revenueSegment, embedded?.businessRevenue, embedded?.segmentRevenue, embedded?.productRevenue, ...restRevenueCandidates];
  ...
  if (!items.length) {
    for (const candidate of canonicalCandidates) {
      ...
      items = parseStockRevenueRowsFromSection(htmlToPlainText(jsonText), year, kind);
      if (items.length) {
        canonicalTotal = revenueTotalFromCandidate(candidate);
        canonicalYear = revenueYearFromCandidate(candidate) || canonicalYear;
        break;
...
function looksLikeInvalidShareholderLabel(label = '') {
  ...
  if (/^(P\/?L|P\/?VP|P\/?Receita|PSR|DY|ROE|ROIC|ROA|VPA|LPA|EV\/?Ebitda|EV\/?Ebit|P\/?Ebitda|P\/?Ebit|Dividend Yield|Margem.*|Liquidez.*|CAGR.*)$/i.test(raw)) return true;
  if (/\b(vou|vender|comprar|iniciante|dinheiro|comentarios?|comentários?|respostas?|enviar|carregar|press[- ]?release|divulgacao|divulgação|selic|duvida|dúvida|pergunta|alguem|alguém|nao consigo|não consigo|avise|banco safra|fii?s? de tijolo|quota|queda da|recuperacao|recuperação)\b/i.test(raw)) return true;
  if (/\b(P\/?VP|P\/?L|DY|ROE|ROIC|Dividend Yield|Margem|Ebitda|Ebit|CAGR)\b/i.test(raw)) return true;
  if (/^\d{1,2}[\/.-]\d{1,2}[\/.-]\d{2,4}\b/.test(raw)) return true;
...
function rowsFromShareholdingCandidate(candidate, source = 'Investidor10 posição acionária') {
  ...
  const headerLooksLikeShareholding = header => {
    ...
    return /acionista|shareholder/.test(joined)
...
function buildStockShareholdingPayload({ html = '', canonical = {}, ticker = '', raw = null } = {}) {
  ...
  if (!rows.length) rows = parseStockShareholdingRowsFromSection(plain);
...
function stockI10ApiKeyFromUrl(url = '') {
  ...
  if (/(?:regioes?.*receita|receita.*regioes?|geograph|geography|region-revenue|revenue-region|revenue-geography|receitas-por-regiao|revenue-by-region)/i.test(clean)) return 'revenueGeography';
  if (/(?:negocios?.*receita|receita.*negocios?|business-revenue|revenue-business|businesses-revenue|segmentos?.*receita|receita.*segmentos?|revenue-segment|revenue-by-business)/i.test(clean)) return 'revenueSegment';
...
async function resolveInvestidor10StockIds({ ticker = '', html = '', timeoutMs = 4500 } = {}) {
  ...
  // v266: regressão de timeout. O modal de ação era travado porque a resolução de IDs
  // fazia até 10 chamadas sequenciais antes de responder. Como o contrato atual deve
  // priorizar o REST informado pelo usuário, limitamos a busca a endpoints de alta
  // confiança e em paralelo, sempre dentro de um orçamento curto.
  const candidates = [
  ...
  const perRequestTimeout = Math.min(1500, Math.max(750, Math.floor((Number(timeoutMs) || 2200) / 2)));
  const headers = {
    'User-Agent': process.env.VALORAE_USER_AGENT || 'Mozilla/5.0',
  ...
  const responses = await Promise.allSettled(candidates.map(async (url) => {
    ...
    return extractInvestidor10StockIdsFromPayload(json || raw, symbol);
  }));
  for (const settled of responses) {
    const ids = settled.status === 'fulfilled' ? settled.value : null;
...
async function fetchInvestidor10StockApiExtras({ ticker = '', html = '', timeoutMs = 4500 } = {}) {
  ...
  const responses = await Promise.all(tasks.slice(0, 24).map(async ([key, url]) => {
    const result = await fetchText(url, {
      timeoutMs: Math.min(2600, Math.max(900, Number(timeoutMs) || 2200)),
      ttlMs: 4 * 60 * 60 * 1000,
      staleMs: 24 * 60 * 60 * 1000,
      retries: 0,
      headers: {
...
function stockModalTimeoutValue(limitMs = 1000, fallback = null) {
  return new Promise(resolve => setTimeout(() => resolve(fallback), Math.max(250, Number(limitMs) || 1000)));
}

async function stockModalWithBudget(promise, limitMs = 1000, fallback = null) {
  return Promise.race([
    Promise.resolve(promise).catch(error => ({ ...(fallback && typeof fallback === 'object' ? fallback : {}), error: error?.message || String(error) })),
    stockModalTimeoutValue(limitMs, fallback)
  ]);
}
...
async function fetchInvestidor10StockBundle(ticker, timeoutMs = 6500) {
  ...
  const symbol = String(ticker || '').toUpperCase();
  const budgetMs = Math.min(7200, Math.max(3200, Number(timeoutMs) || 6500));
  const remaining = (reserveMs = 250, floorMs = 350) => Math.max(floorMs, budgetMs - (Date.now() - startedAt) - reserveMs);
  const url = `https://investidor10.com.br/acoes/${symbol.toLowerCase()}/`;

  const htmlPromise = fetchText(url, {
    timeoutMs: Math.min(4200, remaining(600, 1800)),
    ttlMs: 90_000,
  ...
  }).catch(error => ({ text: '', status: 0, cacheStatus: 'ERROR', error: error?.message || String(error), finalUrl: url }));

  // v266: iniciar o REST em paralelo ao HTML. Antes, o modal aguardava HTML + resolução
  // sequencial de IDs + dezenas de endpoints, extrapolando o timeout do APK.
  const apiExtrasPromise = fetchInvestidor10StockApiExtras({ ticker: symbol, html: '', timeoutMs: Math.min(3000, remaining(800, 1200)) })
  ...
  const htmlResult = await stockModalWithBudget(htmlPromise, Math.min(4300, remaining(700, 1800)), { text: '', status: 0, cacheStatus: 'TIMEOUT', error: 'html_timeout', finalUrl: url });
  const { text, status, cacheStatus, error, finalUrl } = htmlResult || {};
  const rawHtml = text || '';
  ...
  let apiExtras = await stockModalWithBudget(apiExtrasPromise, Math.min(3200, remaining(500, 900)), { embedded: {}, chartsFinanceiros: {}, rawJson: {}, apiStatus: [{ key: 'stockApiExtras', status: 0, ok: false, error: 'api_extras_timeout' }] });
  if (!apiExtras || typeof apiExtras !== 'object') apiExtras = { embedded: {}, chartsFinanceiros: {}, rawJson: {}, apiStatus: [] };
  // Se o HTML chegou rápido mas o REST paralelo não descobriu endpoints embutidos, fazemos
  // apenas uma tentativa curta usando o HTML; nunca bloqueia o modal inteiro.
  const hasApiData = Object.keys(apiExtras?.rawJson || {}).length > 0;
  if (html && !hasApiData && remaining(300, 0) > 900) {
    const htmlExtras = await stockModalWithBudget(
      fetchInvestidor10StockApiExtras({ ticker: symbol, html, timeoutMs: Math.min(1800, remaining(250, 800)) }),
      Math.min(1900, remaining(250, 800)),
      null
    );
    if (htmlExtras?.rawJson && Object.keys(htmlExtras.rawJson).length) apiExtras = htmlExtras;
  }
  ...
  const buyHoldRanking = html && remaining(200, 0) > 900
    ? await stockModalWithBudget(fetchInvestidor10StockBuyHoldRanking(symbol, Math.min(1200, remaining(150, 600))), Math.min(1300, remaining(150, 600)), null)
    : null;
  const historicalIndicatorSources = buildStockHistoricalIndicatorSources({ html, ticker: symbol, apiExtras });
  const historicalIndicators = buildStockHistoricalIndicators(historicalIndicatorSources, symbol, fundamentalIndicators);
  const checklist = html ? extractInvestidor10StockBuyHoldChecklist(html, symbol, { fundamentalIndicators, historicalIndicators, canonical, quickMetrics, buyHoldRanking }) : emptyStockBuyHoldChecklist(symbol, { reason: 'no_html' });
  ...
  const companyData = extractStockCompanyData(html, symbol, companyProfile?.name || extractTitleName(html, symbol));
  const companyInformation = extractStockCompanyInformation(html, symbol);
  ...
  const shareholdingPosition = buildStockShareholdingPayload({ html, canonical, ticker: symbol, raw: [
  ...
  const announcements = html && remaining(200, 0) > 900
    ? await stockModalWithBudget(buildStockAnnouncementsPayload({ ticker: symbol, html, timeoutMs: Math.min(1200, remaining(150, 600)) }), Math.min(1300, remaining(150, 600)), emptyStockAnnouncements(symbol, { reason: 'announcement_budget_timeout' }))
    : emptyStockAnnouncements(symbol, { reason: html ? 'announcement_budget_exhausted' : 'no_html' });
  ...
  return {
    ...
    name: companyProfile?.name || extractTitleName(html, symbol)
  };
}


function metricCard(id = '', label = '', value = '—', numericValue = null, source = 'Investidor10') {
  const cleanId = String(id || label || 'metric').replace(/[^A-Za-z0-9_]+/g, '_').replace(/^_+|_+$/g, '').toLowerCase() || 'metric';
  const numeric = Number(numericValue);
  return {
    id: cleanId,
    label: cleanText(label) || cleanId,
    value: cleanText(value) || '—',
    numericValue: Number.isFinite(numeric) ? round(numeric, 4) : null,
    source
  };
}
...
export async function buildStockModalContract(payload = {}) {
  ...
  const investidorFallback = { status: 0, error: 'stock_bundle_budget_timeout', html: '', quickMetrics: {}, canonical: {}, returnsRows: [], name: ticker, revenueByRegion: buildStockRevenueBreakdownPayload({ ticker, name: ticker }, 'region'), revenueByBusiness: buildStockRevenueBreakdownPayload({ ticker, name: ticker }, 'business') };
  const oneDayFallback = { ok: false, points: [], error: 'quote_1d_budget_timeout' };
  const oneYearFallback = { ok: false, points: [], error: 'quote_1y_budget_timeout' };
  const indexFallback = { id: 'stock_asset_vs_indices_selector', title: `Comparação de ${ticker} com índices`, status: 'ERROR', error: 'index_comparison_budget_timeout', periods: STOCK_COMPARISON_PERIODS, series: [], items: [], seriesByPeriod: {}, itemsByPeriod: {} };
  const [investidor10, oneDayHistory, oneYearHistory, indexComparison] = await Promise.all([
    stockModalWithBudget(
      fetchInvestidor10StockBundle(ticker, Math.min(timeoutMs, 7000)),
      Math.min(6800, Math.max(2400, timeoutMs - 700)),
      investidorFallback
    ),
    stockModalWithBudget(
      fetchYahooHistory(ticker, {
        range: '1D',
        interval: '5m',
        timeoutMs: Math.min(3600, timeoutMs),
        limit: 96,
        cache: true
      }),
      Math.min(3800, Math.max(1400, timeoutMs - 900)),
      oneDayFallback
    ),
    stockModalWithBudget(
      fetchYahooHistory(ticker, {
    ...
    stockModalWithBudget(
      buildStockIndexComparison(ticker, Math.min(timeoutMs, 4200)),
      Math.min(4300, Math.max(1800, timeoutMs - 700)),
      indexFallback
    )
  ]);
