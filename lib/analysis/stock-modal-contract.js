...
export const STOCK_MODAL_VERSION = '26.asset-modal.stock.v26';
...
async function fetchInvestidor10StockBundle(ticker, timeoutMs = 6500) {
  ...
  const apiHistoricalRaw = apiExtras?.rawJson?.historicoIndicadores || apiExtras?.chartsFinanceiros?.historicoIndicadores || apiExtras?.rawJson?.indicatorHistory || apiExtras?.rawJson?.historicalIndicators || {};
  const htmlHistoricalRaw = html ? extractInvestidor10StockHistoricalIndicatorsFromHtml(html, symbol) : {};
  const historicalIndicators = buildStockHistoricalIndicators([htmlHistoricalRaw, canonicalHistoricalRaw, apiHistoricalRaw], symbol, fundamentalIndicators);
