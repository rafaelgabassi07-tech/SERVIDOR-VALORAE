...
function normalizeRevenueCandidateRow(node = {}, index = 0, kind = 'region', source = 'Investidor10 distribuição de receitas API') {
  ...
  const rawLabel = cleanText(
    ...
    || node.region || node.regiao || node.região || node.geography || node.geografia || node.local || node.localidade || node.market || node.mercado || node.country || node.pais || node.país
    || node.segment || node.segmento || node.business || node.bussines || node.negocio || node.negócio || node.product || node.produto || node.category || node.categoria || node.activity || node.atividade || node.linhaNegocio || node.linha_negocio || node.businessLine || node.x || ''
  );
  ...
  const percentRaw = node.percentDisplay || node.percentualDisplay || node.percentageDisplay || node.shareDisplay || node.participationDisplay || node.participacaoDisplay || embeddedPercentDisplay || node.percentual || node.porcentual || node.porcentagem || node.percent || node.percentage || node.sharePercent || node.share || node.shareValue || node.revenueShare || node.yPercent || node.y_percentage || node.pct || node.participation || node.participacao || node.participação || node.custom?.percent || node.custom?.percentage || node.custom?.percentual || node.extra?.percent || node.meta?.percent || node.y;
  ...
  const amountRaw = node.amountDisplay || node.valorDisplay || node.valueDisplay || node.totalDisplay || node.receitaDisplay || node.revenueDisplay || node.formattedValue || node.formatted_value || node.displayValue || node.display_value || node.dataLabel || node.data_label || embeddedAmountDisplay || nestedRevenueValue(node, ['custom.amountDisplay','custom.valorDisplay','custom.valueDisplay','custom.receitaDisplay','custom.revenueDisplay','custom.value','custom.valor','extra.amountDisplay','extra.valorDisplay','meta.amountDisplay','meta.valorDisplay']) || node.amount || node.valor || node.value || node.total || node.receita || node.revenue || node.valorReceita || node.receitaLiquida || node.grossRevenue || node.netRevenue || node.custom?.amount || node.custom?.value || node.custom?.valor || node.custom?.receita || node.extra?.amount || node.meta?.amount;
...
async function fetchInvestidor10StockApiExtras({ ticker = '', html = '', timeoutMs = 4500 } = {}) {
  ...
  const responses = await Promise.all(tasks.slice(0, 72).map(async ([key, url]) => {
    const result = await fetchText(url, {
