...
export const STOCK_MODAL_VERSION = '26.asset-modal.stock.v55';
...
function forceFullModalPayload(payload = {}, defaultTimeoutMs = 12000) {
  ...
  const timeoutMs = Math.min(12000, Math.max(defaultTimeoutMs, Number.isFinite(requestedTimeout) ? requestedTimeout : 0));
  return {
    ...payload,
    stage: 'full',
    mode: 'full',
...
export async function buildStockModalContract(payload = {}) {
  ...
  const rawTicker = fullPayload.ticker || fullPayload.symbol || fullPayload.q || '';
  const ticker = normalizeTicker(rawTicker);
  return withAssetModalRuntime({
    ...
    ttlMs: 45_000,
    staleMs: 120_000,
    producer: () => buildStockModalContractFresh(fullPayload)
  });
}
