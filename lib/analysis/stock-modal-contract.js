...
export const STOCK_MODAL_VERSION = '26.asset-modal.stock.v17';
...
function buildStockPeerBaseRow(symbol = '', quickMetrics = {}, fundamentalIndicators = {}, source = 'Investidor10 indicadores fundamentalistas') {
...
function buildStockPeerCatalogFallbackRows(symbol = '', quickMetrics = {}, fundamentalIndicators = {}) {
  const requested = String(symbol || '').toUpperCase();
  const catalog = buildPeerCatalogEntries(requested, { max: 12, includeBase: true });
  const peers = Array.isArray(catalog?.peers) ? catalog.peers : [];
  if (!peers.length) return requested ? [buildStockPeerBaseRow(requested, quickMetrics, fundamentalIndicators, 'Investidor10 indicadores fundamentalistas')] : [];
  const ordered = [...peers].sort((a, b) => (a.ticker === requested ? -1 : b.ticker === requested ? 1 : a.ticker.localeCompare(b.ticker)));
  return ordered.map(peer => {
    if (peer.ticker === requested) {
      return {
        ...buildStockPeerBaseRow(requested, quickMetrics, fundamentalIndicators, 'Catálogo setorial Valorae + indicadores fundamentalistas do ativo-base'),
        sector: peer.sector,
    ...
    return {
      ...
      segment: peer.segment,
      peerGroup: peer.peerGroup,
      source: 'Catálogo setorial Valorae (mesmo grupo/segmento)'
    };
  });
}
...
function extractInvestidor10StockPeerComparison(html = '', ticker = '', quickMetrics = {}, fundamentalIndicators = {}) {
  ...
  if (rows.length < 2) {
    const fallbackRows = buildStockPeerCatalogFallbackRows(symbol, quickMetrics, fundamentalIndicators);
    if (fallbackRows.length > rows.length) rows.splice(0, rows.length, ...fallbackRows);
  }
  if (!rows.some(row => row.ticker === symbol)) {
    rows.unshift(buildStockPeerBaseRow(symbol, quickMetrics, fundamentalIndicators));
  }
  const bestMin = (field) => {
  ...
  const catalogRows = rows.filter(row => /Catálogo setorial Valorae/i.test(row.source || ''));
  return {
    id: 'stock_peer_comparison',
    title: 'Comparador de ações',
    subtitle: 'Pares setoriais do Investidor10; quando a tabela não vier no HTML, usa catálogo setorial restrito ao mesmo grupo.',
    filterLabel: 'Mesmo setor/segmento do ativo',
    source: catalogRows.length ? 'Investidor10 comparador de ações + catálogo setorial Valorae' : 'Investidor10 comparador de ações',
    status: rows.length >= 2 ? 'OK' : (rows.length ? 'PARTIAL' : 'EMPTY'),
    columns,
    rows: rows.slice(0, 12),
    diagnostics: catalogRows.length ? { fallback: 'catalog_same_peer_group', catalogRows: catalogRows.length } : undefined
  };
}
...
function buildStockRevenueBreakdownPayload({ html = '', canonical = {}, ticker = '', name = '' } = {}, kind = 'region') {
  ...
  return {
    ...
    title,
    status: items.length ? 'OK' : 'EMPTY',
    source: items.some(item => /API/i.test(item.source || '')) ? 'Investidor10 distribuição de receitas API' : (items.length && String(ticker || '').toUpperCase() === 'PETR4' ? 'Investidor10 distribuição de receitas PETR4' : 'Investidor10 distribuição de receitas'),
    selectedYear,
...
function parseStockShareholdingRowsFromHtml(html = '') {
  ...
  for (const tr of slice.matchAll(trRe)) {
    ...
    const normalized = normalizeShareholdingRow({ shareholder, onPercentDisplay: cells[1], pnPercentDisplay: cells[2], totalPercentDisplay: cells[3] }, rows.length, 'Investidor10 posição acionária HTML');
    if (normalized) rows.push(normalized);
  }
  const seen = new Set();
  return rows.filter(row => {
