...
export const STOCK_MODAL_VERSION = '26.asset-modal.stock.v27';
...
async function fetchInvestidor10StockApiExtras({ ticker = '', html = '', timeoutMs = 4500 } = {}) {
  ...
  if (ids.companyId) {
    ...
    addTask('historicoIndicadores', `${base}/api/balancos/indicadores/chart/${ids.companyId}/1825/`);
    addTask('historicoIndicadores', `${base}/api/balancos/indicadores/table/${ids.companyId}/3650/`);
    addTask('historicoIndicadores', `${base}/api/balancos/indicadores/table/${ids.companyId}/1825/`);
    addTask('historicoIndicadores', `${base}/api/balancos/indicadores/${ids.companyId}/3650/`);
    addTask('historicoIndicadores', `${base}/api/balancos/indicadores/${ids.companyId}/1825/`);
    addTask('historicoIndicadores', `${base}/api/acoes/historico-indicadores/${ids.companyId}/10`);
    addTask('historicoIndicadores', `${base}/api/acoes/historico-indicadores/${ids.companyId}/5`);
    ...
    if (ids.tickerId && symbol) {
      ...
      addTask('historicoIndicadores', `${base}/api/acoes/historico-indicadores/${ids.companyId}/${ids.tickerId}/${symbol}/3650`);
      addTask('historicoIndicadores', `${base}/api/acoes/historico-indicadores/${ids.companyId}/${ids.tickerId}/${symbol}/1825`);
    }
  }
  ...
  const responses = await Promise.all(tasks.slice(0, 32).map(async ([key, url]) => {
    const result = await fetchText(url, {
...
async function fetchInvestidor10StockBundle(ticker, timeoutMs = 6500) {
  ...
  const apiHistoricalSources = [
    apiExtras?.rawJson?.historicoIndicadores,
    ...(Array.isArray(apiExtras?.rawJson?.historicoIndicadoresSources) ? apiExtras.rawJson.historicoIndicadoresSources : []),
    apiExtras?.chartsFinanceiros?.historicoIndicadores,
    apiExtras?.rawJson?.indicatorHistory,
    apiExtras?.rawJson?.historicalIndicators,
    apiExtras?.rawJson?.fundamentalIndicatorHistory
  ].filter(Boolean);
  const htmlHistoricalRaw = html ? extractInvestidor10StockHistoricalIndicatorsFromHtml(html, symbol) : {};
  const historicalIndicators = buildStockHistoricalIndicators([htmlHistoricalRaw, canonicalHistoricalRaw, ...apiHistoricalSources], symbol, fundamentalIndicators);
