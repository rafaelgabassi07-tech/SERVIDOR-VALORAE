...
export const STOCK_MODAL_VERSION = '26.asset-modal.stock.v39';
...
function revenueYearFromCandidate(candidate = {}) {
  ...
  const direct = candidate.selectedYear || candidate.year || candidate.ano || candidate.period || candidate.periodo || candidate.label || candidate.name;
  const directMatch = String(direct || '').match(/\b(20\d{2})\b/);
  if (directMatch) return directMatch[1];
  const series = [candidate.datasets, candidate.series, candidate.dataSeries, candidate.chart?.datasets, candidate.chart?.series]
...
function rowsFromRevenueCandidate(candidate, kind = 'region', source = 'Investidor10 distribuição de receitas API') {
  ...
  const rows = [
    ...chartJsRevenueRowsFromCandidate(candidate, kind, source),
    ...highchartsRevenueRowsFromCandidate(candidate, kind, source),
    ...objectMapRevenueRowsFromCandidate(candidate, kind, source),
    ...tupleRevenueRowsFromCandidate(candidate, kind, source),
    ...pairedRows
  ];
  const walk = (node) => {
    if (!node) return;
...
function revenueTotalFromCandidate(candidate = {}) {
  ...
  const totalDisplay = cleanText(candidate.totalAmountDisplay || candidate.totalDisplay || candidate.totalValueDisplay || candidate.valorTotalDisplay || candidate.receitaTotalDisplay || candidate.revenueTotalDisplay || candidate.total || candidate.valorTotal || candidate.receitaTotal || candidate.revenueTotal || '');
...
function buildStockRevenueBreakdownPayload({ html = '', canonical = {}, ticker = '', name = '' } = {}, kind = 'region') {
  ...
  const canonicalCandidates = isRegion
    ? [canonical?.revenueRegion, canonical?.revenueGeography, canonical?.revenueByRegion, canonical?.revenueBreakdowns?.region, canonical?.revenueBreakdowns?.geography, canonical?.embedded?.revenueGeography, canonical?.embedded?.revenueRegion, canonical?.embedded?.revenueByRegion, embedded?.revenueGeography]
    : [canonical?.revenueBusiness, canonical?.revenueSegment, canonical?.revenueByBusiness, canonical?.businessRevenue, canonical?.segmentRevenue, canonical?.productRevenue, canonical?.revenueBreakdowns?.business, canonical?.revenueBreakdowns?.segment, canonical?.revenueBreakdowns?.byBusiness, canonical?.embedded?.revenueSegment, canonical?.embedded?.revenueBusiness, canonical?.embedded?.revenueByBusiness, canonical?.embedded?.businessRevenue, canonical?.embedded?.segmentRevenue, canonical?.embedded?.productRevenue, embedded?.revenueSegment, embedded?.businessRevenue, embedded?.segmentRevenue, embedded?.productRevenue];
