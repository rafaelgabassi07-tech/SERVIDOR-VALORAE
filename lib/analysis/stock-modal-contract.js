...
async function fetchInvestidor10StockBundle(ticker, timeoutMs = 6500, options = {}) {
  ...
  const identityApiStatus = rawHtml && !identityOk
    ...
    : [];
  const buyHoldRankingPromise = html && !fastMode
  ...
  const apiExtras = fastMode
    ? { embedded: {}, chartsFinanceiros: {}, rawJson: {}, apiStatus: [{ key: 'stockApiExtras', status: 204, ok: true, stage: 'fast', deferred: true }] }
    : await fetchInvestidor10StockApiExtras({ ticker: symbol, html, timeoutMs: Math.min(timeoutMs, 5200) })
...
async function buildStockModalContractFresh(payload = {}) {
  ...
  const timeoutMs = stockStageTimeoutMs(payload, fastMode ? 3800 : 8500);
  if (!ticker) {
  ...
  const [investidor10, oneDayHistory, oneYearHistory, indexComparison, yahooLogo] = await Promise.all([
    fetchInvestidor10StockBundle(ticker, Math.min(timeoutMs, fastMode ? 3200 : 7000), { mode: stage }).catch(error => ({ status: 0, error: error?.message || String(error), html: '', quickMetrics: {}, canonical: {}, returnsRows: [], name: ticker, apiStatus: [{ key: 'stockBundle', ok: false, error: error?.message || String(error), stage }] })),
    fetchYahooHistory(ticker, {
      range: '1D',
      interval: '5m',
      timeoutMs: Math.min(3800, Math.max(timeoutMs, 3800)),
      limit: 96,
      cache: true
    }).catch(error => ({ ok: false, points: [], error: error?.message || String(error) })),
    fastMode
    ...
    fetchYahooLogo(ticker, { timeoutMs: Math.min(3800, Math.max(timeoutMs, 3800)), cache: true })
