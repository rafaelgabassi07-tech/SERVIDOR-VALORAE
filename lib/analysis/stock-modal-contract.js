...
async function fetchInvestidor10StockApiExtras({ ticker = '', html = '', timeoutMs = 4500 } = {}) {
  ...
  const dedicatedHistoricalPromise = fetchInvestidor10StockHistoricalIndicatorsRaw({ ticker: symbol, html: html || '', ids, timeoutMs: Math.min(timeoutMs, 5200) })
  ...
  const [dedicatedHistorical, responses] = await Promise.all([
    dedicatedHistoricalPromise,
    Promise.all(tasks.slice(0, 72).map(async ([key, url]) => {
      const result = await fetchText(url, {
...
async function buildStockModalContractFresh(payload = {}) {
  ...
  const [investidor10, oneDayHistory, oneYearHistory, indexComparison, yahooLogo] = await Promise.all([
    ...
    fetchYahooLogo(ticker, { timeoutMs: Math.min(2600, timeoutMs), cache: true })
