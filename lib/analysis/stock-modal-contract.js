...
export const STOCK_MODAL_VERSION = '26.asset-modal.stock.v49';
...
function buildStockRevenueBreakdownPayload({ html = '', canonical = {}, ticker = '', name = '' } = {}, kind = 'region') {
  ...
  let rejectedItems = 0;
  let items = parseStockRevenueRowsFromSection(section, year).filter(row => !looksLikeInvalidStockRevenueLabel(row.label));
  const htmlQuality = stockRevenueRowsQuality(items, kind);
  rejectedItems += htmlQuality.invalidCount;
  items = htmlQuality.accepted ? htmlQuality.rows : [];
