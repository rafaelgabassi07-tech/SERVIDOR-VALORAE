...
export const STOCK_MODAL_VERSION = '26.asset-modal.stock.v46';
...
async function fetchInvestidor10StockBundle(ticker, timeoutMs = 6500) {
  ...
  const identityApiStatus = rawHtml && !identityOk
    ...
    : [];
  const apiExtras = await fetchInvestidor10StockApiExtras({ ticker: symbol, html, timeoutMs: Math.min(timeoutMs, 5200) })
  ...
  const buyHoldRanking = html ? await fetchInvestidor10StockBuyHoldRanking(symbol, Math.min(timeoutMs, 3600)).catch(error => ({ ticker: symbol, status: 'ERROR', source: 'Investidor10 ranking buy and hold', error: error?.message || String(error) })) : null;
