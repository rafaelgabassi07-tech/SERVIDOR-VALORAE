...
function revenueLabelFromNode(node = {}, kind = 'region') {
  const aliases = kind === 'business'
    ? ['label','name','nome','title','titulo','segment','segmento','business','negocio','negócio','produto','product','category','categoria','activity','atividade','description','descricao']
    : ['label','name','nome','title','titulo','region','regiao','região','geography','geografia','local','country','pais','país','mercado','description','descricao'];
...
async function fetchInvestidor10StockBundle(ticker, timeoutMs = 6500) {
  ...
  const identityApiStatus = rawHtml && !identityOk
    ...
    : [];
  const buyHoldRankingPromise = html
  ...
  const announcementsPromise = buildStockAnnouncementsPayload({ ticker: symbol, html, timeoutMs });
  ...
  const buyHoldRanking = await buyHoldRankingPromise;
  ...
  const announcements = await announcementsPromise.catch(error => emptyStockAnnouncements(symbol, { reason: 'stock_announcements_failed', error: error?.message || String(error) }));
...
export async function buildStockModalContract(payload = {}) {
  ...
  return withAssetModalRuntime({
    ...
    ttlMs: 45_000,
    staleMs: 120_000,
    producer: () => buildStockModalContractFresh(payload)
  });
}
