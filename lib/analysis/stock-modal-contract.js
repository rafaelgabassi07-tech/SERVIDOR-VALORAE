...
export const STOCK_MODAL_VERSION = '26.asset-modal.stock.v51';
  ...
  for (const serie of series) {
    ...
    for (const point of data) {
      if (Array.isArray(point)) {
        ...
        const explicitPercent = point.percent ?? point.percentage ?? point.percentual ?? point.porcentagem ?? point.share ?? point.participation ?? point.participacao ?? point.percentDisplay ?? point.percentualDisplay;
        const rawValue = point.y ?? point.value ?? point.valor ?? point.amount ?? point.receita ?? point.revenue ?? explicitPercent;
        entries.push({ ...point, label, rawValue, explicitPercent, amountDisplay: point.amountDisplay || point.valorDisplay || point.valueDisplay || point.receitaDisplay || point.revenueDisplay || point.formattedValue || point.dataLabel, percentDisplay: point.percentDisplay || point.percentualDisplay || point.percentageDisplay });
      }
    }
  }
  ...
  for (const dataset of datasets) {
    ...
    const entries = labels.map((label, index) => {
      ...
      if (rawValue && typeof rawValue === 'object') {
        return {
          ...rawValue,
          label: rawValue.name || rawValue.label || label,
          rawValue: rawValue.y ?? rawValue.value ?? rawValue.valor ?? rawValue.amount ?? rawValue.receita ?? rawValue.revenue ?? rawValue.percent ?? rawValue.percentage,
          explicitPercent: rawValue.percent ?? rawValue.percentage ?? rawValue.percentual ?? rawValue.share,
          amountDisplay: rawValue.amountDisplay || rawValue.valorDisplay || rawValue.valueDisplay || rawValue.receitaDisplay || rawValue.revenueDisplay || rawValue.formattedValue || amountDisplays[index] || revenueAmountDisplayFromCandidate(candidate, index),
          percentDisplay: rawValue.percentDisplay || rawValue.percentualDisplay || rawValue.percentageDisplay
        };
      }
...
async function fetchInvestidor10StockHistoricalIndicatorsRaw({ ticker = '', html = '', ids = {}, timeoutMs = 6500 } = {}) {
  ...
  for (const candidate of candidates) {
    const result = await fetchText(candidate.url, {
      timeoutMs: Math.min(5200, Math.max(2200, Number(timeoutMs) || 5200)),
      ttlMs: 15 * 60 * 1000,
      staleMs: 24 * 60 * 60 * 1000,
      retries: 1,
      headers: {
    ...
    if (json) {
      ...
      if ((normalized?.rows || []).length) rawSources.push(normalized);
    }
    ...
    if (score >= 120 && (bestNormalized?.periods || []).includes('5y')) break;
  }
  return {
...
async function fetchInvestidor10StockApiExtras({ ticker = '', html = '', timeoutMs = 4500 } = {}) {
  ...
  const dedicatedHistorical = await fetchInvestidor10StockHistoricalIndicatorsRaw({ ticker: symbol, html: html || '', ids, timeoutMs: Math.min(timeoutMs, 6200) })
  ...
  const responses = await Promise.all(tasks.slice(0, 48).map(async ([key, url]) => {
    const result = await fetchText(url, {
...
async function buildStockModalContractFresh(payload = {}) {
  ...
  const [investidor10, oneDayHistory, oneYearHistory, indexComparison, yahooLogo] = await Promise.all([
  ...
  return {
    ...
    commodityComparison,
    companyProfile: {
      ...withResolvedStockCompanyProfile(investidor10?.companyProfile || extractStockCompanyProfile('', ticker, name, quick, fundamentalIndicators), { priceDisplay: resolvedPriceDisplay, variation12mDisplay: resolvedVariation12mDisplay !== '—' ? resolvedVariation12mDisplay : '' }),
      logoUrl: resolvedLogoUrl,
      logoSource: yahooLogo?.logoUrl ? 'Yahoo Finance Quote API' : (investidor10?.companyProfile?.logoUrl ? investidor10.companyProfile.source || 'Investidor10' : '')
    },
