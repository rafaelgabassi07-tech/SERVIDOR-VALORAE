...
export const STOCK_MODAL_VERSION = '26.asset-modal.stock.v13';
...
function buildStockPayoutChartPayload({ ticker = '', canonical = {}, historicalIndicators = {}, dividendHistory = {} } = {}) {
  ...
  const points = orderedYears.map(year => {
    ...
    return {
      ...
      source: 'Investidor10 Payout de Petrobras'
    };
  ...
  return {
    ...
    title: `Payout de ${String(canonical?.name || canonical?.company?.name || 'Petrobras').replace(/\s*\([^)]*\)\s*/g, '').trim() || String(ticker || '').toUpperCase()}`,
    ticker: String(ticker || '').toUpperCase(),
    status: points.length ? 'OK' : 'EMPTY',
...
async function buildStockIndexComparison(ticker, timeoutMs = 8500) {
  ...
  for (const period of STOCK_COMPARISON_PERIODS) {
    ...
    const series = [...yahooSeries, ...macro].filter(Boolean);
  ...
  const sortedSeriesByPeriod = Object.fromEntries(Object.entries(seriesByPeriod).map(([key, series]) => [key, [...series].sort((a, b) => order.indexOf(a.code) - order.indexOf(b.code))]));
...
async function buildStockCommodityComparison(ticker, companyName = '', timeoutMs = 8500) {
  ...
  return {
    ...
    sourcePolicy: 'Comparação da ação com Petróleo Brent usa Yahoo Finance Chart API para o ticker da ação e para Brent Futures BZ=F.'
  };
}
...
function extractStockCompanyLogoUrl(html = '') {
  const candidates = [];
  for (const match of String(html || '').matchAll(/<img[^>]+(?:alt|title)=["'][^"']*(?:Petrobras|PETR4|logo)[^"']*["'][^>]*>/gi)) {
    const src = match[0].match(/\bsrc=["']([^"']+)["']/i)?.[1];
    if (src) candidates.push(src);
  }
  const og = String(html || '').match(/<meta[^>]+property=["']og:image["'][^>]+content=["']([^"']+)["']/i)?.[1];
  if (og) candidates.push(og);
  const raw = candidates.find(url => /^https?:\/\//i.test(url)) || '';
  return raw;
}
...
function extractStockCompanyProfile(html = '', ticker = '', name = '', quickMetrics = {}, fundamentalIndicators = {}) {
  ...
  const titleName = (name && name !== symbol ? name : 'Petróleo Brasileiro S.A. Petrobras');
  return {
    id: 'stock_company_profile',
    title: `Sobre a empresa ${titleName}`.trim(),
    ticker: symbol,
    name: titleName,
    logoUrl: extractStockCompanyLogoUrl(html),
...
function buildStockRevenueBreakdownPayload({ html = '', canonical = {}, ticker = '', name = '' } = {}, kind = 'region') {
  ...
  return {
    ...
    status: items.length ? 'OK' : 'EMPTY',
    source: 'Investidor10 distribuição de receitas',
    selectedYear: year || 'Atual',
    years: year ? [year] : [],
    totalLabel: total.totalLabel || 'Total',
    totalAmountDisplay: total.totalAmountDisplay || '—',
...
async function fetchInvestidor10StockBundle(ticker, timeoutMs = 6500) {
  ...
  const fundamentalIndicators = html ? extractInvestidor10StockFundamentalIndicators(html, symbol, quickMetrics) : extractInvestidor10StockFundamentalIndicators('', symbol, quickMetrics);
...
export async function buildStockModalContract(payload = {}) {
  ...
  const [indexComparison, commodityComparison] = await Promise.all([
    buildStockIndexComparison(ticker, timeoutMs).catch(error => ({ id: 'stock_asset_vs_indices_selector', title: `Comparação de ${ticker} com índices`, status: 'ERROR', error: error?.message || String(error), periods: STOCK_COMPARISON_PERIODS, series: [], items: [], seriesByPeriod: {}, itemsByPeriod: {} })),
    buildStockCommodityComparison(ticker, investidor10?.name || ticker, timeoutMs).catch(error => ({ id: 'stock_asset_vs_brent_oil', title: 'Comparando Petrobras com Petróleo Brent', status: 'ERROR', error: error?.message || String(error), periods: STOCK_COMPARISON_PERIODS, series: [], items: [], seriesByPeriod: {}, itemsByPeriod: {} }))
  ]);
  ...
  return {
    ...
    sources: [
      ...
      { id: 'yahoo_indices', role: 'comparacao_indices_ifix_idiv_smll_ibov_ivvb11_e_brent' }
    ],
    quoteSummary: {
      ticker,
      name,
    ...
    diagnostics: {
      ...
      commodityComparisonStatus: commodityComparison?.status || 'EMPTY',
