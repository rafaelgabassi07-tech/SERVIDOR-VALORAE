...
export const STOCK_MODAL_VERSION = '26.asset-modal.stock.v53';
...
function normalizeRevenueCandidateRow(node = {}, index = 0, kind = 'region', source = 'Investidor10 distribuição de receitas API') {
  ...
  const amountRaw = node.amountDisplay || node.valorDisplay || node.valueDisplay || node.totalDisplay || node.receitaDisplay || node.revenueDisplay || node.formattedValue || node.formatted_value || node.displayValue || node.display_value || node.dataLabel || node.data_label || embeddedAmountDisplay || nestedRevenueValue(node, ['custom.amountDisplay','custom.valorDisplay','custom.valueDisplay','custom.receitaDisplay','custom.revenueDisplay','custom.value','custom.valor','custom.valorReceita','custom.receitaBruta','custom.netRevenue','extra.amountDisplay','extra.valorDisplay','extra.valorReceita','meta.amountDisplay','meta.valorDisplay','meta.valorReceita']) || node.amount || node.valor || node.value || node.total || node.receita || node.revenue || node.valorReceita || node.receitaValor || node.vlReceita || node.receitaBruta || node.receitaLiquida || node.grossRevenue || node.netRevenue || node.totalRevenue || node.custom?.amount || node.custom?.value || node.custom?.valor || node.custom?.receita || node.extra?.amount || node.meta?.amount;
...
async function fetchInvestidor10StockApiExtras({ ticker = '', html = '', timeoutMs = 4500 } = {}) {
  ...
  if (!apiExtras.rawJson.revenueGeography || !apiExtras.rawJson.revenueSegment || !revenueRowsExistInRawJson(apiExtras.rawJson, 'region') || !revenueRowsExistInRawJson(apiExtras.rawJson, 'business')) {
    const beforeRevenueSources = stockRevenueSourceCount(apiExtras.rawJson);
    const seenUrls = new Set(tasks.map(([, taskUrl]) => taskUrl));
    const secondary = await fetchMissingInvestidor10StockRevenueSources({ apiExtras, ticker: symbol, ids, html, timeoutMs, seenUrls })
  ...
  if (!revenueRowsExistInRawJson(apiExtras.rawJson, 'region') || !revenueRowsExistInRawJson(apiExtras.rawJson, 'business')) {
    const jsSeenUrls = new Set(tasks.map(([, taskUrl]) => taskUrl));
    const jsDiscovery = await fetchInvestidor10StockRevenueSourcesFromScripts({ apiExtras, ticker: symbol, ids, html, timeoutMs, seenUrls: jsSeenUrls })
      .catch(error => ({ attempted: 0, error: error?.message || String(error) }));
    apiExtras.diagnostics.stockRevenueJsDiscovery = jsDiscovery;
  }
  return apiExtras;
}
