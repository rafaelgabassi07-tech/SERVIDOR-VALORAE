...
function stockModalQualitySections(payload = {}) {
  ...
  return [
    ...
    ['financialCharts', revenueProfitReady && profitQuoteReady && equityEvolutionReady],
    ['financialStatements', hasRenderableRows(payload.resultsStatement?.rows) || hasRenderableRows(payload.balanceSheetStatement?.rows) || hasRenderableTableMap(payload.resultsStatement?.tablesByPeriod) || hasRenderableTableMap(payload.balanceSheetStatement?.tablesByPeriod)],
    ['announcements', countArray(payload.announcements?.items) > 0],
    ['returns', countArray(payload.returns?.rows) > 0]
  ];
}
...
function modalSectionArrival(payload = {}, family = '') {
  ...
  return sections.map(([id, ready]) => {
    ...
    if (!ready && resolvedFamily === 'fii' && id === 'propertyPortfolio' && isFiiWithoutPhysicalProperties(payload)) {
