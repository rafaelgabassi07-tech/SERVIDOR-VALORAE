...
function requestedCriticalSections(payload = {}, family = '') {
  ...
  const supplied = [
    ...parseSectionList(payload.requiredSections),
    ...parseSectionList(payload.missingSections),
    ...parseSectionList(payload.knownMissingSections),
    ...parseSectionList(payload.deferredSections)
  ];
  const defaults = resolvedFamily === 'fii' ? FII_CRITICAL_SECTIONS : STOCK_CRITICAL_SECTIONS;
  const expanded = expandSectionAliases(supplied);
  const allowed = new Set((resolvedFamily === 'fii' ? fiiModalSections(payload) : stockModalSections(payload)).map(([id]) => id));
  const filtered = expanded.filter(id => allowed.has(id));
  return [...new Set(filtered.length ? filtered : defaults)];
}
...
function buildModalDelivery(payload = {}, runtime = {}) {
  ...
  return {
    ...
    unavailableSections: isFinal ? missingSections.filter(id => !critical.requiredSections.includes(id)) : [],
...
function recoveryCacheUpgrade(cachePayload = {}, family = '', requestPayload = {}) {
  ...
  const upgraded = resolvedMissingSections.length > 0 || profile.completeForDelivery ||
