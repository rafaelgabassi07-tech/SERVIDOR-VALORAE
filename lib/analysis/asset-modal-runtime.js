...
export const ASSET_MODAL_RUNTIME_VERSION = '26.asset-modal.runtime.v7-delivery-contract-cancellable';
...
function stockModalSections(payload = {}) {
  return [
    ...
    ['metrics', countArray(payload.metrics) > 0],
    ...
    ['company', countArray(payload.companyProfile?.facts) > 0 || countArray(payload.companyProfile?.sections) > 0 || countArray(payload.companyData?.facts) > 0 || countArray(payload.companyInformation?.facts) > 0],
...
export async function withAssetModalRuntime({ family, ticker, payload = {}, ttlMs = 45_000, staleMs = 120_000, producer }) {
  ...
  if (!refresh) {
    ...
    if (fullCached?.value && fullCached.status === 'HIT' && modalPayloadHasUsefulData(fullCached.value)) {
      return decorateModalPayload(fullCached.value, {
        family,
        requestedMode,
        requestId,
    ...
    if (!staleSnapshot && fullCached?.value && fullCached.status === 'STALE' && modalPayloadHasUsefulData(fullCached.value)) {
  ...
  return coalesce(key, async () => {
    if (!refresh) {
      ...
      if (!staleSnapshot && joinedFullCached?.value && joinedFullCached.status === 'STALE' && modalPayloadHasUsefulData(joinedFullCached.value)) {
