...
import { STOCK_MODAL_CRITICAL_SECTIONS, FII_MODAL_CRITICAL_SECTIONS, canonicalModalSections } from './asset-modal-sections.js';

export const ASSET_MODAL_RUNTIME_VERSION = '27.asset-modal.runtime.v20-contract-batching';
...
function assetModalDeadlineMs(payload = {}) {
  ...
  const fallback = stage === 'fast' ? 3800 : 22000;
  ...
  // O preview fast continua curto. O full precisa acomodar as APIs históricas reais
  // (comparador de FIIs, payout e índices) sem devolver um contrato vazio aos 12 s.
  return Math.min(23000, Math.max(9000, safe));
}
...
function hasResolvedNumericValue(value) {
  if (value === null || value === undefined) return false;
  if (typeof value === 'string' && !value.trim()) return false;
  return Number.isFinite(Number(value));
}

function hasUsefulFiiPeerComparison(peerComparison = {}) {
  const rows = Array.isArray(peerComparison?.rows) ? peerComparison.rows : [];
  if (!rows.length) return false;
  const usable = rows.filter(row => {
    const ticker = String(row?.ticker || '').trim();
    if (!ticker) return false;
    return hasResolvedNumericValue(row?.dividendYield) ||
      hasResolvedNumericValue(row?.pvp) ||
      hasResolvedNumericValue(row?.patrimonialValue) ||
      hasMeaningfulDisplayValue(row?.dividendYieldDisplay) ||
      hasMeaningfulDisplayValue(row?.pvpDisplay) ||
      hasMeaningfulDisplayValue(row?.patrimonialValueDisplay) ||
      hasMeaningfulDisplayValue(row?.fundType) ||
      hasMeaningfulDisplayValue(row?.segment);
  }).length;
  // Patrimônio por cota é enriquecimento opcional. O comparador já é útil quando duas linhas
  // possuem indicadores/tipo/segmento reais, portanto esse enriquecimento não pode bloquear
  // a entrega inteira do bloco.
  const minimum = rows.length > 1 ? Math.min(2, rows.length) : 1;
...
function hasMeaningfulValueItems(value) {
  return Array.isArray(value) && value.some(item =>
    ...
    hasResolvedNumericValue(item?.numericValue)
  );
}

function hasResolvedChecklistItems(value) {
  const items = Array.isArray(value?.items) ? value.items : [];
  const checklistId = String(value?.id || '').toLowerCase();
  const declaredTotal = Number(value?.total);
  const expectedTotal = checklistId.includes('stock')
    ? 10
    : (checklistId.includes('fii') ? 8 : (Number.isFinite(declaredTotal) && declaredTotal > 0 ? declaredTotal : items.length));
  if (!expectedTotal || items.length < expectedTotal) return false;
  return items.slice(0, expectedTotal).every(item =>
...
function hasRenderableRow(row = {}) {
  if (!row || typeof row !== 'object') return false;
  const values = row.values && typeof row.values === 'object' ? Object.values(row.values) : [];
  if (values.some(hasMeaningfulDisplayValue)) return true;
  const candidateKeys = ['value', 'displayValue', 'numericValue', 'amount', 'primaryValue', 'secondaryValue', 'current', 'latest'];
  return candidateKeys.some(key => hasMeaningfulDisplayValue(row[key]) || hasResolvedNumericValue(row[key]));
}

function hasRenderableRows(value) {
  return Array.isArray(value) && value.some(row => hasRenderableRow(row));
}

function hasRenderableTableMap(value) {
  if (!value || typeof value !== 'object') return false;
  return Object.values(value).some(table => {
    if (Array.isArray(table)) return hasRenderableRows(table);
    if (!table || typeof table !== 'object') return false;
    if (hasRenderableRows(table.rows) || hasRenderableRows(table.items)) return true;
    return hasRenderableTableMap(table.tablesByPeriod || table.tables || table.periods);
  });
}

function hasRealSeriesPoints(points, minimum = 2) {
  ...
  const valid = points.filter(point => {
    ...
    const value = point.value ?? point.close ?? point.price ?? point.returnPercent ?? point.accumulatedReturn ?? point.y;
    const timestamp = point.timestamp ?? point.date ?? point.period ?? point.x;
    return (Number.isFinite(Number(value)) || hasMeaningfulDisplayValue(value)) && timestamp !== undefined && timestamp !== null && String(timestamp).trim() !== '';
  });
  return valid.length >= minimum;
}

function hasRealSeriesList(value, minimum = 2) {
  if (!Array.isArray(value)) return false;
  if (hasRealSeriesPoints(value, minimum)) return true;
  return value.some(series => hasRealSeriesPoints(series?.points || series?.values || series?.data, minimum));
}

function hasRealSeriesByPeriod(value, minimum = 2) {
  if (!value || typeof value !== 'object') return false;
  return Object.values(value).some(period => {
    if (hasRealSeriesList(period, minimum)) return true;
    if (!period || typeof period !== 'object') return false;
    return hasRealSeriesList(period.series, minimum) ||
      hasRealSeriesList(period.items, minimum) ||
      hasRealSeriesList(period.points, minimum) ||
      hasRealSeriesByPeriod(period.seriesByPeriod || period.periods, minimum);
  });
}

function hasRealIndexComparison(value = {}) {
  return hasRealSeriesList(value?.series, 2) || hasRealSeriesByPeriod(value?.seriesByPeriod, 2);
}

function normalizedDiagnosticsReason(value = {}) {
  return String(value?.diagnostics?.reason || value?.diagnostics?.status || value?.reason || '').trim().toLowerCase();
}

function sectionBlockStatus(value = {}) {
  const status = String(value?.status || '').trim().toUpperCase();
  const reason = normalizedDiagnosticsReason(value);
  if (['ERROR', 'TIMEOUT'].includes(status) || value?.diagnostics?.error) return 'ERROR';
  if (/deferred|warming|not_requested|no_html|bundle_missing|timeout|failed|route_error/.test(reason)) return 'DEFERRED';
  if (status === 'EMPTY' && /section_not_found|communications_route_empty|no_announcements/.test(reason)) return 'EMPTY_CONFIRMED';
  if (status === 'EMPTY') return 'EMPTY_UNCONFIRMED';
  if (['OK', 'SUCCESS', 'READY'].includes(status)) return 'READY';
  return 'MISSING';
}

function isFiiWithoutPhysicalProperties(payload = {}) {
  const text = JSON.stringify({
    aboutFund: payload.aboutFund,
    infoSections: payload.infoSections,
    metrics: payload.metrics,
    quoteSummary: payload.quoteSummary
  }).normalize?.('NFD').replace?.(/[\u0300-\u036f]/g, '').toLowerCase?.() || '';
  return /fii de papel|fundo de papel|recebiveis|recebivel|certificados de recebiveis|\bcri\b|fundo de fundos|\bfof\b/.test(text);
}
...
function stockModalSections(payload = {}) {
  ...
  const dividendRadarReady = ['OK', 'PARTIAL'].includes(String(payload.dividendRadar?.status || '').toUpperCase()) &&
  ...
  const resultsReady = hasRenderableRows(payload.resultsStatement?.rows) || hasRenderableTableMap(payload.resultsStatement?.tablesByPeriod);
  const balanceReady = hasRenderableRows(payload.balanceSheetStatement?.rows) || hasRenderableTableMap(payload.balanceSheetStatement?.tablesByPeriod);
  return [
    ...
    ['historicalIndicators', hasRenderableRows(payload.historicalIndicators?.rows) || hasRenderableTableMap(payload.historicalIndicators?.tablesByPeriod)],
    ...
    ['payoutChart', payoutReady],
    // Dividendos e payout são blocos independentes no APK. O grupo de dividendos
    // representa somente histórico + radar; payout possui seu próprio section id.
    ['dividends', dividendHistoryReady && dividendRadarReady],
    ['peerComparison', countArray(payload.peerComparison?.rows) > 0],
    ['indexComparison', hasRealIndexComparison(payload.indexComparison)],
...
function stockModalQualitySections(payload = {}) {
  ...
  return [
    ...
    ['equityEvolutionChart', equityEvolutionReady],
    ['checklist', hasResolvedChecklistItems(payload.checklist)],
    ['payoutChart', countArray(payload.payoutChart?.points) > 0],
    ['dividends', dividendHistoryReady && dividendRadarReady],
    ['peerComparison', countArray(payload.peerComparison?.rows) > 0],
    ['indexComparison', hasRealIndexComparison(payload.indexComparison)],
    ['company', companyProfileReady && companyDataReady && companyInformationReady],
...
function fiiModalSections(payload = {}) {
  return [
    ...
    ['chart', countArray(payload.chart?.points) >= 2],
    ['metrics', hasMeaningfulValueItems(payload.metrics)],
    ['indexComparison', hasRealIndexComparison(payload.comparison)],
    ['peerComparison', hasUsefulFiiPeerComparison(payload.peerComparison)],
    ['checklist', countArray(payload.checklist?.items) > 0],
...
const STOCK_CRITICAL_SECTIONS = STOCK_MODAL_CRITICAL_SECTIONS;
const FII_CRITICAL_SECTIONS = FII_MODAL_CRITICAL_SECTIONS;
...
const SECTION_ALIASES = Object.freeze({
  ...
  dividends: ['dividendHistory', 'dividendRadar']
});
...
function criticalSectionStatus(payload = {}, family = '', requestPayload = {}) {
  const arrival = modalSectionArrival(payload, family);
  const settled = new Set(arrival.filter(section => section.settled).map(section => section.id));
  const requiredSections = requestedCriticalSections(requestPayload, family);
  const missingRequiredSections = requiredSections.filter(id => !settled.has(id));
  return { requiredSections, missingRequiredSections, arrival };
}
...
function sectionPayloadFieldMap(family = '') {
  ...
  return {
    ...
    historicalIndicators: ['historicalIndicators'], checklist: ['checklist'], dividends: ['dividendHistory', 'dividendRadar'],
    ...
    revenueProfitChart: ['revenueProfitChart'], profitQuoteChart: ['profitQuoteChart'],
    equityEvolutionChart: ['equityEvolutionChart'],
    financialStatements: ['resultsStatement', 'balanceSheetStatement'], announcements: ['announcements'], returns: ['returns']
  };
}

function modalSectionArrival(payload = {}, family = '') {
  const resolvedFamily = String(family || payload.assetType || '').toLowerCase().includes('fii') || payload.aboutFund || payload.propertyPortfolio
    ? 'fii'
    : 'stock';
  const sections = resolvedFamily === 'fii' ? fiiModalSections(payload) : stockModalSections(payload);
  const fields = sectionPayloadFieldMap(resolvedFamily);
  const sourceFallback = value => value?.source || value?.sourceName || undefined;
  const sourceUrlFallback = value => value?.sourceUrl || value?.url || undefined;
  return sections.map(([id, ready]) => {
    const values = (fields[id] || [id]).map(field => payload[field]).filter(value => value !== undefined && value !== null);
    const statuses = values.map(sectionBlockStatus);
    const firstSource = values.find(value => sourceFallback(value));
    const firstSourceUrl = values.find(value => sourceUrlFallback(value));
    let status = ready ? 'READY' : 'MISSING';
    if (!ready && resolvedFamily === 'fii' && ['propertyPortfolio', 'vacancyHistory'].includes(id) && isFiiWithoutPhysicalProperties(payload)) {
      // FIIs de papel/recebíveis não possuem portfólio físico nem vacância imobiliária.
      // Marcar ambos como não aplicáveis evita settlement/recovery infinito de uma seção que
      // nunca poderá ser preenchida para esta classe de fundo.
      status = 'NOT_APPLICABLE';
    } else if (!ready && id === 'announcements' && statuses.includes('EMPTY_CONFIRMED')) {
      status = 'EMPTY_CONFIRMED';
    } else if (!ready && statuses.includes('ERROR')) {
      status = 'ERROR';
    } else if (!ready && statuses.includes('DEFERRED')) {
      status = 'DEFERRED';
    } else if (!ready && statuses.includes('EMPTY_UNCONFIRMED')) {
      status = 'EMPTY_UNCONFIRMED';
    }
    const settled = ['READY', 'EMPTY_CONFIRMED', 'NOT_APPLICABLE'].includes(status);
    return {
      id,
      status,
      ready: status === 'READY',
      settled,
      retryable: !settled,
      source: firstSource ? sourceFallback(firstSource) : undefined,
      sourceUrl: firstSourceUrl ? sourceUrlFallback(firstSourceUrl) : undefined,
      reasons: [...new Set(values.map(normalizedDiagnosticsReason).filter(Boolean))]
    };
  });
}

function modalSectionArrivalSummary(payload = {}, family = '') {
  const sections = modalSectionArrival(payload, family);
  const byStatus = {};
  for (const section of sections) byStatus[section.status] = (byStatus[section.status] || 0) + 1;
  return {
    status: sections.every(section => section.settled) ? 'SETTLED' : sections.some(section => section.ready) ? 'PARTIAL' : 'EMPTY',
    readyCount: sections.filter(section => section.ready).length,
    settledCount: sections.filter(section => section.settled).length,
...
function requestedRecoverySections(requestPayload = {}, family = '') {
  const allowed = new Set(canonicalModalSections(family));
  const requested = expandSectionAliases(parseSectionList(
    requestPayload.requestedRecoverySections ??
    requestPayload.knownMissingSections ??
    requestPayload.missingSections ??
    requestPayload.deferredSections ??
    ''
  ));
...
function buildModalDelivery(payload = {}, runtime = {}) {
  ...
  const arrival = modalSectionArrival(payload, family);
  const availableSections = arrival.filter(section => section.ready).map(section => section.id);
  const settledSections = arrival.filter(section => section.settled).map(section => section.id);
  const emptyConfirmedSections = arrival.filter(section => section.status === 'EMPTY_CONFIRMED').map(section => section.id);
  const notApplicableSections = arrival.filter(section => section.status === 'NOT_APPLICABLE').map(section => section.id);
  const failedSections = arrival.filter(section => section.status === 'ERROR').map(section => section.id);
  const missingSections = arrival.filter(section => !section.settled).map(section => section.id);
  ...
  const recoverableSections = canonicalModalSections(quality.family);
  const requestedRecovery = requestedRecoverySections(runtime.requestPayload || runtime.payload || {}, quality.family);
  const resolvedRecoverySections = requestedRecovery.filter(id => settledSections.includes(id));
  const remainingRecoverySections = requestedRecovery.filter(id => !settledSections.includes(id));
  ...
  return {
    ...
    failedSections,
    sectionStates: Object.fromEntries(arrival.map(section => [section.id, section.status])),
    recoverableSections,
    criticalSections: critical.requiredSections,
    requestedRecoverySections: requestedRecovery,
    resolvedRecoverySections,
    remainingRecoverySections,
    ...
    unavailableSections: [...new Set([...emptyConfirmedSections, ...notApplicableSections, ...(isFinal ? optionalMissingSections : [])])],
    ...
    maxRecoverySectionsPerRequest: quality.family === 'fii' ? 5 : 6,
    recoveryRetryAfterMs: 250,
    qualityTier,
...
function modalPayloadHasUsefulData(payload = {}) {
  ...
  if (hasRenderableRows(payload.historicalIndicators?.rows) || hasRenderableTableMap(payload.historicalIndicators?.tablesByPeriod)) return true;
  ...
  if (hasRealIndexComparison(payload.indexComparison)) return true;
  ...
  if (hasRenderableRows(payload.resultsStatement?.rows) || hasRenderableRows(payload.balanceSheetStatement?.rows) || hasRenderableTableMap(payload.resultsStatement?.tablesByPeriod) || hasRenderableTableMap(payload.balanceSheetStatement?.tablesByPeriod)) return true;
  ...
  if (hasRealIndexComparison(payload.comparison)) return true;
...
function modalPayloadCompletenessPercent(payload = {}, family = '') {
  ...
  const settled = new Set(modalSectionArrival(payload, resolvedFamily).filter(section => section.settled).map(section => section.id));
  return Math.round((settled.size / sections.length) * 100);
}
...
function modalPayloadQualityProfile(payload = {}, family = '') {
  const resolvedFamily = String(family || payload.assetType || '').toLowerCase().includes('fii') || payload.aboutFund || payload.propertyPortfolio
    ...
    : 'stock';
  const sections = resolvedFamily === 'fii' ? fiiModalSections(payload) : stockModalQualitySections(payload);
  const arrival = modalSectionArrival(payload, resolvedFamily);
  const available = new Set(arrival.filter(section => section.settled).map(section => section.id));
  ...
  const deepIds = resolvedFamily === 'fii'
    ? ['indexComparison', 'peerComparison', 'checklist', 'distributions12m', 'dividendCharts', 'aboutFund', 'propertyPortfolio', 'patrimonialInfo', 'historicalIndicators', 'information', 'returns']
    : ['fundamentalIndicators', 'historicalIndicators', 'revenueProfitChart', 'profitQuoteChart', 'equityEvolutionChart', 'checklist', 'dividends', 'peerComparison', 'indexComparison', 'company', 'revenueBreakdown', 'financialStatements', 'returns'];
...
function parseRecoveryAvailableSections(payload = {}) {
  const raw = payload.knownSettledSections ?? payload.knownAvailableSections ?? payload.currentAvailableSections ?? payload.availableSections;
...
function recoveryClientQuality(payload = {}) {
  ...
  const missingSections = new Set(parseSectionList(payload.requestedRecoverySections ?? payload.knownMissingSections ?? payload.missingSections ?? payload.requiredSections));
...
const RETIRED_ASSET_MODAL_FEATURE_KEYS = new Set([
  'analysisChanges',
  'analysisChange',
  'assetAnalysisChanges',
  'whatChanged',
  'changesSinceLastAnalysis',
  'changesSincePreviousAnalysis',
  'analysisChangeHistory'
]);

function stripRetiredAssetModalFeatures(value) {
  if (Array.isArray(value)) return value.map(stripRetiredAssetModalFeatures);
  if (!value || typeof value !== 'object') return value;
  return Object.fromEntries(
    Object.entries(value)
      .filter(([key]) => !RETIRED_ASSET_MODAL_FEATURE_KEYS.has(key))
      .map(([key, entry]) => [key, stripRetiredAssetModalFeatures(entry)])
  );
}

function decorateModalPayload(payload = {}, runtime = {}) {
  const publicPayload = stripRetiredAssetModalFeatures(payload);
  const diagnostics = publicPayload && typeof publicPayload === 'object' && publicPayload.diagnostics && typeof publicPayload.diagnostics === 'object'
    ? publicPayload.diagnostics
    : {};
  const modalRuntime = {
  ...
  const delivery = buildModalDelivery(publicPayload, { ...runtime, requestPayload: runtime.requestPayload || {} });
  return {
    ...publicPayload,
    progressive: true,
    delivery,
    contractCapabilities: {
      ...
      sourceArrivalDiagnostics: true,
      confirmedEmptySections: true,
      canonicalSectionCatalog: true,
      targetedRecoveryBatches: true,
      deliverySchemaVersion: 4
    },
    diagnostics: {
      ...diagnostics,
      sourceArrival: modalSectionArrivalSummary(publicPayload, runtime.family),
      modalRuntime
    }
  };
}
...
export const _test = {
  ...
  hasRenderableRows,
  hasRenderableTableMap,
  hasRealSeriesPoints,
  hasRealSeriesList,
  hasRealSeriesByPeriod,
  hasRealIndexComparison,
  modalSectionArrival,
  modalSectionArrivalSummary,
  buildModalDelivery,
  stockModalSections,
  ...
  stripRetiredAssetModalFeatures,
