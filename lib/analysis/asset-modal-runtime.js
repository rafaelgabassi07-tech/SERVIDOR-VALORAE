...
export const ASSET_MODAL_RUNTIME_VERSION = '26.asset-modal.runtime.v9-late-producer-cache-retryable-full';
...
function buildModalDelivery(payload = {}, runtime = {}) {
  ...
  const isFinal = deliveredStage === 'full' && !deadlineTimeout && !explicitlyPartial;
  ...
  return {
    ...
    retryable: deadlineTimeout || explicitlyPartial || (isFinal && missingSections.length > 0),
...
function isModalPayloadCacheable(payload = {}, family = '') {
  ...
  if (!terminalClassification && normalizeModalCacheMode(payload) === 'full') {
    // Evita promover para o cache full um contrato que contém só cotação/gráfico. Esse era
    // o motivo de reaberturas permanecerem "básicas" durante toda a janela de cache.
    if (modalPayloadCompletenessPercent(payload, family) < 40) return false;
  }
  return true;
}
...
function startOrJoinModalProducer({ key, family, producer, refresh = false, ttlMs, staleMs } = {}) {
  ...
  promise.then(
    fresh => {
      if (!refresh && isModalPayloadCacheable(fresh, family)) {
        setCache(key, fresh, modalCacheTtlMs(fresh, ttlMs), staleMs);
      }
    },
    () => undefined
  ).finally(() => {
...
export async function withAssetModalRuntime({ family, ticker, payload = {}, ttlMs = 45_000, staleMs = 120_000, producer }) {
  ...
  const coalescedResult = await coalesce(key, async () => {
    ...
    const startedAt = Date.now();
    try {
      const producerFlight = startOrJoinModalProducer({ key, family, producer, refresh, ttlMs, staleMs });
