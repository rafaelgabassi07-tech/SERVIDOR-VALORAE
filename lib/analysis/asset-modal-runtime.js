...
function hasFiiPeerPatrimonialCoverage(peerComparison = {}) {
  const rows = Array.isArray(peerComparison?.rows) ? peerComparison.rows : [];
  if (!rows.length) return false;
  const covered = rows.filter(row =>
    hasPositiveNumber(row?.patrimonialValue) || hasMeaningfulDisplayValue(row?.patrimonialValueDisplay)
  ).length;
  // Uma tabela com vários FIIs não deve ser considerada concluída apenas porque o ticker
  // de referência foi preenchido. Dois valores reais são o mínimo para comparação útil;
  // tabelas unitárias exigem o único valor disponível.
...
function hasRenderableRow(row = {}) {
  ...
  return candidateKeys.some(key => hasMeaningfulDisplayValue(row[key]) || (Number.isFinite(Number(row[key])) && Number(row[key]) !== 0));
}
...
function fiiModalSections(payload = {}) {
  return [
    ...
    ['peerComparison', hasFiiPeerPatrimonialCoverage(payload.peerComparison)],
    ['checklist', hasResolvedChecklistItems(payload.checklist)],
