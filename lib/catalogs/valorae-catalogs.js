export const VALORAE_CATALOG_VERSION = '21.12.0';
...
export const ERROR_CATALOG = Object.freeze([
  ...
  { code: 'VALORAE_CLIENT_AUTH_REQUIRED', http: 401, retryable: false, description: 'Deploy exige x-valorae-app-id e x-valorae-client-key ou assinatura HMAC.' },
  { code: 'RATE_LIMITED', http: 429, retryable: true, description: 'Limite por rota/IP atingido. Respeite Retry-After.' },
  { code: 'MISSING_TARGET_URL', http: 400, retryable: false, description: 'Parâmetro url ausente em /api/scrape; validação esperada, não falha interna.' },
  { code: 'INVALID_TARGET_URL_PROTOCOL', http: 400, retryable: false, description: 'URL alvo em /api/scrape precisa usar HTTPS.' },
  { code: 'SCRAPE_HOST_NOT_ALLOWED', http: 403, retryable: false, description: 'Host alvo fora da allowlist do scraping direto.' },
...
export const ENV_CATALOG = Object.freeze([
  ...
  { name: 'VALORAE_CLIENT_KEYS', required: false, category: 'security', example: 'app-web:chave-secreta,apk:outra-chave', description: 'Ativa autenticação leve opcional por app/cliente sem recurso pago.' },
  { name: 'VALORAE_REQUIRE_CLIENT_AUTH', required: false, category: 'security', defaultValue: '0', description: 'Quando ativo, exige autenticação dos clientes configurados em VALORAE_CLIENT_KEYS.' },
...
export const SCHEMA_CATALOG = Object.freeze({
  asset: { version: 'asset-v21.12.0', required: ['version','ticker','type','status','results','normalized','quality','sourceReport'], volatile: ['requestId','generatedAt','checkedAt'], description: 'Contrato de ativo com dados brutos, normalizados, qualidade e fontes.' },
  portfolio: { version: 'portfolio-v21.12.0', required: ['summary','positions','allocation','income','risk','intelligence'], volatile: ['requestId','generatedAt'], description: 'Contrato de carteira com inteligência, concentração, renda, risco e planos.' },
  ...
  app: { version: 'app-view-v21.12.25', required: ['version','ticker','type','status','appMobileSnapshot','appPayload','appSyncEnvelope','appResponseIntegrity','endpointCoverage'], volatile: ['generatedAt','requestId'], description: 'Contrato oficial enxuto para produção Web/APK.' },
