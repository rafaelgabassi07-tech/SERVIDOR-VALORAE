...
export const VALORAE_NUMBERS_VERSION = '21.12.28-financial-number-normalizer-lru';

const EMPTY = new Set(['', '-', '—', '–', 'n/a', 'na', 'null', 'undefined']);

const NUMBER_PARSE_CACHE_MAX = Number(process.env.VALORAE_NUMBER_PARSE_CACHE_MAX || 2048);
const numberParseCache = new Map();
const numberParseMetrics = { hits: 0, misses: 0, sets: 0, evictions: 0 };
function cacheKeyForNumber(rawInput, maxAbs) { return `${maxAbs}|${rawInput}`; }
function cacheGetNumber(key) {
  if (!NUMBER_PARSE_CACHE_MAX) return undefined;
  if (!numberParseCache.has(key)) { numberParseMetrics.misses += 1; return undefined; }
  const value = numberParseCache.get(key);
  numberParseCache.delete(key);
  numberParseCache.set(key, value);
  numberParseMetrics.hits += 1;
  return value;
}
function cacheSetNumber(key, value) {
  if (!NUMBER_PARSE_CACHE_MAX) return;
  numberParseCache.set(key, value);
  numberParseMetrics.sets += 1;
  while (numberParseCache.size > NUMBER_PARSE_CACHE_MAX) {
    numberParseCache.delete(numberParseCache.keys().next().value);
    numberParseMetrics.evictions += 1;
  }
}
...
export function parseFinancialNumber(value, options = {}) {
  ...
  const key = cacheKeyForNumber(rawInput, maxAbs);
  const cached = cacheGetNumber(key);
  if (cached !== undefined) return cached;
  const negativeByParens = /\([^)]*\d[^)]*\)/.test(rawInput);
  const token = selectNumericToken(rawInput);
  if (!token) { cacheSetNumber(key, null); return null; }
  const multiplier = multiplierFromText(`${token} ${rawInput}`);
  const normalized = normalizeSeparators(token);
  if (!normalized) { cacheSetNumber(key, null); return null; }
  let n = Number(normalized);
  if (!Number.isFinite(n)) { cacheSetNumber(key, null); return null; }
  if (negativeByParens && n > 0) n = -n;
  n *= multiplier;
  if (Math.abs(n) > maxAbs) { cacheSetNumber(key, null); return null; }
  const parsed = Math.round(n * 1e8) / 1e8;
  cacheSetNumber(key, parsed);
  return parsed;
}
...
export function numberNormalizerStats() {
  const total = numberParseMetrics.hits + numberParseMetrics.misses;
  return { version: VALORAE_NUMBERS_VERSION, localePriority: ['pt-BR', 'en-US'], supports: ['currency', 'percent', 'K/M/B', 'mil/milhao/bilhao/trilhao', 'negative-parentheses'], cache: { enabled: NUMBER_PARSE_CACHE_MAX > 0, maxEntries: NUMBER_PARSE_CACHE_MAX, entries: numberParseCache.size, metrics: { ...numberParseMetrics }, hitRatePercent: total ? Math.round((numberParseMetrics.hits / total) * 10000) / 100 : null } };
}
