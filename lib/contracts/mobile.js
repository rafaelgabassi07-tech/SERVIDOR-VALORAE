...
export async function buildMobilePortfolioSync(payload = {}) {
  ...
  const dividendsBlock = await safeBlock('dividends', includeDividends, () => buildDividendsContract(payload));
  const dividendEvents = dividendsBlock[1]?.events || [];

  const results = await Promise.all([
    safeBlock('analysis', includeAnalysis, () => buildPortfolioAnalysis(payload)),
    safeBlock('history', includeHistory, () => buildRealMarketHistory({ ...payload, dividendEvents })),
    safeBlock('ipca', includeIpca, () => getIpcaSeries(payload.historyMonths || payload.months || 12)),
