...
export async function buildMobilePortfolioSync(payload = {}) {
  ...
  const results = await Promise.all([
    ...
    safeBlock('history', includeHistory, () => buildHistory(payload)),
    safeBlock('ipca', includeIpca, () => getIpcaSeries(payload.months || 12)),
    safeBlock('dividends', includeDividends, () => buildDividendsContract(payload)),
    safeBlock('rankings', includeRankings, () => buildRankings(payload))
  ]);
