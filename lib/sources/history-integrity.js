const UNTRUSTED_HISTORY_SOURCE = /(ultimo\s+snapshot\s+conhecido|último\s+snapshot\s+conhecido|last[_ -]?known|mock|fixture|synthetic|sint[eé]tic|simulad|proxy\s*ticker|ticker\s*substitut|etf\s*proxy|reconstructed\s+from\s+yahoo\s+snapshot|reconstructed\s+from\s+monthly\s+returns|curva\s+(?:acumulada\s+)?reconstru[ií]da)/i;
...
function pointHasUntrustedMarker(point = {}) {
  ...
  if (point.simulated === true || point.synthetic === true || point.proxyTickerUsed === true
    ...
    || point.reconstructedFromMonthlyReturns === true
...
export function inspectRealHistoryIntegrity(history = {}) {
  ...
  if (history?.reconstructedFromMonthlyReturns === true) topLevelReasons.push('monthly_return_reconstruction');
