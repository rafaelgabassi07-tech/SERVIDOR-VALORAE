...
function fallbackIpcaSeries(months = 12, reason = '') {
  ...
  for (let i = safeMonths - 1; i >= 0; i--) {
    ...
    points.push({ date, month: date.slice(0, 7), monthlyPercent, value: monthlyPercent, accumulatedPercent: round((factor - 1) * 100, 4), fallback: true, source: 'VALORAE IPCA fallback' });
  }
...
export async function getIpcaSeries(months = 12) {
  ...
  const points = rows.map((row) => {
    ...
    factor *= (1 + monthlyPercent / 100);
    return { date, month: date.slice(0, 7), monthlyPercent, value: monthlyPercent, accumulatedPercent: round((factor - 1) * 100, 4), source: 'VALORAE IPCA BCB SGS 433' };
