...
export async function buildAssetsPayload(payload = {}) {
  ...
  const quotes = await getQuotes(tickers, { timeoutMs: Number(payload.timeoutMs || 3500), max: Number(payload.max || 60) });
  ...
  return { status: items.some(i => i.currentPrice > 0) ? 'OK' : 'EMPTY', endpoint: 'assets', source: 'VALORAE Fonte Oficial', assets: items, items, results: items, partial: items.some(i => i.partial), quotePolicy: { provider: 'Yahoo Finance via Proxy', cacheTtlMs: QUOTE_SAFE_TTL_MS, recommendedClientPollMs: 30_000, backoffMs: QUOTE_BACKOFF_MS, maxTickersPerRequest: Number(payload.max || 60), mode: 'safe-batch-cache' } };
}
