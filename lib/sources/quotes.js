...
function lastPositive(values = []) {
  return [...values].reverse().find(v => Number.isFinite(Number(v)) && Number(v) > 0);
}

function previousPositive(values = []) {
  const clean = values.map(Number).filter(v => Number.isFinite(v) && v > 0);
  return clean.length >= 2 ? clean[clean.length - 2] : undefined;
}
...
function parseChartQuote(ticker, json, status = 0, cacheStatus = 'MISS') {
  ...
  const lastClose = lastPositive(closes);
  const previousSeriesClose = previousPositive(closes);
  const lastVolume = lastPositive(volumes);
  const price = numberValue(meta.regularMarketPrice ?? lastClose ?? meta.chartPreviousClose ?? meta.previousClose, 0);
  const previousClose = numberValue(meta.chartPreviousClose ?? meta.previousClose ?? previousSeriesClose, 0);
  const reportedChange = Number(meta.regularMarketChange);
  const reportedChangePercent = Number(meta.regularMarketChangePercent);
  const change = Number.isFinite(reportedChange) ? reportedChange : (price > 0 && previousClose > 0 ? price - previousClose : 0);
  const changePercent = Number.isFinite(reportedChangePercent) ? reportedChangePercent : (price > 0 && previousClose > 0 ? (change / previousClose) * 100 : 0);
  ...
  return {
    ...
    marketTime: updatedAt,
    updatedAt,
    regularMarketTime: ts || undefined,
    ...
    quoteQuality: {
      ...
      hasVariation: Number.isFinite(changePercent),
      cacheStatus,
...
function mergeQuoteWithFundamentals({ ticker, quote = {}, meta = {}, fundamental = null } = {}) {
  ...
  const changePercent = finiteNumberOrNull(quote?.dayChangePercent, quote?.regularMarketChangePercent, quote?.changePercent, quote?.variationPercent, quote?.variationPct, quote?.variacaoDay, quote?.variacao) ?? 0;
  const dayChangeValue = finiteNumberOrNull(quote?.dayChangeValue, quote?.regularMarketChange, quote?.change);
  ...
  return {
    ...
    quote: { ...(quote || {}), price, currentPrice: price, priceDisplay, changeDisplay, dayChangePercent: round(changePercent, 4), variationPercent: round(changePercent, 4), ...(dayChangeValue !== null ? { dayChangeValue: round(dayChangeValue, 4) } : {}) },
    cotacao: { ...(quote || {}), precoAtual: price, price, currentPrice: price, priceDisplay, changeDisplay, dayChangePercent: round(changePercent, 4), variationPercent: round(changePercent, 4), ...(dayChangeValue !== null ? { dayChangeValue: round(dayChangeValue, 4) } : {}) },
    price,
    currentPrice: price,
    lastPrice: price,
    precoAtual: price,
    ...
    previousClose: finiteNumberOrNull(quote?.previousClose),
    change: dayChangeValue !== null ? round(dayChangeValue, 4) : undefined,
    dayChangeValue: dayChangeValue !== null ? round(dayChangeValue, 4) : undefined,
    changePercent: round(changePercent, 4),
    dayChangePercent: round(changePercent, 4),
    regularMarketChangePercent: round(changePercent, 4),
    variationPercent: round(changePercent, 4),
    ...
    updatedAt: quote?.updatedAt || quote?.marketTime || new Date().toISOString(),
    marketTime: quote?.marketTime || quote?.updatedAt,
    quoteSource: quote?.source || 'Yahoo Finance Chart API',
    quoteOk: hasLiveTradingQuote,
    quoteQuality: quote?.quoteQuality || { hasPrice: hasLiveTradingQuote, hasVariation: Number.isFinite(changePercent), cacheStatus: quote?.cacheStatus || 'MISS' },
...
export async function buildAssetsPayload(payload = {}) {
  ...
  const liveQuoteCount = items.filter(i => i.currentPrice > 0 && i.quoteOk !== false).length;
  return {
    ...
    requestedTickers: clean,
    quoteCoverage: { requested: clean.length, live: liveQuoteCount, missing: Math.max(0, clean.length - liveQuoteCount) },
    quoteFailures: items.filter(i => !(i.currentPrice > 0)).map(i => ({ ticker: i.ticker, status: i.status || 'EMPTY' })),
