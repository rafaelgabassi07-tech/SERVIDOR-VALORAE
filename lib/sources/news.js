...
export async function getNews(payload = {}) {
  ...
  return {
    ...
    items: enriched.map(item => withSourceBranding({ ...item, url: item.url || item.link || '', originalUrl: item.originalUrl || item.url || item.link || '', openInBrowser: true, inAppReader: false })),
    news: enriched.map(item => withSourceBranding({ ...item, url: item.url || item.link || '', originalUrl: item.originalUrl || item.url || item.link || '', openInBrowser: true, inAppReader: false })),
    articles: enriched.map(item => withSourceBranding({ ...item, url: item.url || item.link || '', originalUrl: item.originalUrl || item.url || item.link || '', openInBrowser: true, inAppReader: false })),
