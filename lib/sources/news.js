...
export async function getNews(payload = {}) {
  ...
  if (!items.length && (querySymbols.length || freeTextQuery)) {
    const broadQuery = '(mercado financeiro OR B3 OR ações OR dividendos OR Ibovespa) Brasil when:1d';
    const broadUrl = `https://news.google.com/rss/search?q=${encodeURIComponent(broadQuery)}&hl=pt-BR&gl=BR&ceid=BR:pt-419${freshSuffix}`;
