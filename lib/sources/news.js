...
async function enrichArticle(item, { timeoutMs = 3200, ttlMs = 30 * 60_000 } = {}) {
  ...
  try {
    ...
    return {
      ...
      articleMessage: body.length > 520 ? 'Matéria completa extraída pelo Proxy.' : 'Trecho principal extraído pelo Proxy.'
    };
  } catch (error) {
...
export async function getNews(payload = {}) {
  ...
  const includeArticleBody = String(payload.includeArticleBody ?? payload.full ?? 'true') !== 'false';
  ...
  const fallback = [{
    ...
    body: 'O feed público de notícias não respondeu dentro do prazo. O Valorae mantém a tela nativa funcionando com conteúdo local e tentará atualizar novamente quando a conexão estiver disponível.',
