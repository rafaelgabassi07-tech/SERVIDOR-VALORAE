...
/**
 * Safe, cacheable article-reader endpoint used by the APK before its local fallback.
 * The requested hostname becomes the single-host allowlist and every DNS/redirect hop
 * is validated against public network ranges by fetchAllowedScrapeText.
 */
export async function getArticleContent(payload = {}) {
  ...
  const fetched = await fetchAllowedScrapeText(parsed.toString(), {
