...
export async function getNews(payload = {}) {
  ...
  const query = freeTextQuery
    ...
    : querySymbols.length
      ? `(${querySymbols.join(' OR ')}) (ações OR dividendos OR B3) when:1d`
  ...
  const fetched = await fetchText(url, { timeoutMs, ttlMs: freshSuffix ? 0 : 2 * 60_000, staleMs: freshSuffix ? 0 : 45 * 60_000, headers: { Accept: 'application/rss+xml,text/xml,*/*' } });
  const dayStartMs = new Date(new Date().toLocaleDateString('en-US', { timeZone: 'America/Sao_Paulo' })).getTime();
  const recentWindowMs = 36 * 60 * 60 * 1000;
  const nowMs = Date.now();
  const items = parseItems(fetched.text, Math.max(limit * 3, limit))
    .filter(item => {
  ...
  const normalizedItems = sortNewsNewestFirst(
    enriched.map(item => attachNewsMetadata(item, { querySymbols, freeTextQuery }))
  ...
  return {
    ...
    freshnessPolicy: { source: 'Google News RSS', cacheTtlSeconds: freshSuffix ? 0 : 120, staleSeconds: freshSuffix ? 0 : 2700, newestFirst: true, queryWindow: 'when:1d', maxAgeHours: 36 },
