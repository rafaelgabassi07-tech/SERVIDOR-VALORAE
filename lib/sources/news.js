...
function looksLikeSingleTickerQuery(value = '') {
  return /^[A-Z]{4}[0-9]{1,2}[A-Z]?$/.test(String(value || '').trim().toUpperCase());
}
...
export async function getNews(payload = {}) {
  const symbols = parseSymbols(payload.symbols || payload.tickers || payload.assets).map(normalizeTicker).filter(Boolean).slice(0, 16);
  ...
  const querySymbols = [ticker, ...symbols.filter(symbol => symbol !== ticker)].filter(Boolean).slice(0, 16);
  const query = freeTextQuery
    ? `${freeTextQuery} ações OR dividendos OR B3 OR mercado financeiro Brasil`
    : querySymbols.length
  ...
  const fallback = [withSourceBranding({
  ...
  const enriched = includeArticleBody && items.length
    ...
    : baseItems;
  const normalizedItems = enriched.map(item => attachNewsMetadata(item, { querySymbols, freeTextQuery }));
  return {
    ...
    items: normalizedItems.map(item => withSourceBranding({ ...item, url: item.url || item.link || '', originalUrl: item.originalUrl || item.url || item.link || '', openInBrowser: true, inAppReader: false })),
    news: normalizedItems.map(item => withSourceBranding({ ...item, url: item.url || item.link || '', originalUrl: item.originalUrl || item.url || item.link || '', openInBrowser: true, inAppReader: false })),
    articles: normalizedItems.map(item => withSourceBranding({ ...item, url: item.url || item.link || '', originalUrl: item.originalUrl || item.url || item.link || '', openInBrowser: true, inAppReader: false })),
