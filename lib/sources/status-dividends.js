...
export async function getConfirmedDividendsByTicker(ticker, options = {}) {
  ...
  // A recuperação da Home prefere a página pública visível. O endpoint JSON interno do
  // StatusInvest muda com mais frequência e foi justamente o ponto de regressão observado.
  if (preferVisiblePage && !envOff('VALORAE_STATUSINVEST_HTML_FALLBACK_ENABLED')) {
    const visible = await tryVisiblePages();
    if (visible.length > 0) return { ticker: clean, events: visible, diagnostics };
  }
  ...
  if (!htmlAttempted && !envOff('VALORAE_STATUSINVEST_HTML_FALLBACK_ENABLED')) {
    const visible = await tryVisiblePages();
    if (visible.length > 0) return { ticker: clean, events: visible, diagnostics };
  }
