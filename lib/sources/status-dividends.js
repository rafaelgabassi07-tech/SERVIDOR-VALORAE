...
function parseStatusInvestHtmlEvents(ticker, html = '') {
  ...
  return events.map(item => normalizeStatusEvent(ticker, item, 'confirmed-html-fallback')).filter(e => e.grossValuePerShare > 0 || e.paymentDate || e.dateCom);
}
...
export async function getConfirmedDividendsByTicker(ticker, options = {}) {
  ...
  for (const type of types) {
    ...
    const { json, status, cacheStatus, error, parseError } = await fetchJson(url, {
      ...
      headers: { 'X-Requested-With': 'XMLHttpRequest', Referer: `https://statusinvest.com.br/${type}/${clean.toLowerCase()}` },
  ...
  if (!envOff('VALORAE_STATUSINVEST_HTML_FALLBACK_ENABLED')) {
    for (const type of types) {
      const pageUrl = `https://statusinvest.com.br/${type}/${clean.toLowerCase()}`;
      const fetched = await fetchText(pageUrl, {
        timeoutMs: Math.max(1200, Math.min(statusInvestTimeoutMs(options), Number(options.htmlTimeoutMs || 3500))),
        ttlMs: options.ttlMs || 6 * 60 * 60 * 1000,
        staleMs: options.staleMs || 48 * 60 * 60 * 1000,
        signal: options.signal
      });
      const htmlEvents = fetched.text ? parseStatusInvestHtmlEvents(clean, fetched.text) : [];
      diagnostics.push({ provider: 'statusinvest-html', type, status: fetched.status, cacheStatus: fetched.cacheStatus, count: htmlEvents.length, error: fetched.error });
      if (htmlEvents.length > 0) return { ticker: clean, events: htmlEvents, diagnostics };
    }
  }
