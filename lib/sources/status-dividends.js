...
export function dividendType(item = {}) {
  ...
  if (/\b(JSCP|JCP)\b/.test(text) || compact.includes('JUROSSOBRECAPITALPROPRIO') || compact.includes('JUROSSCAPITALPROPRIO') || compact.includes('JUROSCAPITALPROPRIO')) return 'JCP';
...
export function applyDividendTax(event = {}, raw = {}) {
  ...
  const taxable = Boolean(type === 'JCP' && isStock && gross > 0);
  const taxRate = explicitRate || (taxable ? jcpTaxRateForDate(event.paymentDate || event.dateCom || event.exDate || '') : 0);
  ...
  return {
    ...
    taxWithheldPerShare,
    taxable,
    taxRule: taxable ? `IRRF_JCP_ACOES_${Math.round(taxRate * 1000) / 10}%` : 'NAO_TRIBUTADO_NO_APP'
  };
}
