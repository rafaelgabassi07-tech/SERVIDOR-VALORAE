...
export async function getConfirmedDividendsByTicker(ticker, options = {}) {
  ...
  for (const type of types) {
    const url = `https://statusinvest.com.br/${type}/companytickerprovents?ticker=${encodeURIComponent(clean)}&chartProventsType=2`;
    ...
    diagnostics.push({ provider: 'statusinvest', type, status, cacheStatus, count: models.length, error });
    if (models.length > 0) {
      return {
        ticker: clean,
