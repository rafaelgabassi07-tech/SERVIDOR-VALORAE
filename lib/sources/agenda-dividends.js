...
function tickerFromAny(value = '') {
  const match = String(value || '').toUpperCase().match(/\b(?:[A-Z]{4}\d{1,2}B?|[A-Z0-9]{3,6}\d{1,2})\b/);
...
function buildAgendaEvent(ticker, segment = '', overrides = {}) {
  ...
  const paymentDate = normalizeDate(overrides.paymentDate) || labelledDate(fullSegment, ['data\\s*de\\s*pagamento', 'data\\s*pagamento', 'pagamento', 'pagto', 'pgto', 'payment', 'pay']) || (dates.length >= 2 ? dates[1] : '');
...
function boundedTickerSegments(text = '') {
  const tickerPattern = /\b(?:[A-Z]{4}\d{1,2}B?|[A-Z0-9]{3,6}\d{1,2})\b/g;
...
function agendaFutureMonths(options = {}) {
  const value = Number(options.futureMonths ?? options.agendaMonthsAhead ?? process.env.VALORAE_AGENDA_MONTHS_AHEAD ?? 18);
...
function agendaUrls(options = {}) {
  ...
  const now = new Date();
  const urls = ['https://investidor10.com.br/acoes/dividendos/', 'https://investidor10.com.br/fiis/dividendos/'];
  // Intercala ações e FIIs mês a mês para carteiras mistas não perderem FIIs quando houver maxPages/deadline.
  for (let i = 0; i <= months; i++) {
    const d = new Date(Date.UTC(now.getFullYear(), now.getMonth() + i, 1));
    const year = d.getUTCFullYear();
    const slug = MONTH_SLUGS[d.getUTCMonth()];
    for (const base of ['acoes', 'fiis']) {
      urls.push(`https://investidor10.com.br/${base}/dividendos/${year}/${slug}/`);
      urls.push(`https://investidor10.com.br/${base}/dividendos/${year}/${slug}/data-de-pagamento/`);
    }
  }
...
async function fetchAgendaPage(url, options = {}) {
  ...
  const remaining = deadlineAt > 0 ? Math.max(1, deadlineAt - Date.now() - 80) : 0;
  const requestedTimeout = Number(options.timeoutMs || options.agendaTimeoutMs || 5000);
  const timeoutMs = remaining > 0 ? Math.max(250, Math.min(requestedTimeout || 5000, remaining)) : (requestedTimeout || 5000);
  const res = await fetchText(url, { timeoutMs, ttlMs: 45 * 60 * 1000, staleMs: 36 * 60 * 60 * 1000, signal: options.signal });
  return { url, ...res, timeoutMs };
}
...
export async function getAgendaDividends(tickers = [], options = {}) {
  ...
  const deadlineAt = Number(options.deadlineAt || 0);
  const allUrls = agendaUrls(options);
  const maxPages = Number(options.maxPages || process.env.VALORAE_AGENDA_MAX_PAGES || 0);
  const urls = Number.isFinite(maxPages) && maxPages > 0 ? allUrls.slice(0, Math.max(2, Math.floor(maxPages))) : allUrls;
  const diagnostics = [];
  const events = [];
  ...
  async function worker() {
    while (index < urls.length) {
      if (deadlineAt > 0 && Date.now() >= deadlineAt - 180) {
        deadlineHit = true;
        return;
      }
      const url = urls[index++];
      const res = await fetchAgendaPage(url, { ...options, deadlineAt }).catch(error => ({ url, status: 0, cacheStatus: 'ERROR', error: error?.name === 'AbortError' ? 'timeout-or-aborted' : (error?.message || String(error)), text: '' }));
      diagnostics.push({ provider: 'investidor10-agenda', url: res.url, status: res.status, cacheStatus: res.cacheStatus, error: res.error, attempts: res.attempts, timeoutMs: res.timeoutMs });
      if (res.text) events.push(...parseAgendaHtml(res.text, cleanTickers));
    }
  }
  await Promise.all(Array.from({ length: Math.min(concurrency, urls.length) }, () => worker()));
  if (deadlineHit || index < allUrls.length) {
    diagnostics.push({ provider: 'investidor10-agenda', status: 'PARTIAL', reason: deadlineHit ? 'deadline-exceeded' : 'page-budget-exceeded', completedPages: Math.min(index, urls.length), totalPages: allUrls.length });
  }
  return { events, diagnostics, partial: deadlineHit || index < allUrls.length };
}
