...
async function fetchDirectYahooIndexHistory(indexCode, rangeKey, payload = {}) {
  ...
  const symbol = DIRECT_YAHOO_INDEX_SYMBOLS[code];
  const cfg = PERIOD_MAP[rangeKey] || PERIOD_MAP['1Y'];
  if (!symbol) return { status: 'EMPTY', ticker: code, points: [], error: 'direct-yahoo-index-symbol-unavailable' };
  ...
  for (const host of hosts) {
    ...
    const { json, status, cacheStatus, error } = await fetchJson(url, {
      timeoutMs: Number(payload.timeoutMs || 5200),
      ttlMs: cfg.ttlMs,
    ...
    const limited = points.slice(-Number(payload.limit || cfg.limit || 320));
    attempts.push({ host, status, cacheStatus, parsed: limited.length, error });
    if (limited.length > 1) {
      return {
        status: 'OK',
        ok: true,
        ...
        source: `Yahoo Finance direct index symbol ${symbol}`,
        sourceVersion: '21.12.0-yahoo-direct-index-fallback',
        official: false,
  ...
  return {
    ...
    proxyTickerUsed: false,
    diagnostics: attempts,
    error: `Sem histórico suficiente para ${code} no símbolo direto ${symbol}.`
  };
}
...
export async function getAssetHistory(payload = {}) {
  ...
  if (officialIndex) {
    ...
    return {
      ...
      simulated: false,
      proxyTickerUsed: false,
      fallbackAttempt: yahooDirect,
      error: b3.error || yahooDirect.error || `Sem histórico suficiente para ${officialIndex}.`
    };
  }
