...
function buildSimpleFinancialSeries(indicators = {}, priceHistory = []) {
  const current = priceHistory.at(-1)?.close || priceHistory.at(-1)?.price || priceHistory.at(-1)?.value || 0;
  const revenue = numberValue(indicators.valorDeMercado || indicators.marketValue, 0);
  const equity = numberValue(indicators.patrimonioLiquido || indicators.netWorth, 0);
  const profit = numberValue(indicators.lpa, 0) * 1000000;
  const year = String(new Date().getUTCFullYear());
  const balance = equity > 0 ? [{ label: year, year, netWorth: equity, totalAssets: equity, totalLiabilities: 0 }] : [];
...
function buildAssetChartBundle({ ticker, assetClass, quote, fundamentals, history, dividends }) {
  ...
  const financial = buildSimpleFinancialSeries(indicators, priceHistory);
...
export async function buildAssetDetails(payload = {}) {
  ...
  const [quote, history, fundamentals, dividends] = await Promise.all([
    ...
    fetchFundamentalSnapshot(ticker, { timeoutMs: Number(payload.fundamentalTimeoutMs || payload.timeoutMs || 4200) }).catch(error => ({ status: 'ERROR', indicators: {}, indicatorCards: [], profile: {}, error: error?.message })),
  ...
  const results = {
    ...
    informacoesEmpresa: { valorDeMercado: indicators.valorDeMercado || 0, patrimonioLiquido: indicators.patrimonioLiquido || 0 },
    informacoesFundo: assetClass === 'FII' ? { segmento: 'FII', patrimonioLiquido: indicators.patrimonioLiquido || 0, vacanciaFisica: indicators.vacanciaFisica || 0, numeroCotistas: indicators.cotistas || 0, cotasEmitidas: indicators.cotasEmitidas || 0 } : {},
  ...
  return {
    ...
    subSector: assetClass === 'FII' ? '' : 'Renda variável',
