...
import { fetchB3IndexDailyEvolution, getCdiAccumulatedSeries, getIpcaSeries, getConfirmedDividendsByTicker } from './adapters/index.js';
...
import {
  ...
  findStructuredValues,
} from '../scrape/structured-data-discovery.js';
import { captureDynamicPageData, dynamicNetworkCaptureMode } from '../scrape/dynamic-render-fallback.js';
...
const PERIOD_MAP = {
  ...
  'MAX': { range: 'max', interval: '1mo', ttlMs: 6 * 60 * 60_000, limit: 600 }
};
  ...
  for (const symbol of symbols) {
    for (const requestCfg of chartRequests) {
      ...
      for (const host of hosts) {
        ...
        const points = yahooChartPointsFromPayload(json, symbol, 'Yahoo Finance direct index symbol').slice(-Number(payload.limit || (PERIOD_MAP[rangeKey] || PERIOD_MAP['1Y']).limit || 320));
...
function extractInvestidor10EmbeddedAnalysisData(html = '', pageUrl = 'https://investidor10.com.br/', additionalDocuments = []) {
  const structured = discoverStructuredPageData(html, { url: pageUrl, additionalDocuments });
  const firstStructured = (patterns, kinds = []) => findStructuredValues(structured, patterns, { limit: 1, documentKinds: kinds })[0]?.value || null;
  const advancedMetrics = extractInvestidor10InlineJson(html, [/_sectorIndicators\s*=\s*(\{[\s\S]*?\})\s*;/i, /sectorIndicators\s*=\s*(\{[\s\S]*?\})\s*;/i])
    || firstStructured([/^_?sectorIndicators$/i, /advancedMetrics/i]);
  const revenueGeography = extractInvestidor10InlineJson(html, [/companyRevenuesChartPie\s*=\s*(\{[\s\S]*?\})\s*;/i])
    || firstStructured([/companyRevenuesChartPie/i, /revenueGeography/i, /geograph/i, /region/i]);
  const revenueSegment = extractInvestidor10InlineJson(html, [/companyBussinesRevenuesChartPie\s*=\s*(\{[\s\S]*?\})\s*;/i, /companyBusinessRevenuesChartPie\s*=\s*(\{[\s\S]*?\})\s*;/i])
    || firstStructured([/companyBussinesRevenuesChartPie/i, /companyBusinessRevenuesChartPie/i, /revenueSegment/i, /businessRevenue/i]);
  const rentabilidadeChart = extractInvestidor10RentabilidadeChart(html)
...
async function fetchInvestidor10AnalysisExtras(ticker, type, html = '', options = {}) {
  ...
  const pageUrl = options.pageUrl || `https://investidor10.com.br/${type === 'fii' ? 'fiis' : 'acoes'}/${clean.toLowerCase()}/`;
  const ids = extractInvestidor10ChartIds(html);
  const staticEmbedded = extractInvestidor10EmbeddedAnalysisData(html, pageUrl);
  const staticSignals = [
    staticEmbedded.advancedMetrics,
    staticEmbedded.revenueGeography,
    staticEmbedded.revenueSegment,
    staticEmbedded.rentabilidadeChart,
  ].filter(value => value && typeof value === 'object').length;
  const dynamicNeeded = options.forceDynamicDataCapture === true || (
    dynamicNetworkCaptureMode() !== 'disabled' &&
    (staticSignals < 2 || (staticEmbedded.structuredData?.internalEndpoints || []).length < 2 || (type === 'fii' ? !ids.fiiId : !ids.companyId))
  );
  const dynamicCapture = dynamicNeeded
    ? await captureDynamicPageData({
        url: pageUrl,
        staticHtml: html,
        options: { blocked: options.blocked, waitForSelector: options.waitForSelector },
      })
    : { html: '', networkDocuments: [], diagnostics: { ran: false, reason: 'static-coverage-sufficient', mode: dynamicNetworkCaptureMode() } };
  const candidateEmbedded = dynamicCapture.html || dynamicCapture.networkDocuments?.length
    ? extractInvestidor10EmbeddedAnalysisData(dynamicCapture.html || html, pageUrl, dynamicCapture.networkDocuments || [])
    : null;
  const endpointMap = new Map();
  for (const item of [
    ...(staticEmbedded.structuredData?.internalEndpoints || []),
    ...(candidateEmbedded?.structuredData?.internalEndpoints || []),
  ]) {
    if (!item?.key || !item?.url) continue;
    endpointMap.set(`${item.key}:${item.url}`, item);
  }
  const embedded = {
    advancedMetrics: staticEmbedded.advancedMetrics || candidateEmbedded?.advancedMetrics || null,
    revenueGeography: staticEmbedded.revenueGeography || candidateEmbedded?.revenueGeography || null,
    revenueSegment: staticEmbedded.revenueSegment || candidateEmbedded?.revenueSegment || null,
    rentabilidadeChart: staticEmbedded.rentabilidadeChart || candidateEmbedded?.rentabilidadeChart || null,
    structuredData: {
      ...(candidateEmbedded?.structuredData || {}),
      ...(staticEmbedded.structuredData || {}),
      internalEndpoints: [...endpointMap.values()],
      dynamicCapture: dynamicCapture.diagnostics,
    },
  };
  ...
  const allowCapturedGapFill = dynamicNetworkCaptureMode() === 'known-endpoint-gap-fill';
  if (allowCapturedGapFill) {
    for (const document of dynamicCapture.networkDocuments || []) {
      const key = classifyKnownInternalEndpoint(document.url);
      if (!key || !document.data || typeof document.data !== 'object' || rawJson[key] !== undefined) continue;
      rawJson[key] = document.data;
      existingUrls.add(String(document.url || ''));
      apiStatus.push({
        key,
        url: document.url,
        status: Number(document.status || 200),
        ok: true,
        cacheStatus: 'BROWSER_NETWORK_CAPTURE',
        source: 'playwright-network-json',
      });
    }
  }
  const addTask = (key, url) => {
    const u = String(url || '').trim();
    if (!u || existingUrls.has(u) || rawJson[key] !== undefined) return;
    existingUrls.add(u);
    tasks.push([key, u]);
  };
  for (const discovered of embedded.structuredData?.internalEndpoints || []) {
    addTask(discovered.key, discovered.url);
  }
  ...
  const apiExtras = {
    embedded,
    rawJson,
    chartsFinanceiros: rawJson,
    apiStatus,
    historicoIndicadoresFii: rawJson.historicoIndicadoresFii,
    dynamicCapture: dynamicCapture.diagnostics,
    extractionIntelligence: {
      staticSignals,
      capturedNetworkDocuments: (dynamicCapture.networkDocuments || []).length,
      networkCaptureMode: dynamicNetworkCaptureMode(),
      knownEndpointGapFill: allowCapturedGapFill,
    },
  };
