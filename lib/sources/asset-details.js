...
export async function getAssetHistory(payload = {}) {
  ...
  if (ticker === 'IFIX' || ticker === '^IFIX') {
    const b3 = await fetchB3IndexDailyEvolution('IFIX', { years: rangeKey === 'MAX' ? 5 : 2, timeoutMs: Number(payload.timeoutMs || 6500), limit: Number(payload.limit || 520), bypassCache: payload.bypassCache === true });
    return {
      ...b3,
      status: b3.ok ? 'OK' : (b3.status || 'EMPTY'),
      ticker: 'IFIX',
      range: rangeKey,
      source: 'B3 Oficial - IFIX daily-evolution',
      official: true,
      simulated: false,
