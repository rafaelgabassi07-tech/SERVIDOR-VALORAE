  ...
  for (const symbol of symbols) {
    for (const requestCfg of chartRequests) {
      ...
      for (const host of hosts) {
        ...
        const points = yahooChartPointsFromPayload(json, symbol).slice(-Number(payload.limit || (PERIOD_MAP[rangeKey] || PERIOD_MAP['1Y']).limit || 320));
        ...
        if (points.length > 1 || (rangeKey === '1D' && points.length >= 1)) {
          return {
            ...
            sourceVersion: '21.12.0-yahoo-direct-index-compatible-ranges-v4',
            official: false,
            ...
            warning: requestCfg.reason === 'requested-range'
              ? undefined
              : `${code} obtido pelo Yahoo Finance Chart API em janela compatível (${requestCfg.range}/${requestCfg.interval}) após a janela ampla não retornar pontos suficientes.`
          };
        }
      }
    }
...
export async function getAssetHistory(payload = {}) {
  ...
  if (officialIndex) {
    ...
    return {
      ...
      error: yahooDirect?.error || b3.error || investidor10Index.error || `Sem histórico suficiente para ${officialIndex}.`
    };
  }
