...
function extractMetricFromText(text, label, aliases) {
  ...
  const comparisonNumeric = '([+-]?\\d{1,3}(?:\\.\\d{3})*(?:,\\d+)?|[+-]?\\d+(?:,\\d+)?)(\\s*%|\\s*x)?';
  for (const alias of aliases) {
    const safe = alias.replace(/[.*+?^${}()|[\]\\]/g, '\\$&').replace(/\s+/g, '\\s+');
    const re = new RegExp(`${safe}[^0-9+-]{0,80}${numeric}\\s*(%|x)?`, 'i');
    const match = compact.match(re);
    if (match) {
      const value = numberValue(match[1], 0);
      if (Number.isFinite(value)) {
        const unit = match[2] === '%' || /yield|roe|roic|roa|margem|payout|vacância|cagr/i.test(label) ? '%' : '';
        const result = { label, value, display: `${match[1]}${match[2] || ''}`, unit };
        ...
        for (const [key, comparisonLabel] of [['setor', 'Setor'], ['subsetor', 'Subsetor'], ['segmento', 'Segmento']]) {
          ...
          if (!cmp) continue;
          const cmpValue = numberValue(cmp[1], 0);
          if (!Number.isFinite(cmpValue)) continue;
          const cmpUnit = (cmp[2] || '').trim() || unit;
          comparisons[key] = { label: comparisonLabel, value: cmpValue, display: `${cmp[1]}${cmpUnit}`, unit: cmpUnit === '%' ? '%' : unit };
        }
...
function firstDisplayValueAfterLabel(plain = '', label = '', options = {}) {
  ...
  const money = slice.match(/R\$\s*[+-]?\d{1,3}(?:\.\d{3})*(?:,\d+)?/i);
  ...
  if (options.preferMoney && money) return money[0].trim();
  return (money || percent || number)?.[0]?.trim() || '';
}
...
function extractInvestidor10ChecklistFromHtml(html = '', ticker = '') {
  ...
  const section = slicePlainSection(
    ...
    ['Histórico de Dividendos', 'Lista de Imóveis', 'Informações Adicionais', 'COMPARAÇÃO', 'ÍNDICES COM'],
    2800
  );
  if (!section) return [];
  ...
  const candidates = withoutIntro
    .split(/(?=Empresa\s|FII\s|Dividend Yield\s|Liquidez\s|Número de cotistas\s|Patrimônio líquido\s|Receita\s|Lucro\s|Dívida\s|ROE\s|Pagou\s|Possui\s|Apresentou\s)/i)
  ...
  for (const item of candidates) {
    ...
    if (/ranking|fins informativos|pontua/i.test(cleaned)) continue;
    addDeepFact(rows, seen, `checklist_${rows.length + 1}`, cleaned, 'Critério capturado', 'Checklist buy and hold', 'Investidor10HTML.checklist');
  }
...
export async function buildAssetDetails(payload = {}) {
  ...
  const results = {
    ...
    indicadoresFundamentalistas: { semComparativos: { dividendYield, ...indicators }, comparativosFonte: sourceComparatives },
    sourceComparatives,
    valuationModels,
