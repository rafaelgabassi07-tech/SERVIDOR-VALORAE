...
const DIRECT_YAHOO_INDEX_SYMBOLS = {
  // Origem confirmada no pacote de referência analisado: IBOV -> ^BVSP e IFIX -> IFIX.SA.
  // Para SMLL/IDIV, tentamos primeiro o código direto do índice no Yahoo antes de qualquer
  // outra fonte, sem usar ETF, proxyTicker ou ativo negociável substituto.
  IBOV: ['^BVSP'],
  IFIX: ['IFIX.SA', '^IFIX'],
  SMLL: ['SMLL.SA', '^SMLL'],
  IDIV: ['IDIV.SA', '^IDIV']
...
async function fetchDirectYahooIndexHistory(indexCode, rangeKey, payload = {}) {
  ...
  const params = `range=${encodeURIComponent(cfg.range)}&interval=${encodeURIComponent(cfg.interval)}&includePrePost=false&events=div%2Csplits`;
    ...
    for (const host of hosts) {
      ...
      if (points.length > 1) {
        return {
          ...
          prices: points,
          chartHistory: points,
          source: `Yahoo Finance direct index symbol ${symbol}`,
          sourceVersion: '21.12.0-yahoo-direct-index-fallback-v2',
          official: false,
          ...
          warning: 'B3 oficial indisponível; usando símbolo direto do índice no Yahoo Finance, sem ETF/proxy/ticker substituto.'
        };
      }
    }
  }
  return {
    ...
    prices: [],
    chartHistory: [],
    source: `Yahoo Finance direct index symbol ${symbols.join(',')}`,
    official: false,
    directIndexSymbol: true,
    simulated: false,
    proxyTickerUsed: false,
    diagnostics: attempts,
    error: `Sem histórico suficiente para ${code} nos símbolos diretos ${symbols.join(', ')}.`
  };
}
...
export async function getAssetHistory(payload = {}) {
  ...
  if (officialIndex) {
    ...
    // Relevante para IBOV/IFIX/SMLL/IDIV: usar o símbolo DIRETO do índice no Yahoo Finance
    // quando a página oficial da B3 não entrega dados parseáveis. Isto não usa ETF, proxyTicker
    // nem substituto negociável; é o próprio código do índice no provedor de histórico.
    const yahooDirect = await fetchDirectYahooIndexHistory(officialIndex, rangeKey, payload);
    if (yahooDirect.status === 'OK') {
      return {
        ...
        warning: `B3 oficial sem pontos parseáveis agora; ${officialIndex} sincronizado pelo símbolo direto ${yahooDirect.yahooSymbol}, sem ETF/proxy.`
      };
    }
    ...
    if (investidor10Index.status === 'OK') {
      return {
        ...
        warning: `B3/Yahoo sem pontos parseáveis agora; ${officialIndex} sincronizado pela página de índice do Investidor10, sem ETF/proxy.`
      };
    }
    return {
      ...b3,
      status: b3.status || 'EMPTY',
      ticker: officialIndex,
      range: rangeKey,
      source: `B3 Oficial - ${officialIndex} daily-evolution`,
      official: true,
      ...
      simulated: false,
      proxyTickerUsed: false,
      fallbackAttempt: { yahooDirect, investidor10Index },
      error: b3.error || yahooDirect.error || investidor10Index.error || `Sem histórico suficiente para ${officialIndex}.`
    };
  }
