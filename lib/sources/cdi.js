...
export async function getCdiAccumulatedSeries(months = 12, timeoutMs = 5200) {
  const safeMonths = Math.max(1, Math.min(600, Number(months || 12)));
  const diagnostics = [];

  // Query the daily and monthly official SGS series concurrently. The old sequential chain could
  // spend two full network timeouts before reaching series 4391, which made CDI the slowest member
  // of the eight-item Analysis tape and could cause the whole snapshot request to miss the APK
  // transport deadline. Both sources are official and either one is sufficient.
  const [dailyByDate, monthlyByDate] = await Promise.all([
    fetchBcbByDateRange(CDI_DAILY_SERIES_ID, safeMonths, timeoutMs).catch(error => ({ rows: [], error: error?.message })),
    fetchBcbByDateRange(CDI_MONTHLY_SERIES_ID, safeMonths, timeoutMs).catch(error => ({ rows: [], error: error?.message }))
  ]);
