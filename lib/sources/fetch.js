...
async function attemptFetch(url, { timeoutMs, headers, method, body, signal, validators }) {
  ...
  try {
    ...
    const res = await providerFetch(url, { method, headers: finalHeaders, body, signal: controller.signal, valoraeTransport: { totalTimeoutMs: timeoutMs } });
    ...
    return { text: payload.text, status: res.status, ok: res.ok, validators: responseValidators(res), rawBytes: payload.rawBytes, finalUrl: res.url, contentType: res.headers.get('content-type') || '' };
  } finally {
  ...
  return coalesce(key, async () => {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        else if (payload.ok || payload.text) setCache(key, { text: payload.text, status: payload.status, validators: payload.validators, finalUrl: payload.finalUrl, contentType: payload.contentType }, ttlMs, staleMs);
        if (payload.ok || !isRetryableHttpStatus(payload.status) || attempt >= maxRetries) {
          return payload.revalidated && cached?.value
            ...
            : { text: payload.text, status: payload.status, cacheStatus: payload.ok ? 'LIVE' : 'LIVE_ERROR', url, finalUrl: payload.finalUrl || url, contentType: payload.contentType || '', attempts: attempt + 1 };
        }
      } catch (error) {
