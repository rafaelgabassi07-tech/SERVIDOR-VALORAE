...
async function attemptFetch(url, { timeoutMs, headers, method, body, signal, validators }) {
  ...
  try {
    ...
    if (res.status === 304) return { text: '', status: res.status, ok: true, revalidated: true, validators: responseValidators(res), retryAfterMs: responseRetryAfterMs(res) };
    ...
    return { text: payload.text, status: res.status, ok: res.ok, validators: responseValidators(res), rawBytes: payload.rawBytes, finalUrl: res.url, contentType: res.headers.get('content-type') || '', retryAfterMs: responseRetryAfterMs(res) };
  } finally {
