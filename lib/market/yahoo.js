...
async function fetchYahooLogoNetwork(ticker, { timeoutMs = 4500 } = {}) {
  ...
  for (const host of hosts) {
    ...
    try {
      const res = await providerFetch(url, { signal: controller.signal, headers: { 'User-Agent': process.env.VALORAE_USER_AGENT || 'Mozilla/5.0', Accept: 'application/json' } });
  ...
  for (const host of hosts) {
    ...
    try {
      ...
      return {
        ...
        displayName: result.meta?.longName || result.meta?.shortName || '',
...
export async function fetchYahooQuote(symbol, opts = {}) {
  ...
  return withMarketInflight('yahoo-quote', cacheKey, async () => {
    ...
    const quote = {
      ok: true,
      symbol: data.symbol,
      price: last?.close ?? data.regularMarketPrice,
      previousClose: prev,
      variationPct: prev && last?.close ? Number((((last.close - prev) / prev) * 100).toFixed(2)) : undefined,
      // The 1D chart already contains traded volume. Preserve the accumulated day volume so
      // FII category pages can recover liquidity even when the fundamentals provider has a gap.
      volume: Number(last?.volume || 0),
      regularMarketVolume: Number(data.summary?.totalVolume || last?.volume || 0),
      averageVolume: Number(data.summary?.averageVolume || 0),
      source: 'YahooChart',
      time: last?.date,
