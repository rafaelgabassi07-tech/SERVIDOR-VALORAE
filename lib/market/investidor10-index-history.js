...
export const VALORAE_INVESTIDOR10_INDEX_HISTORY_VERSION = '21.12.407-investidor10-direct-index-history-v2';
...
export function normalizeInvestidor10IndexHistory(raw, {
  ...
  // Comparadores de longo prazo continuam mensais por padrão. O ticker da Análise, porém,
  // precisa do último pregão real; nesse caso `granularity=daily` preserva a série diária.
  if (String(granularity || 'monthly').toLowerCase() !== 'daily') points = monthEndPoints(points);
...
export async function fetchInvestidor10DirectIndexHistory(indexCode = '', {
  ...
  const points = normalizeInvestidor10IndexHistory(response?.json, { indexCode: code, months, limit, granularity });
  const source = `Investidor10 API de cotações do índice ${code}`;
  return {
    ok: points.length >= 2,
    ...
    granularity: String(granularity || 'monthly').toLowerCase(),
    source,
