...
export const B3_CALENDAR_VERSION = '21.12.405-official-2026-session-rules-v3';
...
export function getB3HolidaySet(year) {
  ...
  const holidays = new Set([
    ...
    ...(y === 2026 ? [
      toDateKey(y, 12, 24), // B3 2026: sem sessão na véspera de Natal
      toDateKey(y, 12, 31), // B3 2026: sem sessão na véspera de Ano Novo
    ] : []),
...
export function getB3RegularSessionBounds(dateKey) {
  const key = String(dateKey || '').slice(0, 10);
  const date = parseDateKey(key);
  if (!date) {
    return { openMinute: 10 * 60, closeMinute: 17 * 60, openLabel: '10:00', closeLabel: '17:00' };
  }
  const year = date.getUTCFullYear();
  // Calendário oficial B3 2026: Quarta-feira de Cinzas (18/02) começa às 13h.
  // Mantemos a exceção explicitamente vinculada ao ano publicado pela B3 para não
  // presumir que o mesmo horário especial valerá automaticamente em anos futuros.
  const specialOpen1300 = year === 2026 && key === shiftDateKey(calculateEasterDateKey(year), -46);
  const openMinute = specialOpen1300 ? 13 * 60 : 10 * 60;
  return {
    openMinute,
    closeMinute: 17 * 60,
    openLabel: specialOpen1300 ? '13:00' : '10:00',
    closeLabel: '17:00',
  };
}
...
export function getB3MarketSession(date = new Date()) {
  ...
  const bounds = getB3RegularSessionBounds(sp.dateKey);
  const regularOpen = bounds.openMinute;
  const regularClose = bounds.closeMinute;
  let status = 'closed';
  ...
  return {
    ...
    tradingDay,
    status,
    regularOpen: bounds.openLabel,
    regularClose: bounds.closeLabel,
...
export function normalizeB3Range(rawRange = '1Y') {
  ...
  if (upper === '1A' || compact === '1ANO' || compact === '1YEAR') return '1Y';
  if (upper === '2A' || compact === '2ANOS' || compact === '2YEARS') return '2Y';
  if (upper === '5A' || compact === '5ANOS' || compact === '5YEARS') return '5Y';
  if (upper === 'TUDO' || compact === 'ALL') return 'MAX';
  // O APK usa as mesmas chaves dos seletores Yahoo/Compose (1mo/3mo/6mo/1y).
  // Antes essas chaves caíam silenciosamente em 1Y, o que deixava o gráfico
  // "Preço da carteira" lento, fora do período selecionado e, em alguns casos, vazio.
  if (compact === '1MO' || compact === '1MONTH' || compact === '1MES') return '1M';
  if (compact === '3MO' || compact === '3MONTHS' || compact === '3MESES') return '3M';
  if (compact === '6MO' || compact === '6MONTHS' || compact === '6MESES') return '6M';
