...
export const VALORAE_B3_INDEX_HISTORY_VERSION = '21.12.407-b3-index-column-alignment-v3';
...
function candidateUrls(indexCode, year) {
  ...
  // A rota canônica sem ano representa a visão corrente. Nunca a usamos para
  // preencher silenciosamente um ano histórico, pois isso poderia relabelar
  // cotações do ano atual com uma data passada caso a B3 ignore o parâmetro.
  return y === currentYear
    ? [`${base}?language=pt-br`, ...explicit]
    : explicit;
}

function declaredPageYear(text = '', indexCode = '') {
  const visible = stripTags(text);
  const code = normalizeIndexCode(indexCode).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const patterns = [
    new RegExp(`\\b${code}\\s*[-–—:]\\s*(20\\d{2})\\b`, 'i'),
    /evolu(?:c|ç)[aã]o\s+di[aá]ria[^0-9]{0,40}(20\d{2})/i,
    /daily\s+evolution[^0-9]{0,40}(20\d{2})/i
  ];
...
function responseCanRepresentYear(text = '', requestedYear, indexCode = '') {
  const year = Number(requestedYear);
  const currentYear = new Date().getUTCFullYear();
  if (!Number.isFinite(year) || year === currentYear) return true;
  const declared = declaredPageYear(text, indexCode);
  if (declared != null) return declared === year;
  // Para anos anteriores exigimos evidência explícita no payload. Isso é
  // preferível a fabricar uma série histórica correta apenas na aparência.
  const raw = String(text || '');
  return new RegExp(`(?:^|[^0-9])${year}(?:[-/.]|[^0-9])`).test(raw);
}
...
function parseRowsFromHtml(html = '', year = new Date().getUTCFullYear()) {
  ...
  for (const tr of trMatches) {
    // Preserve empty month cells. Removing them shifts Jul/Ago values into earlier columns and
    // silently makes a current B3 point look stale.
  ...
  return withMarketInflight('b3-index-history', cacheKey, async () => {
    ...
    for (const year of yearList) {
      ...
      for (const url of candidateUrls(code, year)) {
        ...
        const yearCompatible = responseCanRepresentYear(text, year, code);
        if (yearCompatible) {
        ...
        diagnostics.push({ year, url: finalUrl || url, status, cacheStatus, parsed: parsed.length, yearCompatible, declaredYear: declaredPageYear(text, code), error });
