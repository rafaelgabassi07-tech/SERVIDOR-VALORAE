...
const CHART_URL_KEYWORDS = /(chart|grafico|gr[aá]fico|cotacao|cota[cç][aã]o|preco|pre[cç]o|lucro|receita|receitaliquida|balancos|balan[cç]os|ativos|passivos|patrimonio|patrim[oô]nio|payout|historico|hist[oó]rico|indicadores|comparador|compare|indices|[íi]ndices|rentabilidade|profitability|fluxo|cashflow|dre|demonstrativo|statement|provento|dividend|yield|rendimento|distribui[cç][aã]o|carteira|vacancia|vac[âa]ncia|imoveis|im[oó]veis|segmento|negocio|neg[oó]cio|regiao|regi[oõ]es|acionista|acionistas|acionaria|acion[áa]ria|shareholder|shareholders|shareholding|ownership)/i;
...
export function discoverInvestidor10ChartApiUrls(html = '', ticker = '', type = '') {
  ...
  if (type !== 'FII' && ids.companyId) {
    ...
    add(`${base}/api/balancos/ativospassivos/table/${ids.companyId}/3650/`);
    add(`${base}/api/balancos/ativospassivos/table/${ids.companyId}/3650/false/`);
    add(`${base}/api/balancos/balancopatrimonial/table/${ids.companyId}/3650/`);
    add(`${base}/api/balancos/patrimonial/table/${ids.companyId}/3650/`);
...
function fieldForFinancialSeries(name = '') {
  ...
  if (key.includes('patrimonioliquido') || key === 'patrimonio' || key === 'pl' || key.includes('networth') || key.includes('shareholdersequity') || key === 'equity') return 'netWorth';
  // O Investidor10 envia o balanço patrimonial em linhas; ordem importa para não classificar
  // Ativo Circulante como Ativo Total nem Passivo Circulante como Passivo Total.
  if (key.includes('ativonaocirculante') || key.includes('ativonaocorrente') || key.includes('noncurrentassets') || key.includes('noncurrentasset')) return 'nonCurrentAssets';
  if (key.includes('ativocirculante') || key.includes('ativocorrente') || key.includes('currentassets') || key.includes('currentasset')) return 'currentAssets';
  if (key.includes('passivonaocirculante') || key.includes('passivonaocorrente') || key.includes('noncurrentliabilities') || key.includes('noncurrentliability')) return 'nonCurrentLiabilities';
  if (key.includes('passivocirculante') || key.includes('passivocorrente') || key.includes('currentliabilities') || key.includes('currentliability')) return 'currentLiabilities';
...
function tableHeaderLabel(header) {
  ...
  if (typeof header === 'object') return text(header.label ?? header.title ?? header.name ?? header.data ?? header.key ?? header.value ?? header.text ?? '');
  return text(header);
}

function periodFromFinancialTableHeader(header) {
  const raw = tableHeaderLabel(header);
  if (!raw) return '';
  const normalized = raw.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().trim();
  if (!/(20\d{2}|19\d{2}|ult|atual|1t|2t|3t|4t|q[1-4])/.test(normalized)) return '';
  if (/^(?:av|ah|%|var|variacao|varia[cç][aã]o|total|valor|valores)$/.test(normalized)) return '';
  const q1 = normalized.match(/\b([1-4])\s*t\s*[-/]?\s*(20\d{2}|19\d{2})\b/);
  if (q1) return `${q1[1]}T ${q1[2]}`;
  const q2 = normalized.match(/\b(20\d{2}|19\d{2})\s*[-/]?\s*([1-4])\s*t\b/);
  if (q2) return `${q2[2]}T ${q2[1]}`;
  const q3 = normalized.match(/\bq([1-4])\s*[-/]?\s*(20\d{2}|19\d{2})\b/);
  if (q3) return `${q3[1]}T ${q3[2]}`;
  const year = raw.match(/\b(20\d{2}|19\d{2})\b/)?.[1];
  if (year) return year;
  if (/ult(?:imo|\.)?\s*12\s*m/i.test(raw)) return 'Últ 12M';
  if (/atual/i.test(raw)) return 'Atual';
  return '';
}

function normalizeFinancialTablePayload(raw, preferred = '') {
  const out = new Map();
  const seen = new Set();
  const addValue = (period, field, value, index = 0) => {
    const cleanPeriod = text(period || '');
    if (!cleanPeriod || !field) return;
    const n = parseNum(value);
    if (n === null) return;
    const current = out.get(cleanPeriod) || pointBase(cleanPeriod, index);
    current[field] = n;
    out.set(cleanPeriod, current);
  };
  const headersFrom = (value) => arr(value).map(tableHeaderLabel).filter(Boolean);
  const parseRowObject = (row, headers = []) => {
    if (!row || typeof row !== 'object' || Array.isArray(row)) return;
    const rowLabel = text(row.label ?? row.name ?? row.title ?? row.indicador ?? row.indicator ?? row.conta ?? row.descricao ?? row.description ?? row.item ?? row.metric ?? row.campo ?? row[0] ?? '');
    const field = fieldForFinancialSeries(rowLabel) || fieldForFinancialSeries(preferred);
    if (!field) return;
    const valuesObj = row.values || row.valores || row.periodValues || row.periodos || row.dataByPeriod || row.anos;
    if (valuesObj && typeof valuesObj === 'object' && !Array.isArray(valuesObj)) {
      for (const [k, v] of Object.entries(valuesObj)) addValue(periodFromFinancialTableHeader(k) || k, field, v);
    }
    for (const [k, v] of Object.entries(row)) {
      if (['id','key','label','name','title','indicador','indicator','conta','descricao','description','item','metric','campo','source','values','valores','periodValues','periodos','dataByPeriod','anos'].includes(k)) continue;
      const byHeader = /^\d+$/.test(k) ? (headers[Number(k)] || '') : '';
      const period = periodFromFinancialTableHeader(k) || periodFromFinancialTableHeader(byHeader);
      if (period) addValue(period, field, v);
    }
    for (const key of ['values','valores','data','points']) {
      if (Array.isArray(row[key])) parseRowArray([rowLabel, ...row[key]], headers);
    }
  };
  const parseRowArray = (row, headers = []) => {
    const cells = arr(row).map(v => (v && typeof v === 'object' && !Array.isArray(v)) ? (v.value ?? v.valor ?? v.label ?? v.name ?? v.title ?? v.text ?? '') : v);
    if (!cells.length) return;
    let labelIndex = cells.findIndex((cell, idx) => idx <= 4 && fieldForFinancialSeries(cell));
    if (labelIndex < 0) labelIndex = 0;
    const rowLabel = text(cells[labelIndex]);
    const field = fieldForFinancialSeries(rowLabel) || fieldForFinancialSeries(preferred);
    if (!field) return;
    for (let i = 0; i < cells.length; i += 1) {
      if (i === labelIndex) continue;
      const period = periodFromFinancialTableHeader(headers[i] || '');
      if (!period) continue;
      addValue(period, field, cells[i], i);
    }
  };
  const parseRows = (rows, headers = []) => {
    for (const row of arr(rows)) {
      if (Array.isArray(row)) parseRowArray(row, headers);
      else parseRowObject(row, headers);
    }
  };
  const visit = (node, inheritedHeaders = [], depth = 0) => {
    if (depth > 6 || node == null) return;
    if (typeof node !== 'object') return;
    if (seen.has(node)) return;
    seen.add(node);
    if (Array.isArray(node)) {
      if (node.length && Array.isArray(node[0]) && node[0].some(cell => periodFromFinancialTableHeader(cell))) {
        parseRows(node.slice(1), headersFrom(node[0]));
        return;
      }
      parseRows(node, inheritedHeaders);
      node.slice(0, 40).forEach(item => visit(item, inheritedHeaders, depth + 1));
      return;
    }
    const headers = headersFrom(node.columns || node.colunas || node.headers || node.header || node.cabecalho || node.thead || node.years || node.anos || node.periods || node.periodos || inheritedHeaders);
    const rowContainers = ['rows','linhas','items','data','values','valores','metrics','indicadores','indicators','tbody','body','result','results'];
    let parsedContainer = false;
    for (const key of rowContainers) {
      if (Array.isArray(node[key])) {
        parseRows(node[key], headers);
        parsedContainer = true;
      }
    }
    if (!parsedContainer) parseRowObject(node, headers);
    for (const child of Object.values(node).slice(0, 80)) visit(child, headers, depth + 1);
  };
  visit(raw, []);
  return sortByChartPeriod([...out.values()].filter(p => Object.keys(p).some(k => !['label','year','quarter'].includes(k) && Number.isFinite(p[k]))));
}

function normalizeFinancialChart(raw, preferred = '') {
  const out = new Map();
  for (const point of normalizeFinancialTablePayload(raw, preferred)) {
    if (!point?.label) continue;
    out.set(point.label, mergeFinancialPointObjects(out.get(point.label), point) || point);
  }
  const visit = (node, inheritedLabel = '', depth = 0) => {
    ...
    if (Array.isArray(node)) {
      ...
      node.forEach((item, i) => {
        if (item && typeof item === 'object' && !Array.isArray(item)) {
          ...
          const fields = ['netRevenue','netProfit','grossProfit','cost','ebitda','ebit','netWorth','totalAssets','currentAssets','nonCurrentAssets','totalLiabilities','currentLiabilities','nonCurrentLiabilities','grossDebt','netDebt','cash','quote','payout','operatingCashFlow','freeCashFlow','capex','cashFlow'];
    ...
    if (node && typeof node === 'object') {
      ...
      const directFields = ['netRevenue','netProfit','grossProfit','cost','ebitda','ebit','netWorth','totalAssets','currentAssets','nonCurrentAssets','totalLiabilities','currentLiabilities','nonCurrentLiabilities','grossDebt','netDebt','cash','quote','payout','operatingCashFlow','freeCashFlow','capex','cashFlow'];
      let direct = false;
...
function toPt(f) {
  return ({ netRevenue: 'receitaLiquida', netProfit: 'lucroLiquido', grossProfit: 'lucroBruto', netWorth: 'patrimonioLiquido', totalAssets: 'ativos', currentAssets: 'ativoCirculante', nonCurrentAssets: 'ativoNaoCirculante', totalLiabilities: 'passivos', currentLiabilities: 'passivoCirculante', nonCurrentLiabilities: 'passivoNaoCirculante', grossDebt: 'dividaBruta', netDebt: 'dividaLiquida', cash: 'disponibilidade', quote: 'cotacao', operatingCashFlow: 'fluxoCaixaOperacional', freeCashFlow: 'fluxoCaixaLivre', cashFlow: 'fluxoCaixa' })[f] || f;
}
...
function financialFieldAliases(f) {
  return ({
    ...
    ebit: ['ebit','EBIT'],
    netWorth: ['netWorth','net_worth','patrimonioLiquido','patrimonio_liquido','patrimonio','pl','equity','shareholdersEquity','shareholders_equity'],
    ...
    currentAssets: ['currentAssets','current_assets','ativoCirculante','ativo_circulante','ativosCirculantes','ativos_circulantes'],
    nonCurrentAssets: ['nonCurrentAssets','non_current_assets','ativoNaoCirculante','ativo_nao_circulante','ativosNaoCirculantes','ativos_nao_circulantes'],
    ...
    currentLiabilities: ['currentLiabilities','current_liabilities','passivoCirculante','passivo_circulante','passivosCirculantes','passivos_circulantes'],
    nonCurrentLiabilities: ['nonCurrentLiabilities','non_current_liabilities','passivoNaoCirculante','passivo_nao_circulante','passivosNaoCirculantes','passivos_nao_circulantes'],
...
function classifyRoleFromName(name = '', url = '') {
  ...
  if (/posicaoacionaria|acionista|acionistas|shareholder|shareholders|shareholding|ownership/.test(k)) return 'shareholdingPosition';
...
function classifyRoleFromData(value) {
  ...
  if (objectHasKeys(value, ['acionista', 'shareholder', 'shareholders', 'shareholding', 'ownership', 'posicaoacionaria'])) return 'shareholdingPosition';
...
function pointCompleteness(p = {}) {
  return ['netRevenue','netProfit','grossProfit','cost','ebitda','ebit','netWorth','totalAssets','currentAssets','nonCurrentAssets','totalLiabilities','currentLiabilities','nonCurrentLiabilities','grossDebt','netDebt','cash','quote','payout']
...
function mergeFinancialPointObjects(a = null, b = null) {
  ...
  for (const key of ['netRevenue','netProfit','grossProfit','cost','ebitda','ebit','netWorth','totalAssets','currentAssets','nonCurrentAssets','totalLiabilities','currentLiabilities','nonCurrentLiabilities','grossDebt','netDebt','cash','quote','payout']) {
...
export function buildInvestidor10CanonicalCharts({ ticker = '', type = '', html = '', apiExtras = {} } = {}) {
  ...
  const balanceSources = [
    ...collectionValues({ evolucaoPatrimonio: charts.evolucaoPatrimonio, balanceSheet: charts.balanceSheet, balanceSheetTable: charts.balanceSheetTable, balancoPatrimonial: charts.balancoPatrimonial, balancoPatrimonialTable: charts.balancoPatrimonialTable, rawEvolucaoPatrimonio: rawJson.evolucaoPatrimonio, rawBalanceSheet: rawJson.balanceSheet, rawBalanceSheetTable: rawJson.balanceSheetTable, rawBalancoPatrimonial: rawJson.balancoPatrimonial, rawBalancoPatrimonialTable: rawJson.balancoPatrimonialTable }),
  ...
  const revenueRegionSources = [
    ...
    ...(Array.isArray(rawJson.revenueGeographySources) ? rawJson.revenueGeographySources.map((value, index) => ({ key: `rawRevenueGeographySource_${index}`, value })) : []),
    ...(grouped.get('revenueRegion') || []),
  ].filter(x => x.value);
  const revenueBusinessSources = [
    ...collectionValues({ embeddedBusiness: apiExtras.embedded?.revenueSegment, rawRevenueSegment: rawJson.revenueSegment, rawRevenueByBusiness: rawJson.revenueByBusiness, rawBusinessRevenue: rawJson.businessRevenue, rawSegmentRevenue: rawJson.segmentRevenue, rawProductRevenue: rawJson.productRevenue, rawNegociosReceita: rawJson.negociosReceita, rawNegociosQueGeramReceita: rawJson.negociosQueGeramReceita, rawReceitaPorNegocio: rawJson.receitaPorNegocio, chartRevenueSegment: charts.revenueSegment, chartRevenueByBusiness: charts.revenueByBusiness, chartBusinessRevenue: charts.businessRevenue, chartSegmentRevenue: charts.segmentRevenue, chartProductRevenue: charts.productRevenue }),
    ...(Array.isArray(rawJson.revenueSegmentSources) ? rawJson.revenueSegmentSources.map((value, index) => ({ key: `rawRevenueSegmentSource_${index}`, value })) : []),
