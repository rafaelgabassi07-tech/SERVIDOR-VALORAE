...
const CHART_URL_KEYWORDS = /(chart|grafico|gr[aá]fico|cotacao|cota[cç][aã]o|lucro|balancos|balan[cç]os|payout|historico|hist[oó]rico|indicadores|comparador|compare|indices|[íi]ndices|rentabilidade|profitability|fluxo|cashflow|dre|demonstrativo|statement)/i;
...
function normalizeFinancialChart(raw, preferred = '') {
  ...
  const visit = (node, inheritedLabel = '', depth = 0) => {
    ...
    if (Array.isArray(node)) {
      ...
      node.forEach((item, i) => {
        if (item && typeof item === 'object' && !Array.isArray(item)) {
          ...
          const fields = ['netRevenue','netProfit','grossProfit','cost','ebitda','ebit','netWorth','totalAssets','totalLiabilities','quote','payout','operatingCashFlow','freeCashFlow','capex','cashFlow'];
    ...
    if (node && typeof node === 'object') {
      ...
      const directFields = ['netRevenue','netProfit','grossProfit','cost','ebitda','ebit','netWorth','totalAssets','totalLiabilities','quote','payout','operatingCashFlow','freeCashFlow','capex','cashFlow'];
      let direct = false;
...
function toPt(f) {
  return ({ netRevenue: 'receitaLiquida', netProfit: 'lucroLiquido', grossProfit: 'lucroBruto', netWorth: 'patrimonioLiquido', totalAssets: 'ativos', totalLiabilities: 'passivos', quote: 'cotacao', operatingCashFlow: 'fluxoCaixaOperacional', freeCashFlow: 'fluxoCaixaLivre', cashFlow: 'fluxoCaixa' })[f] || f;
}
...
function classifyRoleFromName(name = '', url = '') {
  ...
  if (/fluxodecaixa|cashflow|cashflowstatement/.test(k)) return 'cashFlowStatement';
  if (/resultado|resultados|dre|incomestatement|demonstrativoresultado/.test(k)) return 'incomeStatement';
  if (/receitaliquida|receitaslucros|revenueprofit|receita|lucros/.test(k)) return 'revenueProfit';
...
function classifyRoleFromData(value) {
  ...
  if (objectHasKeys(value, ['fluxocaixa', 'cashflow', 'fcf', 'capex'])) return 'cashFlowStatement';
...
function pointCompleteness(p = {}) {
  return ['netRevenue','netProfit','grossProfit','cost','ebitda','ebit','netWorth','totalAssets','totalLiabilities','quote','payout']
...
function mergeFinancialPointObjects(a = null, b = null) {
  ...
  for (const key of ['netRevenue','netProfit','grossProfit','cost','ebitda','ebit','netWorth','totalAssets','totalLiabilities','quote','payout']) {
...
function mergeFinancialPointLists(lists = []) {
  ...
  return [...map.values()]
    ...
    .sort((a,b) => String(a.year || a.label).localeCompare(String(b.year || b.label), 'pt-BR', { numeric: true }));
}
...
function mergePayoutSourceList(list) {
  ...
  return [...map.values()].filter(p => Number.isFinite(p.value)).sort((a,b) => String(a.year || a.label).localeCompare(String(b.year || b.label), 'pt-BR', { numeric: true }));
}
...
function extractFiiPhysicalAssetsFromHtml(html = '') {
  ...
  if (idx < 0) return [];
  const endRel = plain.slice(idx + 20).search(/hist[oó]rico\s+da\s+taxa\s+de\s+vac[aâ]ncia|COMUNICADOS|Informações\s+sobre\s+valor\s+patrimonial|Média\s+do\s+Tipo|Notícias\s+sobre/i);
  const sec = plain.slice(idx, endRel > 0 ? idx + 20 + endRel : idx + 4200).replace(/\s+/g, ' ').trim();
  const out = [];
  const patterns = [
  ...
  for (const re of patterns) {
    let m;
    while ((m = re.exec(sec)) && out.length < 120) {
      const nome = text(m[1]).replace(/^(Lista de Imóveis\s*)/i, '').replace(/(?:^|\s)(?:São Paulo|Minas Gerais|Rio de Janeiro|Pernambuco|Bahia|Goiás|Santa Catarina|Paraná|Ceará|Amazonas|Pará|Espírito Santo|Distrito Federal)\s+\d+\s+/gi, '').trim();
  ...
  return uniqBy(out, item => `${compactKey(item.nome)}:${compactKey(item.estado)}:${compactKey(item.area_bruta_locavel)}`);
}
