...
export const VALORAE_I10_CHART_EXTRACTOR_VERSION = '21.12.62-i10-complete-asset-charts';
...
function buildCoverage({ html = '', type = '', available = {}, extras = {} } = {}) {
  ...
  return {
    ...
    summary: { visible, captured, totalExpected: checks.length, requiredMissing: checks.filter(x => x.required && x.presentInHtml && !x.captured).map(x => x.key) },
    checks,
  };
}
...
export function buildInvestidor10CanonicalCharts({ ticker = '', type = '', html = '', apiExtras = {} } = {}) {
  ...
  const financial = {
    ...
    balanceSheet: firstNonEmpty(balanceSources, src => normalizeFinancialChart(src, '')),
  ...
  const fiiDividendYieldHistory = firstNonEmpty([
  ...
  const fiiDividendHistory = firstNonEmpty([
  ...
  const available = {
    ...
    equityEvolution: financial.balanceSheet.length > 0,
    ...
    dividendYieldHistory: Boolean(pointCount(fiiDividendYieldHistory) || (grouped.get('dividendYieldHistory') || []).length),
    dividendHistory: Boolean(pointCount(fiiDividendHistory) || (grouped.get('dividendHistory') || []).length),
  ...
  return {
    ...
    fii: type === 'FII' ? {
      ...
      dividendYieldHistory: fiiDividendYieldHistory,
      dividendHistory: fiiDividendHistory,
      physicalAssets: fiiPhysicalAssets,
    } : undefined,
    company: type !== 'FII' ? { info } : undefined,
    sourceDiagnostics: {
