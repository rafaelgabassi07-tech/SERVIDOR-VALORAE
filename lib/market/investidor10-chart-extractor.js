...
export const VALORAE_I10_CHART_EXTRACTOR_VERSION = '21.12.69-asset-charts-performance-web-index';

const I10_BASE = 'https://investidor10.com.br';
const PERIOD_ORDER = ['1 mês', '3 meses', '1 ano', '2 anos', '5 anos', '10 anos'];
const CHART_URL_KEYWORDS = /(chart|grafico|gr[aá]fico|cotacao|cota[cç][aã]o|lucro|balancos|balan[cç]os|payout|historico|hist[oó]rico|indicadores|comparador|compare|indices|[íi]ndices|rentabilidade|profitability)/i;
...
export function extractInvestidor10ChartIds(html = '') {
  const source = String(html || '');
  const companyId = source.match(/\/api\/balancos\/receitaliquida\/chart\/(\d+)\//)?.[1] ||
    source.match(/\/api\/balancos\/ativospassivos\/chart\/(\d+)\//)?.[1] ||
    source.match(/companyId\s*[:=]\s*['"]?(\d+)['"]?/i)?.[1] ||
    source.match(/data-company-id=["'](\d+)["']/i)?.[1] || '';
  const tickerId = source.match(/tickerId\s*[:=]\s*['"]?(\d+)['"]?/i)?.[1] ||
    source.match(/data-ticker-id=["'](\d+)["']/i)?.[1] ||
    source.match(/\/api\/acoes\/payout-chart\/\d+\/(\d+)\//)?.[1] || '';
  const fiiId = source.match(/\/api\/fii\/historico-indicadores\/(\d+)\//i)?.[1] ||
    source.match(/\/api\/fii\/comparador\/table\/(\d+)\//i)?.[1] ||
    source.match(/fiiId\s*[:=]\s*['"]?(\d+)['"]?/i)?.[1] ||
    source.match(/data-fii-id=["'](\d+)["']/i)?.[1] || '';
...
function fieldForFinancialSeries(name = '') {
  ...
  if (key.includes('receitaliquida') || key.includes('receita') || key.includes('revenue') || key.includes('faturamento')) return 'netRevenue';
  ...
  if (key.includes('patrimonioliquido') || key === 'pl' || key.includes('networth') || key.includes('equity')) return 'netWorth';
  if (key.includes('ativototal') || key.includes('totalativos') || key === 'ativos' || key.includes('assets')) return 'totalAssets';
  if (key.includes('passivototal') || key.includes('totalpassivos') || key === 'passivos' || key.includes('liabilities')) return 'totalLiabilities';
...
function normalizeFinancialChart(raw, preferred = '') {
  ...
  const visit = (node, inheritedLabel = '', depth = 0) => {
    ...
    if (Array.isArray(node)) {
      ...
      node.forEach((item, i) => {
        if (item && typeof item === 'object' && !Array.isArray(item)) {
          ...
          const fields = ['netRevenue','netProfit','grossProfit','cost','ebitda','ebit','netWorth','totalAssets','totalLiabilities','quote','payout'];
          for (const f of fields) mergePoint(out, label, f, item[f] ?? item[toSnake(f)] ?? item[toPt(f)], i);
          const field = fieldForFinancialSeries(inheritedLabel || item.name || item.label || item.key || preferred);
          if (field && !fields.some(f => item[f] !== undefined)) mergePoint(out, label, field, valueOfPoint(item), i);
    ...
    if (node && typeof node === 'object') {
      ...
      const directFields = ['netRevenue','netProfit','grossProfit','cost','ebitda','ebit','netWorth','totalAssets','totalLiabilities','quote','payout'];
      let direct = false;
      for (const f of directFields) {
        const val = node[f] ?? node[toSnake(f)] ?? node[toPt(f)];
        if (val !== undefined) { mergePoint(out, directLabel, f, val); direct = true; }
      }
      if (!direct) {
...
function toPt(f) {
  return ({ netRevenue: 'receitaLiquida', netProfit: 'lucroLiquido', grossProfit: 'lucroBruto', netWorth: 'patrimonioLiquido', totalAssets: 'ativos', totalLiabilities: 'passivos', quote: 'cotacao' })[f] || f;
}
...
const ACAO_EXPECTED_CHARTS = [
  ...
  { key: 'indexComparison', title: 'Comparação com Índices', required: true, html: /COMPARAÇÃO\s+DE[\s\S]{0,80}COM\s+ÍNDICES/i, aliases: ['comparacaoIndices','indexComparison','indicesComparison'] },
...
const FII_EXPECTED_CHARTS = [
  ...
  { key: 'physicalAssets', title: 'Lista/Distribuição de imóveis', required: false, html: /Lista\s+de\s+Imóveis|Área\s+bruta\s+locável/i, aliases: ['imoveis','physicalAssets','fiiAssetDistribution'] },
];
...
function classifyRoleFromName(name = '', url = '') {
  ...
  if (/comparacao|comparison|comparador|indices|ifix|ibov|ipca|cdi/.test(k)) return 'indexComparison';
  if (/commodity|commodit|brent|petroleo|ouro|cafe|milho|wti/.test(k)) return 'commodityComparison';
  if (/receitaliquida|receitaslucros|revenueprofit|receita|lucros|resultados/.test(k)) return 'revenueProfit';
  if (/cotacaolucro|lucrocotacao|profitvsquote|lucro.*cotacao|cotacao.*lucro/.test(k)) return 'profitVsQuote';
  if (/ativospassivos|balanco|balancos|balancesheet|evolucaopatrimonio|patrimonio|ativo|passivo/.test(k)) return 'balanceSheet';
  if (/payout/.test(k)) return 'payoutHistory';
  if (/historic.*indicador|indicadores|fundamental/.test(k)) return 'fundamentalIndicatorHistory';
  ...
  if (/dividend|dividendo|provento|distribuicao|rendimento/.test(k)) return 'dividendHistory';
  if (/yield|dyhistory|dividendyield/.test(k)) return 'dividendYieldHistory';
  return '';
}
...
function classifyRoleFromData(value) {
  ...
  if (objectHasKeys(value, ['ativo', 'assets', 'passivo', 'liabilities', 'patrimonioliquido', 'networth'])) return 'balanceSheet';
  ...
  if (objectHasKeys(value, ['dividend', 'dividendo', 'rendimento', 'provento'])) return 'dividendHistory';
  return '';
}
...
function extractSimpleInfoBlock(html = '', type = '') {
  const plain = stripTags(html);
  const startRe = type === 'FII' ? /INFORMAÇÕES\s+SOBRE\s+[A-Z0-9]+/i : /INFORMAÇÕES\s+SOBRE\s+A\s+EMPRESA/i;
  const start = plain.search(startRe);
  if (start < 0) return {};
  const end = plain.slice(start + 20).search(type === 'FII' ? /HISTÓRICO\s+DE|COMPARAÇÃO\s+DE|COMPARANDO\s+COM/i : /Regiões\s+onde|negócios\s+que|POSIÇÃO\s+ACIONÁRIA|Receitas\s+e\s+Lucros/i);
  const sec = plain.slice(start, end > 0 ? start + 20 + end : start + 2000);
  const known = ['Razão Social','CNPJ','PÚBLICO-ALVO','MANDATO','SEGMENTO','TIPO DE FUNDO','PRAZO DE DURAÇÃO','TIPO DE GESTÃO','TAXA DE ADMINISTRAÇÃO','VACÂNCIA','NUMERO DE COTISTAS','COTAS EMITIDAS','VAL. PATRIMONIAL P/ COTA','VALOR PATRIMONIAL','ÚLTIMO RENDIMENTO','Valor de mercado','Valor de firma','Patrimônio Líquido','Nº total de papeis','Ativos','Ativo Circulante','Dívida Bruta','Dívida Líquida','Disponibilidade','Free Float','Tag Along','Liquidez Média Diária'];
  const fields = {};
  for (let i = 0; i < known.length; i++) {
    const label = known[i];
    const re = new RegExp(`${label.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s+([^]+?)(?=${known.map(x => x.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')}|$)`, 'i');
    const m = sec.match(re);
    if (m) {
      const value = text(m[1]).replace(/\s+/g, ' ').trim();
      if (value && value.length < 140) fields[label] = value;
    }
  }
  return fields;
}
  ...
  for (const re of patterns) {
    ...
    while ((m = re.exec(sec)) && out.length < 600) {
      ...
      if (m[1] && /^(JSCP|JCP|Dividend|Rendimento|Amort|Red|Bonif)/i.test(m[1])) {
        kind = m[1]; dataComRaw = m[2]; payRaw = m[3]; valueRaw = m[m.length - 1];
      } else {
...
function buildDividendAggregatesFromEvents(events = [], currentPrice = null) {
  ...
  const dividendYieldHistory = (currentPrice && currentPrice > 0) ? yearly.map(p => ({
    label: 'DY %', year: p.year, value: p.value / currentPrice * 100,
    display: `${(p.value / currentPrice * 100).toFixed(2)}%`, unit: '%',
    source: 'Investidor10HTML.assetDividendHistory.currentPriceEstimate',
    note: 'DY anual estimado por soma de proventos do ano dividida pela cotação atual capturada.'
  })) : [];
  return { yearly, monthly, dividendYieldHistory };
}

function buildCoverage({ html = '', type = '', available = {}, extras = {} } = {}) {
  const defs = type === 'FII' ? FII_EXPECTED_CHARTS : ACAO_EXPECTED_CHARTS;
...
export function buildInvestidor10CanonicalCharts({ ticker = '', type = '', html = '', apiExtras = {} } = {}) {
  ...
  const balanceSheet = firstNonEmpty(balanceSources, src => normalizeFinancialChart(src, ''));
  const equityEvolution = firstNonEmpty(equitySources, src => normalizeFinancialChart(src, ''));
  const financial = {
    revenueProfit: firstNonEmpty(revenueProfitSources, src => normalizeFinancialChart(src, '')),
    profitVsQuote: firstNonEmpty(profitVsQuoteSources, src => normalizeProfitVsQuote(src)),
    equityEvolution: equityEvolution.length ? equityEvolution : balanceSheet,
    balanceSheet,
    payoutHistory: firstNonEmpty(payoutSources, src => normalizePayout(src)),
  };

  const fiiDistribution12m = extractFiiDistribution12mFromHtml(html);
  const info = extractSimpleInfoBlock(html, type);
  ...
  const fiiFundamentalIndicatorHistory = firstNonEmpty([
    ...collectionValues({ historicoIndicadoresFii: apiExtras.historicoIndicadoresFii, rawHistoricoIndicadores: rawJson.historicoIndicadores }).filter(x => x.value),
  ...
  const canonicalDividendYieldHistory = pointCount(apiDividendYieldHistory) ? apiDividendYieldHistory : htmlDividendAggregates.dividendYieldHistory;
  const fiiPhysicalAssets = firstNonEmpty([
    ...collectionValues({ physicalAssets: rawJson.physicalAssets, listaImoveis: rawJson.listaImoveis, imoveis: rawJson.imoveis, ativosFundo: rawJson.ativosFundo }).filter(x => x.value),
  ...
  const available = {
    ...
    fundamentalIndicatorHistory: Boolean(apiExtras.historicoIndicadoresFii || pointCount(fiiFundamentalIndicatorHistory) || (grouped.get('fundamentalIndicatorHistory') || []).length),
    ...
    dividendYieldHistory: Boolean(pointCount(canonicalDividendYieldHistory) || htmlDividendAggregates.dividendYieldHistory.length || (grouped.get('dividendYieldHistory') || []).length),
    ...
    physicalAssets: Boolean(pointCount(fiiPhysicalAssets) || /Lista\s+de\s+Imóveis|Área\s+bruta\s+locável/i.test(html)),
    fiiInfo: Object.keys(info).length > 0,
    revenueRegion: Boolean(apiExtras.embedded?.revenueGeography),
    revenueBusiness: Boolean(apiExtras.embedded?.revenueSegment),
  };
  const extras = {
    ...
    fiiPhysicalAssets,
    info,
    revenueGeography: apiExtras.embedded?.revenueGeography,
    revenueSegment: apiExtras.embedded?.revenueSegment,
  };
  ...
  return {
    ...
    company: type !== 'FII' ? { info, dividendHistory: canonicalDividendHistory, dividendMonthly: htmlDividendAggregates.monthly, dividendYearly: htmlDividendAggregates.yearly, dividendYieldHistory: canonicalDividendYieldHistory } : undefined,
    sourceDiagnostics: {
