...
export const VALORAE_INDICES_MARKET_VERSION = '21.12.405-market-ticker-multisource-v12';
...
async function fetchMarketTickerQuote(name, symbol) {
  ...
  // 3) Investidor10 fica apenas como contingência terciária, nunca como fonte preferencial do tape.
  //
  ...
  if (B3_INDEXES.has(name)) {
    ...
    if (usableMarketQuote(yahoo)) return yahoo;
    if (usableMarketQuote(b3)) return b3;

    const comparison = await fetchComparisonDirectIndexQuote(name)
      .catch(error => ({ ok: false, error: error?.message || String(error) }));
    if (usableMarketQuote(comparison)) return comparison;

    const yahooHistory = await fetchYahooHistoryQuote(name, symbol);
    if (usableMarketQuote(yahooHistory)) return yahooHistory;
    return {
      ok: false,
      error: [yahoo?.error, b3?.error, comparison?.error, yahooHistory?.error].filter(Boolean).join(' | ') || `${name} indisponível`
    };
  }
  ...
  if (name === 'USD') {
    ...
    if (usableMarketQuote(yahoo)) return yahoo;
    if (usableMarketQuote(bcb)) return bcb;
...
export async function fetchIndicesSnapshot({ symbols = INDEX_SYMBOLS, bypassCache = false, cache = true } = {}) {
  ...
  return withMarketInflight('indices', key, async () => {
    ...
    const data = {
      ...
      cache: 'MISS',
      policy: 'Ticker de mercado: IBOV/IFIX/IDIV/SMLL usam Yahoo intradiário quando disponível e fechamento oficial B3 como contingência primária; Investidor10 é apenas fallback terciário. USD usa Yahoo com PTAX/SGS 1 do BCB como contingência; IVVB11 usa Yahoo com cotação pública oficial da B3 como contingência; CDI/IPCA+ usam séries oficiais do BCB. Sem valores sintéticos.'
    };
