...
export const VALORAE_INDICES_MARKET_VERSION = '21.12.0-market-ticker-resilience-v10';
...
// B3 daily-evolution is the final real-source fallback for every direct B3 index shown
// in the Analysis tape. Yahoo coverage for IFIX/IDIV/SMLL is intermittent and must not
// leave those slots permanently empty when the comparison provider is unavailable.
...
function mergePartialTickerWithStale(current, staleData) {
  ...
  const tickerItems = VALORAE_ANALYSIS_TICKER_ORDER.map(code => {
    ...
    if (tickerRowUsable(fresh)) return fresh;
...
async function fetchMarketTickerQuote(name, symbol) {
  // IBOV/IFIX/IDIV/SMLL are already proven by the comparison graph. Run that source and the
  // lightweight Yahoo quote concurrently, then prefer the comparison source when it is usable.
  // This removes the previous sequential 2-3 request chain from the hot path.
  let q;
  if (COMPARISON_DIRECT_INDEXES.has(name)) {
    const [comparison, yahoo] = await Promise.all([
    ...
    q = usableMarketQuote(comparison) ? comparison : yahoo;
  } else {
    q = await fetchYahooQuote(symbol, { interval: DIRECT_YAHOO_INDEXES.has(name) ? '1d' : undefined })
      .catch(error => ({ ok: false, error: error?.message || String(error) }));
  }
  ...
  if (!usableMarketQuote(q) && B3_INDEXES.has(name)) {
    q = await fetchB3FallbackQuote(name);
  }
  return q;
}
