...
export const VALORAE_INDICES_MARKET_VERSION = '21.12.0-yahoo-direct-index-v3';
...
// Último snapshot conhecido informado a partir do próprio Yahoo Finance. Usado somente
// como snapshot atual degradado, nunca para construir histórico/rentabilidade no gráfico.
const LAST_KNOWN_YAHOO_INDEX_SNAPSHOT = {
  IFIX: { price: 3814.03, variationPct: 0.30, asOf: '2026-06-12T17:17:00-03:00' },
  IDIV: { price: 12117.78, variationPct: -0.29, asOf: '2026-06-12T17:17:00-03:00' },
  SMLL: { price: 2214.89, variationPct: -0.40, asOf: '2026-06-12T17:20:00-03:00' }
};
...
function lastKnownFallback(name, symbol, error) {
  const fallback = LAST_KNOWN_YAHOO_INDEX_SNAPSHOT[name];
  ...
  return {
    ...
    source: `Yahoo Finance Chart API índice direto ${symbol} - último snapshot conhecido`,
    time: fallback.asOf,
    cache: 'LAST_KNOWN_IF_ERROR',
    staleFallback: true,
    warning: `${name} sem resposta ao vivo; exibindo último snapshot conhecido do Yahoo Finance.`,
    error
  };
}
...
export async function fetchIndicesSnapshot({ symbols = INDEX_SYMBOLS, bypassCache = false, cache = true } = {}) {
  ...
  return withMarketInflight('indices', key, async () => {
    ...
    const rows = await Promise.all(entries.map(async ([name, symbol]) => {
      ...
      if (!q.ok && B3_INDEXES.has(name)) q = await fetchB3FallbackQuote(name);
      if (!q.ok && DIRECT_YAHOO_INDEXES.has(name)) q = lastKnownFallback(name, symbol, q.error) || q;
      return quoteRow(name, symbol, q);
    }));
    const data = {
      ok: rows.some(r => r.ok),
      source: 'Yahoo Finance Chart API + B3 fallback',
      sourceVersion: VALORAE_INDICES_MARKET_VERSION,
      generatedAt: new Date().toISOString(),
      indices: rows,
      cache: 'MISS',
      policy: 'IFIX, IDIV e SMLL usam símbolos diretos do índice no Yahoo Finance; sem ETF/proxy/ticker substituto.'
    };
    if (data.ok) {
