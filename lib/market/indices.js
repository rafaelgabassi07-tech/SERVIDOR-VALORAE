...
export const VALORAE_INDICES_MARKET_VERSION = '21.12.0-market-ticker-source-parity-v7';
...
function macroRow(name, label, payload = {}, { accumulatedField = 'accumulatedPercent', monthlyField = 'monthlyPercent' } = {}) {
  ...
  return {
    ...
    symbol: name,
    code: name,
    ok: payload.ok === true || payload.status === 'OK' || value != null,
    price: value,
    value,
    variationPct: null,
...
export async function fetchIndicesSnapshot({ symbols = INDEX_SYMBOLS, bypassCache = false, cache = true } = {}) {
  ...
  return withMarketInflight('indices', key, async () => {
    ...
    const tickerOrder = ['USD', 'IFIX', 'IDIV', 'SMLL', 'CDI', 'IPCA', 'IBOV', 'IVVB11'];
    const tickerItems = tickerOrder.map(code => rows.find(row => row.ticker === code)).filter(Boolean);
    const data = {
      ...
      tickerItems,
      items: tickerItems,
      partial: tickerItems.some(row => !row.ok),
      cache: 'MISS',
