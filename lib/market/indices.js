...
export const VALORAE_INDICES_MARKET_VERSION = '21.12.407-market-ticker-infinite-v13';
...
async function fetchMarketTickerQuote(name, symbol) {
  ...
  if (B3_INDEXES.has(name)) {
    ...
    const freshest = chooseFreshestTickerQuote([yahoo, b3, parallelDirect].filter(Boolean));
