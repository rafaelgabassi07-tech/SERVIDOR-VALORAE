...
export const VALORAE_INDICES_MARKET_VERSION = '21.12.0-yahoo-direct-index-only-v4';
...
export async function fetchIndicesSnapshot({ symbols = INDEX_SYMBOLS, bypassCache = false, cache = true } = {}) {
  ...
  return withMarketInflight('indices', key, async () => {
    ...
    const data = {
      ...
      indices: rows,
      cache: 'MISS',
      policy: 'IFIX, IDIV e SMLL usam exclusivamente Yahoo Finance Chart API nos símbolos IFIX.SA, IDIV.SA e SMLL.SA; sem Investidor10, Mais Retorno, B3, ETF/proxy/ticker substituto.'
    };
    if (data.ok) {
