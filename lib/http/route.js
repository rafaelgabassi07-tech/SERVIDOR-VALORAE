...
export const VALORAE_ROUTE_UTILS_VERSION = '21.12.59';
...
export function getBaseUrl(req) {
  ...
  const localDevHost = /^(localhost|127\.0\.0\.1|0\.0\.0\.0|\[?::1\]?)(?::\d{1,5})?$/.test(host);
  const defaultProto = localDevHost && process.env.NODE_ENV !== 'production' ? 'http' : 'https';
  const proto = safeForwardedProto(req?.headers?.['x-forwarded-proto'] || defaultProto);
...
export async function withRouteDeadline(work, timeoutMs, fallbackFactory) {
  const ms = Number(timeoutMs);
  if (!Number.isFinite(ms) || ms <= 0) return typeof work === 'function' ? work() : work;
  let timer;
  ...
  try {
    return await Promise.race([typeof work === 'function' ? work() : work, timeout]);
  } finally {
...
export function beginRoute(req, res, options = {}) {
  ...
  if (rateMax !== false && !req.valoraeGlobalRateApplied) {
