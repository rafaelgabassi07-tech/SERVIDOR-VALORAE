...
import { recordRequestStart } from '../observability/server-metrics.js';
import { resolveClientAuth, shouldRequireClientAuth } from '../security/client-auth.js';

export const VALORAE_ROUTE_UTILS_VERSION = '21.12.0';
...
export function beginRoute(req, res, options = {}) {
  ...
  recordRequestStart(req, { route: options.route || options.profile });
  ...
  if (req.method === 'OPTIONS') {
    res.status(200).end();
    return { done: true, requestId, clientAuth };
  }
  try {
  ...
  const authRequired = shouldRequireClientAuth(options);
  if (authRequired && !clientAuth.ok) {
    sendJson(req, res, {
      version,
      status: 'UNAUTHORIZED',
      requestId,
    ...
    }, { status: 401, engineVersion: version, profile: options.profile || options.route || 'api' });
    return { done: true, requestId, clientAuth };
  }
  ...
  return { done: false, requestId, input: getInput(req), clientAuth };
}


function validationDetailsForCode(code) {
  const details = {
    MISSING_TARGET_URL: {
      expectedValidationError: true,
      hint: 'Envie o parâmetro url com uma URL HTTPS de fonte permitida.',
      example: '/api/scrape?url=https://investidor10.com.br/acoes/petr4/&includeHtml=0',
      benchmarkMeaning: 'expected-client-validation',
    },
    INVALID_TARGET_URL_PROTOCOL: {
      expectedValidationError: true,
      hint: 'Use https://. O scraping direto não aceita http:// por segurança.',
      example: '/api/scrape?url=https://investidor10.com.br/acoes/petr4/&selector=h1',
      benchmarkMeaning: 'expected-security-validation',
    },
    SCRAPE_HOST_NOT_ALLOWED: {
      expectedValidationError: true,
      hint: 'Use um host permitido ou configure VALORAE_ALLOWED_SCRAPE_HOSTS somente para fontes confiáveis.',
      allowedByDefault: ['investidor10.com.br', 'www.investidor10.com.br', 'statusinvest.com.br', 'www.statusinvest.com.br'],
      benchmarkMeaning: 'expected-security-validation',
    },
    TARGET_HOST_PRIVATE: {
      expectedValidationError: true,
      hint: 'Hosts privados/locais são bloqueados para evitar SSRF.',
      benchmarkMeaning: 'expected-security-validation',
    },
    TARGET_URL_TOO_LONG: {
      expectedValidationError: true,
      hint: 'Reduza o tamanho da URL de destino.',
      benchmarkMeaning: 'expected-client-validation',
    },
  };
...
export function sendRouteError(req, res, err, options = {}) {
  const safe = sanitizeError(err);
  const validationDetails = validationDetailsForCode(safe.code);
