...
function undiciPoolConstructor() {
  if (poolConstructor) return poolConstructor;
  const imported = require('undici');
  poolConstructor = imported?.Pool;
  if (typeof poolConstructor !== 'function') {
    const error = new Error('Undici Pool indisponível para o transporte gerenciado.');
    error.code = 'VALORAE_HTTP_POOL_UNAVAILABLE';
    throw error;
  }
