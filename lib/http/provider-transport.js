...
import { VALORAE_HTTP_TRANSPORT_VERSION } from '../core/feature-versions.js';
export { VALORAE_HTTP_TRANSPORT_VERSION } from '../core/feature-versions.js';
export const VALORAE_HTTP_TRANSPORT_POLICY = 'provider-pools-backpressure-fallback-v1';
export const VALORAE_HTTP_TRANSPORT_RUNTIME_POLICY = 'provider-pools-backpressure-bounded-lru-fallback-v2';
export const VALORAE_HTTP_TRANSPORT_IMPLEMENTATION = 'native-fetch-optional-undici-bounded-lru-pool-v3';
...
let poolConstructor;
let poolConstructorResolved = false;

function undiciPoolConstructor() {
  if (poolConstructorResolved) return poolConstructor || null;
  poolConstructorResolved = true;
  try {
    const imported = require('undici');
    poolConstructor = typeof imported?.Pool === 'function' ? imported.Pool : null;
  } catch (error) {
...
const TOTAL_KEYS = [
  'requests', 'managedRequests', 'legacyRequests', 'queued', 'queueRejected',
  'queueTimeouts', 'cancelled', 'legacyFallbacks', 'errors', 'completed',
  'totalLatencyMs', 'poolEvictions', 'poolCloseErrors',
];

function normalizeRuntimeState(candidate) {
  const state = candidate && typeof candidate === 'object' ? candidate : {};
  if (!(state.pools instanceof Map)) state.pools = new Map();
  if (!(state.limiters instanceof Map)) state.limiters = new Map();
  if (!(state.providers instanceof Map)) state.providers = new Map();
  if (!state.totals || typeof state.totals !== 'object') state.totals = {};
  for (const key of TOTAL_KEYS) {
    const value = Number(state.totals[key]);
    state.totals[key] = Number.isFinite(value) ? value : 0;
  }
  const startedAt = Number(state.startedAt);
  state.startedAt = Number.isFinite(startedAt) && startedAt > 0 ? startedAt : Date.now();
  return state;
}

const runtime = normalizeRuntimeState(globalThis.__VALORAE_HTTP_PROVIDER_TRANSPORT__);
...
function maxPoolEntries() {
  const value = Number(process.env.VALORAE_HTTP_MAX_POOLS || 32);
  return Math.max(4, Math.min(128, Number.isFinite(value) ? Math.floor(value) : 32));
}

function closePoolEntry(entry) {
  if (!entry?.dispatcher || typeof entry.dispatcher.close !== 'function') return;
  try {
    const closing = entry.dispatcher.close();
    Promise.resolve(closing).catch(() => { runtime.totals.poolCloseErrors += 1; });
  } catch {
    runtime.totals.poolCloseErrors += 1;
  }
}

function trimPoolsForNewEntry() {
  const limit = maxPoolEntries();
  while (runtime.pools.size >= limit) {
    const oldestKey = runtime.pools.keys().next().value;
    if (!oldestKey) break;
    const oldest = runtime.pools.get(oldestKey);
    runtime.pools.delete(oldestKey);
    runtime.totals.poolEvictions += 1;
    closePoolEntry(oldest);
  }
}
...
function dispatcherFor(url, profile) {
  ...
  if (entry) {
    runtime.pools.delete(key);
    runtime.pools.set(key, entry);
  }
  if (!entry) {
    const Pool = undiciPoolConstructor();
    if (!Pool) return null;
    trimPoolsForNewEntry();
    ...
    entry = { key, provider: profile.provider, origin: parsed.origin, dispatcher, createdAt: Date.now(), lastUsedAt: Date.now(), requests: 0 };
    runtime.pools.set(key, entry);
  }
  entry.requests += 1;
...
export async function providerFetch(url, requestInit = {}) {
  ...
  try {
    ...
    try {
      if (!dispatcher) increment(profile.provider, 'legacyFallbacks');
      const response = await globalThis.fetch(url, dispatcher ? { ...init, dispatcher } : init);
      increment(profile.provider, 'completed');
      return response;
    } catch (error) {
...
export function buildProviderTransportManifest() {
  ...
  return {
    ...
    runtimePolicyVersion: VALORAE_HTTP_TRANSPORT_RUNTIME_POLICY,
    ...
    guarantees: {
      poolsPerOriginAndProvider: Boolean(undiciPoolConstructor()),
      globallyBoundedPoolCountWithLruEviction: true,
      nativeFetchFallbackWhenPoolUnavailable: true,
    ...
    rollback: {
      ...
      maxPools: 'VALORAE_HTTP_MAX_POOLS=32',
    },
    providers,
...
export function setUndiciPoolConstructorForTests(constructor = null) {
  poolConstructor = typeof constructor === 'function' ? constructor : null;
  poolConstructorResolved = true;
}
...
export const _test = { runtime, normalizeRuntimeState, dispatcherFor, maxPoolEntries, trimPoolsForNewEntry, closePoolEntry, setUndiciPoolConstructorForTests };
