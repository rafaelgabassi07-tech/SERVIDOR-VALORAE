...
export const VALORAE_SERVER_METRICS_VERSION = '21.12.65-valorae-i10-dividend-agenda-parser-boundary-fix-monitor';
...
function buildProxyOutputMonitor(summary = {}, routeDetails = [], vercelRuntime = {}) {
  ...
  return {
    ...
    scope: {
      ...
      capturePolicy: 'Rotas de dados nunca são ignoradas por x-valorae-telemetry; apenas rotas internas são isoladas para não inflar o tráfego real.',
    },
    totals: {
...
export function getServerMetricsSnapshot() {
  ...
  const personalReleaseReadiness = buildPersonalReleaseReadiness({ metrics: { summary, eventsStored: state.events.length, routesTracked: state.routes.size, deliveryHarmony }, proxyOutputMonitor, deliveryHarmony });
  return {
    ok: true,
    name: 'Valorae Proxy Server Metrics',
    version: VALORAE_SERVER_METRICS_VERSION,
    releasePatch: '21.12.65-valorae-i10-dividend-agenda-parser-boundary-fix',
