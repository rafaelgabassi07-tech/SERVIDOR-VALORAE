...
export const VALORAE_SERVER_METRICS_VERSION = '21.11.7-vercel-build-safe';
...
function shouldIgnoreMetrics(req, meta = {}) {
  ...
  const telemetryHeader = String(req?.headers?.['x-valorae-telemetry'] || '').toLowerCase();
  if (telemetryHeader && /dashboard|internal|test/.test(telemetryHeader)) return true;
