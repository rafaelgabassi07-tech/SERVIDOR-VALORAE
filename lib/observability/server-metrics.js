...
import { mergeMonitorEvents, monitorPersistenceStatus } from './monitor-persistence.js';

export const VALORAE_SERVER_METRICS_VERSION = `${VALORAE_RELEASE_PATCH}-monitor`;

const DETAILED_METRICS = process.env.VALORAE_METRICS_DETAILED === '1';
const CAPTURE_PAYLOAD_PREVIEW = DETAILED_METRICS && process.env.VALORAE_METRICS_CAPTURE_PREVIEW === '1';
const MAX_EVENTS = Number(process.env.VALORAE_METRICS_MAX_EVENTS || 160);
const MAX_ROUTE_STATS = Number(process.env.VALORAE_METRICS_MAX_ROUTES || 96);
const MAX_CLIENTS = Number(process.env.VALORAE_METRICS_MAX_CLIENTS || 160);
const MAX_ROUTE_SAMPLES = Number(process.env.VALORAE_METRICS_ROUTE_SAMPLES || 64);
...
const MAX_MEMORY_SAMPLES = Number(process.env.VALORAE_METRICS_MEMORY_SAMPLES || 60);
...
function defaultMonitorPersistenceStatus() {
  ...
  return {
    ...
    mode: 'memory',
    table: null,
    readLimit: Number(status.readLimit || MAX_EVENTS),
    queueDepth: 0,
    cachedTotal: 0,
    lastError: null,
  };
...
function mergeMonitorEventHistory(memoryEvents = [], persistedEvents = [], limit = MAX_EVENTS) {
  return mergeMonitorEvents(memoryEvents, persistedEvents, limit);
}
...
function querySignal(req) {
  try {
    ...
    const safeKeys = new Set([
      ...
      'stage', 'class', 'assetType', 'include', 'format', 'envelope', 'action',
    ]);
...
function safePreview(value, limit = Number(process.env.VALORAE_METRICS_PREVIEW_BYTES || 1800)) {
  if (!CAPTURE_PAYLOAD_PREVIEW) return undefined;
  try {
...
function collectSnippet(current = '', chunk, encoding = 'utf8') {
  const limit = DETAILED_METRICS ? Number(process.env.VALORAE_METRICS_BODY_CAPTURE_BYTES || 4096) : 0;
...
function payloadDiagnostics(payload) {
  if (!DETAILED_METRICS) {
    ...
    const syncAction = payload?.action || payload?.appSyncEnvelope?.action || payload?.appSyncEnvelope?.decision || mobile?.sync?.action || mobile?.sync?.decision;
    return {
      kind,
      rootKeys: [],
      signals: {
        status: payload?.status || payload?.data?.status,
        syncCode: payload?.code || payload?.data?.code,
        syncRetryable: payload?.retryable ?? payload?.data?.retryable,
  ...
  const signals = {
    ...
    syncCode: payload?.code || payload?.data?.code,
    syncRetryable: payload?.retryable ?? payload?.data?.retryable,
    syncConflict: payload?.conflict ?? payload?.data?.conflict,
    syncRetryAfterMs: payload?.retryAfterMs ?? payload?.data?.retryAfterMs,
    ticker: payload?.ticker || appPayload?.ticker || mobile?.ticker,
    type: payload?.type || appPayload?.type || mobile?.type,
...
export function recordResponse(req, res, payload, options = {}) {
  try {
    ...
    const event = {
      ...
      syncAction: String(payload?.action || diagnostics.signals.syncAction || started?.signal?.safeQuery?.action || '').slice(0, 120) || undefined,
      errorCode: String(payload?.code || diagnostics.signals.syncCode || res?.getHeader?.('X-Valorae-Sync-Code') || '').slice(0, 120) || undefined,
      retryable: payload?.retryable ?? diagnostics.signals.syncRetryable,
...
export function attachProxyMetricsInterceptor(req, res, meta = {}) {
  ...
  if (typeof originalWrite === 'function') {
    res.write = function patchedWrite(chunk, encoding, cb) {
      try {
        ...
        if (DETAILED_METRICS) res.__valoraeMetricsBodyPreview = collectSnippet(res.__valoraeMetricsBodyPreview, chunk, enc);
      } catch {}
  ...
  res.end = function patchedEnd(chunk, encoding, cb) {
    if (!req.__valoraeMetricsRecorded) {
      ...
      const rawPreview = DETAILED_METRICS ? collectSnippet(res.__valoraeMetricsBodyPreview, chunk, enc) : '';
      const parsedPreview = DETAILED_METRICS ? parseBodyPreview(rawPreview) : undefined;
...
function generateReadiness(summary, routeDetails) {
  return [
    ...
    { name: 'Telemetria isolada em memória', ok: true, detail: 'O monitor não grava nem consulta eventos no Supabase; Redis, KV, WebSocket e cron continuam desnecessários.' },
...
function buildProxyOutputMonitor(summary = {}, routeDetails = [], vercelRuntime = {}, monitorEvents = state.events, persistence = defaultMonitorPersistenceStatus()) {
  ...
  return {
    ...
    scope: {
      ...
      persistence: 'Memória da instância atual; persistência e leitura de telemetria no Supabase estão desativadas.',
      ...
      instanceScope: `Saúde, latência e eventos pertencem somente à instância ${state.instanceId}; cold starts iniciam uma nova janela operacional.`,
    },
    totals: {
...
function buildMonitorBlueprint(summary = {}, routeDetails = [], payloadIntelligence = {}, vercelRuntime = {}) {
  ...
  return {
    ...
    technologies: [
      ...
      { name: 'Supabase REST mínimo', layer: 'Persistência', role: 'Quando configurado, sincroniza somente transações e dividendos; autenticação/sessão é usada para validar acesso e bloquear clientes legados.' },
    ...
    inputChannels: [
      ...
      { name: 'Supabase Sync', method: 'GET/POST/DELETE', examples: ['/api/sync?action=auth_check', '/api/sync?action=download_financial_data', '/api/sync?action=upload_transactions'], headers: ['authorization', 'x-valorae-user-id', 'x-valorae-client-secret', 'x-valorae-sync-token'], risk: 'variáveis/tabelas ausentes ou CORS sem headers de sync', control: 'diagnostics real, headers permitidos e service role apenas no Vercel' },
    ...
    contracts: [
      ...
      { name: 'Supabase Sync', route: 'GET/POST /api/sync', input: 'sessão Supabase + transações/dividendos', output: 'sincronização financeira mínima v2' },
    ],
    supabaseSync: {
      route: '/api/sync',
      ...
      tables: {
        transactions: 'valorae_financial_transactions',
        dividends: 'valorae_financial_dividends',
        state: null,
        snapshots: null,
        clients: null,
        backups: null,
        monitor: null,
        sharedRuntimeState: null,
      },
    },
...
export function getServerMetricsSnapshot(options = {}) {
  // A observabilidade do monitor é deliberadamente efêmera. Argumentos legados de
  // persistência são ignorados para impedir que testes, integrações antigas ou variáveis
  // de ambiente reintroduzam leitura/escrita de telemetria no Supabase.
  void options;
  const persistence = defaultMonitorPersistenceStatus();
  const monitorEvents = mergeMonitorEventHistory(state.events, [], persistence.readLimit || MAX_EVENTS);
  const persistedTotal = 0;
  const now = Date.now();
  ...
  const payloadIntelligence = {
    ...
    detailedDiagnosticsEnabled: DETAILED_METRICS,
    capturePreviewEnabled: CAPTURE_PAYLOAD_PREVIEW,
    note: 'Mostra estrutura, sinais e prévia limitada dos payloads mantidos somente na memória desta instância. O Supabase é reservado à sincronização financeira legítima.',
  };
  ...
  return {
    ...
    serverless: {
      ...
      persistent: false,
      realtimeTransport: 'manual-http-request',
      pollingHintMs: null,
      internalTelemetryRoutes: [...INTERNAL_TELEMETRY_ROUTES],
      note: 'Métricas efêmeras em memória por instância serverless. O endpoint /api/server/metrics só é executado por chamada manual; não há polling, cron ou persistência de telemetria no Supabase.',
    },
    instance: {
      id: state.instanceId,
