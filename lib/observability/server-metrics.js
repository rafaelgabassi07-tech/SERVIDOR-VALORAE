...
export const VALORAE_SERVER_METRICS_VERSION = '21.12.18-proxy-output-server-page';
...
function payloadDiagnostics(payload) {
  ...
  const metricsCount = countMetricsLike(appPayload?.metrics?.canonical) + countMetricsLike(appPayload?.metrics?.aliases) + countMetricsLike(mobile?.metrics) + countMetricsLike(normalized);
  const signals = {
...
function buildProxyOutputMonitor(summary = {}, routeDetails = [], vercelRuntime = {}) {
  ...
  return {
    ...
    scope: {
      captures: 'Toda rota de dados /api/* que não seja telemetria interna do próprio painel.',
      excludes: 'Polling interno do painel e rotas administrativas isoladas para não poluir o tráfego real.',
      persistence: 'memória da instância serverless atual; sem banco/Redis/KV para manter compatibilidade com Vercel Free.',
      important: 'Para ver 100% do ecossistema em produção, todos os apps precisam consumir esta mesma URL/deploy do proxy ou enviar cabeçalhos x-valorae-app/x-valorae-channel.',
    },
    totals: {
