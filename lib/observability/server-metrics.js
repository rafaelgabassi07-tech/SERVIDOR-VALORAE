...
function monitorPersistenceConfigured() {
  // Telemetria do monitor é deliberadamente local. Nenhuma credencial ou variável
  // legada pode transformar eventos operacionais em escritas no Supabase.
  return false;
}
...
export function getServerMetricsSnapshot(options = {}) {
  ...
  return {
    ...
    serverless: {
      ...
      note: 'Métricas reais em memória por instância serverless. O endpoint /api/server/metrics é isolado: o polling do painel não entra em requisições, respostas, status, eventos, cache ou rotas e nenhuma telemetria é lida ou gravada no Supabase.',
    },
    instance: {
      id: state.instanceId,
