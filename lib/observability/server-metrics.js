...
function payloadDiagnostics(payload) {
  const rootKeys = objectKeys(payload, 40);
  ...
  const kind = payload?.ticker || appPayload?.ticker || mobile?.ticker ? 'asset' :
    payload?.portfolio || payload?.allocation || payload?.summary ? 'portfolio' :
    payload?.openapi || payload?.paths ? 'contract' :
    payload?.tests || payload?.readiness ? 'diagnostic' :
    payload && typeof payload === 'object' ? 'json' : 'unknown';
  const chartCount = Array.isArray(charts) ? charts.length : Number(charts?.count || charts?.series?.length || charts?.seriesPreview?.length || 0);
  ...
  const signals = {
    ...
    metrics: countPlainObject(appPayload?.metrics?.canonical) + countPlainObject(appPayload?.metrics?.aliases) + countPlainObject(mobile?.metrics) + countPlainObject(normalized),
    charts: chartCount,
    chartPoints: Number(charts?.bestPointCount || charts?.totalPoints || 0) || (Array.isArray(charts) ? charts.reduce((n, s) => n + arrayLength(s?.points), 0) : 0),
...
export function recordRequestStart(req, meta = {}) {
  ...
  req.__valoraeMetrics = { id: metricsId, startedAt, wallStartedAt, route, method, client, device, app, ua, signal };
  state.activeRequests.set(metricsId, { id: metricsId, route, method, client, device, appName: app.name, channel: app.channel, startedAt: wallStartedAt });
  ...
  updateRouteDetail(route, { request: true, method, device, appName: app.name, channel: app.channel, bytesIn, at: wallStartedAt });
  const b = touchBucket(wallStartedAt);
  b.requests += 1;
  b.bytesIn = (b.bytesIn || 0) + bytesIn;
  const c = state.clients.get(client) || { id: client, firstSeenAt: wallStartedAt, lastSeenAt: wallStartedAt, requests: 0, responses: 0, errors: 0, bytesOut: 0, device, lastRoute: route, appName: app.name, appVersion: app.version, channel: app.channel };
  c.requests += 1;
...
export function recordResponse(req, res, payload, options = {}) {
  try {
    ...
    const event = {
      ...
      payloadKind: payloadDiagnostics(payload).kind,
      payloadRoots: payloadDiagnostics(payload).rootKeys,
      payloadSignals: payloadDiagnostics(payload).signals,
      payloadPreview: payloadDiagnostics(payload).preview,
      deliveryDecision: payloadDiagnostics(payload).signals.syncAction || (payloadDiagnostics(payload).signals.renderSafe ? 'render_safe' : 'inspect_payload'),
    };
    ...
    updateRouteDetail(route, { response: true, status, at: ts, bytesOut, latencyMs, cacheStatus, sourceStatus, appName: event.appName, channel: event.appChannel, payloadKind: event.payloadKind, payloadSignals: event.payloadSignals });
...
function buildDeliveryHarmony(summary = {}, routeDetails = [], payloadEvents = []) {
  ...
  const pipeline = [
    ...
    { stage: 'dashboard_visibility', label: 'Painel mostra saída', ok: true, detail: 'Eventos recentes expõem raízes, sinais, hashes, decisão de sync e prévia limitada do JSON enviado aos apps.' },
  ];
  const byApp = new Map();
  ...
  return {
    ...
    topRoutes: routeDetails.filter(r => r.deliveredPayloads > 0).sort((a, b) => b.deliveredPayloads - a.deliveredPayloads).slice(0, 12).map(r => ({ route: r.route, payloads: r.deliveredPayloads, renderSafeRatePercent: r.renderSafeRatePercent, metrics: r.metricsDelivered, charts: r.chartSeriesDelivered, dividends: r.dividendRowsDelivered, app: r.topApp, channel: r.topChannel })),
  };
}
...
export function getServerMetricsSnapshot() {
  ...
  const dashboardIntegrityScore = scoreClamp(100 - Math.max(0, state.totals.requests - state.totals.responses - state.totals.inFlight) * 5);
  const captureCompletenessPercent = state.totals.requests ? clamp(Math.round((state.totals.responses / Math.max(1, state.totals.requests - state.totals.inFlight)) * 10000) / 100, 0, 100) : 100;
  ...
  const deliveryHarmony = buildDeliveryHarmony(summary, routeDetails, payloadEvents);
  return {
    ok: true,
...
export function resetServerMetricsForTests() {
  ...
  state.status.clear(); state.statusFamily.clear(); state.methods.clear(); state.routes.clear(); state.routeDetails.clear(); state.cache.clear(); state.source.clear(); state.devices.clear(); state.apps.clear(); state.channels.clear(); state.tickers.clear(); state.views.clear(); state.clients.clear(); state.interceptors.clear(); state.buckets.clear();
