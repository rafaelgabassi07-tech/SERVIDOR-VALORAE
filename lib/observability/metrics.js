...
export const OBSERVABILITY_VERSION = '21.5.13-harmony-delivery-observability-free';
...
const state = {
  ...
  dataDeliveries: new Map(),
  dataBytesByRoute: new Map(),
  staticRequests: 0,
  staticCompleted: 0,
  staticErrors: 0,
  staticBytes: 0,
  staticFiles: new Map(),
  staticContentTypes: new Map(),
  staticExtensions: new Map(),
  staticLastRequestAt: null,
  events: [],
  observabilityReads: 0,
  internalProbeReads: 0,
  serverOpenSockets: 0,
...
function contentTypeLabel(value) {
  ...
  if (raw === 'text/html') return 'HTML';
  if (raw === 'text/css') return 'CSS';
  if (raw.includes('javascript')) return 'JavaScript';
  if (raw.startsWith('image/')) return raw.replace('image/', 'Imagem ');
  if (raw.startsWith('text/')) return raw.replace('text/', 'Texto ');
  return raw.slice(0, 80);
}

function extensionLabel(filePath = '') {
  const match = String(filePath || '').toLowerCase().match(/\.([a-z0-9]{1,12})(?:$|[?#])/);
  return match ? `.${match[1]}` : 'sem extensão';
}

function routeFamily(route = '') {
  const r = String(route || '/api');
  if (r.includes('/asset') || r.includes('/assets')) return 'Ativos e cotações';
  if (r.includes('/portfolio')) return 'Carteira';
  if (r.includes('/market')) return 'Mercado';
  if (r.includes('/scrape') || r.includes('/compat')) return 'Scraper';
  if (r.includes('/news')) return 'Notícias';
  if (r.includes('/admin') || r.includes('/cache')) return 'Administração';
  if (r.includes('/health') || r.includes('/ready') || r.includes('/manifest') || r.includes('/schema') || r.includes('/fields') || r.includes('/errors') || r.includes('/openapi')) return 'Controle e contrato';
  return 'API geral';
}
...
function updateDataDelivery(route, status, contentType, outboundBytes) {
  const family = routeFamily(route);
  let item = state.dataDeliveries.get(family);
  if (!item) item = { requests: 0, errors: 0, bytes: 0, lastActivityAt: null, contentTypes: new Map() };
  item.requests += 1;
  if (Number(status) >= 400) item.errors += 1;
  item.bytes += outboundBytes;
  item.lastActivityAt = new Date().toISOString();
  inc(item.contentTypes, contentType);
  state.dataDeliveries.set(family, item);

  state.dataBytesByRoute.set(route, (state.dataBytesByRoute.get(route) || 0) + outboundBytes);
}

function updateStaticFileStats(pathname, status, contentType, extension, outboundBytes, expectedBytes) {
  const route = String(pathname || '/').slice(0, 180);
  let item = state.staticFiles.get(route);
  if (!item) item = { requests: 0, errors: 0, bytes: 0, announcedBytes: 0, contentType, extension, lastActivityAt: null, statuses: new Map() };
  item.requests += 1;
  if (Number(status) >= 400) item.errors += 1;
  item.bytes += outboundBytes;
  item.announcedBytes += Number(expectedBytes || 0);
  item.contentType = contentType;
  item.extension = extension;
  item.lastActivityAt = new Date().toISOString();
  inc(item.statuses, status);
  state.staticFiles.set(route, item);
  if (state.staticFiles.size > TOP_LIMIT_INTERNAL) {
    const keep = [...state.staticFiles.entries()].sort((a, b) => b[1].bytes - a[1].bytes || b[1].requests - a[1].requests).slice(0, TOP_LIMIT_INTERNAL);
    state.staticFiles = new Map(keep);
  }
}

function staticFileRows(limit = 10) {
  return [...state.staticFiles.entries()]
    .sort((a, b) => b[1].bytes - a[1].bytes || b[1].requests - a[1].requests)
    .slice(0, limit)
    .map(([path, item]) => ({
      path,
      ...
      errors: item.errors,
      bytes: item.bytes,
      announcedBytes: item.announcedBytes,
...
function dataDeliveryRows(limit = 10) {
  return [...state.dataDeliveries.entries()]
    .sort((a, b) => b[1].bytes - a[1].bytes || b[1].requests - a[1].requests)
    .slice(0, limit)
    .map(([family, item]) => ({
      family,
      ...
      bytes: item.bytes,
      lastActivityAt: item.lastActivityAt,
      topContentType: top(item.contentTypes, 1)[0]?.name || '-',
    }));
}

function normalizedContextPath(context = {}) {
  return String(context.path || '').replace(/^\/api/, '') || '/';
}

function isInternalObservability(context = {}) {
  const path = normalizedContextPath(context);
  return path === '/observability' || path === '/api/observability';
}

function isDashboardHealthProbe(req, context = {}) {
  const path = normalizedContextPath(context);
  const probe = String(req?.headers?.['x-valorae-dashboard-probe'] || '') === '1';
  return probe && (path === '/health' || path === '/ready');
}
...
function observeFinish(req, res, context, meta) {
  ...
  const responseContentType = contentTypeLabel(res?.getHeader?.('Content-Type') || res?.getHeader?.('content-type'));
  ...
  inc(state.responseContentTypes, responseContentType);
  inc(state.responseProfiles, routeFamily(route));
  updateDataDelivery(route, status, responseContentType, outboundBytes);
...
export function observeProxyRequest(req, res, context = {}) {
  ...
  if (isDashboardHealthProbe(req, context)) {
    state.internalProbeReads += 1;
    return;
  }
  res[OBSERVED] = true;
...
export function observeStaticAssetRequest(req, res, context = {}) {
  if (!req || !res || res[OBSERVED]) return;
  res[OBSERVED] = true;
  state.staticRequests += 1;
  state.staticLastRequestAt = new Date().toISOString();

  const meta = {
    start: performance.now(),
    outboundBytes: 0,
    pathname: String(context.pathname || context.path || normalizePath(req?.url || '/')).slice(0, 180),
    expectedBytes: Number(context.bytes || 0),
    contentType: contentTypeLabel(context.contentType || res?.getHeader?.('Content-Type') || res?.getHeader?.('content-type')),
    extension: extensionLabel(context.filePath || context.pathname || req?.url || ''),
  };
  const originalWrite = res.write?.bind(res);
  const originalEnd = res.end?.bind(res);
  ...
  function finishStatic() {
    ...
    const status = Number(res.statusCode || 0);
    const method = String(req?.method || 'GET').toUpperCase();
    const outboundBytes = Number(meta.outboundBytes || 0);
    state.staticCompleted += 1;
    if (status >= 400) state.staticErrors += 1;
    state.staticBytes += outboundBytes;
    inc(state.staticContentTypes, meta.contentType);
    inc(state.staticExtensions, meta.extension);
    updateStaticFileStats(meta.pathname, status, meta.contentType, meta.extension, outboundBytes, meta.expectedBytes);
    addEvent({
      level: status >= 500 ? 'ERROR' : status >= 400 ? 'WARN' : 'INFO',
      event: 'Arquivo público',
      route: meta.pathname,
      method,
      status,
      latencyMs: Math.max(0, Math.round(performance.now() - meta.start)),
      bytes: outboundBytes,
      client: clientLabel(req),
      message: `${method} ${meta.pathname} ${status || '-'} ${formatBytes(outboundBytes)} ${meta.contentType}`,
    });
  }

  if (originalWrite) {
    res.write = function observedStaticWrite(chunk, encoding, cb) {
      if (chunk) meta.outboundBytes += Buffer.isBuffer(chunk) ? chunk.length : Buffer.byteLength(String(chunk), typeof encoding === 'string' ? encoding : 'utf8');
      return originalWrite(chunk, encoding, cb);
    };
  }
  if (originalEnd) {
    res.end = function observedStaticEnd(chunk, encoding, cb) {
      if (chunk) meta.outboundBytes += Buffer.isBuffer(chunk) ? chunk.length : Buffer.byteLength(String(chunk), typeof encoding === 'string' ? encoding : 'utf8');
      finishStatic();
      return originalEnd(chunk, encoding, cb);
    };
  }
  res.once?.('finish', finishStatic);
  res.once?.('close', finishStatic);
}
...
export function observabilitySnapshot(options = {}) {
  ...
  return {
    ...
    distributions: {
      ...
      contentTypes: mapToDistribution(state.responseContentTypes),
      responseProfiles: mapToDistribution(state.responseProfiles),
      staticContentTypes: mapToDistribution(state.staticContentTypes),
      staticExtensions: mapToDistribution(state.staticExtensions),
      cache: cacheDist,
      auth: authDist,
    },
    top: {
      ...
      dataDeliveries: dataDeliveryRows(10),
      publicFiles: staticFileRows(10),
    ...
    delivery: {
      apiResponses: {
        ...
        contentTypes: top(state.responseContentTypes, 8),
        families: dataDeliveryRows(8),
      },
      publicAssets: {
        requests: state.staticRequests,
        completed: state.staticCompleted,
        errors: state.staticErrors,
        bytes: state.staticBytes,
        lastRequestAt: state.staticLastRequestAt,
        measured: state.staticRequests > 0,
        note: 'Arquivos públicos são medidos pelo server.js local. Em Vercel, assets estáticos podem ser servidos pela própria plataforma fora do runtime Node.',
      },
      privacy: {
        storesBody: false,
        storesRawIp: false,
        note: 'O app armazena metadados técnicos, contagens, bytes, tipos e IPs anonimizados; não guarda corpo de resposta nem IP bruto.'
      }
    },
    system: systemSnapshot(),
    measurement: {
      proxyTraffic: 'Requisições /api/*, exceto /api/observability para não poluir o painel com polling interno. Respostas são analisadas por metadados reais: status, método, bytes e Content-Type.',
      observabilityReads: state.observabilityReads,
      internalProbeReads: state.internalProbeReads,
      internalProbes: 'Health/ready chamados pelo dashboard com X-Valorae-Dashboard-Probe=1 são reais, mas excluídos do tráfego para não poluir as métricas.',
      ...
      publicAssets: { measured: state.staticRequests > 0, reason: state.staticRequests > 0 ? 'Medido pelo servidor local para arquivos públicos entregues aos usuários.' : 'Ainda sem entrega pública observada neste runtime; em Vercel assets estáticos podem ser servidos fora do Node.' },
      payloadPrivacy: { measured: true, reason: 'Mede somente metadados de entrega; não armazena corpos de resposta.' },
    },
...
export function resetObservabilityForTests() {
  ...
  state.responseContentTypes.clear();
  state.responseProfiles.clear();
  state.dataDeliveries.clear();
  state.dataBytesByRoute.clear();
  state.staticRequests = 0;
  state.staticCompleted = 0;
  state.staticErrors = 0;
  state.staticBytes = 0;
  state.staticFiles.clear();
  state.staticContentTypes.clear();
  state.staticExtensions.clear();
  state.staticLastRequestAt = null;
  state.events = [];
  state.observabilityReads = 0;
  state.internalProbeReads = 0;
