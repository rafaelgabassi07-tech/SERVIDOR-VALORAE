...
export function monitorPersistenceConfig() {
  ...
  // Credenciais do Supabase não devem transformar telemetria efêmera em escrita no banco.
  // A persistência é um recurso de diagnóstico excepcional e exige opt-in explícito.
  const enabled = boolValue(process.env.VALORAE_MONITOR_PERSISTENCE_ENABLED, false);
  return {
    url,
    key,
    configured,
