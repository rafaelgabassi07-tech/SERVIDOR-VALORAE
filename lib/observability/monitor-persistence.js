...
export function monitorPersistenceConfig() {
  ...
  // A persistência remota do monitor foi removida do caminho operacional. Métricas do
  // painel são efêmeras e permanecem somente na memória da instância. Mesmo uma variável
  // legada definida como true não pode reativar escrita de telemetria no Supabase.
  const requested = boolValue(process.env.VALORAE_MONITOR_PERSISTENCE_ENABLED, false);
  const enabled = false;
  return {
    url,
    key,
