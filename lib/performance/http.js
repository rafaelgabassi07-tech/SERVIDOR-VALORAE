...
export function sendJson(req, res, payload, options = {}) {
  ...
  if (endpointContractVersion) res.setHeader('X-Valorae-Endpoint-Contract-Version', String(endpointContractVersion).slice(0, 120));
...
export function sendPreparedJson(req, res, prepared = {}, options = {}) {
  ...
  res.setHeader('X-Valorae-Delivery-Schema', String(VALORAE_ASSET_MODAL_DELIVERY_SCHEMA_VERSION));
  res.setHeader('X-Valorae-Baseline-Contract', VALORAE_BASELINE_CONTRACT_VERSION);
  res.setHeader('X-Valorae-Formal-Schema', prepared.formalSchemaVersion || VALORAE_FORMAL_SCHEMA_VERSION);
  res.setHeader('X-Valorae-Contract-Version', VALORAE_CONTRACT_RESPONSE_VERSION);
  const endpointContractVersion = prepared.contractVersion || prepared.schemaVersion;
  if (endpointContractVersion) res.setHeader('X-Valorae-Endpoint-Contract-Version', String(endpointContractVersion).slice(0, 120));
