...
export function sharedStateMode() {
  ...
  // Estado operacional (circuit breaker, cache negativo, continuidade e canários) não é
  // dado financeiro e não deve gerar I/O no Supabase apenas porque a service_role existe.
  // O driver remoto exige dois opt-ins explícitos. Valores antigos como "auto" passam a
  // significar memória, evitando que configurações herdadas reativem carga no banco.
  const raw = String(process.env.VALORAE_SHARED_STATE_MODE || 'memory').trim().toLowerCase();
  if (['off', 'disabled', 'none'].includes(raw)) return 'off';
  if (['memory', 'local', 'auto', 'default'].includes(raw)) return 'memory';

  const remoteEnabled = boolValue(process.env.VALORAE_SHARED_STATE_REMOTE_ENABLED, false);
  if (['supabase', 'remote', 'shared'].includes(raw) && remoteEnabled) return 'supabase';
  return 'memory';
}
