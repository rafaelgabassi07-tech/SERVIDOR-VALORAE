...
function buildFinancialChartContract(payload = {}) {
  const results = payload.results || {};
  const sections = results.sections || {};
  const demonstrativos = sections.demonstrativos || {};
  const chartsFinanceiros = results.chartsFinanceiros || {};
  const normalized = payload.normalized || {};
  const pick = (...values) => firstPresent(...values);
  const receitasLucros = pick(chartsFinanceiros.receitasLucros, demonstrativos.receitasLucros, results.receitasLucros, normalized.receitasLucros);
  const lucroCotacao = pick(chartsFinanceiros.lucroCotacao, demonstrativos.lucroCotacao, results.lucroCotacao, normalized.lucroCotacao);
  const evolucaoPatrimonio = pick(chartsFinanceiros.evolucaoPatrimonio, demonstrativos.evolucaoPatrimonio, results.evolucaoPatrimonio, normalized.evolucaoPatrimonio);
  const balancoPatrimonial = pick(chartsFinanceiros.balancoPatrimonial, demonstrativos.balancoPatrimonial, results.balancoPatrimonial, normalized.balancoPatrimonial, evolucaoPatrimonio);
  const payoutHistorico = pick(chartsFinanceiros.payoutHistorico, demonstrativos.payoutHistorico, results.payoutHistorico, results.payoutHistory, normalized.payoutHistorico);
  const rentabilidade = pick(sections.rentabilidadeChart, sections.rentabilidade, results.rentabilidadeChart, results.rentabilidade, results.profitability, normalized.rentabilidade);
  return {
    receitasLucros: receitasLucros || null,
    revenueProfit: receitasLucros || null,
    incomeStatement: receitasLucros || null,
    dre: receitasLucros || null,
    lucroCotacao: lucroCotacao || null,
    profitVsQuote: lucroCotacao || null,
    quoteProfit: lucroCotacao || null,
    cotacaoLucro: lucroCotacao || null,
    evolucaoPatrimonio: evolucaoPatrimonio || balancoPatrimonial || null,
    equityEvolution: evolucaoPatrimonio || balancoPatrimonial || null,
    balancoPatrimonial: balancoPatrimonial || evolucaoPatrimonio || null,
    balanceSheet: balancoPatrimonial || evolucaoPatrimonio || null,
    payoutHistorico: payoutHistorico || null,
    payoutHistory: payoutHistorico || null,
    payout: payoutHistorico || null,
    rentabilidade: rentabilidade || null,
    profitability: rentabilidade || null,
    realVsNominalReturn: rentabilidade || null,
    hasDre: hasValue(receitasLucros),
    hasProfitQuote: hasValue(lucroCotacao),
    hasBalanceSheet: hasValue(balancoPatrimonial || evolucaoPatrimonio),
    hasPayout: hasValue(payoutHistorico),
    hasProfitability: hasValue(rentabilidade),
  };
}
...
function buildChartContract(payload = {}) {
  ...
  const revenueBreakdowns = buildRevenueBreakdownContract(payload);
  const financialCharts = buildFinancialChartContract(payload);
  return {
    ...
    financialCharts,
    revenueBreakdowns,
    revenueGeography: revenueBreakdowns.revenueGeography,
    regioesReceita: revenueBreakdowns.regioesReceita,
    revenueByRegion: revenueBreakdowns.revenueByRegion,
    revenueSegment: revenueBreakdowns.revenueSegment,
    revenueByBusiness: revenueBreakdowns.revenueByBusiness,
    negociosReceita: revenueBreakdowns.negociosReceita,
    segmentosReceita: revenueBreakdowns.segmentosReceita,
...
function buildRevenueBreakdownContract(payload = {}) {
  const results = payload.results || {};
  const empresa = results.sections?.empresa || {};
  const region = firstPresent(
    results.revenueGeography,
    results.regioesReceita,
    results.geografiaReceita,
    empresa.revenueGeography,
    empresa.regioesReceita
  );
  const business = firstPresent(
    results.revenueByBusiness,
    results.revenueSegment,
    results.negociosReceita,
    results.segmentosReceita,
    empresa.revenueByBusiness,
    empresa.revenueSegment,
    empresa.negociosReceita,
    empresa.segmentosReceita
  );
  return {
    preferredPaths: {
      region: 'appPayload.charts.revenueGeography',
      business: 'appPayload.charts.revenueByBusiness',
    },
    revenueGeography: region || null,
    regioesReceita: region || null,
    revenueByRegion: region || null,
    geography: region || null,
    byRegion: region || null,
    regions: region || null,
    revenueSegment: business || null,
    revenueByBusiness: business || null,
    negociosReceita: business || null,
    segmentosReceita: business || null,
    business: business || null,
    byBusiness: business || null,
    segments: business || null,
    sourceTrace: results.revenueBreakdownSources || results.sections?.apiRevenueBreakdownSources || null,
    hasRegion: hasValue(region),
    hasBusiness: hasValue(business),
    emptyState: (hasValue(region) || hasValue(business)) ? null : {
      title: 'Faturamento por negócio/região indisponível',
      message: 'A fonte pública não entregou os agrupamentos percentuais no payload atual.',
    },
  };
}
