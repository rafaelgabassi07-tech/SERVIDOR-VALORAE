...
export function buildEngineModuleTree({ summaryOnly = false } = {}) {
  const groups = [
    {
      key: 'entrypoints', title: 'Entrada HTTP e Vercel', purpose: 'Recebe chamadas do APK, aplica segurança, CORS, cache e roteamento único sem telemetria por resposta.',
      modules: ['api/router.js', 'routes/_router.js', 'lib/http/route.js'],
    },
    {
    ...
    {
      key: 'monitor-ui', title: 'Página estática do proxy', purpose: 'Informa que o serviço opera sob demanda sem consultar API, Vercel, métricas ou fontes financeiras.',
      modules: ['public/server.html', 'public/index.html', 'public/monitor-valorae.css', 'public/service-worker.js'],
    },
    {
