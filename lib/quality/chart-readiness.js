...
import { buildNormalizedChartSeries } from './chart-series.js';

export const VALORAE_CHART_READINESS_VERSION = '21.12.0-chart-readiness-series-maturity';
...
export function buildChartReadinessReport(results = {}) {
  const normalized = buildNormalizedChartSeries(results, { maxSeries: 24 });
  const series = (normalized.series || []).map(s => {
    const points = s.points || [];
    return {
      key: s.key,
      name: s.name || s.key,
      points: s.pointCount || points.length,
      score: s.score || scoreSeries(points),
      trend: estimateTrend(points),
      lastValue: points.at(-1)?.y ?? null,
      sourceFormat: s.sourceFormat || 'unknown',
    };
  });
  ...
  const formats = [...new Set(series.map(s => s.sourceFormat).filter(Boolean))];
  return {
    ...
    numericPoints,
    normalizedSeriesVersion: normalized.version,
    ...
    recommendations: [
      ...
      ...(formats.length ? [] : ['Não foram detectados formatos padrão como arrays, series/data, labels/datasets ou objetos date/value.']),
    ],
  };
}
