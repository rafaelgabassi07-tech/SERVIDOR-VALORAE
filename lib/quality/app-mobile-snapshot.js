...
export function buildAppMobileSnapshot(payload = {}) {
  ...
  const body = {
    ...
    incomeStatement: payload.appPayload?.charts?.incomeStatement || null,
    revenueVsProfit: payload.appPayload?.charts?.revenueVsProfit || null,
    dre: payload.appPayload?.charts?.dre || null,
    receitasLucros: payload.appPayload?.charts?.receitasLucros || null,
    profitVsQuote: payload.appPayload?.charts?.profitVsQuote || null,
    quoteProfit: payload.appPayload?.charts?.quoteProfit || null,
    cotacaoLucro: payload.appPayload?.charts?.cotacaoLucro || null,
    balanceSheet: payload.appPayload?.charts?.balanceSheet || null,
    balancoPatrimonial: payload.appPayload?.charts?.balancoPatrimonial || null,
    payoutHistory: payload.appPayload?.charts?.payoutHistory || null,
    payout: payload.appPayload?.charts?.payout || null,
    payoutHistorico: payload.appPayload?.charts?.payoutHistorico || null,
    realVsNominalReturn: payload.appPayload?.charts?.realVsNominalReturn || null,
    rentabilidade: payload.appPayload?.charts?.rentabilidade || null,
    chartsFinanceiros: payload.appPayload?.charts?.financialCharts || null,
    financialCharts: payload.appPayload?.charts?.financialCharts || null,
