...
const STOCK_GROUPS = {
  ...
  statements: {
    ...
    fields: ['faturamento12m','lucro12m','receitaLiquida','lucroLiquido','receitasLucros','lucroCotacao','resultados','evolucaoPatrimonio','balancoPatrimonial','regioesReceita','negociosReceita','revenueGeography','revenueSegment','revenueByBusiness']
  },
  peers: {
...
const RESULT_PATHS = {
  ...
  regioesReceita: ['sections.empresa.regioesReceita','sections.empresa.revenueGeography','revenueGeography','regioesReceita','geografiaReceita','appPayload.charts.revenueGeography','appPayload.charts.regioesReceita'],
  revenueGeography: ['sections.empresa.revenueGeography','sections.empresa.regioesReceita','revenueGeography','regioesReceita','geografiaReceita','appPayload.charts.revenueGeography'],
  negociosReceita: ['sections.empresa.negociosReceita','sections.empresa.segmentosReceita','sections.empresa.revenueSegment','sections.empresa.revenueByBusiness','negociosReceita','segmentosReceita','revenueSegment','revenueByBusiness','appPayload.charts.revenueByBusiness','appPayload.charts.negociosReceita'],
  revenueSegment: ['sections.empresa.revenueSegment','sections.empresa.negociosReceita','sections.empresa.segmentosReceita','revenueSegment','segmentosReceita','negociosReceita','revenueByBusiness','appPayload.charts.revenueSegment'],
  revenueByBusiness: ['sections.empresa.revenueByBusiness','sections.empresa.negociosReceita','sections.empresa.segmentosReceita','revenueByBusiness','revenueSegment','negociosReceita','segmentosReceita','appPayload.charts.revenueByBusiness'],
...
function resolveField(payload = {}, field = '') {
  ...
  for (const p of RESULT_PATHS[field] || RESULT_PATHS[canonical] || []) {
    const root = p.startsWith('appPayload.') ? payload : results;
    const prefix = p.startsWith('appPayload.') ? '' : 'results.';
    push(`${prefix}${p}`, getPath(root, p), p.startsWith('appPayload.') ? 'appPayload.chart' : 'results.section', p.startsWith('appPayload.') ? 0.86 : 0.78);
  }
