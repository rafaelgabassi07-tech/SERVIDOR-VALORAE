...
export function encodeTransactionReadCursor(payload = {}) {
  const normalized = {
    v: 2,
    scope: 'transactions',
    offset: Math.max(0, Number(payload.offset) || 0),
  ...
  const body = Buffer.from(JSON.stringify(normalized)).toString('base64url');
  // Transaction cursors are intentionally deployment-stable. Authentication already isolates the
  // user namespace, and offset tampering cannot expose another account. A deterministic checksum
  // catches truncation/corruption without depending on a Vercel-instance secret that may rotate.
  const checksum = crypto.createHash('sha256').update(`valorae-transactions:${body}`).digest('base64url').slice(0, 32);
  return `tx2.${body}.${checksum}`;
}

export function decodeTransactionReadCursor(token, legacySecret = '') {
  const raw = text(token);
  if (!raw.startsWith('tx2.')) {
    try {
      return decodeRevisionCursor(raw, legacySecret);
    } catch (error) {
      // Transaction pagination is already isolated by the authenticated Supabase user. During a
      // rolling Proxy deployment, page 1 can be emitted by an older instance whose internal cursor secret is
      // no longer available to page 2. For this read-only cursor, safely recover only the validated
      // v1 body; changing the offset can at worst skip/duplicate rows from the same account.
      const [body, signature, extra] = raw.split('.');
      if (!body || !signature || extra) throw error;
      let payload;
      try { payload = JSON.parse(Buffer.from(body, 'base64url').toString('utf8')); } catch { payload = null; }
      if (!payload || payload.v !== 1 || !Number.isInteger(payload.offset) || payload.offset < 0) throw error;
      return { ...payload, legacySignatureRecovered: true };
    }
  }
  const [prefix, body, checksum, extra] = raw.split('.');
  if (prefix !== 'tx2' || !body || !checksum || extra) {
    throw Object.assign(new Error('Cursor de transações inválido.'), { code: 'SYNC_CURSOR_INVALID', status: 400 });
  }
  const expected = crypto.createHash('sha256').update(`valorae-transactions:${body}`).digest('base64url').slice(0, 32);
  const actualBuffer = Buffer.from(checksum);
  const expectedBuffer = Buffer.from(expected);
  if (actualBuffer.length !== expectedBuffer.length || !crypto.timingSafeEqual(actualBuffer, expectedBuffer)) {
    throw Object.assign(new Error('Cursor de transações corrompido.'), { code: 'SYNC_CURSOR_INVALID', status: 400 });
  }
  let payload;
  try { payload = JSON.parse(Buffer.from(body, 'base64url').toString('utf8')); } catch { payload = null; }
  if (!payload || payload.v !== 2 || payload.scope !== 'transactions' || !Number.isInteger(payload.offset) || payload.offset < 0) {
    throw Object.assign(new Error('Cursor de transações incompatível.'), { code: 'SYNC_CURSOR_INVALID', status: 400 });
  }
  return payload;
}

export function assertTransactionCursorMatchesState(cursor, state) {
  if (!cursor) return;
  if (cursor.v !== 2) return assertCursorMatchesState(cursor, state);
  ...
  // The global revision also changes for snapshots and dividend events. Those unrelated writes must
  // not invalidate a transaction read between page 1 and page 2. Deletion remains a hard fence.
  if (cursor.deletionGeneration !== deletionGeneration || cursor.tombstone !== tombstone) {
    throw Object.assign(new Error('O Histórico foi excluído ou recriado durante a paginação.'), {
      code: 'SYNC_CURSOR_STALE',
      status: 409,
      details: { expected: { deletionGeneration, tombstone }, cursor },
    });
  }
}
