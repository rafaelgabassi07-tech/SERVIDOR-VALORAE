...
if (!build) {
  console.log('apk-v698-quality-gate-v425 skipped: execute npm run test:cross-stack para validar o APK pareado');
  process.exit(0);
}
...
assert.equal(versionCode, 26082106);
assert.equal(versionName, '2026.08.21.06');
assert.equal(checkpoint, 'v698-quality-gate-kotlin-room');
...
console.log(`apk-v698-quality-gate-v425 ok: APK ${versionName}/${sourceFingerprint}/${buildFingerprint} ↔ Proxy ${RELEASE.publicVersion}; ${expectedRoutes.length} rotas auditadas`);
