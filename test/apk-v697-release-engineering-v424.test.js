...
if (!build) {
  console.log('apk-v697-release-engineering-v424 skipped: execute npm run test:cross-stack para validar o APK pareado');
  process.exit(0);
}
...
assert.equal(versionCode, 26082105);
assert.equal(versionName, '2026.08.21.05');
assert.equal(checkpoint, 'v697-release-engineering-hardening');
...
console.log(`apk-v697-release-engineering-v424 ok: APK ${versionName}/${sourceFingerprint}/${buildFingerprint} ↔ Proxy ${RELEASE.publicVersion}; ${expectedRoutes.length} rotas auditadas`);
