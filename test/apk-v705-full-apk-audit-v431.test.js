...
if (!build) {
  console.log('apk-v705-full-apk-audit-v431 skipped: execute npm run test:cross-stack para validar o APK pareado');
  process.exit(0);
}
...
assert.equal(versionCode, 26082304);
assert.equal(versionName, '2026.08.23.04');
assert.equal(checkpoint, 'v705-full-apk-audit-experience-hardening');
...
assert.match(returnUseCases, /returns-v5-exposure-ledger-v701/, 'auditoria v705 não pode trocar a geração financeira consolidada');
...
console.log(`apk-v705-full-apk-audit-v431 ok: APK ${versionName}/${sourceFingerprint}/${buildFingerprint} ↔ Proxy ${RELEASE.publicVersion}/${RELEASE.patch}; ${expectedRoutes.length} rotas auditadas`);
