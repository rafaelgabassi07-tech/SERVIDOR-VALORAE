...
if (!build) {
  console.log('apk-v704-audit-resilience-v430 skipped: execute npm run test:cross-stack para validar o APK pareado');
  process.exit(0);
}
...
assert.equal(versionCode, 26082303);
assert.equal(versionName, '2026.08.23.03');
assert.equal(checkpoint, 'v704-audit-runtime-resilience');
...
console.log(`apk-v704-audit-resilience-v430 ok: APK ${versionName}/${sourceFingerprint}/${buildFingerprint} ↔ Proxy ${RELEASE.publicVersion}/${RELEASE.patch}; ${expectedRoutes.length} rotas auditadas`);
