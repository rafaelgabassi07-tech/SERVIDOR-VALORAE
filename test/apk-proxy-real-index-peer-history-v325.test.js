...
assert.ok(['21.12.357', '21.12.358', '21.12.359', '21.12.360', '21.12.364'].includes(pkg.valorae.publicVersion));
assert.ok(['21.12.357-real-indices-peer-patrimony-history-v325', '21.12.358-modal-data-truth-audit-v326', '21.12.359-modal-source-arrival-integrity-v327', '21.12.360-news-logos-chart-tooltips-v328', '21.12.364-monthly-variation-logos-return-indices-v332'].includes(pkg.valorae.releasePatch));
assert.ok(['2026.07.13.01', '2026.07.13.02', '2026.07.13.03', '2026.07.13.04', '2026.07.13.07', '2026.07.13.08'].includes(metadata.apkVersion));
assert.match(metadata.contractVersion, /APK v(?:50[5-9]|51[0-2]) \/ Proxy 21\.12\.(?:35[7-9]|36[0-4])/);
...
if (returnsUi && readiness && merge && quality && build) {
  ...
  assert.match(build, /versionCode = 2607130[1-8]/);
  assert.match(build, /versionName = "2026\.07\.13\.0[1-8]"/);
}
