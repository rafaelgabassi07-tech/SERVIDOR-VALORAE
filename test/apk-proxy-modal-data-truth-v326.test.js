...
assert.ok(['21.12.358', '21.12.359', '21.12.360', '21.12.362'].includes(pkg.valorae.publicVersion));
assert.ok(['21.12.358-modal-data-truth-audit-v326', '21.12.359-modal-source-arrival-integrity-v327', '21.12.360-news-logos-chart-tooltips-v328', '21.12.362-modal-source-repair-v330'].includes(pkg.valorae.releasePatch));
assert.ok(['2026.07.13.02', '2026.07.13.03', '2026.07.13.04', '2026.07.13.06'].includes(metadata.apkVersion));
assert.match(metadata.contractVersion, /APK v(?:50[6-9]|510) \/ Proxy 21\.12\.(?:35[89]|36[0-2])/);
...
if ([models, parser, fiiParser, checklistUi, vacancyUi, build].every(Boolean)) {
  ...
  assert.match(build, /versionCode = 2607130[2-6]/);
  assert.match(build, /versionName = "2026\.07\.13\.0[2-6]"/);
}
