...
assert.ok(['21.12.358', '21.12.359', '21.12.360', '21.12.364', '21.12.366'].includes(pkg.valorae.publicVersion));
assert.ok(['21.12.358-modal-data-truth-audit-v326', '21.12.359-modal-source-arrival-integrity-v327', '21.12.360-news-logos-chart-tooltips-v328', '21.12.364-monthly-variation-logos-return-indices-v332', '21.12.366-multisource-official-logo-v334'].includes(pkg.valorae.releasePatch));
assert.ok(['2026.07.13.02', '2026.07.13.03', '2026.07.13.04', '2026.07.13.07', '2026.07.13.08', '2026.07.14.01'].includes(metadata.apkVersion));
assert.match(metadata.contractVersion, /APK v(?:50[6-9]|51[0-4]) \/ Proxy 21\.12\.(?:35[89]|36[0-6])/);
...
if ([models, parser, fiiParser, checklistUi, vacancyUi, build].every(Boolean)) {
  ...
  assert.match(build, /versionCode = 2607(?:130[2-9]|1401)/);
  assert.match(build, /versionName = "2026\.07\.(?:13\.0[2-9]|14\.01)"/);
}
