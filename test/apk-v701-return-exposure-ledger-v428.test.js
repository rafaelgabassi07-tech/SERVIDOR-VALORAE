...
if (!build) {
  console.log('apk-v701-return-exposure-ledger-v428 skipped: execute npm run test:cross-stack para validar o APK pareado');
  process.exit(0);
}
...
assert.equal(versionCode, 26082202);
assert.equal(versionName, '2026.08.22.02');
assert.equal(checkpoint, 'v701-return-exposure-ledger-integrity');
...
console.log(`apk-v701-return-exposure-ledger-v428 ok: APK ${versionName}/${sourceFingerprint}/${buildFingerprint} ↔ Proxy ${RELEASE.publicVersion}; ${expectedRoutes.length} rotas auditadas`);
