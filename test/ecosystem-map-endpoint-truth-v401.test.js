...
const mapJs = fs.readFileSync('public/ecosystem-flow-map.js', 'utf8');
const router = fs.readFileSync('routes/_router.js', 'utf8');

const documented = [...new Set(mapJs.match(/\/api\/v1\/[a-z0-9_/-]+/gi) || [])].sort();
...
for (const required of [
  ...
  '/api/v1/sync',
  '/api/v1/mobile/daily-close',
]) assert.ok(documented.includes(required), `jornada essencial ausente do mapa: ${required}`);

assert.equal(documented.length, 17, `quantidade inesperada de endpoints documentados: ${documented.join(', ')}`);
console.log('ecosystem-map-endpoint-truth-v401 ok');
