...
const first = await withAssetModalRuntime({
  ...
  ttlMs: 1000,
  staleMs: 1000,
  producer: async () => ({ ok: true, status: 'OK', ticker: 'V267A', value: ++calls, updatedAt: '2026-07-06T00:00:00.000Z', quoteSummary: { price: 10.5, priceDisplay: 'R$ 10,50' }, chart: { points: [{ close: 10.1 }, { close: 10.5 }] }, metrics: [{ id: 'price', value: 'R$ 10,50' }], fundamentalIndicators: { items: [{ id: 'pl', value: '8,0' }] }, historicalIndicators: { rows: [{ label: 'P/L' }], tablesByPeriod: {} }, companyProfile: { facts: [{ id: 'segment', value: 'Teste' }], sections: [] }, checklist: { items: [{ id: 'dy', passed: true, status: 'PASSED' }] }, revenueByRegion: { items: [{ label: 'Brasil', value: 100 }] }, returns: { rows: [{ label: '12M', value: '9%' }] }, dividendHistory: { events: [{ date: '2026-06-01', value: 1 }], yieldSeriesByFrequency: {}, dividendSeriesByFrequency: {} }, financialStatements: { rows: [] }, resultsStatement: { rows: [{ label: 'Receita', value: '100' }], tablesByPeriod: {} }, balanceSheetStatement: { rows: [], tablesByPeriod: {} } })
});
assert.equal(first.value, 1);
assert.equal(first.diagnostics.modalRuntime.cacheStatus, 'MISS');
assert.equal(first.diagnostics.modalRuntime.version, '26.asset-modal.runtime.v16-section-complete-skeleton');
