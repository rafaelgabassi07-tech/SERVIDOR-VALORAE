...
assert.equal(pkg.valorae.publicVersion,'21.12.361');
assert.equal(pkg.valorae.releasePatch,'21.12.361-full-stack-alignment-audit-v329');
...
if([application,feed,manifest,build].every(Boolean)){
  assert.match(application,/ValoraeAppGraph\.installHttpCache\(this\)/);assert.equal((feed.match(/executeJsonGetCancellable\(/g)||[]).length,2);assert.equal(feed.includes('executeJsonGet('),false);assert.match(manifest,/ValoraeNotificationRescheduleReceiver[\s\S]{0,160}android:exported="false"/);assert.match(build,/versionCode = 26071305/);
...
console.log('full-stack-alignment-audit-v329 ok');
