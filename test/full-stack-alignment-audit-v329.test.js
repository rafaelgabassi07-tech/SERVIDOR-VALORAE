...
assert.equal(pkg.valorae.publicVersion,'21.12.362');
assert.equal(pkg.valorae.releasePatch,'21.12.362-modal-source-repair-v330');
...
if([application,feed,manifest,build].every(Boolean)){
  assert.match(application,/ValoraeAppGraph\.installHttpCache\(this\)/);assert.equal((feed.match(/executeJsonGetCancellable\(/g)||[]).length,2);assert.equal(feed.includes('executeJsonGet('),false);assert.match(manifest,/ValoraeNotificationRescheduleReceiver[\s\S]{0,160}android:exported="false"/);assert.match(build,/versionCode = 26071306/);
...
console.log('modal-source-repair-v330 ok');
