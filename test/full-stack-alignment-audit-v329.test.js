...
if([application,feed,manifest,build].every(Boolean)){
  assert.match(application,/ValoraeAppGraph\.installHttpCache\(this\)/);assert.equal((feed.match(/executeJsonGetCancellable\(/g)||[]).length,2);assert.equal(feed.includes('executeJsonGet('),false);assert.match(manifest,/ValoraeNotificationRescheduleReceiver[\s\S]{0,160}android:exported="false"/);const apkMetadata=JSON.parse(readSiblingApkFile('metadata.json'));assert.match(build,new RegExp(`versionCode = ${apkMetadata.versionCode}`));
