...
const expectedContract = 'valorae-ecosystem-2026.08.05.03-p403';
...
assert.equal(direct.getHeader('X-Valorae-Public-Version'), '21.12.403');
...
await dispatchRoute({
  ...
  headers: {
    ...
    'x-valorae-app-version': '2026.08.05.03',
...
assert.equal(payload.version, '21.12.403');
assert.equal(payload.publicVersion, '21.12.403');
assert.equal(payload.coreVersion, '21.12.0');
assert.equal(payload.ecosystemContract, expectedContract);
assert.equal(ready.getHeader('X-Valorae-Public-Version'), '21.12.403');
