...
assert.equal(pkg.valorae.publicVersion, '21.12.408');
assert.equal(pkg.valorae.releasePatch, '21.12.408-deep-runtime-audit-v417');
...
assert.equal(direct.getHeader('X-Valorae-Public-Version'), '21.12.408');
...
await dispatchRoute({
  ...
  headers: {
    ...
    'x-valorae-app-version': '2026.08.11.06',
...
assert.equal(payload.version, '21.12.408');
assert.equal(payload.publicVersion, '21.12.408');
assert.equal(payload.coreVersion, '21.12.0');
assert.equal(payload.ecosystemContract, expectedContract);
assert.equal(ready.getHeader('X-Valorae-Public-Version'), '21.12.408');
