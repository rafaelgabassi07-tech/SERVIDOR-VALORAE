...
assert.ok(serverHtml.includes(RELEASE), 'Monitor precisa expor release v21.12.52.');
assert.equal(manifest.version, '21.12.52', 'Manifest PWA precisa apontar para v21.12.52.');
assert.match(sw, /valorae-proxy-server-v21-12-52/, 'Service worker precisa usar cache v21-12-52 para invalidar shell antigo.');
assert.equal(pkg.valorae.releasePatch, RELEASE, 'package.json precisa expor releasePatch v21.12.52.');
assert.equal(meta.releasePatch, RELEASE, 'metadata.json precisa expor releasePatch v21.12.52.');
...
for (const token of ['valorae-logo.svg', 'themeToggle', 'custom-select', 'dataReliability', 'canonicalReliabilityUsed']) {
  assert.ok(serverHtml.includes(token), `Monitor v21.12.52 deve preservar ${token}.`);
}

console.log('post-benchmark-performance-hardening-v21-12-52 OK');
