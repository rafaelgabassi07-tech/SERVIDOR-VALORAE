...
if (!build) {
  console.log('apk-v676-cross-stack-integrity-v423 skipped: execute npm run test:cross-stack para validar o APK pareado');
  process.exit(0);
}
...
assert.equal(versionCode, 26081301);
assert.equal(versionName, '2026.08.13.01');
assert.equal(checkpoint, 'v676-cross-stack-integrity-audit');
...
console.log(`apk-v676-cross-stack-integrity-v423 ok: APK ${versionName}/${sourceFingerprint} ↔ Proxy ${RELEASE.publicVersion}; ${expectedRoutes.length} rotas auditadas`);
