...
assert.equal(pkg.valorae.publicVersion, '21.12.359');
assert.equal(pkg.valorae.releasePatch, '21.12.359-modal-source-arrival-integrity-v327');
...
if ([delivery, parser, loader, quality, protocol, build].every(Boolean)) {
  ...
  assert.match(build, /versionCode = 26071303/);
  assert.match(build, /versionName = "2026\.07\.13\.03"/);
}

const runtime = fs.readFileSync(new URL('../lib/analysis/asset-modal-runtime.js', import.meta.url), 'utf8');
assert.match(runtime, /v18-source-arrival-integrity/);
