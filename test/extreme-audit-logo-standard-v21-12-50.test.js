...
assert.match(html, /21\.12\.\d+-.+/, 'release atual deve estar exposta');
...
assert.match(manifest, /21\.12\.\d+/, 'manifest PWA deve expor a release atual');
assert.match(sw, /valorae-proxy-server-v21-12-\d+/, 'service worker deve usar cache novo da release atual');
