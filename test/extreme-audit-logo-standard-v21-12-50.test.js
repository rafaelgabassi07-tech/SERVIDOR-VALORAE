...
assert.match(html, /21\.12\.51-post-benchmark-performance-hardening/, 'release v21.12.51 deve estar exposta');
...
assert.match(manifest, /21\.12\.51/, 'manifest PWA deve expor v21.12.51');
assert.match(sw, /valorae-proxy-server-v21-12-51/, 'service worker deve usar cache novo v21.12.51');

const script = html.slice(html.indexOf('<script>') + '<script>'.length, html.lastIndexOf('</script>'));
fs.writeFileSync('/tmp/valorae-extreme-audit-logo-standard-v21-12-51.js', script);
execFileSync(process.execPath, ['--check', '/tmp/valorae-extreme-audit-logo-standard-v21-12-51.js'], { stdio: 'inherit' });

console.log('extreme-audit-logo-standard-v21-12-51 OK');
