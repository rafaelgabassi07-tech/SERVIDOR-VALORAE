...
if (!build) {
  console.log('apk-v702-studio-compile-ui-v429 skipped: execute npm run test:cross-stack para validar o APK pareado');
  process.exit(0);
}
...
assert.equal(versionCode, 26082301);
assert.equal(versionName, '2026.08.23.01');
assert.equal(checkpoint, 'v702-studio-compile-ui-integration');
...
console.log(`apk-v702-studio-compile-ui-v429 ok: APK ${versionName}/${sourceFingerprint}/${buildFingerprint} ↔ Proxy ${RELEASE.publicVersion}; ${expectedRoutes.length} rotas auditadas`);
