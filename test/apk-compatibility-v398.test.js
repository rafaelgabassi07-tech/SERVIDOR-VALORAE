...
assert.equal(APK_COMPATIBILITY.pairedVersion, '2026.08.13.01');
...
assert.equal(evaluateApkCompatibility('2026.08.13.02', { allowFuture: false }).reject, true);
...
try {
  ...
  assert.equal(evaluateApkCompatibility('2026.08.13.02').reject, true, 'produção deve rejeitar APK futuro não homologado por padrão');
  process.env.VALORAE_REJECT_UNTESTED_FUTURE_APK = '0';
  assert.equal(evaluateApkCompatibility('2026.08.13.02').reject, false, 'override explícito pode abrir janela futura');
} finally {
