...
assert.equal(APK_COMPATIBILITY.pairedVersion, '2026.07.30.04');
...
assert.equal(evaluateApkCompatibility('2026.07.30.04').status, 'PAIRED');
assert.equal(evaluateApkCompatibility('2026.07.30.05', { allowFuture: false }).reject, true);
...
try {
  ...
  assert.equal(evaluateApkCompatibility('2026.07.30.05').reject, true, 'produção deve rejeitar APK futuro não homologado por padrão');
  process.env.VALORAE_REJECT_UNTESTED_FUTURE_APK = '0';
  assert.equal(evaluateApkCompatibility('2026.07.30.05').reject, false, 'override explícito pode abrir janela futura');
} finally {
