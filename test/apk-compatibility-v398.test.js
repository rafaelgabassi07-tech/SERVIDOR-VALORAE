...
try {
  ...
  assert.equal(evaluateApkCompatibility('2026.08.06.01').reject, true, 'produção deve rejeitar APK futuro não homologado por padrão');
  process.env.VALORAE_REJECT_UNTESTED_FUTURE_APK = '0';
  assert.equal(evaluateApkCompatibility('2026.08.06.01').reject, false, 'override explícito pode abrir janela futura');
} finally {
