...
assert.equal(evaluateApkCompatibility('2026.08.08.07').status, 'PAIRED');
assert.equal(evaluateApkCompatibility('2026.08.08.08', { allowFuture: false }).reject, true);
...
try {
  ...
  assert.equal(evaluateApkCompatibility('2026.08.08.08').reject, true, 'produção deve rejeitar APK futuro não homologado por padrão');
  process.env.VALORAE_REJECT_UNTESTED_FUTURE_APK = '0';
  assert.equal(evaluateApkCompatibility('2026.08.08.08').reject, false, 'override explícito pode abrir janela futura');
} finally {
