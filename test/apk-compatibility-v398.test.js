...
assert.equal(APK_COMPATIBILITY.pairedVersion, '2026.08.09.03');
...
assert.equal(evaluateApkCompatibility('2026.08.09.03').status, 'PAIRED');
assert.equal(evaluateApkCompatibility('2026.08.09.04', { allowFuture: false }).reject, true);
...
try {
  ...
  assert.equal(evaluateApkCompatibility('2026.08.09.04').reject, true, 'produção deve rejeitar APK futuro não homologado por padrão');
  process.env.VALORAE_REJECT_UNTESTED_FUTURE_APK = '0';
  assert.equal(evaluateApkCompatibility('2026.08.09.04').reject, false, 'override explícito pode abrir janela futura');
} finally {
