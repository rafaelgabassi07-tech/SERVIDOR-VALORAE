...
assert.equal(APK_COMPATIBILITY.pairedVersion, '2026.08.07.13');
...
assert.equal(evaluateApkCompatibility('2026.08.07.13').status, 'PAIRED');
assert.equal(evaluateApkCompatibility('2026.08.07.14', { allowFuture: false }).reject, true);
...
try {
  ...
  assert.equal(evaluateApkCompatibility('2026.08.07.14').reject, true, 'produção deve rejeitar APK futuro não homologado por padrão');
  process.env.VALORAE_REJECT_UNTESTED_FUTURE_APK = '0';
  assert.equal(evaluateApkCompatibility('2026.08.07.14').reject, false, 'override explícito pode abrir janela futura');
} finally {
